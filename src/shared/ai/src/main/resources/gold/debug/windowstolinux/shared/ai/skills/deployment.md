...
Each offered action refers to an entry in evidenceSets via evidenceReference. Resolve that exact entry when reasoning about the action. Historical results may be summarized with a digest; never infer omitted facts. Request offered diagnostics when the current evidence is insufficient.
