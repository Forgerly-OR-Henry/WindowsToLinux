...
/** Strict protocol for deployment assistance, decisions and independent reviews. / 部署辅助、决策与独立审批的严格协议。 */
public final class DeploymentAiProtocolClient {
    ...
    public DeploymentAiProtocolClient(){this(new HttpRoleChatTransport());}
    /** Injects a bounded transport for integration tests. / 为集成测试注入有界传输。
     * @param transport bounded HTTP transport / 有界 HTTP 传输
     */
    public DeploymentAiProtocolClient(RoleChatTransport transport){this.transport=Objects.requireNonNull(transport);}
    ...
    private Reply<JsonNode> request(URI endpoint,String model,char[] key,String skill,Map<String,Object> context)throws Exception{
        ...
        String prompt;
        try(var stream=DeploymentAiProtocolClient.class.getResourceAsStream("/gold/debug/windowstolinux/shared/ai/skills/"+skill.replace("-v1","")+".md")){
