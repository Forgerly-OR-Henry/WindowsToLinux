...
public enum AiAnalysisFailureType implements FailureDefinition {
     ...
     * <p>AI分析失败类型中的端点无效分类。
     */
    ENDPOINT_INVALID("ai.configuration.endpoint-invalid", "configuration", "ai.error.endpointInvalid",
     ...
     */
    HTTPS_REQUIRED("ai.configuration.https-required", "configuration", "ai.error.httpsRequired",
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * API KEY MISSING classification within ai analysis failure type.
     * <p>AI分析失败类型中的API键缺失分类。
     */
    API_KEY_MISSING("ai.authentication.api-key-missing", "authentication", "ai.error.apiKeyMissing",
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * HTTP REJECTED classification within ai analysis failure type.
     * <p>AI分析失败类型中的HTTP已拒绝分类。
     */
    HTTP_REJECTED("ai.request.http-rejected", "request", "ai.error.httpRejected",
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * SERVICE UNAVAILABLE classification within ai analysis failure type.
     * <p>AI分析失败类型中的服务不可用分类。
     */
    SERVICE_UNAVAILABLE("ai.request.service-unavailable", "request", "ai.error.unavailable",
     ...
     */
    RESPONSE_CONTENT_MISSING("ai.response.content-missing", "response", "ai.error.responseContentMissing",
            FailureRecoveryAction.NONE),
    /**
     * RESPONSE FORMAT INVALID classification within ai analysis failure type.
     * <p>AI分析失败类型中的响应格式无效分类。
     */
    RESPONSE_FORMAT_INVALID("ai.response.format-invalid", "response", "ai.error.responseFormatInvalid",
            FailureRecoveryAction.NONE),
    /**
     ...
     */
    UNICODE_ESCAPE_INVALID("ai.response.unicode-escape-invalid", "response", "ai.error.unicodeEscapeInvalid",
            FailureRecoveryAction.NONE),
    /**
     * STRING ESCAPE INVALID classification within ai analysis failure type.
     * <p>AI分析失败类型中的字符串转义无效分类。
     */
    STRING_ESCAPE_INVALID("ai.response.string-escape-invalid", "response", "ai.error.stringEscapeInvalid",
            FailureRecoveryAction.NONE),
    /**
     * UNTERMINATED STRING classification within ai analysis failure type.
     * <p>AI分析失败类型中的未终止字符串分类。
     */
    UNTERMINATED_STRING("ai.response.unterminated-string", "response", "ai.error.unterminatedString",
            FailureRecoveryAction.NONE);
    ...
    public FailureSeverityLevel severity() {
        return FailureSeverityLevel.WARNING;
    }
