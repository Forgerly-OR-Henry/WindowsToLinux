...
class AgentEvidenceTextTest {
    @Test
    void redactsBeforeTruncationAndPreservesUsefulErrorContext() {
        ...
        for (String value : new String[]{"topsecret", "tokenvalue", "privatevalue", "user:pass", "keymaterial"})
            assertFalse(result.contains(value), result);
        assertTrue(result.contains("build failed"));
        assertTrue(AgentEvidenceText.redact("a".repeat(9000)).length() < 2100);
    }
}
