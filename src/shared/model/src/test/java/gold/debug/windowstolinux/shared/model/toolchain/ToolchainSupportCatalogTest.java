...
class ToolchainSupportCatalogTest {
    ...
    private List<String> candidates(ToolchainEcosystemType ecosystem, String version) {
        return catalog.candidates(request(ecosystem, version)).stream().map(ToolchainSupportCatalog.Branch::version)
                .toList();
    }

    @Test
    void recognizesPastFutureAndPreviewWithoutApprovingThem() {
    ...
    @Test
    void choosesOnlyTwoHigherBranchesAndNeverChangesAnExactSupportedBranch() {
    ...
    @Test
    void parsesJavaEightAliasesAndOrdersUpdatesNumerically() {
    ...
    @Test
    void preservesSourceAndExactPatchMinimumAndSeriesSemantics() {
    ...
    @Test
    void checksEveryCatalogBoundaryAndStandardOrdering() {
        for (var ecosystem : ToolchainEcosystemType.values()) {
        ...
        assertEquals(List.of("99", "11", "17"),
                catalog.branches(C).stream().map(ToolchainSupportCatalog.Branch::version).toList());
        assertFalse(catalog.permits(ToolchainVersion.parse(NODE, "24.0.0-rc1").orElseThrow()));
    }

    @Test
    void appendsFutureBranchWithoutChangingVersionOrCandidateLogic() {
        ...
        branches.add(new ToolchainSupportCatalog.Branch(JAVA, "29", ToolchainSupportCatalog.ReleaseType.LTS,
                ToolchainSupportCatalog.MaintenanceStatus.MAINTAINED,
                ToolchainSupportCatalog.InstallationType.TEMURIN));
        var future = new ToolchainSupportCatalog("test-future", branches);
        assertEquals(List.of("29"),
                future.candidates(request(JAVA, "28")).stream().map(ToolchainSupportCatalog.Branch::version).toList());
        ...
        assertThrows(IllegalArgumentException.class,
                () -> new ToolchainSupportCatalog("duplicate", List.of(branches.getFirst(), branches.getFirst())));
    }
}
