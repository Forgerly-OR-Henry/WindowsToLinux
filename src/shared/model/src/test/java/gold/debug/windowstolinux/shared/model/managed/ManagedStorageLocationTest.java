...
class ManagedStorageLocationTest {
    @Test void defaultsHaveSeparateFhsRoots() {
        var location = ManagedStorageLocation.defaults();
        assertEquals("/etc/opt/windowstolinux/apps/demo/files/settings", location.resolve("demo", CONFIGURATION, "settings"));
    ...
    @Test void relativePathsRemainInStableApplicationStorage() {
        ...
        assertEquals("/opt/windowstolinux/apps/demo/user-data", ManagedStorageLocation.custom("/opt/windowstolinux/apps/demo/user-data").resolve("demo", FILE, "uploads"));
    }
    @Test void boundariesCannotBeConfusedWithPrefixesOrDefaultMarkers() {
        for (String path : new String[]{"/opt/windowstolinux/apps/demo", "/opt/windowstolinux/apps/demo-other/data",
            ...
            assertThrows(IllegalArgumentException.class, () -> ManagedStorageLocation.custom(path).resolve("demo", FILE, "data"), path);
        }
        assertThrows(IllegalArgumentException.class, () -> new ManagedStorageLocation(ManagedStorageLocation.StorageLocationType.DEFAULT, "/etc"));
        assertThrows(IllegalArgumentException.class, () -> ManagedStorageLocation.unresolved().resolve("demo", FILE, "data"));
    }
    @Test void rejectsTraversalVariablesAndShellSyntax() {
        for (String path : new String[]{"../data", "data/../other", "data//other", "${HOME}/data", "C:/data", "a\\b", "a;touch", "/", ".", ""})
