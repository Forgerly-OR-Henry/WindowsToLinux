...
public record ProjectLanguageFacts(
        Set<LanguageEcosystem> ecosystems,
        Set<SourceLanguage> sourceLanguages,
        Map<LanguageFact, String> values,
        List<AnalysisEvidence> evidence
) {
    /** Creates immutable language facts. / 创建不可变语言事实。 */
    public ProjectLanguageFacts {
        ecosystems = immutableEnumSet(ecosystems, LanguageEcosystem.class);
        sourceLanguages = immutableEnumSet(sourceLanguages, SourceLanguage.class);
        EnumMap<LanguageFact, String> copiedValues = new EnumMap<>(LanguageFact.class);
