...
public record ManagedStorageLocation(StorageLocationType type, String path) {
    ...
    public ManagedStorageLocation {
        ...
        if (type == StorageLocationType.CUSTOM)
            path = validatedPath(path);
        else if (!path.isEmpty())
            throw new IllegalArgumentException("only a custom location may contain a path");
    }
     ...
     */
    public static ManagedStorageLocation defaults() {
        return new ManagedStorageLocation(StorageLocationType.DEFAULT, "");
    }
     ...
     */
    public static ManagedStorageLocation unresolved() {
        return new ManagedStorageLocation(StorageLocationType.UNRESOLVED, "");
    }
     ...
     */
    public static ManagedStorageLocation custom(String path) {
        return new ManagedStorageLocation(StorageLocationType.CUSTOM, path);
    }
    ...
    public String resolve(String applicationId, StorageResourceType resource, String bindingId) {
        ...
        if (type == StorageLocationType.UNRESOLVED)
            throw new IllegalArgumentException("storage location must be reviewed");
        String category = switch (resource) {
            case CONFIGURATION -> "configuration";
            case FILE -> "files";
            case DATABASE -> "databases";
        };
        if (type == StorageLocationType.DEFAULT) {
            return resource == StorageResourceType.CONFIGURATION
                    ? configurationRoot(applicationId) + "/files/" + bindingId
                    : dataRoot(applicationId) + "/" + category + "/" + bindingId;
        }
        if (!path.startsWith("/"))
            return root + "/persistent/" + category + "/" + bindingId;
        if (!path.startsWith(root + "/"))
            throw new IllegalArgumentException("custom storage must remain inside " + root);
        String first = path.substring(root.length() + 1).split("/", 2)[0];
        if (first.equals("releases") || first.equals("current") || first.equals("persistent")
                || first.startsWith(".")) {
     ...
     */
    public static String installationRoot(String applicationId) {
        return "/opt/windowstolinux/apps/" + identifier(applicationId);
    }
     ...
     */
    public static String configurationRoot(String applicationId) {
        return "/etc/opt/windowstolinux/apps/" + identifier(applicationId);
    }
     ...
     */
    public static String dataRoot(String applicationId) {
        return "/var/opt/windowstolinux/apps/" + identifier(applicationId);
    }
    ...
    public static String validatedPath(String input) {
        ...
        if (path.isEmpty() || path.equals("-") || path.length() > 512
    ...
    private static String identifier(String value) {
        if (value == null || !value.matches("[a-z0-9][a-z0-9-]{0,62}"))
            throw new IllegalArgumentException("invalid managed storage identifier");
        return value;
    }
}
