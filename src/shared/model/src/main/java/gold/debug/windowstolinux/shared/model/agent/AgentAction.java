...
/**
 * Immutable nonsecret action offered by trusted code, never arbitrary model shell text. / 可信代码提供的不可变非秘密动作，不接收模型任意 Shell 文本。
...
public record AgentAction(String id,String taskId,String target,String sourceRevision,long planRevision,
    ...
    public AgentAction {
        ...
        if(planRevision<1)throw new IllegalArgumentException("invalid plan revision");
        parameters=bounded(parameters);evidence=bounded(evidence);
    }
    ...
    public String binding(){
        ...
        for(String part:List.of(id,taskId,target,sourceRevision,Long.toString(planRevision),tool.name(),risk.name()))append(value,part);
        append(value,Integer.toString(parameters.size()));new TreeMap<>(parameters).forEach((k,v)->{append(value,k);append(value,v);});
    ...
    private static Map<String,String> bounded(Map<String,String> input){
        if(input.size()>128)throw new IllegalArgumentException("too many action fields");
        int total=0;
        for(var entry:input.entrySet()){
            if(!entry.getKey().matches("[a-zA-Z0-9._/-]{1,160}")||entry.getValue()==null||entry.getValue().length()>8192)throw new IllegalArgumentException("invalid action field");
            total+=entry.getValue().length();
        }
        if(total>32768)throw new IllegalArgumentException("action context too large");
