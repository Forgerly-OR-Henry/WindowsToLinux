...
public final class DatabaseVersionRequirement {
    ...
    public DatabaseVersionRequirement(String declaration) {
        ...
        if (this.declaration.length() > 100)
            throw new IllegalArgumentException("database version requirement is too long");
        for (String term : terms())
            if (!TERM.matcher(term).matches())
                throw new IllegalArgumentException("unsupported database version constraint");
    }
    ...
    public boolean accepts(String observed) {
        if (observed == null || !observed.matches("[0-9]{1,3}(?:\\.[0-9]{1,3}){0,3}"))
            return false;
        int[] actual = numbers(observed);
        for (String term : terms()) {
            var matcher = TERM.matcher(term);
            matcher.matches();
            ...
            if (operator == null || operator.equals("=")) {
                for (int index = 0; index < required.length; index++)
                    if ((index < actual.length ? actual[index] : 0) != required[index])
                        return false;
            } else if (!(switch (operator) {
                case ">=" -> compared >= 0;
                case "<=" -> compared <= 0;
                case ">" -> compared > 0;
                case "<" -> compared < 0;
    ...
    private List<String> terms() {
        return declaration.isEmpty()
                ? List.of()
                : List.of(declaration.replaceAll("([<>=]+)\\s+", "$1").split("[,\\s]+"));
    }
    ...
    private static int compare(int[] left, int[] right) {
        for (int index = 0; index < Math.max(left.length, right.length); index++) {
            int compared = Integer.compare(index < left.length ? left[index] : 0,
