 ...
 */
public record DeploymentRuntimeSuggestion(
        DeploymentProjectType projectType,
        Map<RuntimeInput, String> values,
    ...
    public DeploymentRuntimeSuggestion {
        ...
        suggestedHealthPort.ifPresent(DeploymentRuntimeSuggestion::requirePort);
    ...
    public Optional<String> value(RuntimeInput input) {
    ...
    public static Map<RuntimeInput, String> valuesFor(DeploymentProjectType projectType) {
        Objects.requireNonNull(projectType, "projectType");
        return new EnumMap<>(RuntimeInput.class);
    }

    /** Stable scalar input names exposed by a typed source inference. / 类型化源码推导公开的稳定标量输入名称。 */
    public enum RuntimeInput {
    ...
    private static EnumSet<RuntimeInput> allowedInputs(DeploymentProjectType projectType) {
        return switch (projectType) {
            case SPRING_BOOT, DOCKERFILE_CONTAINER, RECOGNITION_PREVIEW -> EnumSet.noneOf(RuntimeInput.class);
