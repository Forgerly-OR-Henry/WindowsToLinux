 ...
 */
public record AgentReview(AgentReviewDecision decision, AgentRiskLevel risk, String binding, String reason,
    ...
    public AgentReview {
        ...
        java.util.Objects.requireNonNull(risk);
        if (binding == null || !binding.matches("[0-9a-f]{64}") || reason == null || reason.isBlank()
                || reason.length() > 2048)
            throw new IllegalArgumentException("invalid review");
        evidence = List.copyOf(evidence);
        if (evidence.size() > 128 || evidence.stream().anyMatch(v -> v.length() > 160))
