...
public record AssistedDeploymentAdvice(String summary,List<Candidate> suggestions,List<String> unresolved){
     ...
     * @param unresolved unresolved issues requiring human review / 需要人工审阅的未解决问题
     */
    public AssistedDeploymentAdvice{if(summary==null||summary.length()>2048)throw new IllegalArgumentException("invalid assisted summary");
        suggestions=List.copyOf(suggestions);unresolved=List.copyOf(unresolved);
        if(suggestions.size()>64||unresolved.size()>64||unresolved.stream().anyMatch(v->v.length()>1024))throw new IllegalArgumentException("assisted advice too large");}
    /**
     ...
     * @param evidence observed reference identifiers / 观察引用标识
     */
        public Candidate{if(field==null||candidate==null||field.length()>160||candidate.length()>4096)throw new IllegalArgumentException("invalid candidate");
            evidence=List.copyOf(evidence);if(evidence.isEmpty()||evidence.size()>64||evidence.stream().anyMatch(v->v.length()>160))throw new IllegalArgumentException("invalid candidate evidence");}
    }
}
