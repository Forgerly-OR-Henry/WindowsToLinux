...
class NativeDatabaseExceptionTest {
    ...
    void allHelperReasonsExposeStructuredUncheckedFailures(NativeDatabaseFailureType reason) {
        ...
        assertEquals("linux.database." + reason.name().toLowerCase(Locale.ROOT).replace('_', '-'), exception.failure().code());
        ...
        assertEquals("Native database operation requires attention: " + reason.name(), exception.failure().diagnostic());
    }
