...
public final class DeploymentRemoteTaskScope implements AutoCloseable {
    ...
    public DeploymentRemoteTaskScope(String task, Consumer<RemoteWorkspace> intent) {
        requireTask(task);
        if (CURRENT.get() != null)
            throw new IllegalStateException("remote task already active");
        this.task = task;
     ...
     * @param task task identity / 任务身份
     */
    public static void requireTask(String task) {
        if (task == null || !task.matches("[a-zA-Z0-9-]{1,80}"))
            throw new IllegalArgumentException("invalid remote task identity");
    }

    /** Captures the scope on this worker. / 获取当前工作线程作用域。
     * @return optional scope / 可选作用域
     */
    public static Optional<DeploymentRemoteTaskScope> current() {
     ...
     */
    public String preparing(RemoteWorkspace workspace) {
        intent.accept(workspace);
        return task;
    }
    ...
    public void close() {
        if (CURRENT.get() != this)
            throw new IllegalStateException("wrong remote task owner");
        CURRENT.remove();
    }
}
