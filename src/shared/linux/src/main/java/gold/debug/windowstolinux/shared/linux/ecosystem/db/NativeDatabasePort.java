...
public interface NativeDatabasePort {
     ...
     * @throws LinuxOperationException if the authenticated remote operation fails or its evidence is rejected / 已认证远端操作失败或其证据被拒绝时
     */
    Inventory installDatabase(DatabaseEngineType engine, PackageCandidate target, Optional<Replacement> replacement)
     ...
     * @throws LinuxOperationException if the authenticated remote operation fails or its evidence is rejected / 已认证远端操作失败或其证据被拒绝时
     */
    Target inspectDatabaseTarget(Instance instance, String applicationId, String database, String username,
    ...
    record PackageCandidate(String name, String packageVersion, String engineVersion) {
        ...
        public PackageCandidate {
            name = token(name);
            packageVersion = token(packageVersion);
            engineVersion = token(engineVersion);
        }
    }
     ...
     */
    record Instance(DatabaseEngineType engine, String id, String version, int port, String service,
            String dataDirectory, String fingerprint, boolean running) {
        /**
     ...
     */
    record Inventory(DatabaseEngineType engine, List<Instance> instances, Optional<PackageCandidate> candidate,
        ...
        public Inventory {
            ...
            if (instances.size() > 64 || conflicts.size() > 64
                    || instances.stream().anyMatch(instance -> instance.engine() != engine))
                throw new IllegalArgumentException("invalid DB inventory");
        }
    }
     ...
     */
    record Replacement(String serverId, Instance previous, PackageCandidate target, UUID operation,
            boolean backupConfirmed, boolean downtimeConfirmed) {
        /**
        ...
        public Replacement {
            ...
            if (!backupConfirmed || !downtimeConfirmed)
                throw new IllegalArgumentException("DB replacement requires backup and downtime confirmation");
        }
    }
     ...
     */
    record Target(Instance instance, String applicationId, String database, String username, boolean exists,
            boolean empty, String ownershipToken, InitializationState initialization, String initializedSourceSha256) {
        /**
        ...
        public Target {
            ...
            Objects.requireNonNull(ownershipToken);
            Objects.requireNonNull(initialization);
            Objects.requireNonNull(initializedSourceSha256);
            if (!ownershipToken.isEmpty())
                ownershipToken = digest(ownershipToken);
            if (!initializedSourceSha256.isEmpty())
                initializedSourceSha256 = digest(initializedSourceSha256);
            if (!exists && !empty)
                throw new IllegalArgumentException("absent DB cannot contain data");
        }
    ...
    private static String token(String value) {
        if (value == null || !value.matches("[A-Za-z0-9][A-Za-z0-9_.:+~@%=-]{0,255}"))
            throw new IllegalArgumentException("invalid DB token");
        return value;
    }
    ...
    private static String text(String value) {
        if (value == null || value.isEmpty() || value.length() > 512 || value.chars().anyMatch(Character::isISOControl))
            throw new IllegalArgumentException("invalid DB text");
        return value;
    }
    ...
    private static String digest(String value) {
        if (value == null || !value.matches("[a-f0-9]{64}"))
            throw new IllegalArgumentException("invalid DB fingerprint");
        return value;
    }
}
