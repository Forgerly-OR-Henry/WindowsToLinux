...
public record RemoteRestoreMember(String path, long size, String sha256) {
    ...
    public RemoteRestoreMember {
        ...
        if (path.isEmpty() || path.length() > 1024 || path.indexOf('\\') >= 0 || path.startsWith("/")
                || path.endsWith("/") || path.matches("^[A-Za-z]:.*")) {
        ...
        if (size < 0)
            throw new IllegalArgumentException("restore member size must not be negative");
