...
class ManagedRuntimeObservationTest {
    ...
    private Map<String, String> state(String active, String sub, String result, String status, String pid) {
        return new HashMap<>(Map.of("OWNER", "1", "ENABLED", "disabled", "QUERY_OK", "1",
                "ActiveState", active, "SubState", sub, "Result", result,
                "ExecMainCode", "1", "ExecMainStatus", status, "MainPID", pid));
    }
    ...
    @Test void distinguishesHealthyStoppedFailedAndRestartWaiting() {
        ...
        for (String status : new String[]{"1", "37", "129"}) {
            ...
            assertTrue(ManagedRuntimeProtocolExecutor.nativeObservation(app, values).evidence().contains("ExecMainStatus=" + status));
            assertEquals(RuntimeState.ERROR, observe(state("activating", "auto-restart", "exit-code", status, "0")));
        }
    }
    @Test void rejectsIncompleteQueriesLegacyHelperAndTransitions() {
        var values = state("inactive", "dead", "success", "0", "0");
        for (String key : values.keySet()) {
            var missing = new HashMap<>(values); missing.remove(key);
            if (!key.equals("ENABLED")) assertEquals(RuntimeState.UNKNOWN, observe(missing), key);
        ...
        assertTrue(ManagedRuntimeProtocolExecutor.nativeObservation(app, old).evidence().contains("environment preparation"));
        values.put("QUERY_OK", "0"); assertEquals(RuntimeState.UNKNOWN, observe(values));
        ...
        values.put("OWNER", "0"); assertFalse(ManagedRuntimeProtocolExecutor.nativeObservation(app, values).ownershipVerified());
    }
    @Test void preservesStopEvidenceButRejectsUncontrolledText() {
        var evidence = ManagedRuntimeProtocolExecutor.stopEvidence("STOP_BEFORE_QUERY_OK=1\nSTOP_BEFORE_Result=exit-code\nSTOP_BEFORE_ExecMainStatus=129\nSTOP_AFTER_ExecMainStatus=129\nSECRET=private\nSTOP_AFTER_Result=private value\n");
