...
class BuildCompatibilityPolicyTest {
    @Test void retriesOnlyExplicitCompatibilityFailures() {
        ...
        for (String reason : List.of("package requires a different Python: 3.8.20 not in '>=3.9'",
                ...
                "your php version (8.0.30) does not satisfy that requirement", "error NETSDK1045", "npm ERR! EBADENGINE"))
            assertTrue(BuildCompatibilityPolicy.retryable(reason));
        for (String reason : List.of("connection timeout", "Permission denied", "SyntaxError", "OutOfMemoryError", "health check failed"))
