...
class SshdExternalApplicationPortTest {
    ...
    private final String row = "APP\tSYSTEMD\t" + encoded("demo.service") + "\t" + "a".repeat(64) + "\t" + encoded("Demo") + "\tSTOPPED\t1\t1\t0\n";

    @Test void readsPartialScanButRejectsTruncationDuplicatesAndCommandsInIdentity() {
        var scan = SshdExternalApplicationPort.parse(row + "ISSUE\tDOCKER_PERMISSION\nEND\t1\n");
        assertEquals(1, scan.applications().size()); assertEquals(1, scan.issues().size());
