...
class SshdLinuxRemoteSessionTest {
    ...
    Path directory;
    @Test
    void recoveryProbeUsesOnlyAFixedCommandWithoutBashOrHelper() throws Exception {
    ...
    @Test
    void existingCapabilitiesShareOneAuthenticatedConnectionAndFailAfterClose() throws Exception {
        try (var fixture = new Fixture(directory)) {
            ...
            var request = new RemoteDatabasePort.BackupRequest("demo",
                    new RemoteDatabasePort.ConnectionProfile.Sqlite("main",
                            gold.debug.windowstolinux.shared.model.managed.ManagedStorageLocation.defaults(), "app.db"),
                    false, false);
    ...
    @Test
    void knownAndPersistedRuntimeEntriesShareActionsAndPostconditionFailures() throws Exception {
        try (var fixture = new Fixture(directory)) {
            var app = ManagedApplication.forManaged("demo",
                    new ServerIdentity("server", "127.0.0.1", 22, "SHA256:fixture"), "a".repeat(64));
            ...
            for (boolean persisted : new boolean[]{false, true}) {
                ...
                for (var action : new LifecycleAction[]{LifecycleAction.START, LifecycleAction.RESTART,
                    ...
                    var after = persisted
                            ? fixture.session.executeLifecycle(app, action, health)
                    ...
                    if (action == LifecycleAction.START || action == LifecycleAction.RESTART)
                        assertEquals(RuntimeState.RUNNING, after.runtimeState());
                    if (action == LifecycleAction.STOP)
                        assertEquals(RuntimeState.STOPPED, after.runtimeState());
                    if (action == LifecycleAction.ENABLE_AUTOSTART)
                        assertEquals(AutostartState.ENABLED, after.autostartState());
                    if (action == LifecycleAction.DISABLE_AUTOSTART)
                        assertEquals(AutostartState.DISABLED, after.autostartState());
                }
                fixture.running = true;
                var start = assertThrows(LinuxOperationException.class, () -> {
                    if (persisted)
                        fixture.session.executeLifecycle(app, LifecycleAction.START, health);
                    else
                        fixture.session.executeDeploymentLifecycle(app, runtime, LifecycleAction.START);
                });
                assertTrue(start.getMessage().contains("stopped"));
                fixture.ignoreAction = true;
                assertThrows(LinuxOperationException.class, () -> {
                    if (persisted)
                        fixture.session.executeLifecycle(app, LifecycleAction.STOP, health);
                    else
                        fixture.session.executeDeploymentLifecycle(app, runtime, LifecycleAction.STOP);
                });
                fixture.ignoreAction = false;
                fixture.unhealthy = true;
                assertThrows(LinuxOperationException.class, () -> {
                    if (persisted)
                        fixture.session.executeLifecycle(app, LifecycleAction.RESTART, health);
                    else
                        fixture.session.executeDeploymentLifecycle(app, runtime, LifecycleAction.RESTART);
                });
    ...
    private static final class Fixture implements AutoCloseable {
        ...
        Fixture(Path directory) throws Exception {
            ...
            try {
                client.setServerKeyVerifier((connection, address, key) -> true);
                client.start();
                var connection = client.connect("root", "127.0.0.1", server.getPort()).verify(Duration.ofSeconds(10))
                        .getSession();
                connection.addPasswordIdentity("fixture");
                connection.auth().verify(Duration.ofSeconds(10));
        ...
        private void configure(SshServer server) {
            ...
            server.setCommandFactory((channel, command) -> new Command() {
                ...
                @Override
                public void setExitCallback(org.apache.sshd.server.ExitCallback exit) {
                    this.exit = exit;
                }
                ...
                public void start(org.apache.sshd.server.channel.ChannelSession channel,
                    ...
                    lastCommand = command;
                    output.write(reply(command).getBytes(StandardCharsets.UTF_8));
                    output.flush();
        ...
        private String reply(String command) {
            ...
            if (command.equals("printf 'WTL_SSH_READY\\n'"))
                return probeOutput;
            if (command.contains("database-inspect"))
                return "TYPE=sqlite\nENGINE_VERSION=3.40\nTOOL_VERSION=3.40\nTOOL_AVAILABLE=1\nENGINE_COMPATIBLE=1\nONLINE_BACKUP_AVAILABLE=1\nALL_TABLES_TRANSACTIONAL=1\n";
            if (command.contains("restore-preflight"))
                return "AVAILABLE_BYTES=1048576\nMANAGED_ROOT_WRITABLE=1\nFOREIGN_CONFLICT=0\n";
            if (command.contains("inspect-runtime"))
                return "KIND=deployment\n";
            if (command.contains("observe-deployment") && oldHelper)
                return "OWNER=1\nRUNNING=0\nENABLED=disabled\n";
            if (command.contains("observe-deployment"))
                return "OWNER=" + (owner ? 1 : 0) + "\nRUNNING=" + (running ? 1 : 0) + "\nENABLED="
                        + (enabled ? "enabled" : "disabled") + "\n" + "QUERY_OK=1\nActiveState="
                        + (failed ? "failed" : running ? "active" : "inactive") + "\nSubState="
                        + (failed ? "failed" : running ? "running" : "dead") + "\nResult="
                        + (failed ? "exit-code" : "success") + "\nExecMainCode=1\nExecMainStatus=" + (failed ? 129 : 0)
                        + "\nMainPID=" + (running ? 123 : 0) + "\n";
            if (command.contains("lifecycle-deployment")) {
                mutations++;
                if (!ignoreAction) {
                    if (command.contains("'start'") || command.contains("'restart'"))
                        running = true;
    ...
    @Test
    void failedNativeServiceRequiresVerifiedStopBeforeRecoveryForBothEntries() throws Exception {
        try (var fixture = new Fixture(directory)) {
            var app = ManagedApplication.forManaged("demo",
                    new ServerIdentity("server", "127.0.0.1", 22, "SHA256:fixture"), "a".repeat(64));
            var health = new HealthCheck.Tcp(18080, 1, 1);
            ...
            for (boolean persisted : new boolean[]{false, true}) {
                ...
                int mutations = fixture.mutations;
                for (var action : new LifecycleAction[]{LifecycleAction.START, LifecycleAction.RESTART,
                        LifecycleAction.ENABLE_AUTOSTART}) {
                    assertThrows(LinuxOperationException.class, () -> {
                        if (persisted)
                            fixture.session.executeLifecycle(app, action, health);
                        else
                ...
                assertEquals(RuntimeState.STOPPED,
                        fixture.session.executeLifecycle(app, LifecycleAction.STOP, health).runtimeState());
                assertEquals(RuntimeState.RUNNING,
                        fixture.session.executeLifecycle(app, LifecycleAction.START, health).runtimeState());
            }
        }
    }

    @Test
    void missingHelperFieldsAndQueryFailureCannotMasqueradeAsStopped() throws Exception {
        try (var fixture = new Fixture(directory)) {
            var app = ManagedApplication.forManaged("demo",
                    new ServerIdentity("server", "127.0.0.1", 22, "SHA256:fixture"), "a".repeat(64));
            ...
            fixture.oldHelper = false;
            fixture.queryFailed = true;
