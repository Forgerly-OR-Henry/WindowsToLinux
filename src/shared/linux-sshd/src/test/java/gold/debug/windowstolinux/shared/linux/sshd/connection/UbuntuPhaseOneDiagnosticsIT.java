...
/**
 * Opt-in, read-only diagnostics for a disposable Ubuntu phase-one acceptance VM. This does not deploy, build, upload, alter processes, or use sudo.
 *
 * <p>针对一次性 Ubuntu 一期验收虚拟机的可选只读诊断。它不会部署、构建、上传、修改进程或使用 sudo。
 */
@EnabledIfSystemProperty(named = "phase1.diagnostics", matches = "true")
class UbuntuPhaseOneDiagnosticsIT {
    @Test
    void reportsProcessHeadroomWithoutChangingTheTarget() throws Exception {
        ...
        assertEquals(false, host == null || host.isBlank(), "phase1.ssh.host is required");
