    ...
    def test_controller_does_not_hold_volume_writes_during_service_stop(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def service_stopping():
                self.assertFalse(
                    any(
                        path.is_relative_to(root / 'mutable') and flags & (os.O_WRONLY | os.O_RDWR)
...
'''
                ...
                if kind == 'javasource':
                    ...
                    self.assertEqual(
                        shlex.split(('/usr/bin/env ' if selinux == '1' else '') + expected),
                    ...
                    self.assertEqual(
                        ['npm', '--prefix', '/opt/windowstolinux/apps/app/current/source', 'start'],
...
'''
                result = subprocess.run(
                    [bash, '--noprofile', '--norc', '-s'],
                    input=foundation + '\n' + identity + '\n' + setup,
                    env=dict(os.environ, TEST_SELINUX=selinux),
                    text=True,
                    capture_output=True,
                    timeout=10,
                )
                ...
                if selinux == '1':
                    ...
                    self.assertTrue(
                        {'-/run/systemd/private', '-/run/systemd/journal', '-/run/systemd/notify'}.issubset(paths)
                    )
...
class ContainerNamespacePreflightTest(unittest.TestCase):
    def test_uses_only_the_loaded_engine_profile_and_keeps_mapping_failures_blocking(self):
        ...
        cases = [
            ('docker', 'rootlesskit (unconfined)', 'rootlesskit', False),
...
'''
                environment = dict(
                    os.environ, TEST_ENGINE=engine, TEST_PROFILES=profiles, TEST_DENIED='1' if denied else '0'
                )
                result = subprocess.run(
                    [bash, '--noprofile', '--norc', '-s'],
                    input=source + '\n' + setup,
    ...
    def test_socket_existence_does_not_start_a_project_before_engine_readiness(self):
        ...
        for engine in ('docker', 'podman'):
            for ready_after, alive in [(3, True), (999, True), (999, False)]:
                with (
                    self.subTest(engine=engine, ready_after=ready_after, alive=alive),
...
'''
                result = subprocess.run(
                    [bash, '--noprofile', '--norc', '-s'],
                    input=setup + body,
                    env=dict(os.environ, TEST_SCENARIO=scenario),
                    text=True,
                    capture_output=True,
                    timeout=20,
                )
...
class ContainerExportNormalizationTest(unittest.TestCase):
    def test_legacy_aliases_are_removed_without_extracting_or_weakening_publication_validation(self):
        java = (
            ...
            / 'standard/deploy/src/main/java/gold/debug/windowstolinux/shared/standard/deploy/build/workload/ContainerBuildRenderer.java'
        ).read_text()
        normalizer = textwrap.dedent(
            java.split("<<'WTL_PODMAN_EXPORT'\n", 1)[1].split('\n                WTL_PODMAN_EXPORT', 1)[0]
        )
        ...
        for scenario in ('valid', 'escape', 'unknown-layer', 'hardlink', 'root'):
            with self.subTest(scenario=scenario), tempfile.TemporaryDirectory() as temporary:
                ...
                config = json.dumps(
                    {
                        'config': {
                            'User': '0' if scenario == 'root' else '65532:65532',
                            'Labels': {
                                'io.windowstolinux.application': 'demo',
                    ...
                    [
                        {
                            'Config': config_name,
                            'Layers': [layer],
                            'RepoTags': ['localhost/windowstolinux-candidate:demo-0123456789abcdef'],
                        }
                    ]
                ).encode()
                with tarfile.open(archive, 'w') as output:
                    ...
                    item.linkname = (
                        '/etc/passwd'
                        if scenario == 'escape'
                        else '../' + ('c' * 64 + '.tar' if scenario == 'unknown-layer' else layer)
                    )
                    output.addfile(item)
                result = subprocess.run(
                    [sys.executable, '-I', '-c', normalizer, str(archive)], text=True, capture_output=True, timeout=20
                )
