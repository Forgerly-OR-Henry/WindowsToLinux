...
class ExternalApplicationTest(unittest.TestCase):
    def test_stopped_services_containers_deduplicate_and_report_partial_failures(self):
        ...
        out = io.StringIO()
        with patch.object(APP.shutil, 'which', return_value='/usr/bin/tool'), patch.object(APP, 'run', side_effect=run), \
                patch.object(APP, 'systemd', side_effect=systemd), patch.object(APP, 'docker', return_value=('DOCKER', CID, CID, 'Container', 'STOPPED', True, True, False)), contextlib.redirect_stdout(out):
            APP.scan()
    ...
    def test_unit_fingerprint_tracks_original_file_identity_and_contents(self):
        ...
        with patch.object(APP, 'run', return_value=response), patch.object(APP.os, 'fstat', return_value=identity), patch('builtins.open', mock_open(read_data=b'first')):
            first = APP.systemd('app.service')
        with patch.object(APP, 'run', return_value=response), patch.object(APP.os, 'fstat', return_value=identity), patch('builtins.open', mock_open(read_data=b'other')):
    ...
    def test_changed_target_and_managed_marker_prevent_mutation(self):
        out = io.StringIO()
        with patch.object(APP, 'systemd', return_value=('SYSTEMD', 'app.service', FP, 'App', 'STOPPED', True, True, False)), patch.object(APP, 'run') as run, contextlib.redirect_stdout(out):
            self.assertEqual(3, APP.execute('SYSTEMD', 'app.service', 'c' * 64, 'START'))
            run.assert_not_called()
        self.assertIn('IDENTITY_CHANGED', out.getvalue())
        with patch.object(APP, 'systemd', return_value=('SYSTEMD', 'app.service', FP, 'App', 'STOPPED', True, True, True)), patch.object(APP, 'run') as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_lifecycle_uses_fixed_arguments_and_rechecks_after_action(self):
        ...
        with patch.object(APP, 'systemd', side_effect=[before, after]) as inspect, patch.object(APP, 'run') as run, \
                patch.object(APP.os, 'geteuid', return_value=1000, create=True), contextlib.redirect_stdout(io.StringIO()):
            self.assertEqual(0, APP.execute('SYSTEMD', 'app.service', FP, 'START'))
            run.assert_called_once_with(['sudo', '-n', 'systemctl', '--no-ask-password', 'start', '--', 'app.service'], 45)
    ...
    def test_docker_selects_only_named_metadata_and_keeps_empty_label_columns(self):
        ...
        with patch.object(APP.subprocess, 'run', return_value=completed) as run:
            ...
            self.assertNotIn('.Env', str(args)); self.assertNotIn('.Mounts', str(args))
            self.assertFalse(run.call_args.kwargs.get('shell', False))
            self.assertEqual('STOPPED', result[4]); self.assertFalse(result[7])
    ...
    def test_no_runtime_is_distinct_from_empty_success(self):
        ...
        with patch.object(APP.shutil, 'which', return_value=None), contextlib.redirect_stdout(out):
            APP.scan()
        self.assertIn('SYSTEMD_UNAVAILABLE', out.getvalue()); self.assertIn('DOCKER_UNAVAILABLE', out.getvalue())
