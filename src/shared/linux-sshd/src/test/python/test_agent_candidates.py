...
""" + FRAGMENT + "\ncleanup_candidate() { printf 'CLEANED=%s\\n' \"$2\"; }\n" + setup + "\n" + action
            return subprocess.run([BASH, "--noprofile", "--norc", "-s"], input=script, text=True,
    ...
    def test_foreign_and_unbound_markers_cannot_be_queried_or_cleaned(self):
        for setup in ["mkdir work/app-0123456789abcdef", "mkdir work/app-0123456789abcdef; echo other > work/app-0123456789abcdef/.agent-task"]:
