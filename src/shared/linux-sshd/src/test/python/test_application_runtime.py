...
ROOT = (
    ...
    / "main/resources/gold/debug/windowstolinux/shared/linux/sshd/execution/protocol/helper/fragments"
)
...
def payload(mode="ON_DEMAND", endpoints=(), inputs=(), companions=(), verify=True, workers=()):
    fields = [
        "application-v2" if workers else "application-v1",
        mode,
        "1",
        "",
        "",
    ...
    if verify:
        fields += ["main.py", "2", "--self-test", "中文 空格 $HOME %i"]
    ...
    if workers:
        ...
        for identifier, entry, arguments in workers:
            fields += [identifier, entry, str(len(arguments)), *arguments]
    fields += ["PROCESS", "10", "5"]
    return base64.b64encode(("\0".join(fields) + "\0").encode()).decode()


class ApplicationRuntimeTest(unittest.TestCase):
    def test_generic_process_uses_explicit_managed_entry_without_a_language_adapter(self):
        runtime = (ROOT / 'runtime/41-application-runtime.sh').read_text(encoding='utf-8')
        result = execute(
            'ecosystem/35-ecosystem-dispatch.sh',
            runtime
            + '''
application_entry='bin/custom app'
render_ecosystem_runtime_command process /opt/windowstolinux/apps/demo
printf '%s' "$ecosystem_runtime_command_result"
''',
        )
        self.assertEqual(0, result.returncode, result.stderr)
        self.assertEqual('"/opt/windowstolinux/apps/demo/current/source/bin/custom app"', result.stdout)
        for setup in ("application_entry=''", "application_entry=bin/app; set -- unexpected"):
            rejected = execute(
                'ecosystem/35-ecosystem-dispatch.sh',
                runtime + setup + '\nrender_ecosystem_runtime_command process /opt/windowstolinux/apps/demo "$@"\n',
            )
            self.assertEqual(64, rejected.returncode, rejected.stdout + rejected.stderr)
...
''',
        ...
        self.assertEqual(
            ['/opt/php/bin/php', '/opt/apps/demo/current/source/worker.php', '中文 参数', '$HOME %i'], commands[1]
        )
    ...
    def test_workers_preserve_literal_arguments_and_require_unique_daemon_entries(self):
        ...
        for value in (
            ...
            payload('DAEMON', workers=(worker, worker)),
    ...
    def test_unit_working_directory_is_a_literal_path_not_a_quoted_command_argument(self):
        ...
        result = execute(
            'runtime/41-application-runtime.sh',
            foundation
            + '''
...
''',
    ...
    def test_unicode_read_only_paths_remain_literal_and_escape_paths_fail(self):
        result = self.parse(
            payload(inputs=(("source", "/srv/中文 输入", "/inputs/资料"),)),
    ...
    def test_tampered_or_unreviewed_payload_fails_before_any_action(self):
        for data in [
            ...
            base64.b64encode(b"application-v0\0").decode(),
            ...
            with self.subTest(data=data[:40]):
                self.assertEqual(64, self.parse(data).returncode)
    ...
    def test_health_request_is_framed_and_cannot_replace_an_installation_entry(self):
        ...
        result = self.parse(
            payload("DAEMON"),
            f"parse_application_health_payload '{valid}'\nprintf '%s\\n' \"$application_health_port\"",
        )
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        ...
        self.assertEqual(
            64, self.parse(payload("DAEMON"), f"parse_application_health_payload '{valid[:-4]}'").returncode
        )
    ...
    def test_container_mapping_checks_protocol_address_host_and_target(self):
        actual = {
            "9001/tcp": [{"HostIp": "0.0.0.0", "HostPort": "9000"}],
            "9002/udp": [{"HostIp": "0.0.0.0", "HostPort": "9000"}],
        }
        setup = "container_engine=fake\nfake() { printf '%s' '" + json.dumps(actual) + "'; }\n"
        for ports, success in [
            ("0.0.0.0:9000:9001/tcp 0.0.0.0:9000:9002/udp", True),
            ("0.0.0.0:9000:9001/tcp", False),
            ("127.0.0.1:9000:9001/tcp 0.0.0.0:9000:9002/udp", False),
        ]:
            result = execute(
                "runtime/45-application-container-contract.sh",
                setup + f"container_ports=({ports})\nassert_container_ports demo",
            )
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as server:
                ...
                def respond():
                    try:
                        ...
                        if data == b"ping" and reply is not None:
    ...
    def test_container_primary_command_preserves_image_cmd_and_literal_arguments(self):
        for config, expected in [
            ({'Entrypoint': None, 'Cmd': ['python', 'main.py']}, ['python', 'main.py']),
...
""",
        ...
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertEqual(
            ['/opt/app/releases/demo/source/main unit/.venv/bin/python', '-m', 'worker', '中文 空格', '$HOME'],
...
''',
        ...
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertEqual(
            '"/opt/app/current/source/main unit/.venv/bin/python" "/opt/app/current/source/主目录/main.py"',
            result.stdout,
        )
