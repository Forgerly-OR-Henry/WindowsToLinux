...
RESOURCE = (
    ...
    / 'main/resources/gold/debug/windowstolinux/shared/linux/sshd/execution/protocol/helper/fragments/database'
)
CODE = (
    ''.join(
        (RESOURCE / name).read_text(encoding='utf-8') for name in ('10-native-instances.sh', '20-native-targets.sh')
    )
    .split("<<'WTL_NATIVE_DB_PY'\n", 1)[1]
...
class NativeDatabaseTest(unittest.TestCase):
    def test_unknown_installed_version_is_a_conflict_not_an_absent_database(self):
        with (
            patch.dict(
                ...
                {
                    'system': lambda: 'dnf',
                    'packages': lambda manager: {'postgresql-server': 'custom'},
                    'run': lambda *args, **kwargs: types.SimpleNamespace(stdout='', returncode=0),
                    'unit_properties': lambda unit: {'Id': unit, 'LoadState': 'loaded'},
                    'package_candidate': lambda *args: None,
                },
            ),
            patch('shutil.which', return_value='/usr/bin/postgres'),
        ):
    ...
    def test_unparseable_repository_candidate_is_not_used_for_installation(self):
        with patch.dict(
            DB, {'run': lambda *args, **kwargs: types.SimpleNamespace(stdout='  Candidate: custom\n', returncode=0)}
        ):
    ...
    def test_existing_database_cannot_install_without_exact_replacement_approval(self):
        ...
        with patch.dict(
            DB,
            {
                'inspect': lambda *args: {'instances': [self.item], 'candidate': candidate, 'conflicts': []},
                'select': lambda request: self.item,
                'system': lambda: 'apt',
    ...
    def test_stopped_instance_is_started_without_installing_another_server(self):
        ...
        with patch.dict(
            DB,
            {
                'select': lambda request: dict(self.item, running='false'),
                'system': lambda: 'apt',
                'inspect': lambda *args: {'instances': [self.item]},
    ...
    def test_failed_initialization_is_not_reexecuted_on_retry(self):
        ...
        def query(item, sql, *args):
            ...
            if 'count(*)' in sql:
                return '0'
            executed.append(sql)
            raise DB['Failure']('INITIALIZATION_FAILED')

        with patch.dict(DB, {'select': lambda request: self.item, 'sql_query': query}):
            with self.assertRaises(DB['Failure']):
                DB['initialize'](self.request)
            with self.assertRaises(DB['Failure']) as retry:
                DB['initialize'](self.request)
    ...
    def test_initializer_refuses_client_commands_and_privileged_sql(self):
        for sql in [
            ...
            'USE other; DELETE FROM secrets;',
            "CREATE FUNCTION dangerous() RETURNS void AS 'anything';",
        ]:
            with self.subTest(sql=sql), self.assertRaises(DB['Failure']):
                DB['restricted_sql'](sql)
    ...
    def test_launcher_maps_password_and_password_file_without_putting_values_in_arguments(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            (directory / 'WINDOWSTOLINUX_SECRET_DB_123456ABCDEF_ENV_DB_PASSWORD_FILE').write_text(
                'env-secret', encoding='utf-8'
            )
            with (
                patch.dict(os.environ, {'CREDENTIALS_DIRECTORY': folder}, clear=True),
                patch('sys.argv', ['runtime-db-env', '/usr/bin/java', '-jar', 'app.jar']),
