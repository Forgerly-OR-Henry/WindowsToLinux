...
RESOURCE = (
    ...
    / 'main/resources/gold/debug/windowstolinux/shared/linux/sshd/execution/protocol/helper/fragments/runtime/41-application-runtime.sh'
)
PROGRAM = (
    RESOURCE.read_text(encoding='utf-8')
    .split("<<'WTL_WORKER_SUPERVISOR'\n", 1)[1]
    ...
    def run_case(self, primary, terminate=False):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            process = subprocess.Popen(
                [sys.executable, '-I', '-c', PROGRAM, 'program', encoded],
            ...
            try:
                ...
                time.sleep(0.2)
                self.assertEqual(
                    previous, path.read_text() if path.exists() else None, 'worker survived supervisor exit'
                )
            finally:
