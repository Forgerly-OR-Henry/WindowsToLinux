...
SOURCE = (
    pathlib.Path(__file__).resolve().parents[2] / 'main/resources/gold/debug/windowstolinux/shared/linux/sshd/toolchain'
)
import types
...
class OfficialMetadataTest(unittest.TestCase):
    def test_every_ecosystem_lower_middle_upper_metadata(self):
        cases = {
            'JAVA': ['8.0.202+8', '17.0.2+8', '25.0.1+8'],
        ...
        for ecosystem, versions in cases.items():
            for version in versions:
                with self.subTest(ecosystem=ecosystem, version=version):
                    ...
                    filename = {
                        'JAVA': 'jdk.tar.gz',
                        'NODE': 'node-v' + version + '-linux-x64.tar.gz',
                        'PYTHON': 'Python-' + version + '.tgz',
                        'DOTNET': 'sdk.tar.gz',
                        'KOTLIN': 'kotlin-compiler-' + version + '.zip',
                        'GO': 'go' + version + '.linux-amd64.tar.gz',
                        'RUST': 'rust-' + version + '-x86_64-unknown-linux-gnu.tar.gz',
                        'PHP': 'php-' + version + '.tar.gz',
                    ...
                    def json_at(url):
                        if ecosystem == 'JAVA':
                            return [
                                {
                                    'version_data': {'semver': version, 'openjdk_version': version},
                                    'binaries': [
                                        {
                                            'os': 'linux',
                                            'architecture': 'x64',
                                            'package': {
                                                'link': 'https://github.com/adoptium/' + filename,
                                                'checksum': DIGEST,
                                            },
                                        }
                                    ],
                                            ...
                                            'files': [
                                                {
                                                    'rid': 'linux-x64',
                                                    'name': filename,
                                                    'url': 'https://builds.dotnet.microsoft.com/' + filename,
                                                    'hash': 'b' * 128,
                                                }
                                            ],
                                        }
                                ...
                                'assets': [
                                    {
                                        'name': filename,
                                        'digest': None,
                                        'browser_download_url': 'https://github.com/JetBrains/kotlin/' + filename,
                                    }
                                ],
                                'body': '| ' + filename + ' | ' + DIGEST + ' |',
                            }
                        if ecosystem == 'PHP':
                            return {version: {'source': [{'filename': filename, 'sha256': DIGEST}]}}
                    ...
                    def fetch(url, *args):
                        ...
                        if ecosystem == 'PYTHON':
                            if url.endswith('/ftp/python/'):
                                return ('<a href="' + version + '/">release</a>').encode()
                            return ('<tr><td>' + filename + '</td><td>' + DIGEST + '</td></tr>').encode()
                        if ecosystem == 'RUBY':
                            if url.endswith('/downloads/releases/'):
                                return (
                                    '<tr><td>' + filename + '</td><td><a href="/en/news/fixture/">more</a></td></tr>'
                                ).encode()
                            return ('<li>' + filename + '<pre>SHA256: ' + DIGEST + '</pre></li>').encode()
                        raise AssertionError(url)

                    with (
                        mock.patch.object(preparation, 'json_at', side_effect=json_at),
                        mock.patch.object(preparation, 'fetch', side_effect=fetch),
                    ):
    ...
    def test_java_uses_openjdk_identity_instead_of_lts_semver_encoding(self):
        def asset(openjdk, semver):
            return {
                'version_data': {'openjdk_version': openjdk, 'semver': semver},
                'binaries': [
                    {
                        'os': 'linux',
                        'architecture': 'x64',
                        'package': {'link': 'https://github.com/adoptium/' + openjdk + '.tar.gz', 'checksum': DIGEST},
                    }
                ],
            }

        assets = [asset('21.0.12+8-LTS', '21.0.12+8.0.LTS'), asset('21.0.12.1+1-LTS', '21.0.12+101.0.LTS')]
        ...
        with mock.patch.object(preparation, 'json_at', return_value=[asset('1.8.0_492-b09', '8.0.492+9')]):
            self.assertEqual('8.0.492+9', preparation.release('JAVA', '8', '')[0])
        with mock.patch.object(preparation, 'json_at', return_value=[asset('21.0.13-ea+1', '21.0.13-ea+1')]):
            with self.assertRaises(preparation.PreparationFailure):
    ...
    def test_numeric_order_preview_and_ecosystem_specific_java_alias(self):
        ...
        for url in [
            ...
            'https://user@nodejs.org/file',
            'https://nodejs.org:444/file',
        ]:
            with self.assertRaises(preparation.PreparationFailure):
                preparation.official_url(url)
    ...
    def test_exact_identity_includes_build_number_and_probe_checks_the_same_release(self):
        ...
        for reported, expected, valid in [
            ...
            with (
                ...
                mock.patch.object(preparation, 'probe_output', return_value=(0, reported)),
...
class ArchiveAndInstallationTest(unittest.TestCase):
    def test_installation_sealing_repairs_restricted_modes_without_following_links(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                mock.patch.object(
                    pathlib.Path, 'is_symlink', autospec=True, side_effect=lambda path: path == root / 'external-link'
                ),
                mock.patch.object(
                    pathlib.Path, 'chmod', autospec=True, side_effect=lambda path, mode: changed.update({path: mode})
                ),
            ):
    ...
    def test_shared_roots_are_validated_before_explicit_traversal_permission(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            actions = []
            with (
                mock.patch.object(preparation, 'ROOT', root),
                mock.patch.object(
                    preparation, 'owned_directory', side_effect=lambda path: actions.append(('validate', path))
                ),
                mock.patch.object(
                    pathlib.Path,
                    'chmod',
    ...
    def test_rejects_traversal_link_ancestors_and_cycles_before_extraction(self):
        for entries in [
            ...
            with self.subTest(entries=entries), tempfile.TemporaryDirectory() as temporary:
                ...
                destination.mkdir()
                with self.assertRaises(preparation.PreparationFailure):
                    preparation.extract(archive, destination)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with (
                ...
                first = preparation.install(
                    'JAVA', '8.0.202', 'https://github.com/fixture', 'sha256', expected, 'archive'
                )
                second = preparation.install(
                    'JAVA', '11.0.2', 'https://github.com/fixture', 'sha256', expected, 'archive'
                )
                self.assertNotEqual(first['directory'], second['directory'])
                self.assertEqual(
                    first,
                    preparation.install('JAVA', '8.0.202', 'https://github.com/fixture', 'sha256', expected, 'archive'),
                )
                ...
                for error in [preparation.PreparationFailure('probe', 'fixture failure'), KeyboardInterrupt()]:
                    ...
                    with self.assertRaises(type(error)):
                        preparation.install(
                            'JAVA', '17.0.1', 'https://github.com/fixture', 'sha256', expected, 'archive'
                        )
    ...
    def test_lower_middle_upper_installation_recipes_record_the_requested_exact_release(self):
        cases = {
            'JAVA': (['8.0.202', '17.0.2', '25.0.1'], 'archive'),
            ...
            for version in versions:
                with self.subTest(ecosystem=ecosystem, version=version), tempfile.TemporaryDirectory() as temporary:
                    ...
                    with (
                        mock.patch.object(preparation, 'ROOT', root),
                        mock.patch.object(preparation, 'owned_directory'),
                        mock.patch.object(preparation, 'download', side_effect=download),
                        mock.patch.object(preparation, 'run') as run,
                        mock.patch.object(preparation, 'source_dependencies'),
                        mock.patch.object(preparation, 'ruby_legacy_tls', return_value={}),
                        mock.patch.object(preparation, 'probe') as probe,
                    ):
                        record = preparation.install(
                            ecosystem, version, 'https://github.com/fixture', 'sha256', digest, layout
                        )
                        self.assertEqual(
                            (ecosystem, version, digest), (record['ecosystem'], record['version'], record['sha256'])
                        )
                        self.assertEqual(
                            record, json.loads((pathlib.Path(record['directory']) / '.w2l-toolchain.json').read_text())
                        )
    ...
    def test_php_probe_requires_runtime_extensions_before_admission(self):
        ...
        with (
            mock.patch.object(preparation, 'probe_output', return_value=(0, '8.3.33')),
            mock.patch.object(preparation, 'run') as run,
        ):
    ...
    def test_restore_uses_saved_exact_version_and_relocates_system_binding_without_catalog_admission(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            record = {
                'ecosystem': 'PYTHON',
                'version': '3.8.20',
                'directory': str(root / 'new-python'),
                'origin': 'MANAGED',
                'source': 'https://www.python.org/fixture',
                'sha256': DIGEST,
            }
            with (
                mock.patch.object(preparation, 'ROOT', root),
                mock.patch.object(
                    preparation, 'release', return_value=('3.8.20', record['source'], 'sha256', DIGEST, 'python')
                ) as release,
                mock.patch.object(preparation, 'install', return_value=record),
    ...
    def test_repeated_restore_reuses_verified_installation_without_network(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            record = {
                ...
                'origin': 'SYSTEM',
                'source': 'system:/usr/bin/python3.8',
                'sha256': DIGEST,
            }
            ...
            with (
                mock.patch.object(preparation, 'ROOT', root),
                mock.patch.object(preparation, 'owned_directory'),
                mock.patch.object(preparation, 'verify_system'),
                mock.patch.object(preparation, 'probe'),
...
class SourceDependencyTest(unittest.TestCase):
    def test_crb_is_enabled_only_for_the_centos_transaction_and_apt_is_preserved(self):
        for installer, release, crb in [
            ...
            with (
                self.subTest(installer=installer, release=release),
                mock.patch.object(
                    pathlib.Path,
                    'is_file',
                    autospec=True,
