...
application_verification_storage() {
    ...
    for spec in "${managed_data_bindings[@]}"; do
        ...
        source="$(managed_data_binding_root "$binding")"
        target="$application_validation_root/$binding"
        install -d -o "$owner" -g "$owner" -m 700 -- "$target"
        if [ "$storage_kind" = DATABASE ] && [ -f "$source/$database_file" ]; then
            ...
            application_properties+=(--property="BindReadOnlyPaths=$(application_systemd_escape "$(managed_storage_target)")")
            continue
...
run_application_native() {
    local app="$1" operation="$2" entry status=0 kind="$application_runtime_kind"
    shift 2
    ...
    case "$operation" in
        ...
        client)
            ...
            arguments=("${application_client_args[@]}" "$@")
            ;;
        verify)
            entry="$application_verify_entry"
            arguments=("${application_verify_args[@]}")
            ;;
        probe)
            entry="$application_health_entry"
            arguments=("${application_health_args[@]}")
            ;;
    ...
    for secret in "${deployment_secret_names[@]}"; do
        if [[ "$secret" =~ ^WINDOWSTOLINUX_SECRET_DB_[0-9A-F]{12}_ENV_[A-Z0-9_]+_FILE$ ]]; then
            application_exec=(/usr/bin/python3 /usr/local/lib/windowstolinux/runtime-db-env "${application_exec[@]}")
            break
        fi
    done
