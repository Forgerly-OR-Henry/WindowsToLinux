...
prepare_container_builder() {
  ...
  exec 8> "$base_root/.subid-allocation.lock"; flock -x 8
  printf '%s\n' "$user" > "$candidate/.builder-intent"; chmod 400 -- "$candidate/.builder-intent"
  ...
  start="$(/usr/bin/python3 -I - <<'WTL_SUBIDS'
import grp, pwd
...
cleanup_container_builder() {
  ...
  local -a identity; mapfile -t identity < "$candidate/.builder"
  [ "${#identity[@]}" -eq 5 ] || reject container-identity-record
  user="${identity[0]}"; uid="${identity[1]}"; gid="${identity[2]}"; start="${identity[3]}"; end="${identity[4]}"
