...
socket="$XDG_RUNTIME_DIR/engine.sock"; daemon=
cleanup_engine() {
  if [ -n "$daemon" ]; then
...
case "$engine" in
  docker)
    ...
    /usr/bin/rootlesskit --state-dir="$XDG_RUNTIME_DIR/rootlesskit" --net=slirp4netns --disable-host-loopback \
      ...
      > "$mutable/engine.log" 2>&1 & daemon=$!
    ;;
  podman)
    /usr/bin/podman --root "$mutable/engine" --runroot "$XDG_RUNTIME_DIR/podman" --storage-driver=vfs       system service --time=0 "unix://$socket" > "$mutable/engine.log" 2>&1 & daemon=$!
    ;;
  *) exit 64 ;;
esac
wait_engine_ready() {
