...
application_udp_owned() {
    ...
    if [ "$application_backend" = container ]; then
        ...
        for endpoint in "${application_endpoints[@]}"; do
            ...
            if { [ "$protocol" = UDP ] && [ "$transport" = udp ] || [ "$protocol" != UDP ] && [ "$transport" = tcp ]; } && [ "$host" = "$application_health_port" ]; then
                target="$listener"
                break
            fi
        done
...
application_udp_response() {
    ...
    for endpoint in "${application_endpoints[@]}"; do
        ...
        if [ "$protocol" = UDP ] && [ "$host" = "$application_health_port" ]; then
            case "$address" in 0.0.0.0) probe_address=127.0.0.1 ;; ::) probe_address=::1 ;; *) probe_address="$address" ;; esac
            break
        fi
    done
...
application_health() {
    ...
    application_admit_run "$1"
    application_load_current "$1" "$2"
    application_assert_inputs "$1"
...
application_health_loaded() {
    ...
    if [ "$application_mode" = ON_DEMAND ] || [ "$application_health_kind" = COMMAND ]; then
        if [ "$application_mode" != ON_DEMAND ]; then
            application_verify_entry="$application_health_entry"
            application_verify_args=("${application_health_args[@]}")
        ...
        elif [ -n "${application_restore_root:-}" ]; then
            output="$(application_container_candidate_probe verify | head -c 65537)" || reject application-verification-failed
    ...
    while [ "$SECONDS" -lt "$deadline" ]; do
        ...
        if [ -z "$current" ] || [ "$current" != "$initial" ]; then
            initial="$current"
            stable="$SECONDS"
        fi
        case "$application_health_kind" in
            PROCESS) if [ -n "$current" ] && [ "$((SECONDS - stable))" -ge "$application_health_stability" ]; then
                printf 'HEALTHY=1\n'
                return
            fi ;;
            TCP)
                if [ -n "$current" ] && application_udp_owned && application_tcp_response; then
                    if [ "$((SECONDS - stable))" -ge "$application_health_stability" ]; then
                        printf 'HEALTHY=1\n'
                        return
                    fi
            ...
            UDP)
                if [ -n "$current" ] && application_udp_owned; then
                    if [ "$application_health_custom" = 1 ]; then
                        if [ "$application_backend" = native ]; then
                            (run_application_native "$1" probe) >/dev/null || {
                                sleep 1
                                continue
                            }
                        elif [ -n "${application_restore_root:-}" ]; then
                            (application_container_candidate_probe probe) >/dev/null || {
                                sleep 1
                                continue
                            }
                        else (run_application_container "$1" probe) >/dev/null || {
                            sleep 1
                            continue
                        }; fi
                    else application_udp_response || {
                        sleep 1
                        continue
                    }; fi
                    [ "$(application_process_identity)" = "$current" ] || reject application-process-changed
