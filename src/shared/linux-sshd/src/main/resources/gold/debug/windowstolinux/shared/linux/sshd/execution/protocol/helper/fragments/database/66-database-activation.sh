...
database_commit_postgresql() {
  ...
  chown root:root -- "$database_activation_state"; chmod 400 -- "$database_activation_state"
  ...
  PGPASSFILE="$credentials" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname=postgres -v ON_ERROR_STOP=1 -c "$sql" >/dev/null || { rm -f -- "$credentials"; reject database-commit-terminate; }
  if [ "$previous" = 1 ]; then PGPASSFILE="$credentials" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname=postgres -v ON_ERROR_STOP=1 -c "ALTER DATABASE \"$database_name\" RENAME TO \"$database_activation_rollback\"" >/dev/null || { rm -f -- "$credentials"; reject database-commit-rename-old; }; fi
  PGPASSFILE="$credentials" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname=postgres -v ON_ERROR_STOP=1 -c "ALTER DATABASE \"$database_activation_database\" RENAME TO \"$database_name\"" >/dev/null || { rm -f -- "$credentials"; reject database-commit-rename-new; }
  database_postgresql_exists "$credentials" "$database_name" || { rm -f -- "$credentials"; reject database-commit-verify; }
...
database_recover_postgresql() {
  ...
  if [ "$previous" = 1 ] && ! database_postgresql_exists "$credentials" "$database_activation_rollback"; then
    database_postgresql_exists "$credentials" "$database_name" || { rm -f -- "$credentials"; reject database-rollback-missing; }
  else
    if database_postgresql_exists "$credentials" "$database_name"; then PGPASSFILE="$credentials" dropdb --force --no-password --host="$database_host" --port="$database_port" --username="$database_username" "$database_name" >/dev/null || { rm -f -- "$credentials"; reject database-recover-drop; }; fi
    if [ "$previous" = 1 ]; then PGPASSFILE="$credentials" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname=postgres -v ON_ERROR_STOP=1 -c "ALTER DATABASE \"$database_activation_rollback\" RENAME TO \"$database_name\"" >/dev/null || { rm -f -- "$credentials"; reject database-recover-rename; }
      database_postgresql_exists "$credentials" "$database_name" || { rm -f -- "$credentials"; reject database-recover-verify; }
    else ! database_postgresql_exists "$credentials" "$database_name" || { rm -f -- "$credentials"; reject database-recover-absence; }; fi
  fi
  if database_postgresql_exists "$credentials" "$database_activation_database"; then PGPASSFILE="$credentials" dropdb --force --no-password --host="$database_host" --port="$database_port" --username="$database_username" "$database_activation_database" >/dev/null || { rm -f -- "$credentials"; reject database-candidate-discard-failed; }; fi
  rm -f -- "$credentials"
}
