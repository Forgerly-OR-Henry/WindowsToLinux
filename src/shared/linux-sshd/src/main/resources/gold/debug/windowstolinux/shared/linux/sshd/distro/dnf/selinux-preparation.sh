  ...
  if [ -e "$state" ] || [ -L "$state" ]; then
    ...
    case "$(cat "$state/phase")" in
      ...
      enforcing)
        ...
        [ "$config_sha" = "$(cat "$state/permissive-sha256")" ] || selinux_fail configuration-changed
        if [ "$security" = ENFORCING ]; then checkpoint=ENFORCEMENT_PENDING
        elif [ "$security" = PERMISSIVE ]; then checkpoint=READY_TO_ENFORCE
...
selinux_preparation() {
  ...
  if [ "${ID:-}" != centos ] || { [ "${VERSION_ID:-}" != 9 ] && [ "${VERSION_ID:-}" != 10 ]; }; then
    if [ "$operation" = inspect ]; then printf 'APPLICABLE=false\n'; return; fi
