restore_start_deployment_candidate() {
    ...
    deployment_root_override="$(app_root "$app")"
    runtime_identity_override="$(restore_candidate_identity "$token" "$component")"
    application_backend=native
    application_assert_inputs "$app"
    port="$(awk -F: 'NR==1 {print $3}' "$root/ports")"
    if [ -n "$port" ]; then
        require_service_port "$port"
        deployment_service_port_override="$port"
        deployment_bind_address_override=127.0.0.1
    elif [ "${#application_endpoints[@]}" -gt 0 ]; then assert_root_owned_regular "$root/previous"; fi
    unit="$(restore_unit_name "$token" "$component")"
    tmp="$(mktemp /run/systemd/system/.windowstolinux-restore.XXXXXX)"
    ...
    if [ "$application_mode" = ON_DEMAND ]; then
        ...
        command="$application_primary_command"
        render_application_command "$application_runtime_kind" "$(app_root "$app")"
    ...
    install -o root -g root -m 600 -- "$tmp" "/run/systemd/system/$unit"
    rm -f -- "$tmp"
...
restore_application_health() {
    ...
    if [ "$application_health_kind" = UDP ]; then
        ...
        for mapping in "${mappings[@]}"; do
            if [[ "$mapping" = "udp:$application_health_port:"* ]]; then
                application_health_port="${mapping##*:}"
                break
            fi
        done
    fi
