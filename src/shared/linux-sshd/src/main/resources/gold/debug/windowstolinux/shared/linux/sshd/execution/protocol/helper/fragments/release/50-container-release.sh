parse_container_parameters() {
  ...
  while [ "$port_count" -gt 0 ]; do
    ...
    local address="$2"; [[ "$address" != *:* ]] || address="[$address]"
  ...
  for endpoint in "${application_endpoints[@]}"; do
    ...
    expected="$address:$host:$target/$protocol"; found=0
  ...
  for volume in "${container_volumes[@]}"; do
    id="${volume%%:*}"; id="${id#windowstolinux-}"; destination="${volume#*:}"; readonly="${destination##*:}"; destination="${destination%:*}"; found=0
    for spec in "${managed_data_bindings[@]}"; do
      managed_binding_parts "$spec"; [ "$binding" = "$id" ] || continue
      [ "$storage_kind" = FILE ] && [ "$logical" = "$destination" ] || reject container-storage-declaration
      if [ "$readonly" = 1 ]; then [ "$mode" = ro ] || reject container-storage-declaration
...
save_container_parameters() {
  local target="$1" spec source destination mode
  {
    ...
    for spec in "${container_ports[@]}"; do
      ...
      target="${target##*:}"; mapping="${mapping%:*}"; host="${mapping##*:}"; address="${mapping%:*}"; address="${address#[}"; address="${address%]}"
...
start_container_release() {
  ...
  application_backend=container; application_release="$(app_root "$app")/releases/$release_digest"
  application_assert_inputs "$app"; application_container_options "$image"
  if [ "$application_mode" = ON_DEMAND ]; then
    ...
    if [ "$container_engine" = podman ]; then
      ...
      if [ -e "$quadlet" ]; then assert_root_owned_regular "$quadlet"; rm -f -- "$quadlet"; fi
  ...
  if [ "$container_engine" = docker ]; then
    ...
    "${args[@]}" >/dev/null
    assert_container_bind_mounts "$image" "$name"; assert_container_ports "$name"
    ...
    trap 'rm -f -- "$tmp"' EXIT
    {
      ...
      if [ "${#application_container_command[@]}" -gt 0 ]; then
        printf 'Exec='; for spec in "${application_container_command[@]}"; do application_systemd_escape "$spec"; printf ' '; done; printf '\n'
      fi
      index=0
      while [ "$index" -lt "${#application_input_sources[@]}" ]; do
        printf 'Volume=%s\n' "$(application_systemd_escape "${application_input_sources[$index]}:${application_input_targets[$index]}:ro")"; index=$((index+1))
      done
