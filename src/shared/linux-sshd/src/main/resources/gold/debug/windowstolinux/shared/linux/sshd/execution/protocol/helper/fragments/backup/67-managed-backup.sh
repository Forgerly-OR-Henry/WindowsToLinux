...
prepare_backup_operation() {
  ...
  require_backup_operation "$operation"; root="$(backup_operation_root "$operation")"
  install -d -o root -g root -m 700 -- "$backups_root"
  if [ ! -e "$root" ] && [ ! -L "$root" ]; then
    install -d -o root -g root -m 700 -- "$root"
    printf '%s\n' "$deployer" > "$root/.deployer"
    chown root:root -- "$root/.deployer"; chmod 400 -- "$root/.deployer"
  fi
  assert_root_owned_directory "$root"; assert_root_owned_regular "$root/.deployer"
...
assert_backup_tree() {
  ...
  while IFS= read -r -d '' link; do
    relative="${link#"$source"/}"; found=0
    for allowed_link in "$@"; do [ "$relative" = "$allowed_link" ] && found=1; done
    [ "$found" -eq 1 ] || reject backup-link
  done < <(find -P "$source" -xdev -type l -print0)
}
create_pax_artifact() {
  local operation="$1" artifact="$2" source="$3" maximum="$4"; shift 4
  local root target spec
  root="$(backup_operation_root "$operation")"; target="$root/$artifact.pax"
...
backup_create_artifact() {
  ...
  require_backup_operation "$operation"; require_app "$application"; require_app "$component"; require_app "$app"
  require_digest "$release"; require_digest "$manifest"; require_backup_limit "$maximum"
  [[ "$resource" =~ ^[a-z0-9][a-z0-9-]{0,127}$ ]] || reject backup-resource
  case "$kind" in file|release|volume|oci) ;; *) reject backup-kind ;; esac
  prepare_backup_operation "$operation"
  local artifact root target source current_digest spec binding logical found engine image image_id
  artifact="$(backup_artifact_identity "$application" "$component" "$app" "$release" "$kind" "$resource")"
  require_backup_artifact "$artifact"; root="$(backup_operation_root "$operation")"
  if [ "$kind" = release ] && [ -e "$(app_root "$app")/current/.windowstolinux-container-parameters" ]; then
    container_current_release "$app" "$manifest"; [ "$previous_present" -eq 1 ] || reject backup-current
    current_digest="${previous_path##*/}"; [ "$current_digest" = "$release" ] || reject backup-release
    ...
    create_pax_artifact "$operation" "$artifact" "$previous_path" "$maximum"
    target="$root/$artifact.pax"
  elif [ "$kind" = file ] || [ "$kind" = release ]; then
    assert_deployment_current_or_empty "$app" "$manifest"; [ "$previous_present" -eq 1 ] || reject backup-current
    current_digest="${previous_path##*/}"; [ "$current_digest" = "$release" ] || reject backup-release
    [ "$managed_data_application" = "$application" ] && [ "$managed_data_component" = "$component" ] || reject backup-data-owner
    if [ "$kind" = file ]; then
      found=0
      for spec in "${managed_data_bindings[@]}"; do
        binding="${spec%%:*}"
        if [ "$binding" = "$resource" ]; then source="$(managed_data_binding_root "$binding")"; found=1; fi
      done
      [ "$found" -eq 1 ] || reject backup-binding
      create_pax_artifact "$operation" "$artifact" "$source" "$maximum"
    else
      [ "$resource" = release ] || reject backup-resource
      local -a links=()
      for spec in "${managed_data_bindings[@]}"; do
        logical="${spec#*:}"; logical="${logical%:*}"; links+=("source/$logical")
      done
...
backup_read_artifact() {
  ...
  require_backup_operation "$operation"; require_backup_artifact "$artifact"; require_digest "$expected_digest"
  ...
  prepare_backup_operation "$operation"; root="$(backup_operation_root "$operation")"; target="$root/$artifact.$extension"
...
backup_discard_operation() {
  ...
  local operation="$1" root
  require_backup_operation "$operation"; root="$(backup_operation_root "$operation")"
  if [ ! -e "$root" ] && [ ! -L "$root" ]; then printf 'DISCARDED=1\n'; return; fi
  prepare_backup_operation "$operation"
