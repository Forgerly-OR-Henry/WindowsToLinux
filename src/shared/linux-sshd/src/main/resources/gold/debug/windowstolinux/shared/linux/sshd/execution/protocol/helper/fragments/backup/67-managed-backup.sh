...
backup_sqlite_exclusions() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    target="$(managed_storage_target)"; access="$(managed_binding_access "$release/source")"
...
backup_create_artifact() {
  ...
  prepare_backup_operation "$operation"; current_application="$app"
  ...
  if [ -e "$(app_root "$app")/current/.windowstolinux-container-parameters" ]; then container_current_release "$app" "$manifest"
  ...
  if [ "$kind" = release ]; then
    ...
    for spec in "${managed_data_bindings[@]}"; do
      ...
      if [ "$binding" = "$resource" ]; then
        ...
        source="$(managed_storage_target)"; if [ "$storage_kind" = CONFIGURATION ]; then source="${source%/*}"; fi
        found=1; break
      fi
    done
    ...
    if [ "$container_engine" = podman ]; then podman save --format oci-archive --output "$target" "$image"
