...
application_assert_inputs() {
    ...
    while [ "$index" -lt "${#application_input_sources[@]}" ]; do
        source="${application_input_sources[$index]}"
        target="${application_input_targets[$index]}"
        ...
        for current in "$source" "$target"; do
            while [ "$current" != / ]; do
                [ ! -L "$current" ] || reject application-input-symlink
                current="${current%/*}"
                [ -n "$current" ] || current=/
            done
        done
        ...
        for binding in "${managed_data_bindings[@]}"; do
            managed_binding_parts "$binding"
            target_root="$(managed_data_binding_root "$binding")"
...
application_argv() {
    ...
    if [ -z "$entry" ]; then
        ...
        case "$kind" in
            ...
            java | javasource | springboot | kotlin)
                [[ "$entry" = *.jar ]] || reject application-java-entry
                application_exec=("$toolchain_java" -jar "$path")
                ;;
            go | rust | cmake | process)
                [ "${application_rendering:-0}" = 1 ] || [ -x "$path" ] || reject application-entry-not-executable
                application_exec=("$path")
                ;;
...
render_application_command() {
    ...
    if [ -n "$application_entry" ]; then
        ...
        case "$kind" in
            ...
            java | javasource | springboot | kotlin)
                [[ "$application_entry" = *.jar ]] || reject application-java-entry
                command="$toolchain_java -jar"
                ;;
            go | rust | cmake | process) command= ;;
            *) reject application-entry-kind ;;
        esac
...
PY
    ...
    program="$(
        base64 -w0 <<'WTL_WORKER_SUPERVISOR'
...
application_load_current() {
    ...
    root="$(app_root "$app")"
    application_release="$(readlink -f -- "$root/current")"
    case "$application_release" in "$root/releases/"*) ;; *) reject application-current-path ;; esac
    require_digest "${application_release##*/}"
    assert_root_owned_directory "$application_release"
    ...
    if [ -e "$application_release/.windowstolinux-container-parameters" ]; then
        load_container_parameters "$application_release/.windowstolinux-container-parameters"
        application_backend=container
        container_current_release "$app" "$manifest"
    else
        load_deployment_parameters "$application_release/.windowstolinux-deployment-parameters"
        application_backend=native
...
application_job() {
    ...
    local app="$1" manifest="$2" operation="$3" status=0
    shift 3
    [ "${1:-}" = -- ] || reject application-run-argument-separator
    shift
    ...
    application_load_current "$app" "$manifest"
    case "$operation" in
    ...
    if [ "$application_backend" = container ]; then
        run_application_container "$app" "$operation" "$@"
        return
    fi
...
application_assert_no_orphan() {
    ...
    [ ! -e "$record" ] || {
        ...
        if [[ "$first" = windowstolinux-job-*.service ]]; then
            ...
            second="${job[1]:-}"
            [[ "$second" = windowstolinux-job-* ]] || reject application-job-record
...
application_maintenance_control() {
    ...
    case "$action" in
        begin)
            ...
            printf '%s\n' "$token" >"$directory/maintenance"
            chmod 600 -- "$directory/maintenance"
            ;;
