...
database_pgpass() {
  ...
  slash="$(printf '\134')"
  escaped="${password//${slash}/${slash}${slash}}"; escaped="${escaped//:/${slash}:}"
...
database_mysql_defaults() {
  ...
  password="$(cat -- "$password_path")"
  case "$password" in *$'\n'*) reject database-password-newline ;; esac
  slash="$(printf '\134')"; quote='"'
  escaped="${password//${slash}/${slash}${slash}}"; escaped="${escaped//${quote}/${slash}${quote}}"
...
database_server_arguments() {
  ...
  database_type="$1"; database_host="$2"; database_port="$3"; database_name="$4"
  database_username="$5"; database_secret_identifier="$6"; database_secret_revision="$7"; database_tls="$8"
  require_database_type "$database_type"; [ "$database_type" != sqlite ] || reject database-server-type
  require_database_host "$database_host"; require_service_port "$database_port"
  require_database_name "$database_name"; require_database_name "$database_username"
  require_secret_identifier "$database_secret_identifier"; require_revision "$database_secret_revision"
    ...
    if [ "$type" = postgresql ]; then
      if command -v psql >/dev/null 2>&1 && command -v pg_dump >/dev/null 2>&1; then
        ...
        if ! engine="$(PGPASSFILE="$pgpass" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname="$database_name" -Atc 'SHOW server_version' 2>/dev/null)"; then
          rm -f -- "$pgpass"; reject database-inspect-failed
        fi
        tool="$(pg_dump --version | awk '{print $NF}')"; available=1; transactional=1
...
database_export() {
  ...
  shift 5; require_app "$app"; require_boolean "$writes_stopped"; require_boolean "$exclusive"; require_database_type "$type"
  ...
  artifact_id="db-$(cat /proc/sys/kernel/random/uuid | tr -d '-')"; require_artifact_id "$artifact_id"
  ...
  if [ "$type" = sqlite ]; then
    ...
    if [ "$type" = postgresql ]; then
      ...
      local pgpass; pgpass="$(database_pgpass "$app" "$database_host" "$database_port" "$database_name" "$database_username" "$database_secret_identifier" "$database_secret_revision")"
      if ! PGPASSFILE="$pgpass" pg_dump --no-password --format=custom --host="$database_host" --port="$database_port" --username="$database_username" --dbname="$database_name" --file="$tmp"; then
        rm -f -- "$pgpass"; reject database-export-failed
      fi
      rm -f -- "$pgpass"
    else
      ...
      defaults="$(database_mysql_defaults "$app" "$type" "$database_secret_identifier" "$database_secret_revision" "$database_tls")"
      if [ "$mode" = mysql-transaction ] && [ "$transactional" = 1 ]; then transaction_option=--single-transaction
...
database_stage_artifact() {
  ...
  require_artifact_id "$id"; require_artifact_size "$size"; require_digest "$digest"; initialise_controlled_roots
  target="$(database_artifact_path "$id")"; tmp="$(mktemp "$backups_root/.database-stage.XXXXXX")"
  trap 'rm -f -- "$tmp"' EXIT
  head -c "$((size + 1))" > "$tmp"
...
database_read_artifact() {
  ...
  require_artifact_id "$id"; require_artifact_size "$size"; require_digest "$digest"
  target="$(database_artifact_path "$id")"; assert_root_owned_regular "$target"
...
database_discard_artifact() {
  ...
  require_artifact_id "$id"; require_digest "$digest"; target="$(database_artifact_path "$id")"
  if [ -e "$target" ] || [ -L "$target" ]; then
    assert_root_owned_regular "$target"
    [ "$(sha256sum -- "$target" | awk '{print $1}')" = "$digest" ] || reject database-artifact-digest
    rm -f -- "$target"
  ...
  if [ "$type" = sqlite ]; then
    ...
    if [ "$type" = postgresql ]; then
      ...
      if ! PGPASSFILE="$pgpass" pg_restore --no-password --exit-on-error --host="$database_host" --port="$database_port" --username="$database_username" --dbname="$candidate_database" "$artifact"; then
        ...
        rm -f -- "$pgpass"; reject database-restore-failed
      fi
      if [ "$(PGPASSFILE="$pgpass" psql --no-password --host="$database_host" --port="$database_port" --username="$database_username" --dbname="$candidate_database" -Atc 'SELECT 1' 2>/dev/null)" != 1 ]; then
        PGPASSFILE="$pgpass" dropdb --if-exists --force --no-password --host="$database_host" --port="$database_port" --username="$database_username" "$candidate_database" >/dev/null 2>&1 || true
        rm -f -- "$pgpass"; reject database-restore-schema
      fi
      rm -f -- "$pgpass"
    else
