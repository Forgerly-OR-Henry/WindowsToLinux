application_container_cleanup() {
    ...
    if [ -n "${application_job_container:-}" ]; then
        ...
        containers="$("$container_engine" ps --all --format '{{.Names}}')" || {
...
run_application_container() {
    ...
    shift 2
    image="$(container_image "$app" "${application_release##*/}")"
    runtime_user="$(container_nonroot_user "$container_engine" "$image")"
    prepare_container_storage "$app" "$image" "$application_release" "$runtime_user"
    container_storage_mounts "$image"
    local -a args command_args expected_mounts=()
    case "$operation" in
        run)
            entry="$application_entry"
            command_args=("${application_args[@]}" "$@")
            ;;
        client)
            entry="$application_client_entry"
            command_args=("${application_client_args[@]}" "$@")
            ;;
        verify)
            entry="$application_verify_entry"
            command_args=("${application_verify_args[@]}")
            ;;
        probe)
            entry="$application_health_entry"
            command_args=("${application_health_args[@]}")
            ;;
    ...
    while [ "$index" -lt "${#deployment_secret_identifiers[@]}" ]; do
        ...
        args+=(--mount "type=bind,source=$secret,destination=$destination,readonly" --env "$secret_name=$destination")
        expected_mounts+=("$secret:$destination:ro")
        index=$((index + 1))
    done
    args+=("$image" "${command_args[@]}")
    "${args[@]}" >/dev/null || reject application-container-create
