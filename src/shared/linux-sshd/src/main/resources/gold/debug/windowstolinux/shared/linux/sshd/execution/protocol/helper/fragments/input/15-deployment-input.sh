...
stage_configuration() {
  ...
  require_app "$app"; require_digest "$configuration_digest"; require_digest "$payload_digest"; require_payload_size "$payload_size"
...
parse_deployment_inputs() {
  ...
  while [ "$secret_count" -gt 0 ]; do
    ...
    identifier="$1"; revision="$2"; digest="$3"; shift 3
    require_secret_identifier "$identifier"; require_revision "$revision"; require_digest "$digest"
    ...
    deployment_secret_identifiers+=("$identifier"); deployment_secret_revisions+=("$revision")
    deployment_secret_digests+=("$digest"); deployment_secret_names+=("$name")
