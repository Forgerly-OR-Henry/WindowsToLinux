...
database_sqlite_arguments() {
    ...
    local app="$1"
    database_sqlite_binding="$2"
    database_sqlite_location_type="$3"
    database_sqlite_location_path="$4"
    database_sqlite_file="$5"
    require_app "$app"
    require_app "$database_sqlite_binding"
...
database_source_path() {
    ...
    database_sqlite_arguments "$@"
    expected="$database_sqlite_target"
    ...
    if [ -e "$root/.windowstolinux-container-parameters" ]; then
        load_container_parameters "$root/.windowstolinux-container-parameters"
    ...
    for spec in "${managed_data_bindings[@]}"; do
        ...
        if [ "$binding" = "$database_sqlite_binding" ] && [ "$storage_kind" = DATABASE ]; then
            [ "$(managed_storage_target)" = "$expected" ] || reject database-binding-differs
            found=1
        fi
    done
    [ "$found" = 1 ] || reject database-binding-missing
    assert_sqlite_file "$expected"
    printf '%s' "$expected"
}
