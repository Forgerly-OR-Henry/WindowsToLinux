parse_toolchain_binding() {
    ...
    if [ "${1:-}" = tools-v1 ]; then
        ...
        toolchain_binding="$2"
        shift 2
        require_digest "$toolchain_binding"
        ...
        assert_root_owned_directory "$registry"
        assert_root_owned_directory "$registry/bindings"
...
render_ecosystem_runtime_command() {
    ...
    case "$kind" in
        process)
            [ "$#" -eq 0 ] && [ -n "$application_entry" ] || reject process-entry-required
            ecosystem_runtime_command_result="$(application_systemd_escape "$root/current/source/$application_entry")"
            ;;
        go)
        ...
        php)
            ...
            [[ "$3" = *.php ]] || reject php-entrypoint
            if [ "$4" = 0 ]; then
                [ "$2" = source ] || reject php-artifact
                ecosystem_runtime_command_result="$toolchain_php $root/current/source/$3"
                return
            fi
            ...
            require_service_port "$php_port"
            [ "$php_bind" = 0.0.0.0 ] || [ "$php_bind" = 127.0.0.1 ] || reject runtime-bind-address
        ...
        phpcli)
            ...
            require_service_port "$phpcli_port"
            [ "$phpcli_bind" = 0.0.0.0 ] || [ "$phpcli_bind" = 127.0.0.1 ] || reject runtime-bind-address
        ...
        ruby)
            ...
            if [ "$4" = 0 ]; then
                ...
                ecosystem_runtime_command_result="/usr/bin/env PATH=$root/current/source/.w2l/bundler/bin:$toolchain_path GEM_HOME=$root/current/source/.w2l/bundler GEM_PATH=$root/current/source/.w2l/bundler: bundle exec $toolchain_ruby $root/current/source/$3"
                return
            fi
            ...
            require_service_port "$ruby_port"
            [ "$ruby_bind" = 0.0.0.0 ] || [ "$ruby_bind" = 127.0.0.1 ] || reject runtime-bind-address
