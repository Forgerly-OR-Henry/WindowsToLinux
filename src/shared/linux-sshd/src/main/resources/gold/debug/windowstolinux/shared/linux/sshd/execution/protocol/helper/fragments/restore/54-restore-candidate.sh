...
restore_prepare_component() {
  ...
  require_count "$1"; local port_count="$1"; shift; local -a ports=(); local official selected protocol
  while [ "$port_count" -gt 0 ]; do [ "$#" -ge 3 ] || reject restore-ports
    protocol="$1"; official="$2"; selected="$3"; shift 3; case "$protocol" in tcp|udp) ;; *) reject restore-port-protocol ;; esac; require_service_port "$official"; require_service_port "$selected"
    [ "$selected" -ge 49152 ] && [ "$official" != "$selected" ] || reject restore-ports
    ports+=("$protocol:$official:$selected"); port_count=$((port_count-1)); done
...
restore_view_path() {
  ...
  installation="$(app_root "$app")"; data="$data_root/$app"; configuration="$configurations_root/$app"
  case "$path" in
...
restore_binding_link() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if [ "$storage_kind" = FILE ] && [[ "$wanted" = "$logical/"* ]]; then
      printf '%s/data/%s/%s' "$root" "$binding" "${wanted#"$logical"/}"; return
    fi
  done
  if [[ "$wanted" = /* ]]; then restore_view_path "$root" "$app" "$wanted"
...
restore_prepare_native_view() {
  ...
  root="$(restore_component_root "$candidate" "$component")"; mapfile -t saved_identity < "$root/identity"; app="${saved_identity[0]}"
  identity="$(restore_candidate_identity "$token" "$component")"; prepare_service_identity "$identity"; owner="$(service_identity_name "$identity")"
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; target="$(managed_storage_target)"; directory="$(managed_data_binding_root "$binding")"
    if [ "$storage_kind" = DATABASE ]; then
      ...
      source="$(candidate_root "$candidate")/database/$binding"; assert_sqlite_file "$source/$database_file"
      chown -hR "$owner:$owner" -- "$source"; chmod -R u+rwX,go-rwx -- "$source"
      ...
      if [ "$mode" = ro ]; then chown -hR root:"$owner" -- "$source"; chmod -R u=rX,g=rX,o= -- "$source"
      else chown -hR "$owner:$owner" -- "$source"; chmod -R u+rwX,go-rwx -- "$source"; fi
    fi
    if [ "$storage_kind" = CONFIGURATION ]; then
      placeholder="$(restore_view_path "$root" "$app" "$target")"; install -d -o root -g root -m 755 -- "${placeholder%/*}"; : > "$placeholder"
      printf 'BindReadOnlyPaths=%s:%s\n' "$source" "$target" >> "$root/storage-unit"
    else
      placeholder="$(restore_view_path "$root" "$app" "$directory")"; install -d -o root -g root -m 755 -- "$placeholder"
      if [ "$mode" = ro ]; then printf 'BindReadOnlyPaths=%s:%s\n' "$source" "$directory"
...
restore_start_container_candidate() {
  ...
  if [ -z "${mappings[0]:-}" ]; then mappings=(); if [ "${#application_endpoints[@]}" -gt 0 ]; then assert_root_owned_regular "$root/previous"; fi; fi
  ...
  local runtime_user; runtime_user="$(container_nonroot_user "$container_engine" "$image")"
  ...
  for spec in "${container_ports[@]}"; do
    ...
    target="${mapping##*:}"; mapping="${mapping%:*}"; host="${mapping##*:}"; selected="$host"
    for candidate_mapping in "${mappings[@]}"; do
      if [[ "$candidate_mapping" = "$protocol:$host:"* ]]; then selected="${candidate_mapping##*:}"; found=1; break; fi
    done
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; mount="$(container_binding_mount "$image")"; destination="${mount#*:}"; destination="${destination%:*}"
    if [ "$storage_kind" = DATABASE ]; then
      assert_root_owned_regular "$(candidate_root "$candidate")/database/sqlite-binding"
      [ "$(cat -- "$(candidate_root "$candidate")/database/sqlite-binding")" = "$(printf '%s\n' "$app" "$binding" "$location_type" "$location_path" "$database_file")" ] || reject restore-database-binding
      source="$(candidate_root "$candidate")/database/$binding"; assert_sqlite_file "$source/$database_file"
      printf '%s\n' "$runtime_user" > "$(candidate_root "$candidate")/database/sqlite-owner"; chmod 400 -- "$(candidate_root "$candidate")/database/sqlite-owner"
    elif [ "$storage_kind" = CONFIGURATION ]; then
      source="$root/data/$binding/value"; assert_root_owned_regular "$source"
      [ "$(sha256sum -- "$source" | awk '{print $1}')" = "$configuration_sha256" ] || reject restore-configuration-digest
    else source="$root/data/$binding"; (assert_storage_tree "$source"); fi
    if [ "$mode" = ro ]; then chown -hR root:"${runtime_user#*:}" -- "$source"; chmod -R u=rX,g=rX,o= -- "$source"
    else chown -hR "$runtime_user" -- "$source"; chmod -R u+rwX,go-rwx -- "$source"; fi
    args+=(--volume "$source:$destination:$mode,Z"); expected_mounts+=("$source:$destination:$mode")
  done
  for mount in "${container_bind_mounts[@]}"; do
    source="${mount%%:*}"
    if [[ "$source" = "$container_storage_view_root/"* ]]; then args+=(--volume "$mount,Z"); expected_mounts+=("$mount"); fi
  done
  index=0
  while [ "$index" -lt "${#deployment_secret_identifiers[@]}" ]; do
    ...
    args+=(--mount "type=bind,source=$secret,destination=$secret_destination,readonly" --env "$secret_name=$secret_destination"); expected_mounts+=("$secret:$secret_destination:ro"); index=$((index+1))
  done
  application_backend=container; application_release="$root/release"
  ...
  if [ "$application_mode" = ON_DEMAND ]; then application_entry="$application_verify_entry"; application_args=("${application_verify_args[@]}"); fi
  application_container_options "$image"; args+=("${application_container_arguments[@]}" "$image" "${application_container_command[@]}")
