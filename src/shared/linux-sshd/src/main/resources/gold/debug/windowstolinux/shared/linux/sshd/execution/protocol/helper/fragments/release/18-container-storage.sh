container_binding_destination() {
  ...
  require_storage_path "$path"; [[ "$path" = /* ]] || reject container-storage-access
...
container_binding_mount() {
  ...
  destination="$(container_binding_destination "$image")"; source="$(managed_storage_target)"
  if [ "$storage_kind" = DATABASE ]; then
    [ "${destination##*/}" = "$database_file" ] || reject container-database-file-name
    source="${source%/*}"; destination="/run/windowstolinux/databases/$binding"
  fi
...
container_database_link() (
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if [[ "$wanted" = "$parent/"* ]]; then
      ...
      printf '%s/%s' "$source" "${wanted#"$parent"/}"; return
    fi
  done
...
container_storage_mounts() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; resolved="$(container_binding_destination "$image")"
    for saved in "${resolved_access[@]}"; do
      prior_kind="${saved%%:*}"; other="${saved#*:}"
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; mount="$(container_binding_mount "$image")"
    destination="${mount#*:}"; destination="${destination%:*}"
    for previous in "${container_bind_mounts[@]}"; do
      other="${previous#*:}"; other="${other%:*}"
      [ "$destination" != "$other" ] || reject container-storage-overlap
    done
    container_bind_mounts+=("$mount")
    if [ "$storage_kind" = DATABASE ]; then
      link="$(container_database_link "$image")"; view="$(container_view_root "$image")"
      if [[ "$link" = "$view/"* ]]; then
        parent="$(container_binding_destination "$image")"; parent="${parent%/*}"
...
prepare_container_storage() (
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if container_storage_needs_image_seed "$directory"; then
      if [ -z "$cid" ]; then
        ...
        (ulimit -f 8388608; "$container_engine" export --output "$temporary/image.tar" "$cid") || reject container-seed-export
      fi
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if [ "$storage_kind" != CONFIGURATION ]; then
      ...
      if [ "$mode" = ro ]; then chown -hR root:"${user#*:}" -- "$directory"; chmod -R u=rX,g=rX,o= -- "$directory"
      else chown -hR "$user" -- "$directory"; chmod -R u+rwX,go-rwx -- "$directory"; fi
    fi
  done
)
assert_container_bind_mounts() {
  ...
  while IFS=: read -r type source destination mode; do
    ...
    for mount in "${container_bind_mounts[@]}"; do
      row="${mount%:*}"; if [ "${mount##*:}" = ro ]; then row="$row:false"; else row="$row:true"; fi
...
prepare_container_database_links() (
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; [ "$storage_kind" = DATABASE ] || continue
    link="$(container_database_link "$image")"; expected="/run/windowstolinux/databases/$binding/$database_file"
    view="$(container_view_root "$image")"
    if [ -L "$link" ]; then [ "$(readlink -- "$link")" = "$expected" ] || reject container-database-alias; continue; fi
    if [[ "$link" = "$view/"* ]] && [ ! -d "${link%/*}" ]; then
      ...
      destination="$(container_binding_destination "$image")"; destination="${destination%/*}"
      ...
      chown -hR root:root -- "${link%/*}"; chmod -R u=rX,go=rX -- "${link%/*}"
    fi
    ...
    if [ -e "$link" ]; then [ -f "$link" ] && [ ! -L "$link" ] && [ "$(stat -c %h -- "$link")" = 1 ] || reject container-database-alias; rm -f -- "$link"; fi
