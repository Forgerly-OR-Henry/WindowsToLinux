...
application_container_argv() {
    ...
    local image="$1" entry="$2" metadata encoded
    shift 2
    if [ -n "$entry" ]; then
        application_exec=("$(container_binding_destination "$image" "$entry")")
    else
...
application_container_candidate_probe() {
    ...
    if [ "$operation" = verify ]; then
        ...
        args=("${application_verify_args[@]}")
    else
        entry="$application_health_entry"
        args=("${application_health_args[@]}")
    fi
