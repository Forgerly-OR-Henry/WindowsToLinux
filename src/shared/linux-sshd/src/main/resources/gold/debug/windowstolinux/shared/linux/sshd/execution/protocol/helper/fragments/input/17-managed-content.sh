...
managed_storage_directory() {
  ...
  require_app "$app"; require_app "$id"; root="$(app_root "$app")"
  ...
  case "$choice" in
    DEFAULT)
      ...
      if [ "$kind" = CONFIGURATION ]; then printf '%s/%s/files/%s' "$configurations_root" "$app" "$id"
    ...
    CUSTOM)
      ...
      if [[ "$requested" = /* ]]; then
        ...
        first="${requested#"$root"/}"; first="${first%%/*}"
...
parse_managed_data_bindings() {
  ...
  require_app "$managed_data_application"; require_app "$managed_data_component"; require_app "$current_application"
  require_count "$1"; local remaining="$1" existing target other saved item; shift
  managed_data_bindings=()
  while [ "$remaining" -gt 0 ]; do
    [ "$#" -ge 10 ] || reject managed-data-bindings
    binding="$1"; logical="$2"; mode="$3"; storage_kind="$4"; location_type="$5"; location_path="$6"; database_file="$7"; seed_file="$8"; initialization_files="$9"; configuration_sha256="${10}"; shift 10
    require_app "$binding"; require_storage_path "$logical"
    [ "$mode" = ro ] || [ "$mode" = rw ] || reject managed-data-mode
    if [ "$seed_file" != - ]; then require_storage_path "$seed_file"; [[ "$seed_file" != /* ]] || reject managed-storage-seed; fi
    ...
    if [ "$storage_kind" = DATABASE ]; then
      ...
      if [ "$initialization_files" != - ]; then
        local -a sql_files; IFS=, read -r -a sql_files <<< "$initialization_files"
        [ "${#sql_files[@]}" -le 32 ] || reject managed-database-initialization
        for item in "${sql_files[@]}"; do require_storage_path "$item"; [[ "$item" != /* && "$item" = *.sql ]] || reject managed-database-initialization; done
      fi
    ...
    managed_data_bindings+=("$saved"); remaining=$((remaining-1))
  done
...
managed_data_binding_root() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if [ "$binding" = "$requested" ]; then
      managed_storage_directory "$current_application" "$storage_kind" "$binding" "$location_type" "$location_path"; return
    fi
  done
  reject managed-data-binding-missing
}
managed_storage_target() {
  local target; target="$(managed_data_binding_root "$binding")"
...
managed_binding_access() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    ...
    if [ "$storage_kind" = FILE ] && [[ "$wanted" = "$logical/"* ]]; then
      parent="$(managed_data_binding_root "$binding")"; printf '%s/%s' "$parent" "${wanted#"$logical"/}"; return
    fi
  done
...
assert_storage_tree() {
  ...
  while IFS= read -r -d '' link; do
    ...
    for spec in "${managed_data_bindings[@]}"; do
      ...
      expected="$(managed_storage_target)"; access="$(managed_binding_access "${storage_source_root:-/nonexistent}")"
      ...
      if [ "$original/${link#"$target"/}" = "$access" ] && [ "$(readlink -- "$link")" = "$expected" ]; then allowed=1; break; fi
    done
...
assert_managed_data_links() {
  ...
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; directory="$(managed_data_binding_root "$binding")"; assert_storage_parent "$directory"
    target="$(managed_storage_target)"; link="$(managed_binding_access "$source_root")"
    [ ! -L "$target" ] || reject managed-data-target-link
    if [ "$storage_kind" = FILE ]; then (assert_storage_tree "$target")
    ...
    if [ "$storage_kind" = CONFIGURATION ]; then
      assert_root_owned_regular "$target"; [ "$(sha256sum -- "$target" | awk '{print $1}')" = "$configuration_sha256" ] || reject managed-configuration-digest
    fi
  done
}
record_storage_binding() {
  ...
  if [ -e "$record" ] || [ -L "$record" ]; then
    assert_root_owned_regular "$record"; [ "$(cat -- "$record")" = "$expected" ] || reject managed-storage-location-changed
  else
    [ ! -e "$target" ] && [ ! -L "$target" ] || reject managed-storage-unowned-target
    install -d -o root -g root -m 700 -- "${record%/*}"
    printf '%s\n' "$expected" > "$record"; chmod 400 -- "$record"
  fi
}
link_managed_storage() {
  ...
  if [[ "$link" = "$source_root/"* ]]; then
    if [ -e "$link" ] || [ -L "$link" ]; then
      if [ -L "$link" ]; then
        existing="$(readlink -- "$link")"; [ "$existing" = "$target" ] || reject managed-data-alias-changed
...
prepare_managed_data_bindings() {
  ...
  owner="$(service_identity_name "$current_application")"; group="$owner"
  for spec in "${managed_data_bindings[@]}"; do
    managed_binding_parts "$spec"; directory="$(managed_data_binding_root "$binding")"
    target="$(managed_storage_target)"; link="$(managed_binding_access "$source_root")"
    assert_storage_parent "$directory"; record_storage_binding "$directory"
    install -d -o root -g root -m 755 -- "${directory%/*}"
    if [ "$storage_kind" = CONFIGURATION ]; then
      if [ ! -e "$target" ] && [ ! -L "$target" ]; then
        seed="$source_root/$seed_file"; assert_root_owned_regular "$seed"
      ...
      if [ ! -e "$directory" ]; then
        if [ "$storage_kind" = FILE ] && [ "$seed_file" != - ]; then
          seed="$source_root/$seed_file"; [ "$(realpath -m -- "$seed")" = "$seed" ] || reject managed-storage-seed-link
      ...
      if [ "$mode" = ro ]; then chown -hR root:"$group" -- "$directory"; chmod -R u=rX,g=rX,o= -- "$directory"
...
initialize_managed_file_tree() (
  ...
  chown -hR "$owner:$owner" -- "$temporary"; chmod -R u+rwX,go-rwx -- "$temporary"
...
initialize_managed_sqlite() (
  ...
  if [ "$seed_file" != - ]; then
    ...
    for suffix in -wal -shm -journal; do
      if [ -e "$seed$suffix" ] || [ -L "$seed$suffix" ]; then
        assert_root_owned_regular "$seed$suffix"; [ "$(stat -c %h -- "$seed$suffix")" = 1 ] || reject database-seed-link
      fi
    done
    sqlite3 -readonly -- "$seed" ".backup '$temporary/value'" || { rm -rf --one-file-system -- "$temporary"; reject database-seed-invalid; }
    chown "$owner:$owner" -- "$temporary/value"; chmod 600 -- "$temporary/value"
  fi
  input="$temporary/input.sql"; printf '.bail on\n' > "$input"
  if [ "$initialization_files" != - ]; then
    local -a sql_files; IFS=, read -r -a sql_files <<< "$initialization_files"
    for file in "${sql_files[@]}"; do
      seed="$source_root/$file"; assert_root_owned_regular "$seed"
      ...
      cat -- "$seed" >> "$input"; printf '\n' >> "$input"
    done
  fi
  printf 'PRAGMA integrity_check;\n' >> "$input"; chown root:"$owner" -- "$input"; chmod 440 -- "$input"
  if ! systemd-run --quiet --wait --pipe --collect --property="User=$owner" --property="Group=$owner" \
    ...
    rm -rf --one-file-system -- "$temporary"; reject database-initialization-failed
  fi
  [ "$(sqlite3 -- "$temporary/value" 'PRAGMA integrity_check;' 2>/dev/null)" = ok ] || { rm -rf --one-file-system -- "$temporary"; reject database-sqlite-integrity; }
  ...
  chown "$owner:$owner" -- "$temporary/value"; chmod 600 -- "$temporary/value"
  mv -T -- "$temporary/value" "$target"; rm -rf --one-file-system -- "$temporary"
)
