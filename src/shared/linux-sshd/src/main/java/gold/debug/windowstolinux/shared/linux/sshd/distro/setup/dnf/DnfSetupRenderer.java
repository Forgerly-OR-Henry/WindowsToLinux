package gold.debug.windowstolinux.shared.linux.sshd.distro.setup.dnf;
...
public final class DnfSetupRenderer {
    ...
    public static String render(DistributionSetupProfile profile, String username) {
        username = SetupShellSupport.requireUsername(username);
        String packages = String.join(" ", profile.packages());
        String variantCheck = profile.variant().isEmpty() ? ":"
                : "test \"${VARIANT_ID:-}\" = " + SetupShellSupport.quote(profile.variant());
        String preflight = """
                set -euo pipefail
                export LC_ALL=C
                %s
                ...
                """.formatted(SetupShellSupport.renderStageDiagnostics(),
                SetupShellSupport.quote(profile.id()), variantCheck,
                SetupShellSupport.quote(profile.version()),
                SetupShellSupport.quote(profile.packageArchitecture()),
                SetupShellSupport.renderCpuCheck(profile.requiredCpu()),
                SetupShellSupport.quote(username));
        String install = """
        ...
        return preflight
                ...
                + SetupShellSupport.renderSecurityObservationFunctions()
                + "prepare_stage=package-install\n"
                + install
                + "prepare_stage=post-install-checks\n"
                + SetupShellSupport.renderJava21RuntimeInstallation()
                + renderCommonChecks()
                + profile.capabilityChecks().renderChecks()
                + """
                ...
                + SetupShellSupport.renderHelperInstallation(username)
                + "prepare_stage=security-invariant\n"
                + SetupShellSupport.renderSecurityInvariant()
                + """
                printf 'PREPARED_AS=%%s\\n' "$elevation"
                printf 'PACKAGES=%s\\n'
                printf 'SUDOERS=%s\\n'
                printf 'HELPER=%s\\n'
                """.formatted(packages, SetupShellSupport.SUDOERS_PATH,
