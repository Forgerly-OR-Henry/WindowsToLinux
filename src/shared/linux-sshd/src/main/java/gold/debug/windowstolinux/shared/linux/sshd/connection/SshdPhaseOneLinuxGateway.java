...
/**
 * Apache MINA SSHD implementation of the phase-one allowlisted remote contract. It accepts a host key only through the supplied verifier and never exposes a public raw-command method.
 *
 * <p>一期白名单远程契约的 Apache MINA SSHD 实现。它只通过提供的验证器接受主机密钥，并且绝不公开原始命令方法。
 */
public final class SshdPhaseOneLinuxGateway implements PhaseOneLinuxGateway {
    ...
    public PhaseOneRemoteSession connect(SshEndpoint endpoint, SshCredential credential, HostKeyVerifier hostKeyVerifier)
        ...
        try {
            ...
            return new SshdPhaseOneRemoteSession(client, session, endpoint, observedFingerprint.get());
        } catch (Exception exception) {
            closeQuietly(client);
