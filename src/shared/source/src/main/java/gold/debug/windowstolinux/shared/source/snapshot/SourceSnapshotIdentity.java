...
public final class SourceSnapshotIdentity {
    ...
    public static String digest(Path directory) throws Exception {
        var validator = new SourceBoundaryValidator();
        Path root = validator.validateSourceDirectory(directory);
        ...
        for (var entry : manifest.entries()) {
            ...
            digest.update((entry.relativePath().length() + ":" + entry.relativePath() + ":" + entry.byteCount() + ":")
                    .getBytes(StandardCharsets.UTF_8));
            try (var stream = Files.newInputStream(entry.path(), LinkOption.NOFOLLOW_LINKS)) {
                int n;
