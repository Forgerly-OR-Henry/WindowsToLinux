package gold.debug.windowstolinux.shared.standard.analyze.contract.policy;
...
public final class SourceMutationPolicy {
    ...
    private static final Pattern DATABASE_MIGRATION = Pattern.compile(
            "\\b(flyway|liquibase|alembic|prisma(?:\\s+migrate)?|knex)\\b"
                    ...
                    + "|(?:spring\\.sql\\.init\\.mode|initialization-mode|sql:\\s+init:\\s+mode)\\s*[:=]\\s*always\\b",
    ...
    public void validate(SourceInspectionFacts source, List<RejectionReason> rejections,
        ...
        if (review.filter(value -> value.inspectionSha256().equals(inspectionDigest(source)))
                .filter(value -> !DATABASE_MIGRATION.matcher(source.scannedText()).find()
                        || value.frameworkMigrationsDisabled())
                .filter(value -> value.existingSchemaChangeApproved()
                        || source.relativeFiles().stream().filter(SourceMutationPolicy::databaseChangePath).allMatch(
                                path -> value.initializedSqlPaths().contains(path.toString().replace('\\', '/'))))
                .isPresent())
            return;
        if (source.relativeFiles().stream().anyMatch(SourceMutationPolicy::databaseChangePath)) {
            rejections
                    .add(rejection("AUTOMATIC_SCHEMA_MUTATION_DETECTED", "analysis.rejection.schemaMutationDetected"));
        }
