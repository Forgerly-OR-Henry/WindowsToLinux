...
public final class ServiceInspectionAssembler {
     ...
     */
    public static DeploymentTypeAssessment assemble(Path root, DeploymentProjectType projectType,
            DeploymentBuildToolType buildTool, ProjectLanguageFacts languageFacts, ServiceProjectFacts shape,
            boolean requiresServicePort) {
        List<LocalizedMessage> missing = new ArrayList<>();
        shape.missingFiles()
                .forEach(file -> missing.add(LocalizedMessage.of("analysis.service.missingFile", "file", file)));
        ...
        DeploymentProjectFacts facts = new DeploymentProjectFacts(root, ProjectIdentityResolver.rootApplicationId(root),
                projectType, buildTool, languageFacts, List.of(evidence("analysis.service.metadata",
                        shape.primaryMetadata(), "analysis.deployment.evidence.detected")),
                List.of(), missing);
