...
public final class ManagedToolchainPreparer {
     ...
     */
    public ManagedToolchainPreparer(RemoteCommandExecutor commands, ToolchainSupportCatalog catalog) {
    ...
    public ResolvedToolchainSet prepare(List<ToolchainRequirement> requirements, int attempt, int timeoutSeconds)
        ...
        if (attempt < 0 || attempt > 1)
            throw new IllegalArgumentException("only two toolchain attempts are allowed");
        List<ResolvedToolchainSet.Selection> result = new ArrayList<>();
        for (var requirement : requirements) {
            var candidates = catalog.candidates(requirement);
            if (candidates.isEmpty())
                throw unsupported("No catalog candidate: " + requirement.ecosystem() + " " + requirement.declaration());
            var branch = candidates.get(Math.min(attempt, candidates.size() - 1));
            if (branch.installation() == ToolchainSupportCatalog.InstallationType.SYSTEM_COMPILER)
                continue;
            ...
            var response = commands.exec(command(List.of("prepare", requirement.ecosystem().name(), branch.version(),
                    exact, Integer.toString(timeout)), result), Duration.ofSeconds(timeout + 15L), true);
            var fields = gold.debug.windowstolinux.shared.linux.command.CommandText.lines(response.output());
            ...
            try {
                ...
                if (f.length != 6 || !f[0].equals(requirement.ecosystem().name()))
                    throw new IllegalArgumentException();
                var version = ToolchainVersion.parse(requirement.ecosystem(), f[1]).orElseThrow();
                if (requirement.constraint() == ToolchainRequirement.ConstraintType.MINIMUM && catalog.permits(version)
                        && version.branch().equals(branch.version())
                        && version.compareTo(requirement.version().orElseThrow()) < 0) {
                    if (attempt == 0 && candidates.size() > 1)
                        return prepare(requirements, 1, timeoutSeconds);
        ...
        if (!result.isEmpty()) {
            String encoded = Base64.getEncoder()
                    .encodeToString(ToolchainBindingCodec.encode(set).getBytes(StandardCharsets.UTF_8));
            var bound = commands.exec(command(List.of("bind", encoded), result), Duration.ofMinutes(3), true);
            if (!bound.succeeded() || !ToolchainBindingCodec.identity(set)
                    .equals(gold.debug.windowstolinux.shared.linux.command.CommandText.lines(bound.output())
                            .get("TOOLCHAIN_BINDING")))
                throw LinuxOperationException.create(LinuxOperationFailureType.ENVIRONMENT_PREPARATION_FAILED,
    ...
    static String command(List<String> arguments, List<ResolvedToolchainSet.Selection> prepared) {
        ...
        if (arguments.getFirst().equals("prepare"))
            values.add(javaHome.isEmpty() ? "-" : javaHome);
        String prefix = "sudo -n "
                ...
                + " prepare-toolchains";
        return prefix + " " + values.stream().map(gold.debug.windowstolinux.shared.linux.command.CommandText::quote)
