 ...
 */
public record ReviewedDeploymentRequest(ServerIdentity server, DeploymentProjectFacts facts,
        SourceRevision sourceRevision, SourceArchiveDescriptor archive, ConfigurationSnapshot configuration,
        ...
        DeploymentRuntimeSpecification runtime, Optional<UserAccessUrl> userAccessUrl, BuildLimitConfiguration limits,
        DeploymentApproval approval, boolean containerDaemonRiskAccepted, boolean experimentalAdapterRiskAccepted) {
    /**
    ...
    public ReviewedDeploymentRequest {
        ...
        if (runtime instanceof DeploymentRuntimeSpecification.Container container) {
            ...
            for (var volume : container.volumes()) {
                if (mutable.stream().noneMatch(binding -> binding.bindingId().equals(volume.bindingId())))
                    mutable.add(new gold.debug.windowstolinux.shared.config.resource.ManagedFileBinding(
                            volume.bindingId(),
                            new gold.debug.windowstolinux.shared.model.project.component.ComponentDataPath(
            ...
            for (var volume : container.volumes()) {
                var binding = mutable.stream().filter(file -> file.bindingId().equals(volume.bindingId())).findFirst()
                        .orElseThrow();
                if (!binding.dataPath().path().equals(volume.containerPath()) || binding
                        .resourceType() != gold.debug.windowstolinux.shared.model.managed.ManagedStorageLocation.StorageResourceType.FILE
                        || (binding.dataPath()
                                .access() == gold.debug.windowstolinux.shared.model.project.component.ComponentDataPath.AccessMode.READ_ONLY) != volume
                                        .readOnly())
        ...
        gold.debug.windowstolinux.shared.config.resource.ManagedStoragePlan.resolve(facts.applicationId(),
                new gold.debug.windowstolinux.shared.config.resource.ManagedComponentResourceBindings(fileBindings,
                        databaseBindings),
                runtime);
        ...
        if (!facts.readyForPlanning()) {
            throw new IllegalArgumentException(
                    "typed deployment requests require complete deterministic project facts");
        }
        ...
        new gold.debug.windowstolinux.shared.model.managed.ManagedApplicationRuntimeConfiguration(runtime.healthCheck(),
                userAccessUrl);
        if (!facts.applicationId().equals(configuration.applicationId())
            ...
            throw new IllegalArgumentException(
                    "application, configuration, server, and approval identities must match");
        }
        ...
        if (limits.runAsRoot() || approval.rootBuildAccepted()) {
            throw new IllegalArgumentException(
                    "Root builds are no longer supported; root manages the project and a restricted identity builds it");
        }
        if (runtime instanceof DeploymentRuntimeSpecification.Container container
        ...
        if (databaseBindings.isPresent() && databaseBindings.orElseThrow().stream()
            ...
            throw new IllegalArgumentException(
                    "server database password references must belong to the reviewed deployment");
        }
    }
    ...
    public ReviewedDeploymentRequest(ServerIdentity server, DeploymentProjectFacts facts, SourceRevision sourceRevision,
            SourceArchiveDescriptor archive, ConfigurationSnapshot configuration,
            List<SecretReference> secretReferences, Optional<List<ManagedDatabaseBinding>> databaseBindings,
            DeploymentRuntimeSpecification runtime, Optional<UserAccessUrl> userAccessUrl,
            BuildLimitConfiguration limits, DeploymentApproval approval, boolean containerDaemonRiskAccepted,
            boolean experimentalAdapterRiskAccepted) {
        this(server, facts, sourceRevision, archive, configuration, secretReferences, databaseBindings, List.of(),
                runtime, userAccessUrl, limits, approval, containerDaemonRiskAccepted, experimentalAdapterRiskAccepted);
    }
     ...
     */
    public ReviewedDeploymentRequest withFiles(
            List<gold.debug.windowstolinux.shared.config.resource.ManagedFileBinding> files) {
        return new ReviewedDeploymentRequest(server, facts, sourceRevision, archive, configuration, secretReferences,
                databaseBindings, files, runtime, userAccessUrl, limits, approval, containerDaemonRiskAccepted,
