...
public final class ToolchainBuildEnvironment {
    ...
    public static String render(ResolvedToolchainSet tools) {
        ...
        for (var s : tools.selections()) {
            ...
            switch (s.version().ecosystem()) {
                ...
                case GO -> {
                    variable(script, "GOROOT", path);
                    variable(script, "GOTOOLCHAIN", "local");
                }
                case RUST -> {
                    variable(script, "RUSTC", path + "/bin/rustc");
                    variable(script, "RUSTDOC", path + "/bin/rustdoc");
                }
                default -> {
                }
            }
        ...
        for (var selection : tools.selections())
            verifyExecutables(script, selection);
    ...
    private static void verifyExecutables(StringBuilder script, ResolvedToolchainSet.Selection selection) {
        ...
        for (String name : names) {
            String executable = selection.directory() + (ecosystem == ToolchainEcosystemType.DOTNET ? "/" : "/bin/")
                    + name;
            String quoted = gold.debug.windowstolinux.shared.linux.command.CommandText.quote(executable);
            script.append("if [ \"$(command -v ").append(name).append(")\" != ").append(quoted).append(" ] || [ ! -x ")
                    .append(quoted).append(" ]; then\n").append("  printf 'TOOLCHAIN_FAILURE=integrity:selected ")
                    .append(ecosystem).append(' ').append(name)
    ...
    private static void variable(StringBuilder out, String name, String value) {
        out.append("export ").append(name).append('=')
                .append(gold.debug.windowstolinux.shared.linux.command.CommandText.quote(value)).append('\n');
    }
}
