package gold.debug.windowstolinux.shared.standard.deploy.build.ecosystem.python;
...
public final class PipBuildRenderer implements DeploymentBuildRenderer {
    ...
    public Set<DeploymentBuildToolType> buildTools() {
        return Set.of(DeploymentBuildToolType.PIP_LOCKED, DeploymentBuildToolType.PYTHON_STDLIB);
    }

    /**
     * Renders pip build as text without executing the rendered command.
     * <p>渲染Pip构建为文本，不执行所渲染命令。
     *
    ...
    public String render(DeploymentProjectFacts facts, DeploymentRuntimeSpecification runtime,
        ...
        if (facts.buildTool() == DeploymentBuildToolType.PYTHON_STDLIB)
            return PythonArchitectureBuildRenderer.render(DeploymentBuildToolType.PYTHON_STDLIB, facts, runtime,
                    workspace, limits, null, "test -f ./pyproject.toml");
