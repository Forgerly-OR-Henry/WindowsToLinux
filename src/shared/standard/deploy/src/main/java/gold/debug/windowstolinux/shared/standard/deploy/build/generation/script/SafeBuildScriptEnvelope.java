...
public final class SafeBuildScriptEnvelope {
     ...
     */
    public static String wrap(DeploymentProjectFacts facts, RemoteWorkspace workspace, BuildLimitConfiguration limits,
            String command) {
        ...
        boolean container = facts
                .buildTool() == gold.debug.windowstolinux.shared.model.project.DeploymentBuildToolType.CONTAINER_BUILD;
        return jvmEnvironment(facts, limits)
                + """
                        ...
                        .formatted(shellQuote(workspace.candidateRoot()), shellQuote(mutable), shellQuote(source),
                                shellQuote(workspace.sourceSha256()), extractSource(container),
                                facts.buildDirectory().isEmpty() ? "" : "cd -- " + shellQuote(facts.buildDirectory()),
                                command, workspaceAccounting(container, limits), facts.buildTool().name());
    }
    ...
    private static String workspaceAccounting(boolean container, BuildLimitConfiguration limits) {
        ...
        if (container)
            return "printf 'BUILD_WORKSPACE_LIMIT=fixed-volume\\n'";
        return """
    ...
    private static String jvmEnvironment(DeploymentProjectFacts facts, BuildLimitConfiguration limits) {
        ...
        return "export JAVA_TOOL_OPTIONS='-Xms16m -Xmx" + Math.min(768, memory / 4) + "m -XX:MaxMetaspaceSize="
                + Math.min(384, memory / 8) + "m -XX:CompressedClassSpaceSize=" + Math.min(64, memory / 16)
