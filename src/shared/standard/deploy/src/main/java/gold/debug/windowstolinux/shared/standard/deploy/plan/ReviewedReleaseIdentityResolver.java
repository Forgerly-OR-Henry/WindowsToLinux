...
public final class ReviewedReleaseIdentityResolver {
     ...
     */
    public static String bind(String reviewedIdentity,
        ...
        try {
            ...
            update(digest, "reviewed-release-tools-v1");
            update(digest, reviewedIdentity);
    ...
    public static String from(ReviewedDeploymentRequest request) {
        try {
            ...
            request.secretReferences().stream().sorted(
                    Comparator.comparing(SecretReference::identifier).thenComparingLong(SecretReference::revision))
                    .forEach(reference -> {
                        update(digest, reference.identifier());
                        update(digest, Long.toString(reference.revision()));
                    });
            request.fileBindings().stream()
                    .sorted(Comparator
                            .comparing(gold.debug.windowstolinux.shared.config.resource.ManagedFileBinding::bindingId))
                    .forEach(binding -> {
                        update(digest, binding.bindingId());
                        update(digest, binding.dataPath().path());
                        update(digest, binding.dataPath().access().name());
                        update(digest, binding.dataPath().schemaId());
                        update(digest, Boolean.toString(binding.dataPath().reversible()));
                        update(digest, binding.resourceType().name());
                        update(digest, binding.location().type().name());
                        update(digest, binding.location().path());
                        update(digest, binding.seedFile());
                        update(digest, binding.contentSha256());
                    });
        ...
        try {
            digest.update(
                    new gold.debug.windowstolinux.shared.config.persistence.serialization.DeploymentRuntimePersistenceCodec()
                            .write(runtime));
            digest.update(new gold.debug.windowstolinux.shared.config.persistence.serialization.HealthCheckCodec()
                    .write(runtime.healthCheck()));
        } catch (java.io.IOException failure) {
            throw new IllegalArgumentException("invalid release runtime", failure);
        }
    }
    ...
    private static void databaseBindings(MessageDigest digest, ReviewedDeploymentRequest request) {
        ...
        request.databaseBindings().orElseThrow().forEach(binding -> {
            ...
            if (binding.connection() instanceof ManagedDatabaseConnection.Sqlite sqlite) {
                ...
                update(digest, sqlite.location().type().name());
                update(digest, sqlite.location().path());
                update(digest, sqlite.accessPath());
                update(digest, sqlite.seedFile());
                sqlite.initializationFiles().forEach(file -> update(digest, file));
                return;
            }
