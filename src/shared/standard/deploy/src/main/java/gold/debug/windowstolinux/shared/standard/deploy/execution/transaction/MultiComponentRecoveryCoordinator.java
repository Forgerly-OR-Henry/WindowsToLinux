...
/**
 * Cleans uncommitted standard build candidates; publication rollback belongs to the common transaction. / 清理未提交标准构建候选，发布回滚归公共事务。
 */
final class MultiComponentRecoveryCoordinator {
     ...
     */
    static MultiComponentDeploymentResult recover(MultiComponentDeploymentPlan plan,
            Map<String, MultiComponentTransactionContext> contexts, DeploymentLinuxGateway gateway,
        ...
        try (DeploymentRemoteSession session = gateway.connect(endpoint, credential.duplicate(), verifier)) {
            ...
            applicationEvents
                    .add(DeploymentEvent.failed(DeploymentTraceEvent.CANDIDATE_CLEANUP_RECONNECT, failure.failure()));
            requireManualCleanup(contexts);
            return result(DeploymentStatus.MANUAL_RECOVERY_REQUIRED, applicationEvents, contexts);
        }
    }
     ...
     */
    static boolean cleanupAll(Map<String, MultiComponentTransactionContext> contexts, DeploymentRemoteSession session) {
    ...
    static void markCleanupFailure(Map<String, MultiComponentTransactionContext> contexts) {
        contexts.values().stream()
                .filter(context -> context.workspace != null && context.events.stream().anyMatch(
                        event -> event.step() == DeploymentTraceEvent.CANDIDATE_CLEANUP && !event.succeeded()))
    ...
    private static MultiComponentDeploymentResult result(DeploymentStatus status,
            List<DeploymentEvent> applicationEvents, Map<String, MultiComponentTransactionContext> contexts) {
