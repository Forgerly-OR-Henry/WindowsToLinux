...
import gold.debug.windowstolinux.shared.standard.deploy.distro.generation.script.EcosystemCapabilityScriptRenderer;
import gold.debug.windowstolinux.shared.standard.deploy.distro.generation.script.SetupScriptRenderer;
...
public final class DnfSetupRenderer {
    ...
    private static String render(DistributionSetupProfile profile, String username, boolean withCrb) {
        ...
        String preflight = """
                ...
                elevation=root
                """.formatted(SetupScriptRenderer.renderStageDiagnostics(), SetupScriptRenderer.quote(profile.id()),
                variantCheck, SetupScriptRenderer.quote(profile.version()),
                SetupScriptRenderer.quote(profile.packageArchitecture()),
                SetupScriptRenderer.renderCpuCheck(profile.requiredCpu()), SetupScriptRenderer.quote(username));
        String install = """
                  /usr/bin/dnf %s-y install %s
                """.formatted(withCrb ? "--enablerepo=crb " : "", packages);
        return preflight + "prepare_stage=security-observation\n"
                + SetupScriptRenderer.renderSecurityObservationFunctions() + "prepare_stage=package-install\n" + install
                + "prepare_stage=post-install-checks\n"
                + "prepare_check=sshd-configuration-and-libraries\n/usr/sbin/sshd -t\n"
                + SetupScriptRenderer.renderJava21RuntimeInstallation() + SetupScriptRenderer.renderCommonChecks()
                ...
                + SetupScriptRenderer.renderSecurityInvariant() + """
                        ...
                        printf 'PACKAGES=%s\\n'
                        printf 'HELPER=%s\\n'
                        """.formatted(packages, ManagedHelperProtocol.PATH);
    }
