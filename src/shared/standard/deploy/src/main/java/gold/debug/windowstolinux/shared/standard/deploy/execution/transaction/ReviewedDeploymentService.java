...
public final class ReviewedDeploymentService {
    ...
    public DeploymentResult deploy(ReviewedDeploymentRequest request, ManagedApplication application,
        ...
        return deploy(request, application, gateway, endpoint, credential, hostKeyVerifier, resolvedSecrets,
                ignored -> {
                });
    }
    ...
    public DeploymentResult deploy(ReviewedDeploymentRequest request, ManagedApplication application,
        ...
        if (!resolvedSecrets.stream().map(ResolvedSecretRevision::reference).toList()
                .equals(request.secretReferences())) {
        ...
        for (var message : gold.debug.windowstolinux.shared.standard.deploy.input.ManagedStoragePreparation
                .preview(request))
            events.add(new DeploymentEvent(DeploymentTraceEvent.APPLICATION_PREFLIGHT, true, message,
        ...
        try (DeploymentRemoteSession session = gateway.connect(endpoint, credential.duplicate(), hostKeyVerifier)) {
            ...
            if (!receipt.contentSha256().equals(request.archive().contentSha256())
                ...
                if (!cleanup(session, workspace, events))
                    return new DeploymentResult(DeploymentStatus.MANUAL_RECOVERY_REQUIRED, events, Optional.empty(),
                            Optional.empty());
            ...
            if (!build.succeeded()) {
                ...
                return new DeploymentResult(
                        cleaned ? DeploymentStatus.FAILED_BUILD : DeploymentStatus.MANUAL_RECOVERY_REQUIRED, events,
            ...
            var prepared = new gold.debug.windowstolinux.shared.deploy.publication.PreparedPublication(application.id(),
                    List.of(), application, workspace, build, releaseIdentity, request.runtime(),
                    Optional.of(request.facts()), DeploymentInputMapper.manifest(inputs),
                    new ManagedContentPublication(application.id(), application.id(), DeploymentInputMapper.storage(
                            application.id(),
                            new gold.debug.windowstolinux.shared.config.resource.ManagedComponentResourceBindings(
                                    request.fileBindings(), request.databaseBindings()),
                            request.runtime())));
            var publication = new gold.debug.windowstolinux.shared.deploy.publication.ManagedPublicationTransaction()
                    .publish(session, List.of(prepared), (id, event) -> events.add(event));
            if (publication.status() != DeploymentStatus.SUCCEEDED) {
                ...
                return new DeploymentResult(cleaned ? publication.status() : DeploymentStatus.MANUAL_RECOVERY_REQUIRED,
                        events, Optional.ofNullable(publication.observations().get(application.id())),
                        Optional.empty());
            }
            LifecycleObservation observation = publication.observations().get(application.id());
            ...
            if (committedObservation != null) {
                return new DeploymentResult(DeploymentStatus.SUCCEEDED, events, Optional.of(committedObservation),
                        ...
                        .withNonFatalFailure(failure(DeploymentExecutionFailureType.POST_PUBLICATION_CLEANUP_PENDING,
                                "Release identity was verified before the session cleanup failed"));
            }
            if (candidateMayExist) {
                return cleanupAfterInterruptedSession(gateway, endpoint, credential, hostKeyVerifier, workspace,
                        events);
            }
            return new DeploymentResult(DeploymentStatus.PRECONDITION_REJECTED, events, Optional.empty(),
                    Optional.empty());
    ...
    private static DeploymentResult finishPublication(DeploymentRemoteSession session, ManagedApplication application,
        ...
        return retained && cleaned
                ? result
                : result.withNonFatalFailure(failure(DeploymentExecutionFailureType.POST_PUBLICATION_CLEANUP_PENDING,
     ...
     */
    private static DeploymentResult validateRequestBinding(ReviewedDeploymentRequest request,
            ManagedApplication application, SshEndpoint endpoint, List<DeploymentEvent> events) {
     ...
     */
    private static DeploymentResult preflight(ReviewedDeploymentRequest request, DeploymentRemoteSession session,
            List<DeploymentEvent> events) throws LinuxOperationException {
        ...
        if (platform.support() != HostSupportStatus.READY_FOR_RUNTIME_VALIDATION)
            return rejected(events, DeploymentTraceEvent.TYPED_HOST_COMPATIBILITY,
        ...
        HostSupportDecision typedCompatibility = HostSupportEvaluator.evaluate(prepared.capabilities(), request.facts(),
                request.runtime(), prepared.toolchains());
        ...
        return typedCompatibility.support() == HostSupportStatus.READY_FOR_RUNTIME_VALIDATION
                ? null
                : new DeploymentResult(DeploymentStatus.PRECONDITION_REJECTED, events, Optional.empty(),
                        Optional.empty());
    }
     ...
     * @return constructed or resolved deployment result / 构造或解析得到的部署结果
     */
    private static DeploymentResult cleanupAfterInterruptedSession(DeploymentLinuxGateway gateway, SshEndpoint endpoint,
            SshCredential credential, HostKeyEvaluator hostKeyVerifier, RemoteWorkspace workspace,
            List<DeploymentEvent> events) {
        try (DeploymentRemoteSession cleanupSession = gateway.connect(endpoint, credential.duplicate(),
                hostKeyVerifier)) {
            ...
            events.add(DeploymentEvent.failed(DeploymentTraceEvent.CANDIDATE_CLEANUP_RECONNECT, exception.failure()));
        }
        return new DeploymentResult(DeploymentStatus.MANUAL_RECOVERY_REQUIRED, events, Optional.empty(),
                Optional.empty());
    }
     ...
     */
    private static DeploymentResult rejected(List<DeploymentEvent> events, DeploymentTraceEvent step, String evidence) {
        events.add(
                DeploymentEvent.failed(step, failure(DeploymentExecutionFailureType.PRECONDITION_REJECTED, evidence)));
     ...
     */
    private static boolean cleanup(DeploymentRemoteSession session, RemoteWorkspace workspace,
            List<DeploymentEvent> events) {
        try {
            RemoteStepResult cleanup = session.cleanupCandidate(workspace);
            events.add(DeploymentEvent.result(DeploymentTraceEvent.CANDIDATE_CLEANUP, cleanup.succeeded(),
                    cleanup.evidence()));
