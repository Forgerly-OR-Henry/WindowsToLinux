...
public final class AutomaticRuntimeResolver {
    ...
    public Map<String, String> defaults(DeploymentProjectType type, DeploymentRuntimeAssessment suggested, Path root) {
        ...
        values.put("healthMode",
                type == DeploymentProjectType.STATIC_SITE || type == DeploymentProjectType.SPRING_BOOT
                        ? "HTTP"
                        : "TCP");
        values.put("expectedStatus", "200");
        values.put("timeout", "10");
        values.put("stability", "5");
        values.put("containerEngine", "PODMAN");
        if (type == DeploymentProjectType.STATIC_SITE && Files.isRegularFile(root.resolve("package.json")))
            values.put("nodeBuild", "true");
        if (suggested != null) {
            suggested.values().forEach((key, value) -> values.put(switch (key) {
                case JAVA_JAR_PATH, JAVA_SOURCE_ROOT, PYTHON_VERSION, STATIC_OUTPUT_DIRECTORY, SERVICE_ARTIFACT,
                        CMAKE_ARTIFACT ->
                    "primary";
            ...
            if (!suggested.suggestedContainerPorts().isEmpty())
                values.put("ports",
                        suggested.suggestedContainerPorts().entrySet().stream()
                                .map(entry -> entry.getKey() + ":" + entry.getValue()).reduce((a, b) -> a + ";" + b)
                                .orElseThrow());
            if (!suggested.suggestedManagedVolumes().isEmpty())
                values.put("volumes",
                        suggested.suggestedManagedVolumes().stream()
                                .map(volume -> volume.name() + ":" + volume.containerPath()
                                        + (volume.readOnly() ? ":ro" : ":rw"))
                                .reduce((a, b) -> a + ";" + b).orElseThrow());
        }
        if (type == DeploymentProjectType.STATIC_SITE && Files.isRegularFile(root.resolve("index.html"))
                && !Files.exists(root.resolve("package.json")))
            if (Files.isDirectory(root.resolve("public")))
                values.put("primary", "public");
        values.put("executionMode", "DAEMON");
        if (type != DeploymentProjectType.STATIC_SITE && type != DeploymentProjectType.SPRING_BOOT
                && !values.containsKey("port"))
            values.put("healthMode", "PROCESS");
        if (type == DeploymentProjectType.STATIC_SITE)
            values.put("exposure", "EXTERNAL");
     ...
     */
    public List<DeploymentInputField> missing(String component, DeploymentProjectType type,
     ...
     */
    private List<DeploymentInputField> missingCompleted(String component, DeploymentProjectType type,
            Map<String, String> values) {
        ...
        switch (type) {
            ...
            case JAVA_JAR, JAVA_SOURCE, GO_SERVICE, RUST_SERVICE, DOTNET_SERVICE, KOTLIN_SERVICE, PHP_SERVICE,
                    RUBY_SERVICE, CMAKE_SERVICE ->
                required.addAll(List.of("primary", "secondary", "version"));
            case NODE_SERVICE -> required.add("version");
            case PYTHON_SERVICE -> required.addAll(List.of("primary", "secondary"));
            case STATIC_SITE -> {
                required.add("primary");
                if (values.containsKey("nodeBuild"))
                    required.add("version");
            }
            case DOCKERFILE_CONTAINER -> {
                if (!values.containsKey("application.mode") && required.contains("port"))
                    required.add("ports");
            }
            default -> {
            }
        }
        if (type == DeploymentProjectType.KOTLIN_SERVICE)
            required.add("jvmTarget");
        return required.stream().filter(key -> values.getOrDefault(key, "").isBlank())
                .map(key -> field(component, key,
                        key.equals("exposure") && !values.containsKey("port")
                                && values.getOrDefault("healthMode", "PROCESS").equals("PROCESS") ? "INTERNAL" : "",
                        List.of()))
                .toList();
    }
    ...
    public static DeploymentInputField field(String component, String key, String value, List<String> choices) {
        if (key.equals("executionMode"))
            choices = List.of("DAEMON", "ON_DEMAND");
        if (key.equals("exposure"))
            choices = List.of("EXTERNAL", "INTERNAL");
    ...
    public List<DeploymentInputField> corrections(String component, Map<String, String> values) {
        Map<String, String> labels = Map.ofEntries(Map.entry("healthMode", "field.healthMode"),
                Map.entry("healthEndpoint", "field.healthEndpoint"),
                ...
                Map.entry("configuration", "field.configurationEntries"),
                Map.entry("secrets", "field.secretReferences"), Map.entry("databaseMode", "field.databaseReviewMode"),
                Map.entry("databaseDetails", "field.databaseDetails"),
        ...
        for (String key : List.of("applicationDeclaration", "executionMode", "exposure", "port", "primary", "secondary",
                "version", "ports", "healthMode", "healthEndpoint", "expectedStatus", "timeout", "stability",
                "accessUrl", "configuration", "secrets", "databaseMode", "databaseDetails", "containerEngine",
                "volumes", "jvmArguments", "arguments", "jvmTarget")) {
            ...
            if (!values.containsKey(key) && !missingDatabaseDetails && !key.equals("applicationDeclaration"))
                continue;
            ...
            fields.add(labels.containsKey(key)
                    ? new DeploymentInputField(component + "/" + key, labels.get(key), "help." + labels.get(key),
                            values.getOrDefault(key, ""), choices)
    ...
    public DeploymentRuntimeSpecification runtime(DeploymentProjectType type, Map<String, String> v) {
        ...
        String primary = v.getOrDefault("primary", ""), secondary = v.getOrDefault("secondary", ""),
                version = v.getOrDefault("version", "");
        DeploymentRuntimeSpecification result = switch (type) {
            ...
            case JAVA_JAR -> new DeploymentRuntimeSpecification.JavaJar(primary, secondary, version,
                    DeploymentRuntimeParser.arguments(v.getOrDefault("jvmArguments", "")),
                    DeploymentRuntimeParser.arguments(v.getOrDefault("arguments", "")), health);
            ...
            case STATIC_SITE -> new DeploymentRuntimeSpecification.StaticSite(primary,
                    version.isBlank() ? OptionalInt.empty() : OptionalInt.of(Integer.parseInt(version)),
                    (HealthCheck.Http) health);
            case DOCKERFILE_CONTAINER -> new DeploymentRuntimeSpecification.Container(
                    DeploymentRuntimeSpecification.ContainerEngineType
                            .valueOf(v.getOrDefault("containerEngine", "PODMAN")),
                    DeploymentRuntimeParser.ports(v.getOrDefault("ports", "")),
                    DeploymentRuntimeParser.volumes(v.getOrDefault("volumes", "")), health);
    ...
    public List<String> artifacts(Path relativeRoot, DeploymentProjectFacts facts, Map<String, String> values) {
        String artifact = switch (facts.projectType()) {
            ...
            case SPRING_BOOT -> (facts.buildTool() == DeploymentBuildToolType.MAVEN
                    || facts.buildTool() == DeploymentBuildToolType.MAVEN_WRAPPER) ? "target" : "build/libs";
            case STATIC_SITE, JAVA_JAR, GO_SERVICE, RUST_SERVICE, DOTNET_SERVICE, KOTLIN_SERVICE, PHP_SERVICE,
                    RUBY_SERVICE, CMAKE_SERVICE ->
                values.get("primary");
    ...
    public HealthCheck health(Map<String, String> v) {
        ...
        if (v.getOrDefault("executionMode", "DAEMON").equals("ON_DEMAND") || mode.equals("COMMAND"))
            return new HealthCheck.Command(ApplicationDeclaration.command(v, "verification"),
                    v.getOrDefault("application.expectedOutput", ""), timeout);
        if (mode.equals("PROCESS"))
            return new HealthCheck.Process(timeout, Integer.parseInt(v.getOrDefault("stability", "5")));
        int port = Integer.parseInt(v.get("port"));
        if (mode.equals("UDP"))
            return new HealthCheck.Udp(port, v.getOrDefault("requestHex", ""), v.getOrDefault("responseHex", ""),
                    v.keySet().stream().anyMatch(key -> key.startsWith("application.verification."))
                            ? Optional.of(ApplicationDeclaration.command(v, "verification"))
                            : Optional.empty(),
                    timeout);
        if (mode.equals("HTTP"))
            return new HealthCheck.Http(URI.create(v.getOrDefault("healthEndpoint", "http://127.0.0.1:" + port + "/")),
                    Integer.parseInt(v.getOrDefault("expectedStatus", "200")), timeout);
        if (!mode.equals("TCP"))
            throw new IllegalArgumentException("unsupported health mode");
    ...
    public Optional<UserAccessUrl> access(Map<String, String> v, String host) {
        v = ApplicationDeclaration.completed(v);
        var external = ApplicationDeclaration.resolve(v).endpoints().stream().filter(endpoint -> endpoint
                .exposure() == gold.debug.windowstolinux.shared.model.project.application.ApplicationEndpoint.ExposureType.EXTERNAL)
                .filter(endpoint -> endpoint
                        .protocol() == gold.debug.windowstolinux.shared.model.project.application.ApplicationEndpoint.ProtocolType.HTTP
                        || endpoint
                                .protocol() == gold.debug.windowstolinux.shared.model.project.application.ApplicationEndpoint.ProtocolType.HTTPS)
                .findFirst();
        if (external.isEmpty()) {
            if (!v.getOrDefault("accessUrl", "").isBlank())
                throw new IllegalArgumentException("An access URL requires a declared external HTTP endpoint");
            return Optional.empty();
        }
        var endpoint = external.orElseThrow();
        String address = host.contains(":") ? "[" + host + "]" : host;
        String url = endpoint.accessUrl().isEmpty()
                ? endpoint.protocol().name().toLowerCase(java.util.Locale.ROOT) + "://" + address + ":"
                        + endpoint.hostPort() + "/"
                : endpoint.accessUrl();
