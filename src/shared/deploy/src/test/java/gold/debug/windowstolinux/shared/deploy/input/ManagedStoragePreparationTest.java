...
class ManagedStoragePreparationTest {
    @TempDir Path root;
    private ConfigurationSnapshot configuration() { return ConfigurationSnapshot.create("demo",1,"v1",Instant.EPOCH,List.of(new gold.debug.windowstolinux.shared.config.revision.ConfigurationEntry("PORT",gold.debug.windowstolinux.shared.config.contract.definition.ConfigurationScope.RUNTIME,new gold.debug.windowstolinux.shared.config.contract.definition.ConfigurationValue.Number(8080)))); }
    private DeploymentRuntimeSpecification runtime() { return new DeploymentRuntimeSpecification.SpringBoot(new HealthCheck.Tcp(8080,10,2)); }
    ...
    @Test void passesDefaultStorageToAnExplicitApplicationEnvironment() throws Exception {
        ...
        assertTrue(result.configuration().entries().stream().anyMatch(entry -> entry.key().equals("UPLOADS_DIRECTORY") && entry.value().toString().contains("/var/opt/windowstolinux/apps/demo/files/uploads")));
    }
    @Test void relativePathsRemainStableAndConfigurationRevisionsChangeOnlyWithContent() throws Exception {
    ...
    @Test void unknownInputsAndExplicitOutOfBoundsPathsNeverFallBack() throws Exception {
    ...
    @Test void applicationConfigurationCannotSilentlyOverrideStorageHandoff() throws Exception {
        var initial=prepare("storage=a\nstorage.a.environment=DATA_DIR\n");
        Files.writeString(root.resolve("windowstolinux-storage.properties"),"storage=a\nstorage.a.path=other\nstorage.a.environment=DATA_DIR\n");
        assertThrows(IllegalArgumentException.class,()->ManagedStoragePreparation.prepare(root,"demo",initial.configuration(),runtime(),List.of()));
    }
}
