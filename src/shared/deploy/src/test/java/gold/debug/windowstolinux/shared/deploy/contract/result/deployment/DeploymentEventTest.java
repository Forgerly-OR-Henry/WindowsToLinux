...
class DeploymentEventTest {
    ...
    void structuredFailuresKeepTheirDiagnosticAndRecoveryDescriptorInProgress() {
        var failure = gold.debug.windowstolinux.shared.model.failure.FailureDescriptor.create(
                ...
                gold.debug.windowstolinux.shared.model.failure.OperationIdentity.create(), "Target free space is insufficient");
        ...
        assertEquals(event.message(), event.withOperationIdentity(
                gold.debug.windowstolinux.shared.model.failure.OperationIdentity.create()).message());
    }
    @Test
