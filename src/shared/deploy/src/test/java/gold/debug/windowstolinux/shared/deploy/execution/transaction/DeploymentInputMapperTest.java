...
class DeploymentInputMapperTest {
    private static ConfigurationSnapshot snapshot() {
        return ConfigurationSnapshot.create("demo", 3, "runtime-v1", Instant.EPOCH, List.of(
                new ConfigurationEntry("Z_LABEL", ConfigurationScope.RUNTIME,
                        new ConfigurationValue.Text("quote'\"\\ $value\ttab")),
    ...
    @Test
    void separatesScopesAndPreservesCanonicalBindings() {
        ...
        var onlyRuntime = ConfigurationSnapshot.create("demo", 1, "runtime-v1", Instant.EPOCH, List
    ...
    @Test
    void preservesExistingSmallValuesWithoutInventingAnEntryCountLimit() {
        var entries = java.util.stream.IntStream.range(0, 130)
                .mapToObj(index -> new ConfigurationEntry("FIELD_" + index, ConfigurationScope.RUNTIME,
                        new ConfigurationValue.Flag(true)))
                .toList();
    ...
    @Test
    void closesPayloadsOnSuccessFailureAndMismatchedRemoteEvidence() throws Exception {
        for (int outcome = 0; outcome < 3; outcome++) {
            ...
            var session = (DeploymentRemoteSession) Proxy.newProxyInstance(getClass().getClassLoader(),
                    new Class<?>[]{DeploymentRemoteSession.class}, (proxy, method, arguments) -> {
                        ...
                        var values = (List<RemoteSecretPayload>) arguments[2];
                        captured.addAll(values);
                        byte[] bytes = values.getFirst().copyValue();
                        try {
                            assertArrayEquals(new byte[]{115, 101, 99, 114, 101, 116}, bytes);
                        } finally {
                            Arrays.fill(bytes, (byte) 0);
                        }
                        if (selected == 1)
                            throw new IllegalStateException("fixture staging failure");
            ...
            try (var secret = new ResolvedSecretRevision(new SecretReference("database-password", 4),
                ...
                assertArrayEquals(new byte[captured.getFirst().digest().byteCount()],
                        (byte[]) owned.get(captured.getFirst()));
    ...
    @Test
    void rejectsDuplicateBindingsAndInterruptionBeforeRemoteMutation() {
        var session = (DeploymentRemoteSession) Proxy.newProxyInstance(getClass().getClassLoader(),
                new Class<?>[]{DeploymentRemoteSession.class}, (proxy, method, args) -> {
                    throw new AssertionError("must not reach transport");
                });
    ...
    private static ManagedApplication application() {
        return ManagedApplication.forManaged("demo", new ServerIdentity("server", "127.0.0.1", 22, "SHA256:fixture"),
                "a".repeat(64));
    }
}
