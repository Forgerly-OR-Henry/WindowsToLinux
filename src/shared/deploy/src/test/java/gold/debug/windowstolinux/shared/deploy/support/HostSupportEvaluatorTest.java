...
class HostSupportEvaluatorTest {
    ...
    void requiresTheMatchingContainerRuntimeAndPreservesLegacyWarning() {
        var docker = new DeploymentRuntimeSpecification.Container(DeploymentRuntimeSpecification.ContainerEngineType.DOCKER,
        ...
        assertEquals(HostSupportStatus.UNSUPPORTED,
                HostSupportEvaluator.evaluate(capabilities(LinuxDistroType.UBUNTU, "22.04", "apt", false, false),
                        facts(DeploymentProjectType.DOCKERFILE_CONTAINER, DeploymentBuildToolType.CONTAINER_BUILD), docker).support());
        assertEquals(HostSupportStatus.LEGACY_RISK_CONFIRMATION_REQUIRED,
                HostSupportEvaluator.evaluate(capabilities(LinuxDistroType.LEGACY_CENTOS, "7", "yum", false, false),
    ...
    void requiresMavenOnlyForTheSystemMavenSpringBootBuild() {
        ...
        assertEquals(HostSupportStatus.READY_FOR_RUNTIME_VALIDATION,
                HostSupportEvaluator.evaluate(noMaven,
                        facts(DeploymentProjectType.SPRING_BOOT, DeploymentBuildToolType.MAVEN_WRAPPER), runtime).support());
        assertEquals(HostSupportStatus.READY_FOR_RUNTIME_VALIDATION,
                HostSupportEvaluator.evaluate(noMaven,
                        facts(DeploymentProjectType.SPRING_BOOT, DeploymentBuildToolType.GRADLE_WRAPPER), runtime).support());
    }
