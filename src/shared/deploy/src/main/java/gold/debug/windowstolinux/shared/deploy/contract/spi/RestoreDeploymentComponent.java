 ...
 */
public record RestoreDeploymentComponent(String componentId, String managedApplicationId,
        String ownershipManifestSha256, String releaseSha256, List<SecretReference> secretReferences,
        String releaseManifestPath, String configurationSnapshotPath, String serviceDefinitionPath,
        List<String> dependsOn, DeploymentRuntimeSpecification runtime, Optional<DeploymentInputManifest> inputManifest,
        List<String> persistentArchivePaths, Optional<String> ociArchivePath) {
    /**
     * SECRET ORDER.
     * <p>秘密顺序。
     */
    ...
    public RestoreDeploymentComponent {
        ...
        dependsOn = Objects.requireNonNull(dependsOn, "dependsOn").stream().map(value -> managedId(value, "dependsOn"))
                .toList();
        ...
        if (inputManifest.isPresent()) {
            ...
            if (!staged.equals(secretReferences)) {
                throw new IllegalArgumentException(
                        "staged deployment inputs differ from exact backup secret references");
            }
        }
        ...
        if (persistentArchivePaths.size() > 256
                || persistentArchivePaths.stream()
                        .anyMatch(path -> !memberPath(path, persistentPrefix, "persistentArchivePath").equals(path))
     ...
     * @param runtime reviewed language, process and health specification / 已审阅的语言、进程及健康规格
     */
    public RestoreDeploymentComponent(String componentId, String managedApplicationId, String ownershipManifestSha256,
            String releaseSha256, List<SecretReference> secretReferences, String releaseManifestPath,
            String configurationSnapshotPath, String serviceDefinitionPath, List<String> dependsOn,
    ...
    private static String memberPath(String value, String prefix, String field) {
        value = Objects.requireNonNull(value, field).trim();
        if (!value.startsWith(prefix) || value.startsWith("/") || value.indexOf('\\') >= 0 || value.contains("//")
                || value.split("/").length < 2 || java.util.Arrays.stream(value.split("/"))
