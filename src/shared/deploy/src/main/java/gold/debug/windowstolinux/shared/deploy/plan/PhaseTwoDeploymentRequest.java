...
/**
 * Fully reviewed Phase Two deployment input that contains identities and typed definitions, never a shell command.
 *
 * <p>经过完整审阅的二期部署输入，包含身份和类型化定义，绝不包含 Shell 命令。
 *
 ...
 */
public record PhaseTwoDeploymentRequest(
        ServerIdentity server,
        PhaseTwoProjectFacts facts,
        SourceRevision sourceRevision,
        SourceArchiveDescriptor archive,
        ConfigurationSnapshot configuration,
        List<SecretReference> secretReferences,
        PhaseTwoRuntimeSpecification runtime,
        BuildLimits limits,
        DeploymentApproval approval,
        boolean containerDaemonRiskAccepted
) {
    /**
     * Creates a {@code PhaseTwoDeploymentRequest} instance.
     *
     * <p>创建 {@code PhaseTwoDeploymentRequest} 实例。
     */
    public PhaseTwoDeploymentRequest {
        ...
        if (!facts.readyForPlanning()) {
            throw new IllegalArgumentException("Phase Two deployment requests require complete deterministic project facts");
        }
        ...
        if (runtime instanceof PhaseTwoRuntimeSpecification.Container container
                && container.engine() == PhaseTwoRuntimeSpecification.ContainerEngine.DOCKER && !containerDaemonRiskAccepted) {
