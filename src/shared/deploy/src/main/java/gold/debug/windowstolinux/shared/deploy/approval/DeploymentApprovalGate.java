...
public final class DeploymentApprovalGate {
    /** Prevents construction. / 禁止实例化。 */
    private DeploymentApprovalGate() {
    }
    ...
    public static AgentRiskLevel admit(AgentAction action, AgentRiskLevel local, AgentReview review) {
        if (local == AgentRiskLevel.FORBIDDEN || action.risk() == AgentRiskLevel.FORBIDDEN || review == null
                ...
                || !action.evidence().keySet().containsAll(review.evidence()))
            throw new SecurityException("action not approved");
        return AgentRiskLevel.values()[Math.max(action.risk().ordinal(),
                Math.max(local.ordinal(), review.risk().ordinal()))];
    }
    ...
    public static boolean needsHuman(AgentApprovalMode mode, AgentRiskLevel risk) {
        if (risk == AgentRiskLevel.FORBIDDEN)
            throw new SecurityException("forbidden action");
        return mode == AgentApprovalMode.MANUAL_REVIEW
                || mode == AgentApprovalMode.AUTOMATIC && risk == AgentRiskLevel.HIGH;
    }
}
