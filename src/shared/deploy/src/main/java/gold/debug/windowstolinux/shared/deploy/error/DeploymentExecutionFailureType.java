...
public enum DeploymentExecutionFailureType implements FailureDefinition {
     ...
     */
    PRECONDITION_REJECTED("deployment.preflight.rejected", "preflight", "deployment.error.preconditionRejected",
     ...
     * <p>部署执行失败类型中的构建失败分类。
     */
    BUILD_FAILED("deployment.build.failed", "build", "deployment.error.buildFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.CLEANUP),
    /**
     * SWITCH UNVERIFIED classification within deployment execution failure type.
     * <p>部署执行失败类型中的切换未验证分类。
     */
    SWITCH_UNVERIFIED("deployment.switch.unverified", "switch", "deployment.error.switchUnverified",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.ROLLBACK),
    /**
     * PUBLISH FAILED classification within deployment execution failure type.
     * <p>部署执行失败类型中的发布失败分类。
     */
    PUBLISH_FAILED("deployment.publish.failed", "publish", "deployment.error.publishFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.ROLLBACK),
    /**
     * HEALTH FAILED classification within deployment execution failure type.
     * <p>部署执行失败类型中的健康失败分类。
     */
    HEALTH_FAILED("deployment.health.failed", "health", "deployment.error.healthFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.ROLLBACK),
    /**
     * OBSERVATION UNVERIFIED classification within deployment execution failure type.
     * <p>部署执行失败类型中的观测未验证分类。
     */
    OBSERVATION_UNVERIFIED("deployment.observation.unverified", "observation", "deployment.error.observationUnverified",
     ...
     * <p>部署执行失败类型中的算术溢出分类。
     */
    ARITHMETIC_OVERFLOW("deployment.preflight.arithmetic-overflow", "preflight", "deployment.error.arithmeticOverflow",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * CLEANUP UNVERIFIED classification within deployment execution failure type.
     * <p>部署执行失败类型中的清理未验证分类。
     */
    CLEANUP_UNVERIFIED("deployment.cleanup.unverified", "cleanup", "deployment.error.cleanupUnverified",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * ROLLBACK UNVERIFIED classification within deployment execution failure type.
     * <p>部署执行失败类型中的回滚未验证分类。
     */
    ROLLBACK_UNVERIFIED("deployment.rollback.unverified", "rollback", "deployment.error.rollbackUnverified",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * POST PUBLICATION CLEANUP PENDING classification within deployment execution failure type.
     * <p>部署执行失败类型中的后置发布清理待处理分类。
     */
    POST_PUBLICATION_CLEANUP_PENDING("deployment.cleanup.pending", "cleanup", "deployment.error.cleanupPending",
            FailureSeverityLevel.WARNING, FailureRecoveryAction.CLEANUP),
    /**
     * LOCAL OBSERVATION PERSISTENCE FAILED classification within deployment execution failure type.
     * <p>部署执行失败类型中的本地观测持久化失败分类。
     */
    LOCAL_OBSERVATION_PERSISTENCE_FAILED("deployment.persistence.observation-save-failed", "persistence",
            "deployment.error.observationSaveFailed", FailureSeverityLevel.WARNING, FailureRecoveryAction.RETRY);
     ...
     */
    DeploymentExecutionFailureType(String code, String phase, String messageKey, FailureSeverityLevel severity,
