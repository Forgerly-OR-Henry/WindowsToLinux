...
/**
 * Orchestrates the fixed managed-deployment short-downtime publish transaction.
 *
 * <p>编排固定的受管部署短停机发布事务。
 */
public final class ManagedDeploymentService {
    ...
    public DeploymentResult deploy(
        ...
        if (request.approval().rootBuildAccepted() && !"root".equals(endpoint.username())) {
            try {
                return rejected(events, "root-build-session",
                        "managed service root builds require a root SSH session; the current SSH user is not root, so no "
        ...
        if ("root".equals(endpoint.username()) && !request.approval().rootBuildAccepted()) {
            try {
                return rejected(events, "root-build-session",
                        "A root SSH session may be used only for a separately confirmed managed-deployment root build; the "
        ...
        try (LinuxRemoteSession session = gateway.connect(endpoint, connectionCredential(credential), hostKeyVerifier)) {
            ...
            if (!capabilities.supportsManagedDeployment(request.source().usesMavenWrapper(), request.healthCheck())) {
    ...
    private static DeploymentResult recoverAfterInterruptedSession(
        ...
        try (LinuxRemoteSession recoverySession = gateway.connect(endpoint, connectionCredential(credential), hostKeyVerifier)) {
    ...
    private static DeploymentResult cleanupCandidateAfterSnapshotFailure(
        ...
        try (LinuxRemoteSession cleanupSession = gateway.connect(
