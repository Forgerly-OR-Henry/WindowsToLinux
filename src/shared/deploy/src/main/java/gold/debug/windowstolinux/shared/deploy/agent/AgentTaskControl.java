...
public final class AgentTaskControl {
     ...
     */
    public AgentTaskControl(Consumer<AgentTaskState> listener){this.listener=Objects.requireNonNull(listener);}
     ...
     */
    public synchronized AgentTaskState state(){return state;}
     ...
     */
    public synchronized void awaitUser(String reasonCode){reason=reasonCode; if(state==AgentTaskState.RUNNING)change(AgentTaskState.PAUSE_REQUESTED);checkpoint();}
    ...
    public synchronized void finish(AgentTaskState terminal){
        if(terminal!=AgentTaskState.SUCCEEDED&&terminal!=AgentTaskState.FAILED&&terminal!=AgentTaskState.UNKNOWN&&terminal!=AgentTaskState.CANCELLED)throw new IllegalArgumentException("not terminal");
        change(terminal);notifyAll();
    }
    /** Publishes a state transition. / 发布状态转换。
     * @param next new state / 新状态
     */
    private void change(AgentTaskState next){state=next;listener.accept(next);}
}
