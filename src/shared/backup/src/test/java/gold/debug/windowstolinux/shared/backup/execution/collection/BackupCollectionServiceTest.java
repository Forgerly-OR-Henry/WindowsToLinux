...
class BackupCollectionServiceTest {
    ...
    private final Map<String, RuntimeState> states = new HashMap<>(
            Map.of("store", RuntimeState.RUNNING, "api", RuntimeState.RUNNING));
    ...
    private BackupCollectionRequest request(boolean owned) {
        var start = List.of("store", "api");
        var reverse = List.of("api", "store");
        var plan = new MultiComponentDeploymentPlan("demo", List.of(List.of("store"), List.of("api")), reverse, start,
                start, reverse, Map.of("store", "demo-store", "api", "demo-api"),
                Map.of("store", List.of(), "api", List.of("store")));
        var health = new HealthCheck.Tcp(18080, 10, 1);
        var components = start.stream()
                .map(id -> new BackupCollectionRequest.Component(id,
                        ManagedApplication.forManaged(id,
                                new ServerIdentity("server-one", "example.test", 22, "SHA256:fixture"), "a".repeat(64)),
                        new DeploymentRuntimeSpecification.NodeService(22, health), "b".repeat(64),
                        new ManagedComponentResourceBindings(List.of(), Optional.of(List.of()))))
                .toList();
    ...
    @Test
    void collectsInDependencyOrderAndRestoresOnlyOriginallyRunningComponents() throws Exception {
        ...
        assertFalse(calls.contains("stop:store"));
        assertFalse(calls.contains("start:store"));
    ...
    @Test
    void partialStopFailureStillRecoversEveryComponentWhoseStopWasAttempted() throws Exception {
        stopFailure = "store";
    ...
    @Test
    void cancellationClearsTheFlagForRecoveryAndRestoresItBeforeReturning() throws Exception {
        interrupted = true;
        try {
            assertThrows(InterruptedException.class, () -> collect(true));
            assertTrue(Thread.currentThread().isInterrupted());
            assertTrue(calls.contains("start:store"));
            assertTrue(calls.contains("start:api"));
            assertTrue(calls.contains("end:api"));
            assertTrue(calls.contains("discard"));
        } finally {
            Thread.interrupted();
        }
    }
    ...
    @Test
    void failedRecoveryRetainsMarkersAndRemoteMaterialAndBothFailures() throws Exception {
        collectFailure = true;
        startFailure = "store";
    ...
    @Test
    void cleanupErrorsAreSuppressedWithoutReplacingCollectionFailure() throws Exception {
        collectFailure = true;
        cleanupFailure = true;
    ...
    @Test
    void callerOwnedMaintenanceIsNeverReleased() throws Exception {
        collect(false);
    ...
    @Test
    void acquisitionFailureReleasesOnlyAcknowledgedMarkersWithoutStopping() throws Exception {
        maintenanceFailure = true;
        assertThrows(LinuxOperationException.class, () -> collect(true));
        assertTrue(calls.contains("end:store"));
        assertFalse(calls.contains("end:api"));
    ...
    private DeploymentRemoteSession session() throws Exception {
        ...
        return (DeploymentRemoteSession) Proxy.newProxyInstance(getClass().getClassLoader(),
                ...
                (proxy, method, args) -> {
                    String id = args != null && args.length > 0 && args[0] instanceof ManagedApplication app
                            ? app.id()
                            : "";
                    return switch (method.getName()) {
                        ...
                        case "collectCapabilities" ->
                            new ServerCapabilityFacts("Ubuntu 24.04", "x86_64", true, true, true, true, true, true,
                                    true, true, ManagedHelperProtocolVersion.CURRENT, 16L << 30, "fixture");
                        ...
                        case "beginMaintenance", "endMaintenance" -> {
                            ...
                            boolean begin = method.getName().equals("beginMaintenance");
                            calls.add((begin ? "begin:" : "end:") + id);
                            if (begin && maintenanceFailure && id.equals("api") || !begin && cleanupFailure)
                                throw remoteFailure();
                            yield null;
                        }
                        case "executeDeploymentLifecycle" -> {
                            boolean stop = args[2] == LifecycleAction.STOP;
                            calls.add((stop ? "stop:" : "start:") + id);
                            assertFalse(Thread.currentThread().isInterrupted(),
                                    "recovery must not inherit collection cancellation");
                            states.put(id, stop ? RuntimeState.STOPPED : RuntimeState.RUNNING);
                            if (stop && id.equals(stopFailure) || !stop && id.equals(startFailure))
                                throw remoteFailure();
                            yield observation((ManagedApplication) args[0], states.get(id));
                        }
                        case "checkDeploymentHealth" -> {
                            calls.add("health:" + id);
                            yield new HealthCheckResult(true, "fixture");
                        }
                        case "createBackupArtifact" -> {
                            var request = (RemoteBackupArtifactRequest) args[0];
                            calls.add("collect:" + request.componentId());
                            if (collectFailure)
                                throw remoteFailure();
                            yield new RemoteBackupArtifact(operation, "artifact-" + "b".repeat(32), request.kind(),
                                    content.length, sha);
                        }
                        case "copyBackupArtifact" -> {
                            ((OutputStream) args[1]).write(content);
                            if (interrupted)
                        ...
                        case "discardBackupOperation" -> {
                            calls.add("discard");
                            yield new RemoteStepResult(!cleanupFailure, false, "fixture");
                        }
    ...
    private static LinuxCapabilityFacts linuxFacts() {
        return new LinuxCapabilityFacts(LinuxDistroType.UBUNTU, "24.04", "x86_64", "apt", "amd64", true, false, false,
                false, Set.of(21), Set.of(22), true, true, Set.of("3.12"), true, Map.of(), Map.of(), false, false,
