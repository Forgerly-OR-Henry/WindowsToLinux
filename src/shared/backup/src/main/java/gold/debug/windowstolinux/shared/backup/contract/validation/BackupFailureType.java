...
public enum BackupFailureType implements FailureDefinition {
     ...
     * <p>备份失败类型中的采集预检失败分类。
     */
    COLLECTION_PREFLIGHT_FAILED("backup.collection.preflight-failed", "preflight", "backup.error.collectionPreflightFailed", FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * COLLECTION FAILED classification within backup failure type.
     * <p>备份失败类型中的采集失败分类。
     */
    COLLECTION_FAILED("backup.collection.failed", "collection", "backup.error.collectionFailed", FailureRecoveryAction.CLEANUP),
    /**
     * COLLECTION RECOVERY FAILED classification within backup failure type.
     * <p>备份失败类型中的采集恢复失败分类。
     */
    COLLECTION_RECOVERY_FAILED("backup.collection.recovery-failed", "recovery", "backup.error.collectionRecoveryFailed", FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
