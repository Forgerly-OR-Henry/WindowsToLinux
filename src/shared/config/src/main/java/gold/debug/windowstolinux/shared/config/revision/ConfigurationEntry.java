...
public record ConfigurationEntry(String key, ConfigurationScope scope, ConfigurationValue value) {
    ...
    public ConfigurationEntry {
        ...
        if (!key.matches("[A-Z][A-Z0-9_]{0,63}")) {
            throw ConfigurationException.create(ConfigurationFailureType.CONFIGURATION_KEY_INVALID,
                    "Configuration keys must use uppercase environment-style identifiers");
        }
        scope = Objects.requireNonNull(scope, "scope");
        value = Objects.requireNonNull(value, "value");
        if (key.endsWith("_PASSWORD") || key.endsWith("_SECRET") || key.endsWith("_TOKEN") || key.endsWith("_KEY")) {
            throw ConfigurationException.create(ConfigurationFailureType.SECRET_LIKE_KEY,
                    "Secret-like keys must use a SecretReference");
        }
        if ("PORT".equals(key) && (!(value instanceof ConfigurationValue.Number number) || number.value() < 1
                || number.value() > 65535)) {
            throw ConfigurationException.create(ConfigurationFailureType.PORT_INVALID,
