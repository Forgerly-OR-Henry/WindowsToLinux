...
public final class ApplicationRuntimeConfigurationCodec {
    ...
    public byte[] write(ManagedApplicationRuntimeConfiguration value) throws IOException {
        ...
        try (var out = new DataOutputStream(bytes)) {
            out.writeInt(0x57544c41);
            out.writeByte(value.workload().workers().isEmpty() ? 1 : 2);
            byte[] health = new HealthCheckCodec().write(value.healthCheck());
            out.writeInt(health.length);
            out.write(health);
            out.writeUTF(value.identityPolicy().name());
            out.writeUTF(value.userAccessUrl().map(url -> url.url().toString()).orElse(""));
            ApplicationWorkloadCodec.write(out, value.workload());
        }
        if (bytes.size() > 1_048_576)
            throw new IOException("application runtime exceeds size limit");
    ...
    public ManagedApplicationRuntimeConfiguration read(byte[] bytes) throws IOException {
        if (bytes == null || bytes.length > 1_048_576)
            throw new IOException("invalid application runtime size");
        try (var in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            if (in.readInt() != 0x57544c41)
                throw new IOException("unsupported application runtime format");
            int version = in.readUnsignedByte();
            if (version != 1 && version != 2)
                throw new IOException("unsupported application runtime format");
            int count = in.readInt();
            if (count < 1 || count > 65536)
                throw new IOException("invalid health size");
            var health = new HealthCheckCodec().read(in.readNBytes(count));
            var identity = RuntimeIdentityMode.valueOf(in.readUTF());
            String url = in.readUTF();
            var result = new ManagedApplicationRuntimeConfiguration(health,
                    url.isEmpty() ? Optional.empty() : Optional.of(new UserAccessUrl(URI.create(url))), identity,
                    ApplicationWorkloadCodec.read(in, version == 2));
            if (in.read() != -1)
                throw new IOException("trailing application runtime data");
            return result;
        } catch (IllegalArgumentException failure) {
            throw new IOException("invalid application runtime", failure);
        }
    }
}
