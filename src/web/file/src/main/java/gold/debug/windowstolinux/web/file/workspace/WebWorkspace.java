...
public final class WebWorkspace {
    ...
    public WebWorkspace(Path root, UploadQuota quota, long minimumFreeBytes) throws IOException {
        if (minimumFreeBytes < 0)
            throw new IllegalArgumentException("Invalid free-space reserve");
        this.root = root.toAbsolutePath().normalize();
        this.quota = quota;
        this.minimumFreeBytes = minimumFreeBytes;
        safeAncestors(this.root);
    ...
    public synchronized java.util.List<WorkspaceAddress> owned(String workspaceId) throws IOException {
        ...
        Path parent = root.resolve(workspaceId);
        safeAncestors(parent);
        if (!Files.isDirectory(parent, LinkOption.NOFOLLOW_LINKS))
            return java.util.List.of();
        var result = new java.util.ArrayList<WorkspaceAddress>();
        try (var children = Files.list(parent)) {
            for (Path child : children.toList()) {
                if (!child.getFileName().toString().matches("[a-f0-9-]{36}")
                ...
                try {
                    directory(address);
                    result.add(address);
                } catch (IOException unowned) {
                    /* Preserve anything without exact ownership. / 保留任何无法确认精确归属的内容。 */ }
            }
        }
    ...
    public boolean exists(WorkspaceAddress address) throws IOException {
        Path path = root.resolve(address.workspaceId()).resolve(address.resourceId());
        safeAncestors(path);
    ...
    public synchronized void checkCapacity(WorkspaceAddress address, long additionalBytes) throws IOException {
        ...
        if (additionalBytes < 0 || size(directory) + additionalBytes > quota.projectBytes()
                || size(root) + additionalBytes > quota.totalBytes()
                || Files.getFileStore(root).getUsableSpace() < additionalBytes + minimumFreeBytes)
            throw new IOException("Workspace capacity exceeded");
    }
    ...
    public synchronized void discardChild(WorkspaceAddress address, String name) throws IOException {
        if (!name.matches("[a-z][a-z0-9.-]*"))
            throw new IOException("Invalid cleanup target");
        Path directory = directory(address), child = directory.resolve(name).normalize();
        if (!child.startsWith(directory) || child.equals(directory))
            throw new IOException("Cleanup boundary mismatch");
        ...
        try (var paths = Files.walk(child)) {
            for (Path path : paths.sorted(Comparator.reverseOrder()).toList()) {
                if (!path.startsWith(child))
                    throw new IOException("Cleanup boundary mismatch");
                if (!Files.isSymbolicLink(path) && Files.getFileStore(path).supportsFileAttributeView("dos"))
    ...
    public Path directory(WorkspaceAddress address) throws IOException {
        ...
        if (!Files.isRegularFile(marker, LinkOption.NOFOLLOW_LINKS) || Files.size(marker) > 250
                || !Files.readString(marker).equals(address.marker()))
            throw new IOException("Workspace ownership mismatch");
        return directory;
    }
     ...
     * @throws IOException if the required file or stream operation fails / 所需文件或流操作失败时
     */
    public Path source(WorkspaceAddress address) throws IOException {
        return directory(address).resolve("source");
    }
    ...
    public synchronized long upload(WorkspaceAddress address, String relative, InputStream input) throws IOException {
        Path directory = directory(address);
        if (Files.exists(directory.resolve(".complete"), LinkOption.NOFOLLOW_LINKS))
            throw new IOException("Source already finalized");
        ...
        try (var paths = Files.walk(directory.resolve("source"))) {
            if (paths.filter(path -> Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS)).limit(quota.members())
                    .count() >= quota.members())
            ...
            try (var output = Files.newOutputStream(destination, StandardOpenOption.CREATE_NEW,
                ...
                for (int read; (read = input.read(buffer)) != -1;) {
                    ...
                    if (Thread.currentThread().isInterrupted())
                        throw new java.io.InterruptedIOException("Upload cancelled");
                    if (count > quota.fileBytes() || used + count > quota.projectBytes()
                            || total + count > quota.totalBytes())
    ...
    public synchronized void complete(WorkspaceAddress address) throws IOException {
        Path directory = directory(address);
        if (size(directory.resolve("source")) == 0)
            throw new IOException("Source has no content");
        ...
        try (var paths = Files.walk(directory)) {
            for (Path path : paths.sorted(Comparator.reverseOrder()).toList()) {
                ...
                if (Files.isSymbolicLink(path))
                    throw new IOException("Workspace contains a link");
                Files.delete(path);
            }
        }
    }
    ...
    public Path safeMember(Path base, String relative) throws IOException {
        if (relative == null || relative.isBlank() || relative.length() > quota.pathLength() || relative.startsWith("/")
            ...
            throw new IOException("Invalid source member");
        for (String part : relative.split("/", -1)) {
            ...
            if (part.isBlank() || part.equals(".") || part.equals("..") || part.endsWith(".") || part.endsWith(" ")
                    || part.chars().anyMatch(ch -> ch < 32) || part.matches(".*[<>\"|?*].*"))
                throw new IOException("Invalid source member");
            if (upper.matches("(CON|PRN|AUX|NUL|COM[0-9]|LPT[0-9])(\\..*)?"))
                throw new IOException("Reserved source member");
        }
        Path path = base.resolve(relative).normalize();
        if (!path.startsWith(base.normalize()) || path.equals(base))
            throw new IOException("Source boundary escape");
        safeAncestors(path);
        return path;
    }
        ...
        try (var paths = Files.walk(directory)) {
            for (Path path : paths.toList()) {
                ...
                if (attributes.isSymbolicLink() || attributes.isOther())
                    throw new IOException("Unsafe workspace member");
