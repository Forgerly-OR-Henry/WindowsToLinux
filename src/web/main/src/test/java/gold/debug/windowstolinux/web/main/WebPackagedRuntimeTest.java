...
class WebPackagedRuntimeTest {
    ...
    @Test void externalDbJarStartsRestartsAndRelocatesWithYamlOnly() throws Exception {
        ...
        Path first = root.resolve("first/web"), keys = root.resolve("keys"); WebMainTest.copy(distribution, first);
        String id;
        ...
        Path moved = root.resolve("relocated"); moveStoppedDistribution(first, moved);
        renameDbJar(moved);
        ...
        try (var files = Files.list(keys)) { assertEquals(1, files.filter(path -> path.toString().endsWith(".key")).count()); }
    }

    private static void moveStoppedDistribution(Path source, Path destination) throws Exception {
        long deadline = System.nanoTime() + java.util.concurrent.TimeUnit.SECONDS.toNanos(5);
        while (true) {
            try { Files.move(source, destination); return; }
                ...
                if (!System.getProperty("os.name").startsWith("Windows") || System.nanoTime() >= deadline) throw failure;
                Thread.sleep(100);
            }
        }
    }
    ...
    private static void renameDbJar(Path distribution) throws Exception {
        ...
        try (var files = Files.list(distribution.resolve("lib"))) {
            old = files.filter(path -> path.getFileName().toString().startsWith("windowstolinux-web-db-")).findFirst().orElseThrow();
        }
        Path renamed = old.resolveSibling("windowstolinux-web-db-alternate-version.jar"); Files.move(old, renamed);
        ...
        try (var original = new JarFile(launcher.toFile())) {
            var manifest = original.getManifest(); var attributes = manifest.getMainAttributes();
            attributes.putValue("Class-Path", attributes.getValue("Class-Path").replace(old.getFileName().toString(), renamed.getFileName().toString()));
            try (var output = new JarOutputStream(Files.newOutputStream(updated), manifest)) {
                for (var entry : original.stream().toList()) {
                    ...
                    if (!entry.isDirectory()) try (var input = original.getInputStream(entry)) { input.transferTo(output); }
                    output.closeEntry();
                }
            }
        }
