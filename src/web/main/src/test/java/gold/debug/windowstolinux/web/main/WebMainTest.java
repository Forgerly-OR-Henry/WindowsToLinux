...
class WebMainTest {
    ...
    @Test void classesStartWithoutArgumentsOrMasterKeyEnvironmentAndIgnoreWorkingDirectoryForDbResolution() throws Exception {
        ...
        String classpath = Arrays.stream(System.getProperty("surefire.test.class.path", System.getProperty("java.class.path")).split(java.io.File.pathSeparator))
                .map(entry -> Path.of(entry).toAbsolutePath().normalize().equals(original.normalize()) ? classes.toString() : entry)
                .collect(java.util.stream.Collectors.joining(java.io.File.pathSeparator));
        Path keys = root.resolve("keys");
        for (String working : List.of("unrelated-first", "unrelated-second")) {
            try (var process = WebProcessProbe.launch(List.of("-cp", classpath, WebMain.class.getName()), root.resolve(working), keys)) {
    ...
    @Test void obsoleteInternalPathConfigurationAndPublicAddressAreRejected() {
        assertThrows(RuntimeException.class, () -> WebTestApplication.start(root, Map.of("w2l.storage.upload-directory", root.resolve("elsewhere").toString())));
