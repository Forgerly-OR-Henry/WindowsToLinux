...
final class WebProcessProbe implements AutoCloseable {
    ...
    private WebProcessProbe(Process process, Path log, int port) {
    ...
    static WebProcessProbe launch(List<String> arguments, Path working, Path keys) throws Exception {
        ...
        var builder = new ProcessBuilder(command).directory(working.toFile()).redirectErrorStream(true)
                .redirectOutput(log.toFile());
        for (String name : List.of("WEB_TEST_MASTER_KEY", "JAVA_TOOL_OPTIONS", "JDK_JAVA_OPTIONS", "_JAVA_OPTIONS"))
    ...
    private void awaitReady() throws Exception {
        ...
        while (System.nanoTime() < deadline) {
            ...
            try {
                if (get("/api/v1/health").statusCode() == 200)
                    return;
    ...
    HttpResponse<String> get(String path) throws Exception {
        return client.send(
                HttpRequest.newBuilder(URI.create(origin + path)).timeout(Duration.ofSeconds(2)).GET().build(),
