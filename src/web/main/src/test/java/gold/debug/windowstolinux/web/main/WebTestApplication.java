...
final class WebTestApplication implements AutoCloseable {
    ...
    private WebTestApplication(ConfigurableApplicationContext context) { this.context = context; }
    static WebTestApplication start(Path root, Map<String,Object> changes) {
        ...
        app.addInitializers(context -> context.getEnvironment().getPropertySources().addFirst(new MapPropertySource("test-values", properties)));
        return new WebTestApplication(app.run());
    }
    String origin() { return "http://127.0.0.1:" + context.getEnvironment().getRequiredProperty("local.server.port"); }
    ...
    static class IsolatedGateway {
        @Bean @Primary DeploymentLinuxGateway isolatedGateway() {
            return (endpoint, credential, verifier) -> { throw new AssertionError("Local Web tests must not contact SSH"); };
        }
    }
}
