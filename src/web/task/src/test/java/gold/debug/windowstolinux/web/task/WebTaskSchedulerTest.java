...
class WebTaskSchedulerTest {
    ...
    @Test
    void tasksOutliveTheirSubmitterAndEventsReplayInOrder() throws Exception {
        ...
        try (var scheduler = new WebTaskScheduler(repository, policy(Duration.ofMinutes(30)))) {
            String id = scheduler.submit(CONTEXT, work(false, interaction -> {
                interaction.progress("ANALYSIS_STARTED", WebJsonCodec.object());
                return WebJsonCodec.object().put("value", 42);
            }));
    ...
    @Test
    void decisionsPersistResumeExactlyOnceAndNeverAcceptAnExpiredAnswer() throws Exception {
        var repository = repository();
        try (var scheduler = new WebTaskScheduler(repository, policy(Duration.ofSeconds(5)))) {
            String id = scheduler.submit(CONTEXT, work(false, interaction -> interaction.decide("HOST_KEY",
                    WebJsonCodec.object().put("fingerprint", "SHA256:test"))));
        ...
        try (var scheduler = new WebTaskScheduler(repository, policy(Duration.ofMillis(30)))) {
            String id = scheduler.submit(CONTEXT,
                    work(false, interaction -> interaction.decide("REVIEW", WebJsonCodec.object())));
    ...
    @Test
    void cancellationWakesPendingDecisionAndDoesNotMarkMutationSuccessful() throws Exception {
        try (var scheduler = new WebTaskScheduler(repository(), policy(Duration.ofMinutes(30)))) {
            String id = scheduler.submit(CONTEXT,
                    work(true, interaction -> interaction.decide("REVIEW", WebJsonCodec.object())));
            await(scheduler, id, "WAITING_DECISION");
    ...
    @Test
    void sameTargetMutationsAreSerialized() throws Exception {
        try (var scheduler = new WebTaskScheduler(repository(), policy(Duration.ofMinutes(30)))) {
            ...
            String first = scheduler.submit(CONTEXT, operation);
            assertTrue(firstEntered.await(5, TimeUnit.SECONDS));
            String second = scheduler.submit(CONTEXT, operation);
            release.countDown();
            await(scheduler, first, "SUCCEEDED");
            await(scheduler, second, "SUCCEEDED");
            assertEquals(1, peak.get());
        }
    }

    @Test
    void restartMarksEveryUnfinishedTaskForRevalidationAndLeavesTerminalResults() throws Exception {
        var repository = repository();
        var scope = WebPersistence.INTERNAL_SCOPE;
    ...
    @Test
    void rawExceptionsNeverBecomePersistedTaskErrors() throws Exception {
        try (var scheduler = new WebTaskScheduler(repository(), policy(Duration.ofMinutes(30)))) {
            String id = scheduler.submit(CONTEXT, work(false, interaction -> {
                throw new Exception("password=sensitive-value /private/path SQL");
            }));
            await(scheduler, id, "FAILED");
    ...
    @Test
    void rejectedDecisionPayloadsDoNotPersistAndValidAnswersRemainPossible() throws Exception {
        var repository = repository();
        try (var scheduler = new WebTaskScheduler(repository, policy(Duration.ofMinutes(30)))) {
            String id = scheduler.submit(CONTEXT, work(false,
                    interaction -> interaction.decide("SECRET", WebJsonCodec.object().put("code", "backup.password"))));
            await(scheduler, id, "WAITING_DECISION");
            String decision = scheduler.get(CONTEXT, id).path("decision").path("id").asText();
            assertThrows(IllegalArgumentException.class, () -> scheduler.answer(CONTEXT, id, decision,
                    WebJsonCodec.object().put("value", "must-not-persist")));
            assertEquals("WAITING_DECISION", scheduler.get(CONTEXT, id).path("state").asText());
            scheduler.answer(CONTEXT, id, decision, WebJsonCodec.object().put("secretId", "encrypted-reference"));
            await(scheduler, id, "SUCCEEDED");

            try (var connection = database.source().getConnection();
                    var q = connection.createStatement();
                    var rows = q.executeQuery("SELECT answer_json FROM task_decisions")) {
                assertTrue(rows.next());
                assertFalse(rows.getString(1).contains("must-not-persist"));
                assertTrue(rows.getString(1).contains("encrypted-reference"));
            }
        }
    }

    @Test
    void exactInputFieldsAndChoicesAreCheckedBeforeResuming() throws Exception {
        try (var scheduler = new WebTaskScheduler(repository(), policy(Duration.ofMinutes(30)))) {
            ...
            await(scheduler, id, "WAITING_DECISION");
            String decision = scheduler.get(CONTEXT, id).path("decision").path("id").asText();
            for (String invalid : List.of("{\"values\":{\"password\":\"private\"}}",
                    "{\"values\":{\"app/type\":\"SHELL\"}}", "{\"values\":{\"app/type\":true}}"))
                assertThrows(IllegalArgumentException.class,
                        () -> scheduler.answer(CONTEXT, id, decision, WebJsonCodec.read(invalid)));
            scheduler.answer(CONTEXT, id, decision, WebJsonCodec.read("{\"values\":{\"app/type\":\"STATIC_SITE\"}}"));
            await(scheduler, id, "SUCCEEDED");
        }
    }

    @Test
    void secretLikeConfigurationIsRejectedBeforeDecisionPersistence() throws Exception {
        try (var scheduler = new WebTaskScheduler(repository(), policy(Duration.ofMinutes(30)))) {
            ...
            await(scheduler, id, "WAITING_DECISION");
            String decision = scheduler.get(CONTEXT, id).path("decision").path("id").asText();
            assertThrows(RuntimeException.class, () -> scheduler.answer(CONTEXT, id, decision,
                    WebJsonCodec.read("{\"values\":{\"app/configuration\":\"API_TOKEN=must-not-persist\"}}")));
            scheduler.answer(CONTEXT, id, decision,
                    WebJsonCodec.read("{\"values\":{\"app/configuration\":\"PORT=8080\"}}"));
            await(scheduler, id, "SUCCEEDED");
    ...
    private static gold.debug.windowstolinux.web.task.model.WebTaskPolicy policy(Duration timeout) {
        return new gold.debug.windowstolinux.web.task.model.WebTaskPolicy(4, 32, 10000, timeout, Duration.ofSeconds(30),
                Duration.ofSeconds(10));
    }

    private WebTaskRepository repository() throws Exception {
        if (database == null)
            database = new WebDatabaseTestContext(root);
        return database.tasks();
    }

    private static PreparedWebOperation work(boolean mutating, PreparedWebOperation.WebWork action) {
        return new PreparedWebOperation("TEST", WebJsonCodec.object(), List.of(), List.of("test.invalid:22"), mutating,
    ...
    private static void await(WebTaskScheduler scheduler, String id, String state) throws Exception {
        ...
        while (System.nanoTime() < until) {
            if (scheduler.get(CONTEXT, id).path("state").asText().equals(state))
                return;
            Thread.sleep(10);
        }
