 ...
 */
public record WebTaskPolicy(int workers, int maxActive, int maxEvents, Duration decisionTimeout,
        Duration shutdownTimeout, Duration forcedShutdownTimeout) {
    /**
    ...
    public WebTaskPolicy {
        if (workers < 1 || workers > 64 || maxActive < workers || maxEvents < 1 || decisionTimeout == null
                ...
                || forcedShutdownTimeout.isNegative() || forcedShutdownTimeout.isZero())
            throw new IllegalArgumentException("Invalid w2l.tasks limits");
    }
}
