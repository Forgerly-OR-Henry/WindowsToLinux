...
public final class WebTaskScheduler implements AutoCloseable {
    ...
    public synchronized String submit(WebRequestContext context, PreparedWebOperation operation) {
        if (closed || running.size() >= policy.maxActive())
            throw new IllegalStateException("Task queue is unavailable or full");
        String id = UUID.randomUUID().toString();
        ResourceScope scope = scope(context);
        repository.create(scope, id, operation.kind(), WebJsonCodec.write(operation.request()), operation.serverIds(),
                operation.mutating(), operation.sourceId(), operation.applicationId(), operation.backupId());
        var task = new RunningTask(scope, id);
        running.put(key(scope, id), task);
        task.future = workers.submit(() -> execute(task, operation));
        return id;
    }
    ...
    public JsonNode list(WebRequestContext context) {
        ...
        for (var row : repository.list(scope(context)))
            result.add(snapshot(scope(context), row));
     ...
     */
    public JsonNode get(WebRequestContext context, String id) {
        return snapshot(scope(context), require(scope(context), id));
    }
    ...
    public JsonNode events(WebRequestContext context, String id, long after) {
        ResourceScope scope = scope(context);
        require(scope, id);
        return WebJsonCodec.tree(repository.events(scope, id, after).stream()
                .map(event -> Map.of("sequence", event.sequence(), "kind", event.kind(), "message", event.message(),
                        "details", WebJsonCodec.read(event.detailJson()), "createdAt", event.createdAt()))
                .toList());
    }
    ...
    public void cancel(WebRequestContext context, String id) {
        ResourceScope scope = scope(context);
        StoredTask row = require(scope, id);
        if (TaskState.valueOf(row.state()).terminal())
            return;
        RunningTask task = running.get(key(scope, id));
        if (task == null)
            throw new IllegalStateException("Task requires revalidation");
        task.cancelled.set(true);
        repository.transition(scope, id, Set.of("QUEUED", "ANALYZING", "RUNNING", "WAITING_DECISION"), "CANCELLING",
                null, null);
        synchronized (task) {
            task.notifyAll();
            if (task.thread != null)
    ...
    private static void validateAnswer(String kind, JsonNode prompt, JsonNode answer) {
        if (kind == null)
            throw new IllegalStateException("No pending decision");
        switch (kind) {
            case "HOST_KEY", "CONFIRM" -> {
                WebRequestValidator.fields(answer, "accepted");
                if (!answer.path("accepted").isBoolean())
                    throw new IllegalArgumentException("Expected boolean decision");
            }
            case "SECRET" -> {
            ...
            case "DATABASE_REPLACEMENT" -> {
                ...
                for (String field : List.of("backupConfirmed", "downtimeConfirmed", "replacementConfirmed"))
                    if (!answer.path(field).isBoolean())
                        throw new IllegalArgumentException("Expected explicit boolean acknowledgements");
            }
            case "INPUTS" -> {
                WebRequestValidator.fields(answer, "values");
                var names = new ArrayList<String>();
                for (var field : prompt.path("fields"))
                    names.add(field.path("id").asText());
                var values = answer.path("values");
                WebRequestValidator.fields(values, names.toArray(String[]::new));
                ...
                for (var field : prompt.path("fields")) {
                    ...
                    if (!field.path("choices").isEmpty()) {
                        ...
                        for (var choice : field.path("choices"))
                            if (choice.equals(value))
                                allowed = true;
                        if (!allowed)
                            throw new IllegalArgumentException("Unexpected field choice");
                    }
                }
            }
    ...
    private void execute(RunningTask task, PreparedWebOperation operation) {
        ...
        try {
            ...
            if (operation.mutating())
                for (String target : operation.lockKeys().stream().distinct().sorted().toList()) {
                    var lock = locks.computeIfAbsent(target,
                            ignored -> new java.util.concurrent.locks.ReentrantLock(true));
                    lock.lockInterruptibly();
                    held.add(lock);
                    task.checkCancelled();
                }
            if (!repository.transition(task.scope, task.id, Set.of("QUEUED"), "RUNNING", null, null))
                task.checkCancelled();
            JsonNode result = operation.work().execute(task);
            task.checkCancelled();
            repository.transition(task.scope, task.id, Set.of("RUNNING", "ANALYZING"), task.completion.name(),
                    WebJsonCodec.write(result),
                    task.completion == OperationCompletionState.SUCCEEDED ? null : "BUSINESS_OPERATION_FAILED");
        } catch (Exception failure) {
            boolean interrupted = task.cancelled.get() || Thread.currentThread().isInterrupted()
            ...
            try {
                ...
                String state = task.completion == OperationCompletionState.REVALIDATION_REQUIRED
                ...
                System.err.println("Web task state could not be persisted; restart revalidation required.");
            }
        } finally {
    ...
    private JsonNode snapshot(ResourceScope scope, StoredTask task) {
        var json = WebJsonCodec.object().put("id", task.id()).put("kind", task.kind()).put("state", task.state())
                .put("createdAt", task.createdAt()).put("updatedAt", task.updatedAt())
        ...
        if (!decision.isEmpty())
            json.set("decision",
                    WebJsonCodec.object().put("id", decision.get("id")).put("kind", decision.get("kind"))
                            .put("expiresAt", decision.get("expiresAt"))
                            .set("prompt", WebJsonCodec.read(decision.get("prompt"))));
        return json;
    }
     ...
     * @return key text / 键文本
     */
    private static String key(ResourceScope scope, String id) {
    ...
    public void close() {
        ...
        try {
            if (!workers.awaitTermination(policy.shutdownTimeout().toMillis(), TimeUnit.MILLISECONDS)) {
                workers.shutdownNow();
                if (!workers.awaitTermination(policy.forcedShutdownTimeout().toMillis(), TimeUnit.MILLISECONDS))
                    throw new IllegalStateException("Web tasks did not stop");
            }
    ...
    private final class RunningTask implements TaskInteraction {
         ...
         * @param id stable identifier within the owning registry / 所属登记表内的稳定标识
         */
        private RunningTask(ResourceScope scope, String id) {
            this.scope = scope;
            this.id = id;
        }
        ...
        public void completion(OperationCompletionState state) {
            completion = Objects.requireNonNull(state);
        }
         ...
         */
        @Override
        public void progress(String code, JsonNode details) throws Exception {
            checkCancelled();
            if (!code.matches("[A-Za-z0-9_.:-]{1,120}") || ++events > policy.maxEvents())
         ...
         */
        @Override
        public synchronized JsonNode decide(String kind, JsonNode prompt) throws Exception {
            checkCancelled();
            decisionId = UUID.randomUUID().toString();
            answer = null;
            decisionKind = kind;
            decisionPrompt = prompt.deepCopy();
            ...
            if (!repository.transition(scope, id, Set.of("RUNNING", "ANALYZING"), "WAITING_DECISION", null, null)) {
                checkCancelled();
                throw new IllegalStateException("Task cannot ask for a decision");
            }
            while (answer == null) {
                checkCancelled();
                long remaining = Duration.between(Instant.now(), expires).toMillis();
                if (remaining <= 0)
                    throw new TimeoutException("Decision expired");
                wait(Math.min(1000, remaining));
            }
            checkCancelled();
            if (!repository.transition(scope, id, Set.of("WAITING_DECISION"), "RUNNING", null, null)) {
                checkCancelled();
                throw new IllegalStateException("Task cannot continue");
            }
