...
public final class WebRequestFilter extends OncePerRequestFilter {
    ...
    public WebRequestFilter(WebHttpPolicy policy) {
        this.policy = policy;
        requests = new Semaphore(policy.requests());
        streams = new Semaphore(policy.streams());
        uploads = new Semaphore(policy.uploads());
    }
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        try {
            ...
            if (json && request.getContentLengthLong() > policy.jsonBytes())
                throw new ResponseStatusException(HttpStatus.PAYLOAD_TOO_LARGE);
            chain.doFilter(json ? new BoundedWebRequest(request, policy.jsonBytes()) : request, response);
        } catch (Exception failure) {
            if (!response.isCommitted()) {
                var error = WebErrorResponse.from(failure);
                response.setStatus(error.status());
                response.setContentType("application/json;charset=UTF-8");
                response.getWriter().write(WebJsonCodec.write(error));
            }
        } finally {
            if (operation != null)
                operation.release();
    ...
    private void guard(HttpServletRequest request) throws IOException {
        ...
        if (!InetAddress.getByName(request.getRemoteAddr()).isLoopbackAddress()
                || Collections.list(request.getHeaders("Host")).size() != 1 || !host.equals(request.getHeader("Host")))
            throw new SecurityException();
        String supplied = request.getHeader("Origin"), fetch = request.getHeader("Sec-Fetch-Site");
        if (supplied != null && !origin.equals(supplied)
                || fetch != null && !Set.of("same-origin", "none").contains(fetch))
            throw new SecurityException();
        String method = request.getMethod(), path = request.getRequestURI();
        if (!Set.of("GET", "HEAD", "POST", "PUT", "DELETE").contains(method)
                || method.equals("HEAD") && path.startsWith("/api/"))
            throw new ResponseStatusException(HttpStatus.METHOD_NOT_ALLOWED);
        if (!path.startsWith("/api/v1/") && !path.equals("/") && !path.equals("/index.html")
                && !path.matches("/assets/[A-Za-z0-9_.-]+\\.(js|css|svg|woff2|png)"))
            throw new ResponseStatusException(HttpStatus.NOT_FOUND);
        if (!Set.of("GET", "HEAD").contains(method)) {
            if (!origin.equals(supplied) || !"web".equals(request.getHeader("X-W2L-Client")))
                throw new SecurityException();
            String type = request.getContentType();
            if (type == null || !(upload(path)
                    ? type.equals("application/octet-stream")
                    : type.matches("application/json(?:;\\s*charset=[Uu][Tt][Ff]-8)?")))
    ...
    private void query(HttpServletRequest request) {
        ...
        if (query == null)
            return;
        if (query.length() > policy.queryCharacters())
            throw new ResponseStatusException(HttpStatus.PAYLOAD_TOO_LARGE);
        var names = new HashSet<String>();
        for (String pair : query.split("&")) {
            String[] parts = pair.split("=", 2);
            if (!names.add(URLDecoder.decode(parts[0], StandardCharsets.UTF_8)))
