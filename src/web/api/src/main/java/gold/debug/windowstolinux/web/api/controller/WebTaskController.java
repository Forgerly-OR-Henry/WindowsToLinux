...
public final class WebTaskController {
    ...
    public JsonNode create(@RequestBody JsonNode body) throws Exception {
        ...
        return WebJsonCodec.object().put("id", tasks.submit(context, service.prepare(context, WebRequestValidator.text(body, "kind", 60), body.path("input"))));
    }
    /**
    ...
    public JsonNode cancel(@PathVariable String id, @RequestBody JsonNode body) {
        WebRequestValidator.fields(body); tasks.cancel(context, id); return tasks.get(context, id);
    }
    /**
