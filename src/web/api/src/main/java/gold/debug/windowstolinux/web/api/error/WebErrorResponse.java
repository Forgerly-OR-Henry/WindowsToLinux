...
public record WebErrorResponse(String code, String message, String correlationId) {
    ...
    public static WebErrorResponse from(Exception failure) {
        ...
        if (failure instanceof org.springframework.web.HttpRequestMethodNotSupportedException)
            return http(405);
        if (failure instanceof org.springframework.web.HttpMediaTypeNotSupportedException)
            return http(415);
        if (failure instanceof org.springframework.web.servlet.resource.NoResourceFoundException)
            return http(404);
        if (failure instanceof org.springframework.http.converter.HttpMessageNotReadableException
            ...
            return http(400);
        String code = failure instanceof NoSuchElementException
                ? "NOT_FOUND"
                : failure instanceof IllegalArgumentException
                        ? "INVALID_INPUT"
                        : failure instanceof IllegalStateException
                                || failure instanceof org.springframework.dao.OptimisticLockingFailureException
