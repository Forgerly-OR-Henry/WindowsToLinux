...
public final class WebServerService {
    ...
    public JsonNode list(WebRequestContext context) throws Exception {
        return WebJsonCodec.tree(repository.list(scope(context), ResourceType.SERVER).stream().map(WebServerService::view).toList());
    }
    /**
    ...
    public JsonNode save(WebRequestContext context, String id, JsonNode input) throws Exception {
        ...
        var endpoint = new SshEndpoint(id, WebRequestValidator.text(input, "host", 253), input.path("port").asInt(22), WebRequestValidator.text(input, "username", 128));
    ...
    public PreparedWebOperation probe(WebRequestContext context, String id) throws Exception {
        ...
        return new PreparedWebOperation("SERVER_PROBE", WebJsonCodec.object().put("serverId", id), List.of(id), List.of(lockKey(server)), false,
                null, null, null, interaction -> {
                    try {
                        ...
                        var result = WebJsonCodec.object().put("connected", true).put("observedAt", Instant.now().toString())
                        ...
                        observation(context, id, WebJsonCodec.object().put("connected", false).put("observedAt", Instant.now().toString()).put("operatingSystem", "UNKNOWN"));
                        throw failure;
                    }
                });
    }
    ...
    private void observation(WebRequestContext context, String id, JsonNode observation) throws Exception {
        ...
        repository.save(scope(context), ResourceType.SERVER, id, current.name(), current.attributes(), WebJsonCodec.write(observation), current.version());
    }
        ...
        return new HostKeyEvaluator() {
            ...
            @Override public HostKeyDecision verify(SshEndpoint endpoint, String fingerprint) {
                try {
                    ...
                    JsonNode answer = interaction.decide("HOST_KEY", WebJsonCodec.object().put("serverId", captured.id()).put("host", endpoint.host()).put("fingerprint", fingerprint));
    ...
    private static JsonNode view(StoredResource server) {
        ...
        var result = WebJsonCodec.object().put("id", server.id()).put("name", server.name()).put("version", server.version())
        ...
        result.set("observation", WebJsonCodec.read(server.document())); return result;
    }
    /**
     ...
     * @param <T> type of the contract payload / 契约载荷的类型
     */
    @FunctionalInterface public interface SessionAction<T> {
    /**
     * Executes T.
     * <p>执行T。
     *
     ...
     * @param <T> type of the contract payload / 契约载荷的类型
     */
    @FunctionalInterface public interface ConnectionAction<T> {
    /**
     * Executes T.
     * <p>执行T。
     *
