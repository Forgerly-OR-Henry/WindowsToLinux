...
public final class WebDatabaseService {
     ...
     */
    public WebDatabaseService(WebApplicationSecrets secrets, WebServerService servers,
     ...
     */
    public DatabaseProjectInspector.Assessment inputs(WebRequestContext context, Path root, String applicationId,
            Map<String, String> values, TaskInteraction interaction) throws Exception {
        var assessment = new DatabaseProjectInspector().inspect(root);
        if (values.get("type").equals("DOCKERFILE_CONTAINER"))
            assessment = DatabaseProjectInspector.containerStorage(assessment);
        ...
        if (values.containsKey("databaseMode")) {
            ...
            if (Set.of("NONE", "UNREVIEWED").contains(mode) || assessment.databases().size() > 1
                    || assessment.databases().stream().anyMatch(db -> !db.engine().name().equals(mode))
                    || assessment.schemaReviewRequired())
                throw new IllegalArgumentException(
                        "Manual DB bindings conflict with declared database/schema requirements");
            return assessment;
        }
        if (values.get("type").equals("DOCKERFILE_CONTAINER")
            ...
            throw new IllegalArgumentException("Container databases require an explicitly reachable binding");
        if (assessment.endpointConfirmationRequired())
            WebTaskInteractionService.approve(interaction, "db.nativeEndpoint", WebJsonCodec.object());
        return new NativeDatabasePreparationService(secrets.databaseCredentials(context)).completeInputs(root,
                applicationId, assessment, interaction(context, interaction));
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public AutomaticDatabasePreparation prepare(WebRequestContext context, String serverId, Path root,
            String applicationId, DatabaseProjectInspector.Assessment assessment, Map<String, String> values,
            TaskInteraction interaction) throws Exception {
        if (values.containsKey("databaseMode") || assessment.databases().isEmpty())
            return AutomaticDatabasePreparation.empty();
        return servers.withSession(context, serverId, interaction,
                session -> new NativeDatabasePreparationService(secrets.databaseCredentials(context)).prepare(root,
                        applicationId, serverId, session.nativeDatabases(), assessment,
                        interaction(context, interaction), message -> WebTaskInteractionService.progress(interaction,
                                message.key(), WebJsonCodec.object())));
    }
    ...
    private AutomaticDeploymentInteraction interaction(WebRequestContext context, TaskInteraction task) {
        return new AutomaticDeploymentInteraction() {
            ...
            public Optional<Map<String, String>> requestInputs(List<DeploymentInputField> fields) {
                try {
                    return Optional.of(ai.inputs(context, fields, task));
                } catch (InterruptedException cancelled) {
                    Thread.currentThread().interrupt();
                    throw new CancellationException();
                } catch (Exception failure) {
                    throw new IllegalStateException("Cannot collect database inputs", failure);
                }
            }
            ...
            public boolean confirm(String key, Map<String, ?> details) {
                return WebTaskInteractionService.confirm(task, key, WebJsonCodec.tree(details));
            }
            ...
            public boolean confirmDatabaseReplacement(Map<String, ?> details) {
                try {
                    ...
                    return answer.path("backupConfirmed").asBoolean(false)
                            && answer.path("downtimeConfirmed").asBoolean(false)
                            && answer.path("replacementConfirmed").asBoolean(false);
                } catch (InterruptedException cancelled) {
                    Thread.currentThread().interrupt();
                    throw new CancellationException();
                } catch (Exception failure) {
                    throw new IllegalStateException("Cannot confirm database replacement", failure);
                }
            }
            ...
            public char[] requestSecret(String key) {
                try {
                    return secrets.decisionSecret(context, task, key);
                } catch (InterruptedException cancelled) {
                    Thread.currentThread().interrupt();
                    throw new CancellationException();
                } catch (Exception failure) {
                    throw new IllegalStateException("Cannot resolve database credential", failure);
                }
            }
        };
    }
