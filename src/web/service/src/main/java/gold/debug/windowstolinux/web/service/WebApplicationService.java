...
public final class WebApplicationService {
    ...
    public WebApplicationService(WebServerService servers, WebSourceService sources, WebResourceRepository repository,
            WebApplicationInventory applications, WebDeploymentService deployment, WebAiService ai,
            WebApplicationSecrets secrets, gold.debug.windowstolinux.web.service.backup.WebBackupService backups) {
     ...
     */
    public JsonNode listBackups(WebRequestContext context) throws Exception {
        return backups.list(context);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode uploadBackup(WebRequestContext context, String name, InputStream input) throws Exception {
        return backups.upload(context, name, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public java.nio.file.Path downloadBackup(WebRequestContext context, String id) throws Exception {
        return backups.download(context, id);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode listApplications(WebRequestContext context) throws Exception {
        return applications.list(context);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode editApplication(WebRequestContext context, String id, JsonNode input) throws Exception {
        return applications.presentation(context, id, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode saveSecret(WebRequestContext context, JsonNode input) throws Exception {
        return secrets.save(context, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode listAi(WebRequestContext context) throws Exception {
        return ai.list(context);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public PreparedWebOperation saveAi(WebRequestContext context, String id, JsonNode input) throws Exception {
        return ai.save(context, id, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode enableAi(WebRequestContext context, String id, JsonNode input) throws Exception {
        return ai.enabled(context, id, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode reorderAi(WebRequestContext context, JsonNode input) throws Exception {
        return ai.reorder(context, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public void deleteAi(WebRequestContext context, String id, long version) throws Exception {
        ai.delete(context, id, version);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode listServers(WebRequestContext context) throws Exception {
        return servers.list(context);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode saveServer(WebRequestContext context, String id, JsonNode body) throws Exception {
        return servers.save(context, id, body);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public void deleteServer(WebRequestContext context, String id, long version) throws Exception {
        servers.delete(context, id, version);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode listSources(WebRequestContext context) throws Exception {
        return sources.list(context);
    }
    ...
    public JsonNode beginSource(WebRequestContext context, JsonNode input) throws Exception {
        WebRequestValidator.fields(input, "name");
        return sources.begin(context, WebRequestValidator.text(input, "name", 100));
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public void uploadSource(WebRequestContext context, String id, String path, InputStream input) throws Exception {
        sources.upload(context, id, path, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public void uploadArchive(WebRequestContext context, String id, String format, InputStream input) throws Exception {
        sources.archive(context, id, format, input);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode finishSource(WebRequestContext context, String id) throws Exception {
        return sources.finish(context, id);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public JsonNode preferences(WebRequestContext context) throws Exception {
        return WebJsonCodec.tree(repository.preferences(scope(context)));
    }
    ...
    public JsonNode savePreferences(WebRequestContext context, JsonNode body) throws Exception {
        ...
        repository.savePreferences(scope(context), values);
    ...
    public PreparedWebOperation prepare(WebRequestContext context, String kind, JsonNode input) throws Exception {
        var operation = switch (kind) {
            case "SERVER_PROBE" -> {
                WebRequestValidator.fields(input, "serverId");
                yield servers.probe(context, WebRequestValidator.text(input, "serverId", 63));
            }
            case "GIT_SNAPSHOT" -> sources.git(context, input);
            case "ANALYZE" -> {
                WebRequestValidator.fields(input, "sourceId");
                yield sources.analyze(context, WebRequestValidator.text(input, "sourceId", 63));
            }
            case "DEPLOY" -> deployment.prepare(context, input);
            case "LIFECYCLE" -> {
                WebRequestValidator.fields(input, "applicationId", "action");
                yield applications.lifecycle(context, WebRequestValidator.text(input, "applicationId", 63),
                        WebRequestValidator.text(input, "action", 40));
            }
            case "APPLICATION_SCAN" -> {
                WebRequestValidator.fields(input, "serverId");
                yield applications.scan(context, WebRequestValidator.text(input, "serverId", 63));
            }
            case "APPLICATION_ADOPT" -> {
                WebRequestValidator.fields(input, "scanTaskId", "candidateKey");
                yield applications.adopt(context, WebRequestValidator.text(input, "scanTaskId", 63),
                        WebRequestValidator.text(input, "candidateKey", 512));
            }
     ...
     */
    private PreparedWebOperation pinTargets(WebRequestContext context, PreparedWebOperation operation)
            throws Exception {
        var endpoints = new java.util.LinkedHashMap<String, gold.debug.windowstolinux.shared.linux.connection.SshEndpoint>();
        for (String id : operation.serverIds())
            endpoints.put(id, WebServerService.endpoint(servers.require(context, id)));
        var currentLocks = endpoints.keySet().stream()
                .map(id -> endpoints.get(id).host().toLowerCase(java.util.Locale.ROOT) + ":" + endpoints.get(id).port())
                .toList();
        if (operation.mutating()
                && !new java.util.HashSet<>(currentLocks).equals(new java.util.HashSet<>(operation.lockKeys())))
            throw new IllegalStateException("Server endpoint changed while preparing the task");
        return new PreparedWebOperation(operation.kind(), operation.request(), operation.serverIds(),
                operation.lockKeys(), operation.mutating(), operation.sourceId(), operation.applicationId(),
                operation.backupId(), interaction -> {
                    for (var entry : endpoints.entrySet())
                        if (!entry.getValue()
                                .equals(WebServerService.endpoint(servers.require(context, entry.getKey()))))
                            throw new IllegalStateException("Server endpoint changed before task execution");
