...
public final class WebBackupService {
     ...
     */
    public JsonNode list(WebRequestContext context) throws Exception { return WebJsonCodec.tree(repository.list(scope(context),ResourceType.BACKUP).stream().map(WebBackupService::view).toList()); }
    /**
     * Recovers temporary.
     * <p>恢复临时。
     *
    ...
    public PreparedWebOperation prepare(WebRequestContext context,String kind,JsonNode input) throws Exception {
        if(kind.equals("BACKUP_CREATE")) {
            WebRequestValidator.fields(input,"applicationId");String id=WebRequestValidator.text(input,"applicationId",63);var row=applications.require(context,id);
            ...
            return new PreparedWebOperation(kind,input,List.of(serverId),List.of(WebServerService.lockKey(servers.require(context,serverId))),true,null,id,null,interaction -> {
                WebTaskInteractionService.approve(interaction,"BACKUP_STOP_WINDOW",WebJsonCodec.object().put("applicationId",id));
        ...
        if(kind.equals("RESTORE") || kind.equals("RESTORE_PREFLIGHT")) {
            WebRequestValidator.fields(input,"backupId","serverId");String id=WebRequestValidator.text(input,"backupId",63),serverId=WebRequestValidator.text(input,"serverId",63);require(context,id);
            return new PreparedWebOperation(kind,input,List.of(serverId),List.of(WebServerService.lockKey(servers.require(context,serverId))),true,null,null,id,interaction -> {
                if(kind.equals("RESTORE")) WebTaskInteractionService.approve(interaction,"RESTORE_TARGET",WebJsonCodec.object().put("backupId",id).put("serverId",serverId));
                char[] password=secrets.decisionSecret(context,interaction,"backup.password");
    ...
    public synchronized JsonNode create(WebRequestContext context,String applicationId,char[] password,TaskInteraction interaction,String heldMaintenance) throws Exception {
        ...
        if(Set.of("RUNNING","REVALIDATION_REQUIRED","MANUAL_RECOVERY_REQUIRED").contains(WebJsonCodec.read(row.document()).path("deploymentState").asText()))throw new IllegalStateException("Revalidate publication before backup");
    ...
    private StoredResource save(WebRequestContext context,String id,String name,String applicationId,BackupArchiveValidation validation) throws Exception {
        ...
        var manifest=validation.manifest();var document=WebJsonCodec.object().put("application",manifest.applicationId()).put("schema",manifest.schemaVersion()).put("canRestore",manifest.supportsAutomaticActivation())
     ...
     * @return constructed or resolved json node / 构造或解析得到的JSON节点
     */
    private static JsonNode view(StoredResource row) { var result=WebJsonCodec.object().put("id",row.id()).put("name",row.name()).put("version",row.version()).put("createdAt",row.createdAt()).put("digest",row.attributes().get("digest").toString()).put("byteCount",((Number)row.attributes().get("byte_count")).longValue());result.set("inspection",WebJsonCodec.read(row.document()));return result; }
    /**
