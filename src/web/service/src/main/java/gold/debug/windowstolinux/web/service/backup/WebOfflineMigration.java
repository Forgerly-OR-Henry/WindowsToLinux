...
public final class WebOfflineMigration {
    ...
    public PreparedWebOperation prepare(WebRequestContext context,JsonNode input) throws Exception {
        WebRequestValidator.fields(input,"applicationId","targetServerId");String id=WebRequestValidator.text(input,"applicationId",63),target=WebRequestValidator.text(input,"targetServerId",63);
        ...
        return new PreparedWebOperation("MIGRATE",input,List.of(source,target),List.of(WebServerService.lockKey(sourceServer),WebServerService.lockKey(targetServer)),true,null,id,null,interaction -> {
            WebTaskInteractionService.approve(interaction,"MIGRATION_STOP_WINDOW",WebJsonCodec.object().put("applicationId",id).put("sourceServerId",source).put("targetServerId",target));
            ...
            try {
                ...
                return WebJsonCodec.object().put("status",result.status().name()).put("sourceRetained",true).put("externalTrafficChanged",false).put("sourceTaskAdmission",admission);
     ...
     * @param <T> type of the contract payload / 契约载荷的类型
     */
    @FunctionalInterface private interface CheckedAction<T> {
    /**
     * Runs T.
     * <p>运行T。
     *
