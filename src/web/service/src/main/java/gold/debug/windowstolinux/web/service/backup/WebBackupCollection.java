...
public final class WebBackupCollection {
    ...
    public BackupArchiveValidation create(ManagedWebGraph graph,DeploymentRemoteSession session,Path directory,Path output,
        ...
        for (var component : graph.components()) components.add(
        ...
        var request = new gold.debug.windowstolinux.shared.backup.contract.definition.BackupCollectionRequest(
                ...
                heldMaintenance == null, new gold.debug.windowstolinux.shared.deploy.contract.ApplicationHealthGate(
                        graph.healthOwner(), graph.components().stream().filter(component -> component.id().equals(graph.healthOwner()))
                                .findFirst().orElseThrow().runtime().healthCheck()), Set.of(graph.healthOwner()),
                bindings.entrySet().stream().findFirst().map(entry ->
                        new gold.debug.windowstolinux.shared.backup.contract.definition.BackupCollectionRequest.Database(
                                entry.getKey().id(), entry.getValue().databaseId(), profile(entry.getValue()))), quota.fileBytes());
        gold.debug.windowstolinux.shared.backup.contract.definition.BackupCollectionResult collected;
        try {
            collected = new gold.debug.windowstolinux.shared.backup.execution.collection.BackupCollectionService(policy).collect(
                    request, session, new BackupCollectionMaterialPort() {
                        /**
                         ...
                         */
                        @Override public Path member(String name) throws IOException { return WebBackupCollection.member(directory, name); }
                        /**
                         * Opens output stream.
                         * <p>打开输出流。
                         *
                         ...
                         * @throws IOException if the required file or stream operation fails / 所需文件或流操作失败时
                         */
                        @Override public OutputStream open(Path path) throws IOException { return bounded(path); }
                         ...
                         */
                        @Override public void checkCancelled() throws InterruptedException { interaction.checkCancelled(); }
                        /**
                         ...
                         * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
                         */
                        @Override public void collecting(String id) throws Exception {
        ...
        return packageArchive(graph, directory, output, password, secrets, new LinkedHashMap<>(collected.materials()), collected);
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    private BackupArchiveValidation packageArchive(ManagedWebGraph graph,Path directory,Path output,
            char[] password,List<ResolvedSecretRevision> secrets,Map<BackupMember,Path> material,gold.debug.windowstolinux.shared.backup.contract.definition.BackupCollectionResult collected) throws Exception {
        ...
        var inventory=new BackupInventory(entries.stream().map(BackupComponent::releaseManifestPath).toList(),entries.stream().map(BackupComponent::configurationSnapshotPath).toList(),references,
                ...
                collected.database(),new BackupIdentity(graph.applicationId(),graph.components().getFirst().application().server().id(),"/opt/windowstolinux/apps/"+graph.applicationId(),BackupInventory.computeReleaseSetSha256(entries)),
