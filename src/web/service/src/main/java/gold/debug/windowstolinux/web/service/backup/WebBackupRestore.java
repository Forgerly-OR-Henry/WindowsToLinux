...
public final class WebBackupRestore {
     ...
     */
    public WebBackupRestore(WebServerService servers, WebApplicationInventory applications,
     ...
     */
    public JsonNode execute(WebRequestContext context, String serverId, WebRestoreMaterial material,
        ...
        return servers.withSession(context, serverId, interaction, session -> {
            var manifest = material.validation().manifest();
            var targetIdentity = servers.identity(context, serverId);
            var current = applications.managed(context, serverId, manifest.applicationId());
            boolean owned = current.isPresent()
                    && applications.graph(current.orElseThrow()).components().size() == manifest.inventory()
                            .components().size()
                    && applications.graph(current.orElseThrow()).components().stream()
                            .allMatch(value -> manifest.inventory().components().stream()
                                    .anyMatch(component -> component.componentId().equals(value.id())
                                            && component.managedApplicationId().equals(value.application().id())
                                            && component.ownershipManifestSha256()
                                                    .equals(value.application().ownershipManifestSha256())
                                            && value.application().server().equals(targetIdentity)));
            if (current.isPresent() && !owned)
                throw new IllegalStateException("Target ownership differs from backup");
            var evidence = session.restoreActivation().inspectRestoreActivation(manifest.applicationId(),
                    material.validation().verifiedBytes());
            var target = new RestoreTargetEvaluator().evaluate(manifest, material.database().isPresent(), serverId,
                    session.collectCapabilities(), session.collectDeploymentCapabilities(), evidence, owned);
            var plan = new BackupRestorePlan(material.validation(), material.candidate(),
                    material.candidate().root().getParent(), material.candidateId(), RestoreMaterialKind.BINARY_RELEASE,
                    target, material.database());
            new BackupRestorePreflight().verify(plan);
            if (preflightOnly)
                return WebJsonCodec.object().put("status", "PREFLIGHT_PASSED").put("availableBytes",
                        target.availableBytes());
            var documents = material.configurations();
            var components = new ArrayList<ManagedWebComponent>();
            for (var component : manifest.inventory().components()) {
                var document = documents.get(component.componentId());
                components.add(new ManagedWebComponent(component.componentId(),
                        ManagedApplication.forManaged(component.managedApplicationId(), targetIdentity,
                                component.ownershipManifestSha256()),
                        Map.of("type", component.runtime().projectType().name()),
                        Base64.getEncoder().encodeToString(
                                new gold.debug.windowstolinux.shared.backup.format.BackupConfigurationCodec()
                                        .writeActivation(document)),
                        component.dependsOn(), component.releaseSha256().orElseThrow(),
                        component.secretReferences().orElseThrow(),
                        Base64.getEncoder().encodeToString(material.runtimeDocuments().get(component.componentId()))));
            }
            var graph = new ManagedWebGraph(manifest.applicationId(),
                    manifest.inventory().applicationHealthComponentId(), components);
            ...
            for (var revision : material.secrets()) {
                ...
                if (existing.isPresent() && existing.orElseThrow().revision() >= revision.reference().revision()) {
                    ...
                    try {
                        if (!Arrays.equals(stored, incoming))
                            throw new IllegalStateException("Backup secret revision conflicts with existing material");
                    } finally {
                        Arrays.fill(stored, '\0');
                        Arrays.fill(incoming, '\0');
                    }
                } else
                    credentialPort.save(revision.reference(), revision.copyCharacters());
            }
            ...
            try {
                ...
                for (var component : components)
                    staged.put(component.id(), DeploymentInputMapper.stage(session, component.application(),
                            documents.get(component.id()).configuration(), material.secrets().stream()
                                    .filter(value -> component.secrets().contains(value.reference())).toList()));
                var databaseOperations = new LinuxDatabaseOperationPort(session.databaseOperations());
                boolean databaseStaged = false;
                BackupRestoreResult result;
                try {
                    if (material.database().isPresent()) {
                        try (var input = Files.newInputStream(material.databaseFile().orElseThrow())) {
                            databaseOperations.stageArtifact(material.database().orElseThrow().artifact(), input);
                        }
                        databaseStaged = true;
                    }
                    interaction.progress("RESTORE_STARTED", WebJsonCodec.object());
                    var candidates = new LinuxRestoreCandidateAdapter(session,
                            new ManagedRestoreDeploymentPort(session.restoreActivation()), Map.copyOf(staged));
                    ...
                    if (databaseStaged)
                        try {
                            databaseOperations.discardArtifact(material.database().orElseThrow().artifact());
                        } catch (java.io.IOException cleanup) {
                            interaction.progress("REMOTE_CLEANUP_REQUIRED", WebJsonCodec.object());
                        }
                }
                if (result.status() != BackupRestoreStatus.SUCCEEDED)
                    interaction.completion(result.status() == BackupRestoreStatus.MANUAL_RECOVERY_REQUIRED
                            ? OperationCompletionState.REVALIDATION_REQUIRED
                            : OperationCompletionState.FAILED);
                applications.finish(context, record.id(), result.status().name(),
                        result.status() == BackupRestoreStatus.SUCCEEDED ? graph : null, null);
