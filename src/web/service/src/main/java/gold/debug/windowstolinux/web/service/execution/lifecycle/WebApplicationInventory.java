...
public final class WebApplicationInventory {
    ...
    public JsonNode list(WebRequestContext context) throws Exception {
        return WebJsonCodec.tree(repository.list(scope(context), ResourceType.APPLICATION).stream().map(WebApplicationInventory::view).toList());
    }
    /**
    ...
    public StoredResource stage(WebRequestContext context, String serverId, String name, ManagedWebGraph graph) throws Exception {
        ...
        ObjectNode document = old.isPresent() ? (ObjectNode) WebJsonCodec.read(old.orElseThrow().document()) : WebJsonCodec.object();
        document.set("pendingGraph", WebJsonCodec.tree(graph)); document.put("deploymentState", "RUNNING");
        if (!document.has("graph")) document.set("graph", WebJsonCodec.tree(graph));
    ...
    public JsonNode finish(WebRequestContext context, String id, String status, ManagedWebGraph published, JsonNode observation) throws Exception {
        var row = require(context, id); var document = (ObjectNode) WebJsonCodec.read(row.document());
        document.put("deploymentState", status);
        if (published != null) {
            document.set("graph", WebJsonCodec.tree(published)); document.remove("pendingGraph"); document.put("deployedAt", Instant.now().toString());
        ...
        return view(repository.save(scope(context), ResourceType.APPLICATION, id, row.name(), row.attributes(), WebJsonCodec.write(document), row.version()));
    }
    /**
    ...
    public JsonNode presentation(WebRequestContext context, String id, JsonNode input) throws Exception {
        ...
        var document = (ObjectNode) WebJsonCodec.read(row.document()); document.put("category", category).put("accessUrl", access);
        return view(repository.save(scope(context), ResourceType.APPLICATION, id, name, row.attributes(), WebJsonCodec.write(document), row.version()));
    }
    /**
    ...
    public PreparedWebOperation lifecycle(WebRequestContext context, String id, String requestedAction) throws Exception {
        ...
        return new PreparedWebOperation("LIFECYCLE", WebJsonCodec.object().put("applicationId", id).put("action", action.name()), List.of(serverId),
                List.of(WebServerService.lockKey(servers.require(context, serverId))), action != LifecycleAction.REFRESH_STATUS, null, id, null, interaction -> {
                    var current = require(context, id); var document = (ObjectNode) WebJsonCodec.read(current.document());
                    ...
                    if (external) {
                        ...
                        observation = WebJsonCodec.object().put("state", result.state().name()).put("observedAt", Instant.now().toString());
                    } else {
                    ...
                    repository.save(scope(context), ResourceType.APPLICATION, id, latest.name(), latest.attributes(), WebJsonCodec.write(document), latest.version());
                    return safe;
                });
    }
    /**
    ...
    public PreparedWebOperation scan(WebRequestContext context, String serverId) throws Exception {
        ...
        return new PreparedWebOperation("APPLICATION_SCAN", WebJsonCodec.object().put("serverId", serverId), List.of(serverId), List.of(), false,
    ...
    public PreparedWebOperation adopt(WebRequestContext context, String taskId, String key) throws Exception {
        ...
        var result = WebJsonCodec.read(scanTask.resultJson()); String serverId = result.path("serverId").asText();
        ...
        return new PreparedWebOperation("APPLICATION_ADOPT", WebJsonCodec.object().put("scanTaskId", taskId).put("candidateKey", key), List.of(serverId),
                List.of(WebServerService.lockKey(servers.require(context, serverId))), true, null, null, null, interaction -> {
                    ...
                    var document = WebJsonCodec.object().put("canStart", checked.canStart()).put("canStop", checked.canStop());
                    document.set("target", WebJsonCodec.tree(checked.target()));
                    document.set("observation", WebJsonCodec.object().put("state", checked.state().name()).put("observedAt", Instant.now().toString()));
                    var created = repository.save(scope(context), ResourceType.APPLICATION, UUID.randomUUID().toString(), checked.name(),
                            Map.of("server_id", serverId, "kind", "EXTERNAL", "remote_identity", checked.target().key()), WebJsonCodec.write(document), 0);
                    return view(created);
                });
    }
    ...
    private static JsonNode statusProjection(JsonNode raw) {
        if (raw.has("state")) return WebJsonCodec.object().put("state", raw.path("state").asText()).put("observedAt", raw.path("observedAt").asText());
        var result = WebJsonCodec.object().put("observedAt", Instant.now().toString()).put("state", raw.path("runtimeState").asText("UNKNOWN"));
        var items = new ArrayList<JsonNode>();
        for (var component : raw.path("componentResults")) {
            var observation = component.path("observation");
            items.add(WebJsonCodec.object().put("id", component.path("componentId").asText()).put("state", observation.path("runtimeState").asText("UNKNOWN"))
        ...
        result.set("components", WebJsonCodec.tree(items)); return result;
    }
    /**
    ...
    private static JsonNode view(StoredResource row) {
        ...
        var view = WebJsonCodec.object().put("id", row.id()).put("name", row.name()).put("serverId", row.attributes().get("server_id").toString())
        ...
        if (data.has("graph")) view.set("types", WebJsonCodec.tree(graphTypes(data.path("graph"))));
        return view;
    }
    /**
