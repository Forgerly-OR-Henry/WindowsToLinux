...
public final class WebAiService {
     ...
     */
    public JsonNode list(WebRequestContext context) throws Exception { return WebJsonCodec.tree(ordered(context).stream().map(WebAiService::view).toList()); }
    /**
    ...
    public PreparedWebOperation save(WebRequestContext context,String id,JsonNode input) throws Exception {
        ...
        String name=WebRequestValidator.text(input,"name",120),model=new ProviderEndpointPolicy().requireModel(WebRequestValidator.text(input,"model",128));
        URI endpoint=new ProviderEndpointPolicy().validateEndpoint(URI.create(WebRequestValidator.text(input,"endpoint",2048)));
        ...
        if(supplied.isEmpty()) {
            ...
            if(!WebJsonCodec.read(saved.document()).path("endpoint").asText().equals(endpoint.toString())) throw new IllegalArgumentException("Endpoint changes require a new key");
        ...
        var safe=WebJsonCodec.object().put("profileId",profileId).put("name",name).put("endpoint",endpoint.toString()).put("model",model);
        return new PreparedWebOperation("AI_SAVE",safe,List.of(),List.of("ai:"+context.workspaceId()),true,null,null,null,interaction -> {
            ...
            if(result.evidence().status()!=AiInvocationStatus.VALIDATED) {
                interaction.completion(OperationCompletionState.FAILED); return WebJsonCodec.object().put("status","AI_TEST_FAILED");
            }
            var document=WebJsonCodec.object().put("endpoint",endpoint.toString()).put("model",model).put("verifiedAt",Instant.now().toString());
            ...
            return view(repository.save(scope(context),ResourceType.AI_PROFILE,profileId,name,Map.of("priority",priority,"enabled",enabled,"secret_id",secret,"secret_version",1),WebJsonCodec.write(document),old.map(StoredResource::version).orElse(0L)));
        });
    }
    /**
    ...
    public JsonNode enabled(WebRequestContext context,String id,JsonNode input) throws Exception {
        WebRequestValidator.fields(input,"enabled","version"); if(!input.path("enabled").isBoolean()) throw new IllegalArgumentException("Invalid enabled flag");
    ...
    public JsonNode reorder(WebRequestContext context,JsonNode input) throws Exception {
        WebRequestValidator.fields(input,"ids"); if(!input.path("ids").isArray()) throw new IllegalArgumentException("Invalid model order");
    ...
    public Optional<AiRoleInvocationResult> invoke(WebRequestContext context,AiRoleContext role,TaskInteraction interaction) throws Exception {
        ...
        for(var row:snapshot) {
            interaction.checkCancelled(); var document=WebJsonCodec.read(row.document()); AiRoleInvocationResult result=null;
            ...
            catch(Exception unavailable) { interaction.progress("AI_PROVIDER_UNAVAILABLE",WebJsonCodec.object().put("profileId",row.id())); }
            interaction.checkCancelled();
            if(result!=null) {
                interaction.progress("AI_ATTEMPT",WebJsonCodec.object().put("profileId",row.id()).put("status",result.evidence().status().name()));
    ...
    public Map<String,String> inputs(WebRequestContext context,List<DeploymentInputField> fields,TaskInteraction interaction) throws Exception {
        ...
        supplied.putAll(WebTaskInteractionService.inputs(interaction,fields.stream().filter(field -> !supplied.containsKey(field.id())).toList()));
    ...
    private static JsonNode view(StoredResource row) {
        var document=WebJsonCodec.read(row.document());return WebJsonCodec.object().put("id",row.id()).put("name",row.name()).put("version",row.version())
