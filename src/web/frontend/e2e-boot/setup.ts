...
export default async function setup() {
  ...
  if (
    await stat(join(source, "lib/data")).then(
      () => true,
      () => false,
    )
  )
    throw new Error("Build distribution contains runtime data");
  const owned = await mkdtemp(join(target, "boot-browser-"));
  const distribution = join(owned, "web"),
    working = join(owned, "working"),
    keys = join(owned, "keys");
  ...
  await writeFile(
    join(working, "application.yml"),
    `server:\n  address: 127.0.0.1\n  port: ${port}\nw2l:\n  secrets:\n    directory: '${keys.replaceAll("'", "''")}'\n`,
  );
  const environment = { ...process.env };
  for (const name of ["WEB_TEST_MASTER_KEY", "JAVA_TOOL_OPTIONS", "JDK_JAVA_OPTIONS", "_JAVA_OPTIONS"])
  ...
  const java = environment.JAVA_HOME
    ...
    : "java";
  const log = join(owned, "startup.log"),
    output = openSync(log, "w");
  const child = spawn(java, ["-jar", join(distribution, "web.jar")], {
    cwd: working,
    env: environment,
    ...
    if (child.exitCode === null && child.signalCode === null && !startupError) {
      ...
      await Promise.race([
        ...
        new Promise<void>((_, reject) =>
          setTimeout(() => reject(new Error("Spring Boot test process did not stop")), 10000),
        ),
      ]);
    }
    if (dirname(owned) !== target || !basename(owned).startsWith("boot-browser-"))
      throw new Error("Invalid owned cleanup directory");
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const ready = await fetch(`${origin}/api/v1/health`, { signal: AbortSignal.timeout(1000) }).then(
        (result) => result.ok,
        () => false,
      );
      if (ready) {
        process.env.W2L_BOOT_TEST_ORIGIN = origin;
        return cleanup;
      }
