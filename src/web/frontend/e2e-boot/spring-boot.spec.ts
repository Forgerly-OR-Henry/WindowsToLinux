...
test('built Vue uses real Spring Boot CRUD, upload, tasks and persistent preferences', async ({ page }) => {
  ...
  expect(servers.status()).toBe(200); expect(await servers.text()).not.toContain('synthetic-browser-password')
  await page.getByRole('button', { name: '设置', exact: true }).click()
  await page.getByRole('button', { name: '深色', exact: true }).click()
  await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark')
  await expect.poll(async () => (await (await page.request.get(`${origin}/api/v1/preferences`)).json()).theme).toBe('dark')
  await page.reload(); await expect(page.locator('html')).toHaveAttribute('data-theme', 'dark')
  await page.locator('input[type=file][multiple]:not([webkitdirectory])').setInputFiles({ name: 'index.html', mimeType: 'text/html', buffer: Buffer.from('<h1>Local Spring Boot</h1>') })
  await expect.poll(async () => (await (await page.request.get(`${origin}/api/v1/sources`)).json())[0]?.state).toBe('READY')
  const sources = await (await page.request.get(`${origin}/api/v1/sources`)).json()
  const task = await page.request.post(`${origin}/api/v1/tasks`, { headers: { Origin: origin, 'X-W2L-Client': 'web' }, data: { kind: 'ANALYZE', input: { sourceId: sources[0].id } } })
  expect(task.status()).toBe(202); const id = (await task.json()).id
  await expect.poll(async () => (await (await page.request.get(`${origin}/api/v1/tasks/${id}`)).json()).state).toBe('SUCCEEDED')
