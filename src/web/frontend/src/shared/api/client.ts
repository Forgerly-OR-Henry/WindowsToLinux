  ...
  try {
    response = await fetch(`/api/v1/${path}`, {
      ...
      headers:
        method === "GET"
          ? {}
          : { "Content-Type": binary ? "application/octet-stream" : "application/json", "X-W2L-Client": "web" },
      body: method === "GET" ? undefined : binary ? body : JSON.stringify(body ?? {}),
    });
  } catch {
    throw new ApiError("CONNECTION_FAILED", "Connection failed");
  }
  const value = await response.json().catch(() => null);
  if (!response.ok || value === null)
    throw new ApiError(value?.code ?? "CONNECTION_FAILED", value?.message ?? "Connection failed", value?.correlationId);
  return value as T;
}
...
export const terminal = (state: string) =>
  ["SUCCEEDED", "FAILED", "CANCELLED", "INTERRUPTED", "REVALIDATION_REQUIRED"].includes(state);
