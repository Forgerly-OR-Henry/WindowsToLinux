...
class WebStorageLocationTest {
    @TempDir Path root;
    @Test void classLocationComesFromTheDbModuleAndNeverWorkingDirectory() throws Exception {
        var actual = WebStorageLocation.resolve("db.data");
        assertEquals(WebStorageLocation.Mode.CLASS, actual.mode());
        assertTrue(Files.readString(actual.root().getParent().resolve("pom.xml")).contains("<artifactId>windowstolinux-web-db</artifactId>"));
    ...
    @Test void externalJarUsesItsPhysicalParentRegardlessOfFilename() throws Exception {
        ...
        assertEquals(WebStorageLocation.Mode.JAR, location.mode()); assertEquals(libraries.resolve("data/abc"), location.root());
    ...
    @Test void invalidLegacyNestedAndUnknownLocationsFailWithoutFallback() throws Exception {
        ...
        assertThrows(IOException.class, () -> WebStorageLocation.resolve("db.data", java.net.URI.create("jar:file:/outer.jar!/lib/db.jar")));
