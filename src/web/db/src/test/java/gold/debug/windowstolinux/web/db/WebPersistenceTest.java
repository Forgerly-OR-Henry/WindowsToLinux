...
class WebPersistenceTest {
    ...
    @Test
    void schemaKeepsReservedRelationsAndEveryConnectionEnforcesForeignKeys() throws Exception {
        ...
        assertTrue(tables
                .containsAll(Set.of("roles", "permissions", "role_permissions", "member_roles", "user_credentials",
    ...
    @Test
    void compositeIdentityAndMembershipPreventCrossWorkspaceAccess() {
        otherWorkspace();
        var other = new ResourceScope("other", "other");
        ...
        assertThrows(DataIntegrityViolationException.class, () -> resources.save(other, ResourceType.APPLICATION, "app",
                "App", Map.of("server_id", "one", "kind", "MANAGED", "remote_identity", "app"), "{}", 0));
        assertThrows(SecurityException.class,
                () -> resources.list(new ResourceScope("internal", "other"), ResourceType.SERVER));
        ...
        assertThrows(OptimisticLockingFailureException.class,
                () -> resources.delete(other, ResourceType.SERVER, "one", 2));
        assertEquals("Other", resources.find(other, ResourceType.SERVER, "one").orElseThrow().name());
    }

    @Test
    void compareAndSetSurvivesReopenAndNullUpdates() throws Exception {
        var resources = database.resources();
        var first = server();
        first.put("fingerprint", "SHA256:test");
        ...
        assertNull(second.attributes().get("fingerprint"));
        assertThrows(OptimisticLockingFailureException.class,
                () -> resources.save(scope, ResourceType.SERVER, "one", "Stale", server(), "{}", 1));
        database.close();
        database = new WebDatabaseTestContext(root);
    ...
    @Test
    void relationalValidationRejectsMalformedDocumentsPortsAndDuplicateEndpoints() {
        var resources = database.resources();
        assertThrows(DataIntegrityViolationException.class,
                () -> resources.save(scope, ResourceType.SERVER, "one", "First", server(), "no json", 0));
        var bad = server();
        bad.put("port", 0);
        assertThrows(DataIntegrityViolationException.class,
                () -> resources.save(scope, ResourceType.SERVER, "one", "First", bad, "{}", 0));
        resources.save(scope, ResourceType.SERVER, "one", "First", server(), "{}", 0);
        assertThrows(DataIntegrityViolationException.class,
                () -> resources.save(scope, ResourceType.SERVER, "two", "Second", server(), "{}", 0));
    }

    @Test
    void preferencesRollbackAsAUnitAndRejectDisabledMembership() {
        var resources = database.resources();
        resources.savePreferences(scope, Map.of("theme", "dark"));
        var changes = new LinkedHashMap<String, String>();
        changes.put("theme", "light");
        changes.put("invalid", "value");
    ...
    @Test
    void taskTargetsAndInitialEventRollbackTogether() {
    ...
    @Test
    void eventFailureRollsBackStateAndSuccessPublishesExactlyOneEvent() {
        var tasks = database.tasks();
        tasks.create(scope, "task", "PROBE", "{}", List.of(), false, null, null, null);
        database.jdbc().execute(
                "CREATE TRIGGER reject_event BEFORE INSERT ON task_events BEGIN SELECT RAISE(ABORT,'test'); END");
        assertThrows(RuntimeException.class,
                () -> tasks.transition(scope, "task", Set.of("QUEUED"), "RUNNING", null, null));
    ...
    @Test
    void activeTasksPinEndpointAndRecoveryRetainsScopedEvents() {
        var resources = database.resources();
        var tasks = database.tasks();
        resources.save(scope, ResourceType.SERVER, "one", "First", server(), "{}", 0);
        tasks.create(scope, "task", "SERVER_PROBE", "{}", List.of("one"), false, null, null, null);
        var changed = server();
        changed.put("host", "changed.invalid");
        assertThrows(OptimisticLockingFailureException.class,
                () -> resources.save(scope, ResourceType.SERVER, "one", "First", changed, "{}", 1));
        resources.save(scope, ResourceType.SERVER, "one", "First", server(), "{\"connected\":true}", 1);
        assertEquals(1, tasks.recoverInterrupted());
        assertEquals(0, tasks.recoverInterrupted());
        assertEquals("REVALIDATION_REQUIRED", tasks.events(scope, "task", 1).getFirst().message());
        assertEquals("changed.invalid",
                resources.save(scope, ResourceType.SERVER, "one", "First", changed, "{}", 2).attributes().get("host"));
    }

    @Test
    void resourceAndConfigurationRevisionCommitOrRollbackTogether() {
        var resources = database.resources();
        resources.save(scope, ResourceType.SERVER, "one", "First", server(), "{}", 0);
        ...
        database.jdbc().execute(
                "CREATE TRIGGER reject_revision BEFORE INSERT ON configuration_revisions BEGIN SELECT RAISE(ABORT,'test'); END");
    ...
    @Test
    void schemaFailureRollsBackAndUnknownVersionIsPreserved() throws Exception {
        Path broken = root.resolve("broken");
        java.nio.file.Files.createDirectory(broken);
        var source = new SQLiteDataSource();
        source.setUrl("jdbc:sqlite:" + broken.resolve(WebStorageLocation.DATABASE_NAME));
        var jdbc = new JdbcTemplate(source);
        jdbc.execute("CREATE TABLE tasks (keep_value TEXT)");
    ...
    private static Map<String, Object> server() {
        ...
        values.put("host", "test.invalid");
        values.put("port", 22);
        values.put("username", "deploy");
        values.put("fingerprint", null);
        values.put("secret_id", null);
        values.put("secret_version", null);
        return values;
    }
}
