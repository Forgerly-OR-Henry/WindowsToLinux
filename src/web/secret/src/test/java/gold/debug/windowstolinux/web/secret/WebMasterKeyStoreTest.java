...
class WebMasterKeyStoreTest {
    @TempDir Path root;
    @Test void firstRunCreatesPrivateKeyAndRelocationReusesIt() throws Exception {
        ...
        if (posix != null) assertEquals(PosixFilePermissions.fromString("rw-------"), posix.readAttributes().permissions());
            ...
            assertTrue(acl.getAcl().stream().filter(entry -> entry.type() == AclEntryType.ALLOW).allMatch(entry -> entry.principal().equals(owner)));
        }
    ...
    @Test void missingOrDamagedExistingKeyNeverGetsReplaced() throws Exception {
        ...
        Files.delete(saved);
        assertThrows(IOException.class, () -> WebMasterKeyStore.loadOrCreate(data, keys));
        assertFalse(Files.exists(saved)); assertEquals("keep", Files.readString(data.resolve(WebStorageLocation.DATABASE_NAME)));
        Files.delete(data.resolve(".master-key-id"));
        assertThrows(IOException.class, () -> WebMasterKeyStore.loadOrCreate(data, keys));
        assertFalse(Files.exists(data.resolve(".master-key-id")));
    }
    @Test void keysCannotBeStoredInsideTheDataRoot() throws Exception {
