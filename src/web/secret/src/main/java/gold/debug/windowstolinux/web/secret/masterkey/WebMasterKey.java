...
public final class WebMasterKey implements AutoCloseable {
    ...
    public WebMasterKey(byte[] supplied) {
        if (supplied == null)
            throw new IllegalArgumentException("Web master key is missing");
        try {
            if (supplied.length != 32)
                throw new IllegalArgumentException("Expected a 256-bit Web key");
    ...
    public synchronized byte[] copy() {
        if (closed)
            throw new IllegalStateException("Web master key is closed");
        return bytes.clone();
    }
    /**
     * Closed.
