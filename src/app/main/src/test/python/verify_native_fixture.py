    ...
    with tempfile.TemporaryDirectory(prefix='native-fixture-', dir=work_parent) as temporary:
        ...
        def request(path):
            result = subprocess.run(
                [str(executable), path], cwd=work, env=environment, capture_output=True, check=True, timeout=5
            )
        ...
        broken = subprocess.run(
            [part for part in command if part != 'src/summary' + suffix], cwd=work, capture_output=True, timeout=60
        )
