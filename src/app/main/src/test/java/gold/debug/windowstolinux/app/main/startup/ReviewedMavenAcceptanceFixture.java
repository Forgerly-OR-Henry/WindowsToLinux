...
final class ReviewedMavenAcceptanceFixture {
    ...
    static ReviewedDeploymentRequest request(DesktopApplicationFacade service, ReviewedSourcePreparation preparation,
            ServerIdentity server, HealthCheck health, Optional<UserAccessUrl> userAccessUrl,
            BuildLimitConfiguration limits, boolean rootBuild) throws Exception {
        ...
        ConfigurationSnapshot configuration = ConfigurationSnapshot.create(applicationId, revision,
                "maven-acceptance-v1", Instant.now(), List.of(new ConfigurationEntry("ACCEPTANCE_RUN_ID",
                        ConfigurationScope.RUNTIME, new ConfigurationValue.Text("maven-acceptance-" + revision))));
