...
class ManagedSystemPreparationAcceptanceTest {
    ...
    @Test void confirmsSystemChangesSeparatelyThenPreparesTheEnvironmentTwice() throws Exception {
        ...
        try (var context = new LiveTypedDeploymentContext(directory)) {
            ...
            if (before.securityPosture().state() != LinuxSecurityState.ENFORCING) {
                assertThrows(DeploymentApprovalException.class, () -> context.prepareEnvironmentWithSystemConfirmation(plan -> {
                    declined.set(plan);
            ...
            var result = context.prepareEnvironmentWithSystemConfirmation(plan -> {
                if (declined.get() != null) assertEquals(declined.get(), plan, "Declined preparation changed target facts");
            ...
            var repeated = context.prepareEnvironmentWithSystemConfirmation(plan -> fail("Verified preparation asked for system changes again"));
