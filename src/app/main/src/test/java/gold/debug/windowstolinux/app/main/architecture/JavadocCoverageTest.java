...
class JavadocCoverageTest {
    @Test void responsibilityLineLimitsExcludeOnlyActualDocumentation() throws Exception {
        String source = "class Example {\n    String script = \"\"\"\n/** protocol text */\n\"\"\";\n\n// implementation rationale\nvoid run() {}\n}\n";
        assertEquals(8, JavadocCoverageInspector.implementationLines(source));
        assertEquals(8, JavadocCoverageInspector.implementationLines(source.replace("void run()", "/**\n * Operation documentation.\n */\nvoid run()")));
        assertEquals(8, JavadocCoverageInspector.implementationLines(source.replace("void run()", "/** Inline documentation. */ void run()")));
        assertEquals(9, JavadocCoverageInspector.implementationLines(source.replace("void run()", "int additionalState;\nvoid run()")));
    }
    @Test void everyExplicitProductionDeclarationHasBilingualDocumentationAndTags() throws Exception {
        var result = JavadocCoverageInspector.production();
        Path report = Path.of("target/javadoc-coverage.txt"); Files.createDirectories(report.getParent());
        Files.writeString(report, "Explicit declarations: " + result.declarations() + "\n" + String.join("\n", result.violations()));
        assertTrue(result.violations().isEmpty(), () -> String.join("\n", result.violations().stream().limit(80).toList())
                + "\nFull report: " + report.toAbsolutePath());
    }

    @Test void missingMonolingualAndIncompleteTagsFailWithoutRequiringSyntheticMembers() throws Exception {
        assertFalse(JavadocCoverageInspector.snippets("class Example { int value; void run() {} }").violations().isEmpty());
        ...
        assertEquals(2, result.declarations()); assertTrue(result.violations().isEmpty(), result.violations().toString());
        var annotated = JavadocCoverageInspector.snippets(valid.replace("(T value)", "(@SuppressWarnings(\"private\") T value)"));
        assertEquals(2, annotated.declarations()); assertTrue(annotated.violations().isEmpty(), annotated.violations().toString());
        ...
        assertFalse(JavadocCoverageInspector.snippets(valid.replace("unchanged input / 未变的输入", "unchanged input")).violations().isEmpty());
    }

    @Test void privateAnonymousAndEnumDeclarationsAreIncluded() throws Exception {
        String source = """
                /** Fixture owner. / 夹具持有者。 */
                class Example {
                    private int count;
    ...
    @Test void jdk21DoclintAcceptsProductionSyntaxAndReferences() throws Exception {
        assertEquals(21, Runtime.version().feature(), "Use the documented JDK 21 toolchain");
        var tool = ToolProvider.getSystemDocumentationTool(); assertNotNull(tool);
        ...
        try (var manager = tool.getStandardFileManager(null, null, StandardCharsets.UTF_8)) {
            ...
            assertTrue(valid, () -> diagnostics.toString().substring(0, Math.min(12000, diagnostics.toString().length())));
        }
    }
}
