...
class UbuntuYarnStopAcceptanceTest {
    @Test
    void observesFailedReleasesThenStopsAndVerifiesIdempotenceWithoutReplacingData() throws Exception {
        ...
        try (var client = SshClient.setUpDefaultClient()) {
            client.setServerKeyVerifier(
                    (session, address, key) -> pin.equals(KeyUtils.getFingerPrint(BuiltinDigests.sha256, key)));
            client.start();
            try (var connection = client.connect("root", host, port).verify(Duration.ofSeconds(20)).getSession()) {
                connection.addPasswordIdentity(password);
                connection.auth().verify(Duration.ofSeconds(20));
                var diagnostics = new SshCommandExecutor(connection);
                try (var session = new SshdLinuxGateway(
                        ...
                        gold.debug.windowstolinux.shared.standard.deploy.distro.extension.registry.DistributionSetupRegistry
                                .defaults())
                        .connect(endpoint, new SshCredential.Password(password.toCharArray()),
                                (remote, fingerprint) -> pin.equals(fingerprint)
                                        ? HostKeyDecision.ACCEPT_EXISTING
                    ...
                    for (String id : System.getProperty("managed.runtime.yarn.existing").split(",")) {
                        ...
                        var manifest = diagnostics.execProtocol("cat " + root + "/current/.windowstolinux-owner",
                        ...
                        var release = diagnostics.execProtocol("readlink -f " + root + "/current",
                                Duration.ofSeconds(10), true);
                        ...
                        for (int attempt = 1; attempt <= 2; attempt++) {
                            var stopped = session.executeLifecycle(application, LifecycleAction.STOP,
                                    new HealthCheck.Tcp(18080, 1, 1));
                            assertTrue(stopped.ownershipVerified());
                            ...
                            String script = "set -eu\nunit="
                                    + gold.debug.windowstolinux.shared.linux.command.CommandText.quote(unit) + "\n"
                            ...
                            System.out.println("LIVE_YARN_STOP " + id + " attempt=" + attempt + " " + stopped.evidence()
                                    + " " + checked.output().strip());
                        }
                    }
                } finally {
                    SshSessionLifecycleExecutor.closeQuietly(connection);
                }
            } finally {
