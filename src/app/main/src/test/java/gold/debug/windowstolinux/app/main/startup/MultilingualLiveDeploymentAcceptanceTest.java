...
class MultilingualLiveDeploymentAcceptanceTest {
    ...
    final org.junit.jupiter.api.extension.TestWatcher failureEvidence = new org.junit.jupiter.api.extension.TestWatcher() {
        @Override public void testFailed(org.junit.jupiter.api.extension.ExtensionContext context, Throwable failure) {
    ...
    void deploysTaskBoardThroughAutomaticEntry() throws Exception {
        ...
        values.put("backend/configuration", "HOST=0.0.0.0;PORT=" + apiPort + ";WEB_ORIGIN=http://" + host + ":" + webPort);
        ...
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("state"))) {
            try (AutoCloseable cleanup = context::stopTestApplications) {
                ...
                var result = context.service.deployAutomatically(new AutomaticDeploymentRequest(Optional.of(source), Optional.empty(),
                        profile, values), masterPassword(), interaction(), fingerprint -> true,
                ...
                var created = requestJson(api + "/api/tasks", Map.of(
                        "projectId", 1, "title", "实机部署持久化验证", "description", "Java + TypeScript",
                        "ownerId", 1, "priority", 2, "labels", List.of("live"), "dependsOn", List.of(),
                        "dueDate", "2030-10-01", "version", 0, "actorId", 1));
                ...
                System.out.println("POLYGLOT_BUSINESS task-board create + stop/start/restart + SQLite persistence passed");
            }
        }
    }
    ...
    void deploysGatewayAndBackendThroughAutomaticEntry(String project) throws Exception {
        ...
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("state"));
            ...
            var result = context.service.deployAutomatically(new AutomaticDeploymentRequest(Optional.of(source), Optional.empty(),
                    context.service.findServerProfile("typed-live").orElseThrow(), values), masterPassword(), interaction(),
            ...
            if (project.equals("file-transfer")) {
                ...
                String sha = java.util.HexFormat.of().formatHex(java.security.MessageDigest.getInstance("SHA-256").digest(fileContent));
                var upload = requestJson(url + "/api/uploads", Map.of("folderId", created.path("id").asLong(),
                        "name", "实机文件.bin", "size", fileContent.length, "sha256", sha, "chunkSize", fileContent.length));
                ...
                var asset = requestJson(url + "/api/assets", Map.of("name", "实机借还资产", "serial", "LIVE-001",
                        "categoryId", created.path("id").asLong()));
                long assetId = asset.path("id").asLong(); assertTrue(assetId > 0, asset::toString);
            ...
            if (project.equals("file-transfer")) assertArrayEquals(fileContent, transferBytes(persistedUrl, "GET", null));
            else assertHttp(persistedUrl, "closed");
    ...
    void deploysSurveyScoringThroughAutomaticEntry() throws Exception {
        ...
        values.put("backend/exposure", "EXTERNAL");
        values.put("backend/configuration", "HOST=0.0.0.0;PORT=" + apiPort + ";WEB_ORIGIN=http://" + host + ":" + webPort
                + ";SCORER_URL=http://127.0.0.1:18142");
        ...
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("state"));
            ...
            var scored = requestJson(api + "/api/submissions", Map.of("revisionId", 1, "requestId", "live-score-001",
                    "respondent", "林同学", "answers", Map.of("participated", "yes", "quality", 4,
                            "support", List.of("docs", "peer"), "friction", 2)));
            ...
            System.out.println("POLYGLOT_BUSINESS survey-scoring Kotlin/Ruby score + restart + SQLite persistence passed");
        }
    }

    private Path copyFixture(String project, String name) throws Exception {
        Path original = MultilingualDeploymentPlanningTest.repositoryRoot().resolve("test/multi-language/success-" + project);
    ...
    void deploysCsvQueueAndAnalyzerThroughAutomaticEntry() throws Exception {
        ...
        Map<String, String> values = Map.of("web/dependencies", "analyzer", "web/configuration", "ANALYZER_URL=http://127.0.0.1:18131",
                "analyzer/configuration", "HOST=127.0.0.1;PORT=18131");
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("state"));
             AutoCloseable cleanup = context::stopTestApplications) {
            ...
            var job = requestJson(url + "/api/jobs", Map.of("datasetId", "demo", "templateId", "basic", "requestId", "live-csv-001"));
            ...
            while (!completed.path("status").asText().equals("completed") && System.nanoTime() < deadline) {
                Thread.sleep(250); completed = getJson(jobUrl);
            ...
            System.out.println("POLYGLOT_BUSINESS csv-inspector PHP queue/Python report + unified restart + SQLite persistence passed");
        }
    }
    ...
    private static byte[] transferBytes(String url, String method, byte[] body) throws Exception {
        ...
        connection.setConnectTimeout(10000); connection.setReadTimeout(20000); connection.setRequestMethod(method);
        try {
            if (body != null) {
                connection.setDoOutput(true); connection.setRequestProperty("Content-Type", "application/octet-stream");
    ...
    private static com.fasterxml.jackson.databind.JsonNode getJson(String url) throws Exception {
        ...
        try {
            assertEquals(200, connection.getResponseCode(), url);
            try (var input = connection.getInputStream()) { return new com.fasterxml.jackson.databind.ObjectMapper().readTree(input); }
        } finally { connection.disconnect(); }
    }
    ...
    void deploysOnDemandMixedApplications(String project) throws Exception {
        ...
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("state"))) {
            var result = context.service.deployAutomatically(new AutomaticDeploymentRequest(Optional.of(source), Optional.empty(),
                    context.service.findServerProfile("typed-live").orElseThrow(), Map.of()), masterPassword(), interaction(),
                    fingerprint -> true, message -> System.out.println("POLYGLOT_PROGRESS " + message));
            System.out.println("POLYGLOT_DEPLOY " + project + " " + result);
            assertEquals(DeploymentStatus.SUCCEEDED, result.status());
            assertEquals(id, result.applicationId());
            assertEquals(1, result.handoffs().size());
            var entry = assertInstanceOf(gold.debug.windowstolinux.app.service.deployment.single.DeploymentHandoff.ApplicationEntry.class,
            ...
            String arguments = switch (project) {
                ...
                case "directory-diff" -> " --db data/snapshots.db --format json snapshot --root samples/baseline --name live-baseline";
            ...
            if (project.equals("log-analyzer")) {
                ...
                var rejected = executePublishedEntry(context, entry.command() + " --input samples/corrupt.bin --format json");
                assertEquals(2, rejected.exitStatus(), rejected::failureEvidence);
            } else {
                var history = executePublishedEntry(context, entry.command() + " --db data/snapshots.db --format json list");
    ...
    private static void selectedProject(String project) {
        ...
        org.junit.jupiter.api.Assumptions.assumeTrue(projects.isBlank() || List.of(projects.split(",")).contains(project),
                "project not selected for this live run");
    }

    private static gold.debug.windowstolinux.shared.linux.sshd.command.SshCommandExecutor.CommandResult executePublishedEntry(
        ...
        return withSshCommands(context, executor -> executor.execProtocol(command, java.time.Duration.ofMinutes(3), true));
    }
    ...
    void cleansConfirmedInterruptedCandidatesThroughProductAdapter() throws Exception {
        ...
        try (LiveTypedDeploymentContext context = new LiveTypedDeploymentContext(temporaryDirectory.resolve("repair-state"))) {
            withSshCommands(context, executor -> {
                var adapter = new gold.debug.windowstolinux.shared.linux.sshd.execution.protocol.CandidateWorkspaceExecutor(executor);
                for (String declaration : supplied.split(",")) {
                    String[] fields = declaration.split(":", -1);
                    assertEquals(2, fields.length);
                    assertTrue(fields[0].startsWith("wtl-polyglot-"));
                    var candidate = new gold.debug.windowstolinux.shared.linux.transfer.RemoteWorkspace(fields[0], fields[1]);
    ...
    private static <T> T withSshCommands(LiveTypedDeploymentContext context, SshAction<T> action) throws Exception {
        try (var client = org.apache.sshd.client.SshClient.setUpDefaultClient()) {
            client.setServerKeyVerifier((session, address, key) -> context.trustedServer().hostKeySha256().equals(
                    org.apache.sshd.common.config.keys.KeyUtils.getFingerPrint(org.apache.sshd.common.digest.BuiltinDigests.sha256, key)));
            client.start();
            try (var session = client.connect("root", System.getProperty("managed.ssh.host"), Integer.getInteger("managed.ssh.port", 22))
                ...
                return action.apply(new gold.debug.windowstolinux.shared.linux.sshd.command.SshCommandExecutor(session));
            }
        }
    }
        ...
        return new AutomaticDeploymentInteraction() {
            public Optional<Map<String, String>> requestInputs(List<DeploymentInputField> fields) {
                ...
                for (var field : fields) {
                    ...
                    if (field.id().endsWith("/dependencies")) answers.put(field.id(), "");
                    else if (field.id().endsWith("/healthOwner") && field.choices().contains("frontend")) answers.put(field.id(), "frontend");
                    else if (field.id().endsWith("/healthOwner") && field.choices().contains("web")) answers.put(field.id(), "web");
                    else throw new AssertionError("unresolved deployment input: " + field);
                }
            ...
            public char[] requestSecret(String key) { throw new AssertionError("unexpected secret request: " + key); }
        };
    }

    private static char[] masterPassword() {
        return System.getenv().getOrDefault("WINDOWSTOLINUX_TEST_MASTER_PASSWORD", "typed-live-acceptance-master").toCharArray();
    }
    ...
    private static void assertHttp(String url, String expected) throws Exception {
        ...
        try {
            ...
            try (var input = connection.getInputStream()) {
                assertTrue(new String(input.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8).contains(expected), url);
            }
        } finally { connection.disconnect(); }
    }

    private static com.fasterxml.jackson.databind.JsonNode requestJson(String url, Map<String, Object> body) throws Exception {
        ...
        try {
            try (var output = connection.getOutputStream()) { output.write(mapper.writeValueAsBytes(body)); }
            assertEquals(200, connection.getResponseCode(), url);
            try (var input = connection.getInputStream()) { return mapper.readTree(input); }
        } finally { connection.disconnect(); }
    }
}
