...
export async function verify(page, expect) {
  ...
  await expect(page.locator("#folder option:checked")).toHaveText("浏览器文件夹");
  ...
  await expect(page.locator("#progress-text")).toHaveText("批量.txt 已校验并发布");
  await page.locator("#files").setInputFiles({ ...file, buffer: Buffer.from("版本二") });
  await page.locator("#upload-button").click();
  await expect(page.locator("#progress-text")).toHaveText("浏览器 中文.txt 已校验并发布");
  ...
  const [download] = await Promise.all([
    ...
    page.locator("#versions tr").nth(1).getByRole("link", { name: "下载" }).click(),
  ]);
  if (!file.buffer.equals(await fs.readFile(await download.path()))) throw new Error("Historical bytes changed");
  await page.getByRole("link", { name: "上传 / 续传" }).click();
  ...
  await page.locator("#upload-button").click();
  await expect(page.locator("#progress-text")).toHaveText("暂停恢复.bin 已校验并发布");
  ...
  await page.locator("#history tr").first().getByRole("button", { name: "查看 / 恢复" }).click();
