...
public final class AiPage {
     ...
     */
    public AiPage(AiApplicationFacade service, ReviewContext reviewContext, DesktopComponentFactory c,
        ...
        for (var capability : AiCapabilityType.values()) {
            ...
            test.addActionListener(event -> test(capability));
            panel.addOption(test);
        }
        JButton explain = c.primaryButton(messages.text("button.requestAi"));
        explain.addActionListener(event -> explain());
        panel.addOption(explain);
        cancel = c.secondaryButton(messages.text("button.cancel"));
        cancel.setEnabled(false);
        cancel.addActionListener(event -> {
            if (task != null) {
                task.cancel();
                output.setText(messages.text("ai.models.cancelling"));
            }
        });
        panel.addOption(cancel);
        output.setRows(16);
        panel.addOption(new JScrollPane(output));
    ...
    public AiPageState captureState() {
        return new AiPageState(masterPassword.getPassword(), output.getText(), inventory.capture());
    }
    ...
    public void restoreState(AiPageState state) {
        char[] password = state.masterPassword();
        try {
            masterPassword.setText(new String(password));
        } finally {
    ...
    private void edit(AiProviderSummary existing) {
        if (busy || service == null)
            return;
        new AiProviderDialog(SwingUtilities.getWindowAncestor(panel), service, c, messages, existing,
                inventory::refresh).setVisible(true);
    }
    ...
    private void test(AiCapabilityType capability) {
        var model = inventory.current();
        if (busy || service == null || model == null)
            return;
        char[] master = masterPassword.getPassword();
        setBusy(true);
        output.setText(messages.text("ai.models.testing"));
        task = DesktopTaskExecutor.submit(() -> {
            try {
                service.testAiCapability(model.profile().id(), capability, master);
                return true;
            } finally {
            ...
            setBusy(false);
            output.setText(messages.text("ai.capability.passed"));
            inventory.refresh();
        }, failure -> {
            setBusy(false);
    ...
    private void explain() {
        ...
        if (preparation.isEmpty() || preparation.get().assessment().facts().isEmpty()) {
        ...
        task = DesktopTaskExecutor.submit(() -> {
            try {
                return service.invokeAiRole(
                        ProjectAnalysisRoleContext.from(preparation.get().assessment().facts().orElseThrow()), master);
            } finally {
                Arrays.fill(master, '\0');
            }
        }, result -> {
            setBusy(false);
            output.setText(
                    result.map(this::evidenceText).orElseGet(() -> messages.text("ai.status.noEnabledProviders")));
        }, failure -> {
            boolean cancelled = task != null && task.cancelled();
            setBusy(false);
            output.setText(cancelled ? messages.text("ai.models.cancelled") : messages.safe(failure));
        });
    }
    ...
    private String evidenceText(AiRoleInvocationResult result) {
        ...
        return messages.text("ai.roleEvidence",
                Map.of("provider", e.providerId(), "model", e.model(), "status",
                        messages.text("ai.invocation." + e.status().name().toLowerCase(Locale.ROOT)), "digest",
                        e.inputSha256(), "validation", e.validationDetail(), "decision",
                        e.output()
                                .map(v -> messages.text("ai.decision." + v.decision().name().toLowerCase(Locale.ROOT)))
                                .orElse("-"),
