...
public final class DeploymentInputDialog implements AutomaticDeploymentInteraction, DesktopRecoveryInteraction {
    ...
    public Optional<Map<String, String>> requestInputs(List<DeploymentInputField> fields) {
        return onEdt(() -> {
            ...
            inputs.forEach((id, input) -> values.put(id,
                    input instanceof JTextField text
                            ? text.getText().trim()
                            : input instanceof JScrollPane scroll
                                    ? ((JTextArea) scroll.getViewport().getView()).getText().trim()
    ...
    private JComponent input(DeploymentInputField field) {
        ...
        if (field.choices().isEmpty())
            return new JTextField(field.value(), 24);
        ...
        choices.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean selected,
                ...
                return super.getListCellRendererComponent(list,
                        messages.inputChoice(field, Objects.toString(value, "")), index, selected, focus);
            }
        });
        if (!field.value().isBlank())
            choices.setSelectedItem(field.value());
        return choices;
    }
    ...
    public boolean confirmDatabaseReplacement(Map<String, ?> details) {
        return onEdt(() -> {
            ...
            explanation.setText(messages.text("db.replaceConfirmed", details));
            explanation.setRows(7);
            ...
            panel.add(checks, BorderLayout.SOUTH);
            while (JOptionPane.showConfirmDialog(owner, panel, messages.text("auto.waiting"),
                    JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE) == JOptionPane.OK_OPTION) {
                if (backup.isSelected() && downtime.isSelected() && replacement.isSelected())
                    return true;
    ...
    public boolean confirm(String key, Map<String, ?> details) {
        if (key.startsWith("deployment.agent."))
            return onEdt(() -> {
                JTextArea text = new JTextArea(messages.text(key, details), 24, 90);
                text.setEditable(false);
                text.setLineWrap(true);
                text.setWrapStyleWord(true);
                text.setCaretPosition(0);
                return JOptionPane.showConfirmDialog(owner, new JScrollPane(text), messages.text("auto.waiting"),
                        JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION;
            });
        if (key.equals("environment.confirm")) {
            return gold.debug.windowstolinux.app.ui.component.SystemPreparationDialog.confirmEnvironment(owner,
                    messages, details);
        }
        ...
        Map<String, ?> arguments = key.equals("db.conflict")
                ? Map.of("detail",
                        ((List<?>) details.get("conflicts")).stream().map(value -> value.toString().split("\\|", 2))
                                .map(parts -> messages.text("db.conflict." + parts[0],
                                        Map.of("instance", parts.length > 1 ? parts[1] : "")))
                                .collect(java.util.stream.Collectors.joining("\n")))
                : details;
        return onEdt(
                () -> JOptionPane.showConfirmDialog(owner, messages.text(key, arguments), messages.text("auto.waiting"),
                        JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION);
    }
     ...
     */
    @Override
    public boolean offerSshRecovery(gold.debug.windowstolinux.app.service.server.ServerProfile server) {
     ...
     */
    @Override
    public void showSshRecovery(gold.debug.windowstolinux.app.service.server.ServerProfile server,
            gold.debug.windowstolinux.app.service.contract.SshRecoverySession session) {
        onEdt(() -> new gold.debug.windowstolinux.app.ui.recovery.SshRecoveryDialog(owner, server, session, service,
                components, messages));
    }
}
