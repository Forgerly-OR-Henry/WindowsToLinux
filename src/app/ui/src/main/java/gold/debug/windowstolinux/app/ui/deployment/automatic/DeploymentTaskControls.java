...
final class DeploymentTaskControls extends JPanel {
     ...
     */
    DeploymentTaskControls(AutomaticDeploymentApplicationFacade service, PageMessagePresenter messages,
            Consumer<String> output, java.util.function.Supplier<char[]> credentials) {
        ...
        setOpaque(false);
        pause.setText(messages.text("deployment.task.pause"));
        resume.setText(messages.text("deployment.task.resume"));
        cancel.setText(messages.text("deployment.task.cancel"));
        refreshModels.setText(messages.text("deployment.task.refreshModels"));
        refreshModels.addActionListener(e -> command(AgentTaskCommandAction.REFRESH_MODELS));
        pause.addActionListener(e -> command(AgentTaskCommandAction.PAUSE));
        resume.addActionListener(e -> command(AgentTaskCommandAction.RESUME));
        cancel.addActionListener(e -> command(AgentTaskCommandAction.CANCEL));
        JButton history = new JButton(messages.text("deployment.task.history"));
        ...
        add(state);
        add(history);
        timer = new javax.swing.Timer(400, e -> refresh());
        finish();
    }
    ...
    void begin(String id) {
        taskId = id;
        state.setText(messages.text("deployment.task.starting"));
        timer.start();
    }
     ...
     */
    private void command(AgentTaskCommandAction action) {
        if (taskId == null)
            return;
        try {
            service.controlDeployment(taskId, action);
            refresh();
        } catch (Exception failure) {
            output.accept(messages.safe(failure));
        }
    }
    ...
    private void refresh() {
        if (taskId == null)
            return;
        try {
            var snapshot = service.deploymentTaskState(taskId);
            String current = snapshot.getOrDefault("state", "");
            pause.setEnabled(current.equals("RUNNING"));
            resume.setEnabled(current.equals("PAUSED"));
            refreshModels.setEnabled(current.equals("PAUSED"));
            cancel.setEnabled(Set.of("RUNNING", "PAUSED", "PAUSE_REQUESTED").contains(current));
            state.setText(current.isEmpty()
                    ? messages.text("deployment.task.starting")
                    : messages.text("deployment.task.state." + current));
            String reason = snapshot.getOrDefault("reason", "");
            state.setToolTipText(reason.isEmpty() ? null : messages.text("deployment.task.reason." + reason));
        } catch (Exception failure) {
            timer.stop();
            output.accept(messages.safe(failure));
        }
    ...
    private void history() {
        DesktopTaskExecutor.run(service::deploymentTaskHistory, rows -> {
            ...
            rows.forEach(row -> list.addElement(row.get("started_at") + " · " + row.get("server_id") + " · "
                    + row.get("state") + " · " + messages.text(
                            "deployment.task.semantics." + row.getOrDefault("execution_semantics_version", "1"))));
            JList<String> tasks = new JList<>(list);
            JTextArea details = new JTextArea(18, 72);
            details.setEditable(false);
            details.setLineWrap(true);
            details.setWrapStyleWord(true);
            tasks.addListSelectionListener(event -> {
                if (event.getValueIsAdjusting() || tasks.getSelectedIndex() < 0)
                    return;
                String id = rows.get(tasks.getSelectedIndex()).get("id");
                DesktopTaskExecutor.run(() -> service.deploymentTaskEvents(id),
                        events -> details.setText(events.stream()
                                .map(value -> value.get("created_at") + " " + value.get("event_type") + " "
                                        + value.get("action_id") + " " + value.get("detail"))
                                .collect(java.util.stream.Collectors.joining("\n"))),
                        failure -> details.setText(messages.safe(failure)));
            });
            JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(tasks),
                    new JScrollPane(details));
            split.setResizeWeight(.35);
            split.setPreferredSize(new Dimension(900, 480));
            ...
            inspect.addActionListener(event -> {
                if (tasks.getSelectedIndex() < 0)
                    return;
                String id = rows.get(tasks.getSelectedIndex()).get("id");
                char[] master = credentials.get();
                inspect.setEnabled(false);
                DesktopTaskExecutor.run(() -> {
                    try {
                        return service.inspectDeploymentTask(id, master);
                    } finally {
            ...
            historyPanel.add(inspect, BorderLayout.SOUTH);
            JOptionPane.showMessageDialog(this, historyPanel, messages.text("deployment.task.history"),
