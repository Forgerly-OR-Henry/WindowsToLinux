...
public final class DeploymentSourceCard extends JPanel {
     ...
     */
    public DeploymentSourceCard(AutomaticDeploymentApplicationFacade service, DesktopComponentFactory c,
            PageMessagePresenter messages, Consumer<DeploymentSourceInput> selected) {
        ...
        browse.setHorizontalTextPosition(SwingConstants.CENTER);
        browse.setVerticalTextPosition(SwingConstants.BOTTOM);
        browse.setIconTextGap(10);
        browse.setPreferredSize(new Dimension(0, com.formdev.flatlaf.util.UIScale.scale(88)));
        browse.addActionListener(event -> choose());
        add(browse);
        ...
        entry.add(status, "selected");
        JPanel bottom = c.transparent(new BorderLayout(8, 0));
        bottom.add(entry);
        clear = c.secondaryButton(messages.text("source.clear"));
        clear.setEnabled(false);
        clear.addActionListener(event -> {
            restore("", false);
            selected.accept(new DeploymentSourceInput(Optional.empty(), "", 0, ""));
        });
        bottom.add(clear, BorderLayout.EAST);
        add(bottom, BorderLayout.SOUTH);
        debounce = new Timer(300, event -> identify());
        ...
        TransferHandler transfer = new TransferHandler() {
            ...
            public boolean canImport(TransferSupport support) {
                return selectedValue.isEmpty() && input.isEnabled()
            ...
            public boolean importData(TransferSupport support) {
                ...
                try {
                    if (support.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
                        ...
                        if (files.size() != 1 || !(files.getFirst() instanceof File file))
                            throw new IllegalArgumentException("one folder required");
                        input.setText(file.toString());
                    } else
                        input.setText((String) support.getTransferable().getTransferData(DataFlavor.stringFlavor));
                    return true;
        ...
        browse.setTransferHandler(transfer);
    }
    ...
    public String inputText() {
        return selectedValue.isEmpty() ? input.getText() : selectedValue;
    }
    ...
    private void refresh() {
        ...
        ((CardLayout) entry.getLayout()).show(entry, occupied ? "selected" : "input");
        input.setEnabled(!occupied);
        browse.setEnabled(!occupied);
        browse.setText(occupied ? selectedValue : messages.text("source.drop"));
    ...
    private void identify() {
        ...
        if (!selectedValue.isEmpty() || text.isBlank() || service == null)
            return;
        DesktopTaskExecutor.run(() -> service.identifyDeploymentSource(text), value -> {
            ...
            restoring = false;
            refresh();
            status.setText(messages.text(value.directory().isPresent() ? "source.folderReady" : "source.gitReady"));
    ...
    private void invalid() {
        browse.setText(messages.text("source.invalid"));
        browse.setToolTipText(messages.text("source.invalid"));
    }
