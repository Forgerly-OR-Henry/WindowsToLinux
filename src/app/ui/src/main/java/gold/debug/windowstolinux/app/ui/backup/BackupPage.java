...
public final class BackupPage {
     ...
     */
    public BackupPage(Component owner, BackupApplicationFacade service, DesktopComponentFactory components,
    ...
    public BackupPageState captureState() {
        return new BackupPageState(selections.applicationId(), selections.targetServerId(), archivePath.getText(),
                destinationPath.getText(), output.getText(), preparedCandidate, task.getSelectedIndex(),
                backupPassword.getPassword(), masterPassword.getPassword());
    }
    ...
    private JPanel createPanel(DesktopComponentFactory components) {
        ...
        task.addItem(messages.text("backup.task.create"));
        task.addItem(messages.text("backup.task.restore"));
        task.addItem(messages.text("backup.task.migrate"));
        inputRows.add(task);
        inputRows.add(selections);
        selections.advanced(advanced);
        ...
        advanced.addOption(assessButton);
        ...
        rowConstraints.gridwidth = java.awt.GridBagConstraints.REMAINDER;
        rowConstraints.weightx = 1;
        rowConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
        rowConstraints.insets = new java.awt.Insets(0, 0, 8, 0);
        for (Component row : inputRows.getComponents())
            ((java.awt.GridBagLayout) inputRows.getLayout()).setConstraints(row, rowConstraints);
        ...
        page.add(components.outputCard(messages.text("backup.output.title"), messages.text("backup.output.description"),
                output), BorderLayout.CENTER);
     ...
     */
    private JPanel actionButtons(DesktopComponentFactory components,
            gold.debug.windowstolinux.app.ui.component.AdvancedOptionsPane advanced) {
        ...
        refresh.addActionListener(event -> selections.refresh());
        actions.add(refresh);
        return actions;
    }
    ...
    private void updateTask() {
        ...
        archiveRow.setVisible(selected == 1);
        destinationRow.setVisible(selected == 0);
        createButton.setVisible(selected == 0);
        restoreButton.setVisible(selected == 1);
        migrateButton.setVisible(selected == 2);
        if (panel != null)
            panel.revalidate();
    }
    ...
    private void restoreManagedBackup() {
        ...
        if (!confirmOperation("backup.restore.confirm", Map.of("target", target, "archive", selected)))
            return;
        ...
        DesktopTaskExecutor.run(() -> {
            ...
            output.setText(formatRestored(restored));
    ...
    private void prepareOfflineMigration() {
        ...
        if (JOptionPane.showConfirmDialog(owner, messages.text("backup.migration.confirm"),
            ...
            output.setText(messages.text("backup.migration.notApproved"));
            return;
        }
        char[] independent = backupPassword.getPassword();
        char[] master = masterPassword.getPassword();
        ...
        setBusy(true);
        output.setText(messages.text("backup.migrating"));
        DesktopTaskExecutor.run(() -> {
            try {
                return service.prepareManagedOfflineMigration(application, target, independent, master, true,
            ...
            output.setText(formatMigrated(migrated));
            output.setCaretPosition(0);
            setBusy(false);
        }, this::showFailure);
    }
    ...
    private void createManagedBackup() {
        ...
        try {
            destination = destinationPath.getText().isBlank()
                    ? null
        ...
        if (!confirmOperation("backup.create.confirm", Map.of("application", application)))
            return;
        char[] independent = backupPassword.getPassword();
        char[] master = masterPassword.getPassword();
        backupPassword.setText("");
    ...
    private boolean confirmOperation(String key, Map<String, ?> details) {
        boolean accepted = JOptionPane.showConfirmDialog(owner, messages.text(key, details),
                ...
                JOptionPane.WARNING_MESSAGE) == JOptionPane.YES_OPTION;
        if (!accepted)
            output.setText(messages.text("auto.cancelled"));
        return accepted;
    }
    ...
    private void prepareSelectedArchiveWithSecrets() {
        ...
        DesktopTaskExecutor.run(() -> {
            try (PreparedBackupSecrets prepared = service.prepareBackupCandidateWithSecrets(selected, password)) {
                return new AuthenticatedCandidate(prepared.candidate(), prepared.secrets().revisions().size());
            } finally {
                Arrays.fill(password, '\0');
            }
        }, authenticated -> {
            ...
            output.setText(
                    formatAuthenticatedCandidate(authenticated.candidate(), authenticated.secretRevisionCount()));
            output.setCaretPosition(0);
            setBusy(false);
        }, this::showFailure);
    }
    ...
    private String formatInspection(BackupArchiveInspection inspection) {
        String provenance = messages
                .text("backup.provenance." + inspection.provenanceStatus().name().toLowerCase(Locale.ROOT));
        return messages.text("backup.inspection",
                Map.of("application", inspection.applicationId(), "schema", inspection.schemaVersion(), "created",
                        inspection.createdAtUtc(), "components", inspection.componentCount(), "members",
                        inspection.memberCount(), "bytes", inspection.verifiedBytes(), "sha256",
                        inspection.archiveSha256(), "provenance", provenance));
    }
    ...
    private String formatCandidate(PreparedBackupCandidate candidate) {
        ...
        return messages.text("backup.prepared", Map.of("application", inspection.applicationId(), "path",
                candidate.candidateRoot(), "bytes", candidate.extractedBytes(), "sha256", inspection.archiveSha256()));
    }
    ...
    private String formatCreated(CreatedBackupArchive created) {
        ...
        return messages.text("backup.created",
                Map.of("application", inspection.applicationId(), "path", created.archive(), "components",
                        inspection.componentCount(), "members", inspection.memberCount(), "bytes",
                        inspection.verifiedBytes(), "sha256", inspection.archiveSha256()));
    }
    ...
    private String formatRestored(ManagedRestoreOutcome restored) {
        String warnings = restored.warnings().isEmpty()
                ? messages.text("backup.inputs.none")
                : restored.warnings().stream().map(messages::text).collect(java.util.stream.Collectors.joining("\n"));
        return messages.text("backup.restored", Map.of("target", restored.targetServerId(), "status",
                messages.text("backup.restore.status." + restored.restore().status().name().toLowerCase(Locale.ROOT)),
                "control",
                messages.text("backup.restore.control." + restored.controlState().name().toLowerCase(Locale.ROOT)),
                "warnings", warnings));
    }
    ...
    private String formatMigrated(ManagedOfflineMigrationOutcome migrated) {
        ...
        String warnings = migrated.warnings().isEmpty()
                ? messages.text("backup.inputs.none")
                : migrated.warnings().stream().map(messages::text).collect(java.util.stream.Collectors.joining("\n"));
        return messages.text("backup.migrated", Map.of("status",
                messages.text(
                        "backup.migration.status." + migrated.migration().status().name().toLowerCase(Locale.ROOT)),
    ...
    String formatAssessment(ManagedBackupInputAssessment assessment) {
        String components = assessment.componentIds().isEmpty()
                ? messages.text("backup.inputs.none")
                : String.join(", ", assessment.componentIds());
        String releases = assessment.currentReleaseIdentities().entrySet().stream()
                .map(entry -> messages.text("backup.inputs.release",
                        Map.of("component", entry.getKey(), "release", entry.getValue())))
                .reduce((left, right) -> left + "\n" + right).orElse(messages.text("backup.inputs.none"));
        java.util.List<String> missing = new java.util.ArrayList<>();
        assessment.applicationMissingInputs().forEach(reason -> missing
                .add(messages.text("backup.inputs.applicationBlocker", Map.of("reason", missingInput(reason)))));
        assessment.componentMissingInputs()
                .forEach((component,
                        reasons) -> missing.add(messages.text("backup.inputs.componentBlocker",
                                Map.of("component", component, "reasons", reasons.stream().map(this::missingInput)
                                        .reduce((left, right) -> left + ", " + right).orElseThrow()))));
        String blockers = missing.isEmpty() ? messages.text("backup.inputs.none") : String.join("\n", missing);
        String status = messages.text(assessment.persistedInputsComplete()
                ? "backup.inputs.status.complete"
                : "backup.inputs.status.incomplete");
        return messages.text("backup.inputs.result", Map.of("application", assessment.applicationId(), "status", status,
                "components", components, "releases", releases, "missing", blockers));
    }
    ...
    String formatAuthenticatedCandidate(PreparedBackupCandidate candidate, int secretRevisionCount) {
        ...
        return messages.text("backup.preparedSecrets",
                Map.of("application", candidate.inspection().applicationId(), "path", candidate.candidateRoot(),
                        "count", secretRevisionCount, "sha256", candidate.inspection().archiveSha256()));
    }
    ...
    private void setBusy(boolean busy) {
        if (panel instanceof gold.debug.windowstolinux.app.ui.component.AdvancedOptionsPane advanced)
            advanced.setBusy(busy);
        task.setEnabled(!busy);
        selections.busy(busy);
     ...
     * @param secretRevisionCount secret revision count / 秘密修订数量
     */
    private record AuthenticatedCandidate(PreparedBackupCandidate candidate, int secretRevisionCount) {
    }
}
