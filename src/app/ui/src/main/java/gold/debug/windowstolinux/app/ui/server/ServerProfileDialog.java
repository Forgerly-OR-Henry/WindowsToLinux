...
public final class ServerProfileDialog extends JDialog {
    ...
    public ServerProfileDialog(Window owner, ServerApplicationFacade service, DesktopComponentFactory c,
        ...
        storage.addActionListener(
                event -> master.setEnabled(storage.getSelectedItem() == CredentialStorageMode.MASTER_PASSWORD));
        ...
        if (existing) {
            ...
            password.setToolTipText(messages.text("server.password.keep"));
        }
        ...
        save = c.primaryButton(messages.text("button.saveServer"));
        ...
        body.add(new JScrollPane(status), BorderLayout.CENTER);
        body.add(actions, BorderLayout.SOUTH);
        setContentPane(body);
        setMinimumSize(new Dimension(560, 490));
        pack();
    ...
    private void save() {
        ...
        try {
            ...
            if (!existing && secret.length == 0) {
                ...
                throw new IllegalArgumentException(messages.text("server.password.required"));
            }
            busy = true;
            save.setEnabled(false);
            setInputs(false);
            status.setText(messages.text("server.connecting"));
            ...
            DesktopTaskExecutor.run(() -> {
                try {
                    ...
                    written.set(true);
                    service.verifyServer(profile, mode, unlock.clone(),
