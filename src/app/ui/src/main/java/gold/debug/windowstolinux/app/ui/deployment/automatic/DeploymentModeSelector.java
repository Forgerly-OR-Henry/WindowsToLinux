...
final class DeploymentModeSelector extends JPanel {
    ...
    DeploymentModeSelector(PageMessagePresenter messages) {
        ...
        setOpaque(false);
        slider.setName("deployment.automation");
        slider.setMajorTickSpacing(1);
        slider.setSnapToTicks(true);
        slider.setPaintTicks(false);
        slider.setPaintLabels(true);
        slider.setPreferredSize(UIScale.scale(new Dimension(220, 48)));
        slider.setMinimumSize(UIScale.scale(new Dimension(200, 48)));
        var labels = new Hashtable<Integer, JLabel>();
        for (var mode : DeploymentAutomationMode.values())
            labels.put(mode.ordinal(), new JLabel(messages.text("deployment.mode." + mode.name())));
        slider.setLabelTable(labels);
        slider.getAccessibleContext().setAccessibleName(messages.text("deployment.mode.label"));
        approval.setName("deployment.approval");
        approval.setSelectedItem(AgentApprovalMode.AUTOMATIC);
        messages.localize(approval, "deployment.approval.");
        approval.getAccessibleContext().setAccessibleName(messages.text("deployment.approval.label"));
        installApprovalIcons();
        modeButton.setName("deployment.modePicker");
        modeButton.putClientProperty("JButton.buttonType", "roundRect");
        modeButton.setMargin(new Insets(5, 12, 5, 10));
        modeButton.setHorizontalTextPosition(SwingConstants.LEFT);
        modeButton.setIcon(DesktopIcons.icon("chevron-down", 12, modeButton::getForeground));
        modeButton.setIconTextGap(UIScale.scale(8));
        ...
        heading.add(modeHeading, BorderLayout.NORTH);
        heading.add(description);
        JPanel contents = new JPanel(new BorderLayout(0, 2));
        contents.setOpaque(false);
        contents.setBorder(BorderFactory.createEmptyBorder(4, 6, 2, 6));
        contents.add(heading, BorderLayout.NORTH);
        contents.add(slider);
        modePopup.setLayout(new BorderLayout());
        modePopup.add(contents);
        modePopup.setName("deployment.modePopup");
        modePopup.getAccessibleContext().setAccessibleName(messages.text("deployment.mode.label"));
        installPopupInteractions();
        add(row);
        slider.addChangeListener(event -> update());
        approval.addActionListener(event -> update());
        update();
    }
     ...
     */
    DeploymentAutomationMode mode() {
        return DeploymentAutomationMode.values()[slider.getValue()];
    }
    ...
    AgentApprovalMode approval() {
        return (AgentApprovalMode) approval.getSelectedItem();
    }
     ...
     */
    void restore(DeploymentAutomationMode mode, AgentApprovalMode policy) {
        slider.setValue(mode.ordinal());
        approval.setSelectedItem(policy);
        update();
    }
    ...
    private void installApprovalIcons() {
        ...
        approval.setRenderer((list, value, index, selected, focus) -> {
            ...
            label.setIconTextGap(UIScale.scale(8));
            return label;
        });
    }
    ...
    private void togglePopup() {
        if (modePopup.isVisible()) {
            hidePopup();
            return;
        }
        if (!modeButton.isEnabled())
            return;
    ...
    private void installPopupInteractions() {
        ...
        slider.getActionMap().put("closeModePopup", new AbstractAction() {
            ...
            public void actionPerformed(ActionEvent event) {
                hidePopup();
                modeButton.requestFocusInWindow();
            }
        });
    ...
    /** Displays current policy and the manual-review workload warning. / 展示当前策略及人工审核工作量提示。 */
    private void update() {
        approval.setVisible(mode() != DeploymentAutomationMode.STATIC);
        String key = mode() != DeploymentAutomationMode.STATIC && approval() == AgentApprovalMode.MANUAL_REVIEW
                ? "deployment.approval.manualHint"
                : "deployment.mode.description." + mode().name();
        ...
        description.setText(messages.text(approval() == AgentApprovalMode.MANUAL_REVIEW
                ? "deployment.approval.manualHint"
                : "deployment.mode.description.AGENT"));
        int width = UIScale.scale(220);
        View view = description.getUI().getRootView(description);
        view.setSize(width, Float.MAX_VALUE);
        int height = Math.max(description.getFontMetrics(description.getFont()).getHeight() * 2,
        ...
        modeButton.setText(title);
        modeHeading.setText(title);
        ...
        approval.setToolTipText(approval() == AgentApprovalMode.MANUAL_REVIEW
                ? messages.text("deployment.approval.manualHint")
                : messages.text("deployment.approval.label"));
    ...
    private static final class ModeSliderUI extends FlatSliderUI {
         ...
         */
        @Override
        protected ChangeListener createChangeListener(JSlider control) {
        ...
        protected void calculateThumbLocation() {
            if (!isDragging() || !slider.getValueIsAdjusting())
         ...
         */
        @Override
        protected TrackListener createTrackListener(JSlider control) {
            return new FlatTrackListener() {
                ...
                public void mouseDragged(MouseEvent event) {
                    ...
                    double value = slider.getMinimum() + (thumbRect.getCenterX() - left) / Math.max(1, right - left)
        ...
        private double visualValue() {
            return Double.isNaN(displayedValue) ? slider.getValue() : displayedValue;
        }
        ...
        private void animateTo(double target) {
            ...
            animation = new Timer(15, event -> {
                if (!slider.isShowing() || !slider.isEnabled()) {
                    finishAnimation();
                    return;
                }
                double progress = Math.min(1, (System.nanoTime() - started) / 200_000_000.0);
                double eased = 1 - Math.pow(1 - progress, 3);
                displayedValue = from + (target - from) * eased;
                slider.repaint();
                if (progress >= 1)
                    stopAnimation();
            });
         ...
         */
        @Override
        protected void installDefaults(JSlider control) {
            super.installDefaults(control);
            trackWidth = 26;
            thumbSize = new Dimension(22, 22);
            focusWidth = 2;
            trackValueColor = UIManager.getColor("Component.focusColor");
            trackColor = UIManager.getColor("Component.borderColor");
            thumbColor = Color.WHITE;
            hoverThumbColor = Color.WHITE;
            pressedThumbColor = new Color(245, 247, 250);
        ...
        public void paintThumb(Graphics graphics) {
            Graphics2D g = (Graphics2D) graphics.create();
            try {
                g.translate(visualCenter() - thumbRect.getCenterX(), 0);
                super.paintThumb(g);
            } finally {
                g.dispose();
            }
