...
public record RecoveryInteractionPresenter(Component owner, AiApplicationFacade ai, DesktopComponentFactory components,
     ...
     */
    @Override public boolean offerSshRecovery(ServerProfile server) {
        if (ai == null) return false;
        return onEdt(() -> JOptionPane.showConfirmDialog(owner, messages.text("recovery.offer"), messages.text("recovery.title"),
     ...
     */
    @Override public void showSshRecovery(ServerProfile server, SshRecoverySession session) {
    ...
    private static <T> T onEdt(Callable<T> action) {
        try {
            ...
            var task = new FutureTask<>(action); SwingUtilities.invokeLater(task); return task.get();
        } catch (InterruptedException failure) { Thread.currentThread().interrupt(); throw new CancellationException(); }
        catch (Exception failure) { throw new IllegalStateException("recovery-ui-unavailable"); }
    }
}
