...
public final class ManagedPage {
     ...
     */
    public ManagedPage(ManagedApplicationFacade service, ServerContext serverContext, DesktopComponentFactory c,
     ...
     */
    public ManagedPageState captureState() {
        return new ManagedPageState(applicationId.getText(), output.getText(), (String) type.getSelectedItem(),
                selectedServer());
    }
    ...
    public void restoreState(ManagedPageState state) {
        ...
        desiredServer = state.serverFilter();
        loading = true;
        if (!desiredServer.isEmpty()) {
            server.addItem(new ServerChoice(desiredServer, desiredServer));
            server.setSelectedIndex(server.getItemCount() - 1);
        }
        loading = false;
    ...
    public void selectApplication(String key) {
        applicationId.setText(key);
        output.setText(messages.text("deployment.selected", Map.of("application", key)));
        refresh();
    }
    ...
    private JPanel createPanel() {
        ...
        type.setRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean selected,
                ...
                return super.getListCellRendererComponent(list, messages
                        .text(value == null || value.toString().isEmpty() ? "apps.allTypes" : "apps.category." + value),
                        index, selected, focus);
            }
        });
        type.addActionListener(event -> render());
        server.addItem(new ServerChoice("", messages.text("apps.allServers")));
        server.addActionListener(event -> {
            if (!loading) {
                desiredServer = selectedServer();
                render();
            }
        });
        GridBagConstraints filterLayout = new GridBagConstraints();
        filterLayout.gridy = 0;
        filterLayout.weighty = 1;
        filterLayout.fill = GridBagConstraints.VERTICAL;
        filterLayout.insets = new Insets(0, 0, 0, 8);
        filters.add(type, filterLayout);
        filters.add(server, filterLayout);
        filterLayout.weightx = 1;
        filters.add(Box.createHorizontalGlue(), filterLayout);
        toolbar.add(filters);
        ...
        JButton add = c.primaryButton(messages.text("apps.add"));
        add.addActionListener(event -> {
            if (service != null)
                new ApplicationScanDialog(SwingUtilities.getWindowAncestor(panel), service, c, messages,
                        this::selectApplication).setVisible(true);
        });
        ...
        output.setRows(12);
        advanced.addOption(new JScrollPane(output));
        return advanced;
    }
    ...
    private void refresh() {
        if (service == null || loading || busy)
            return;
        loading = true;
        status.setText(messages.text("apps.loading"));
        DesktopTaskExecutor.run(service::listApplications, values -> {
            applications = values;
            String chosen = desiredServer;
            server.removeAllItems();
            server.addItem(new ServerChoice("", messages.text("apps.allServers")));
            values.stream()
                    .collect(java.util.stream.Collectors.toMap(ApplicationSummary::serverId, value -> value,
                            (first, next) -> first, java.util.TreeMap::new))
                    .values().forEach(value -> server.addItem(new ServerChoice(value.serverId(), value.serverName())));
            for (int index = 0; index < server.getItemCount(); index++)
                if (server.getItemAt(index).id.equals(chosen))
    ...
    private void render() {
        if (loading)
            return;
        cards.removeAll();
        var shown = applications.stream()
                .filter(value -> value.matches((String) type.getSelectedItem(), selectedServer())).toList();
        for (ApplicationSummary app : shown)
            cards.add(card(app));
        status.setText(shown.isEmpty() ? messages.text("apps.empty") : "");
        cards.revalidate();
        cards.repaint();
    }
    ...
    private JPanel card(ApplicationSummary app) {
        JPanel wrapper = c.transparent(new BorderLayout());
        ...
        name.setIcon(DesktopIcons.icon(app.category().equals("WEBSITE") ? "panels-top-left" : "archive", 22,
                name::getForeground));
        title.add(name);
        title.add(c.badge(messages.text("apps.category." + app.category())), BorderLayout.EAST);
        ...
        String date = app.deployedAt().map(value -> messages.text("apps.deployedAt", Map.of("time", time(value))))
                .orElseGet(
                        () -> app.adoptedAt().map(value -> messages.text("apps.adoptedAt", Map.of("time", time(value))))
                                .orElse(messages.text("apps.unknownDate")));
        details.add(new JLabel(
                date + "  ·  " + messages.text("runtime.state." + app.lastState().name().toLowerCase(Locale.ROOT))
                        + app.observedAt()
                                .map(value -> "  ·  " + messages.text("apps.observedAt", Map.of("time", time(value))))
                                .orElse("")));
        card.add(details);
        JPanel actions = c.transparent(new FlowLayout(FlowLayout.RIGHT, 8, 0));
        JButton access = c.secondaryButton(messages.text("apps.open"));
        access.setEnabled(app.accessUrl().isPresent());
        access.setToolTipText(
                app.accessUrl().map(value -> value.url().toString()).orElse(messages.text("managed.noWebEntry")));
        access.addActionListener(event -> app.accessUrl().ifPresent(value -> {
            try {
                Desktop.getDesktop().browse(value.url());
            } catch (Exception failure) {
                status.setText(messages.safe(failure));
            }
        }));
        if (app.accessUrl().isPresent())
            actions.add(access);
        addUsage(app, details, actions);
        for (LifecycleAction action : List.of(LifecycleAction.REFRESH_STATUS, LifecycleAction.START,
                LifecycleAction.STOP, LifecycleAction.RESTART, LifecycleAction.ENABLE_AUTOSTART,
                LifecycleAction.DISABLE_AUTOSTART)) {
            if (action != LifecycleAction.REFRESH_STATUS && app.usage().map(usage -> !usage.lifecycle()).orElse(false))
                continue;
            if (app.external()
                    && (action == LifecycleAction.ENABLE_AUTOSTART || action == LifecycleAction.DISABLE_AUTOSTART))
                continue;
            String label = switch (action) {
                case REFRESH_STATUS -> "refreshStatus";
                case ENABLE_AUTOSTART -> "enableAutostart";
                case DISABLE_AUTOSTART -> "disableAutostart";
                default -> action.name().toLowerCase(Locale.ROOT);
            };
            JButton button = c.secondaryButton(messages.text("button." + label));
            button.setEnabled(switch (action) {
                case START -> app.canStart();
                case STOP -> app.canStop();
                case RESTART -> app.canStart() && app.canStop();
                default -> true;
            });
            button.addActionListener(event -> execute(app, action));
            actions.add(button);
        }
        JButton edit = c.secondaryButton(messages.text("apps.edit"));
        edit.addActionListener(event -> {
            applicationId.setText(app.key());
            new ApplicationPresentationDialog(SwingUtilities.getWindowAncestor(panel), service, c, messages, app,
    ...
    private void addUsage(ApplicationSummary app, JPanel details, JPanel actions) {
        app.usage().ifPresent(usage -> {
            if (!usage.reviewed())
                details.add(new JLabel(messages.text("apps.reanalysisRequired")));
            for (String endpoint : usage.endpoints())
                details.add(new JLabel(endpoint));
            if (usage.category().equals("APP") && !usage.command().isBlank()) {
                ...
                command.addActionListener(event -> {
                    JTextArea text = new JTextArea(usage.command(), 3, 60);
                    text.setEditable(false);
                    text.setLineWrap(true);
                    text.setWrapStyleWord(true);
                    int result = JOptionPane.showOptionDialog(panel, new JScrollPane(text),
                            messages.text("apps.command"), JOptionPane.DEFAULT_OPTION, JOptionPane.PLAIN_MESSAGE, null,
                            new String[]{messages.text("apps.copyCommand"), messages.text("button.close")}, null);
                    if (result == 0)
                        Toolkit.getDefaultToolkit().getSystemClipboard()
                                .setContents(new java.awt.datatransfer.StringSelection(usage.command()), null);
                });
                actions.add(command);
            }
        });
    ...
    private void execute(ApplicationSummary app, LifecycleAction action) {
        ...
        if (app.needsMasterPassword() && master.length == 0) {
            ...
            if (JOptionPane.showConfirmDialog(panel, field, messages.text("field.masterPassword"),
                    JOptionPane.OK_CANCEL_OPTION) != JOptionPane.OK_OPTION)
                return;
            master = field.getPassword();
            field.setText("");
        }
        final char[] unlock = master;
        applicationId.setText(app.key());
        busy = true;
        ((AdvancedOptionsPane) panel).setBusy(true);
        status.setText(messages.text("lifecycle.running",
                Map.of("action", messages.text("lifecycle.action." + action.name().toLowerCase(Locale.ROOT)))));
        DesktopTaskExecutor.run(() -> {
            try {
                return service.executeApplicationLifecycle(app.key(), action, unlock,
                        serverContext::confirmFingerprint);
            } finally {
                java.util.Arrays.fill(unlock, '\0');
            }
        }, result -> {
            finish();
            output.setText(describe(result));
            refresh();
        }, failure -> {
            finish();
    ...
    private String describe(ApplicationLifecycleResult result) {
        String summary = messages.text("runtime.state." + result.state().name().toLowerCase(Locale.ROOT)) + "  ·  "
                + time(result.observedAt());
        return result.managed()
                .map(value -> summary + "\n"
                        + messages.text(value.accepted() ? "lifecycle.accepted" : "lifecycle.rejected",
                                Map.of("message", messages.catalog().text(value.message()), "observation",
                                        messages.lifecycle(value.observation().orElse(null))))
                        + "\n"
                        + messages.text(
                                "failure.operation.summary",
                                Map.of("operationId", value.operationIdentity().toString()))
                        + value.failure().map(failure -> "\n" + failure.code()).orElse("")
                        + value.nonFatalFailures().stream()
                                .map(failure -> "\n" + failure.code() + " "
                                        + messages.catalog().text(failure.userMessage()))
     ...
     */
    private boolean matchesSelection(ApplicationSummary app) {
        return app.key().equals(applicationId.getText()) || app.key().equals("managed:" + applicationId.getText());
    }
    ...
    private String selectedServer() {
        return server.getSelectedItem() instanceof ServerChoice choice ? choice.id : desiredServer;
    }
     ...
     */
    private static String time(Instant time) {
        return DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm").withZone(ZoneId.systemDefault()).format(time);
    }
    /**
