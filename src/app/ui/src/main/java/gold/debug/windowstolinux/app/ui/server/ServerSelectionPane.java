...
public final class ServerSelectionPane extends JPanel {
    ...
    public ServerSelectionPane(ServerApplicationFacade service, DesktopComponentFactory c,
        ...
        choose.setVerticalTextPosition(SwingConstants.BOTTOM);
        ...
        clear = c.secondaryButton(messages.text("server.clear"));
    ...
    public String selectedId() {
        return desiredId != null ? desiredId : profile == null ? "" : profile.id();
    }
    ...
    public void select(String serverId) {
        desiredId = serverId;
        choose.setEnabled(serverId.isBlank());
        clear.setEnabled(!serverId.isBlank());
        reload();
    }
    ...
    public void reload() {
        ...
        DesktopTaskExecutor.run(service::listServerSummaries, values -> {
            ...
            ServerSummary chosen = values.stream().filter(value -> value.profile().id().equals(id)).findFirst()
                    .orElse(null);
            if (chosen == null && !restoring && id.isBlank() && !values.isEmpty())
                chosen = values.getFirst();
            profile = chosen == null ? null : chosen.profile();
            desiredId = profile == null ? "" : null;
            loading = false;
            refresh(chosen);
            ...
            loading = false;
            endpoint.setText(messages.safe(failure));
        });
    }
    ...
    public void open(Window owner) {
        if (service == null || profile != null || !selectedId().isBlank())
            return;
        ...
        dialog.setContentPane(inventory);
    ...
    private void refresh(ServerSummary summary) {
        ...
        choose.setEnabled(profile == null);
        clear.setEnabled(profile != null);
        ...
        choose.setToolTipText(description);
        endpoint.setToolTipText(description);
        revalidate();
        repaint();
    }
}
