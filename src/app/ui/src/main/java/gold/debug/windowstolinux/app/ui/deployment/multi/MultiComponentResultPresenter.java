...
final class MultiComponentResultPresenter {
    ...
    String review(ReviewedMultiComponentApplication candidate) {
        ...
        String supports = candidate.components().stream()
                .map(component -> component.componentId() + " = "
                        + messages.text("support.level."
                                + component.request().facts().support().level().name().toLowerCase(Locale.ROOT)))
                .reduce((left, right) -> left + "\n" + right).orElse("");
        String databases = candidate.components().stream().map(
                component -> component.componentId() + " = " + databaseReview(component.request().databaseBindings()))
                .reduce((left, right) -> left + "\n" + right).orElse("");
        return messages.text("component.review.body", Map.of("application", candidate.plan().applicationId(), "waves",
                waves, "stop", String.join(" -> ", candidate.plan().stopOrder()), "start",
                String.join(" -> ", candidate.plan().startOrder()), "supports", supports, "databases", databases));
    }
    ...
    private String databaseReview(Optional<java.util.List<ManagedDatabaseBinding>> bindings) {
        if (bindings.isEmpty())
            return messages.text("database.review.unreviewed");
        if (bindings.orElseThrow().isEmpty())
            return messages.text("database.review.none");
    ...
    private String databaseBindingReview(ManagedDatabaseBinding binding) {
        if (binding.connection() instanceof ManagedDatabaseConnection.Sqlite sqlite) {
            return messages.text("database.review.sqlite",
                    Map.of("id", binding.databaseId(), "file", sqlite.fileName()));
        }
        ManagedDatabaseConnection.Server connection = (ManagedDatabaseConnection.Server) binding.connection();
        return messages.text("database.review.server",
                Map.of("engine", messages.text("db.engine." + connection.engine().name().toLowerCase(Locale.ROOT)),
                        "id", binding.databaseId(), "host", connection.host(), "port", connection.port(), "database",
                        connection.database()));
    }
    ...
    String analysis(PreparedMultiComponentSource prepared) {
        String components = prepared.assessment().components().stream().map(component -> "- " + component.componentId()
                + " | "
                + messages.text("project.type." + component.facts().projectType().name().toLowerCase(Locale.ROOT))
                + " | "
                ...
                .reduce((left, right) -> left + "\n" + right).orElse("");
        String issues = prepared.assessment().issues().stream()
                .map(issue -> "- " + issue.code() + " [" + String.join(",", issue.componentIds()) + "]")
                .reduce((left, right) -> left + "\n" + right).orElse(messages.text("component.analysis.noIssues"));
        return messages.text("component.analysis.result", Map.of("admission",
                messages.text(
                        "analysis.admission." + prepared.assessment().admission().name().toLowerCase(Locale.ROOT)),
    ...
    String deployment(MultiComponentDeploymentResult result) {
        String components = result.componentResults().stream()
                .map(component -> "- " + component.componentId() + " = "
                        + messages.text("component.state." + component.state().name().toLowerCase(Locale.ROOT)) + "\n"
                        + component.events().stream()
                                .map(event -> "  " + messages.catalog().text(event.message()) + " = "
                                        + messages.text(event.succeeded() ? "value.success" : "value.failed")
        ...
        String warnings = result.nonFatalFailures().stream().map(failure -> messages.text("failure.warning.summary",
                Map.of("code", failure.code(), "message", messages.catalog().text(failure.userMessage()), "recovery",
                        messages.text(
                                "failure.recovery." + failure.recoveryDisposition().name().toLowerCase(Locale.ROOT)))))
                .reduce((left, right) -> left + "\n" + right).orElse("");
        return messages.text("component.deployment.result",
                Map.of("status", messages.text("deployment.status." + result.status().name().toLowerCase(Locale.ROOT)),
    ...
    String lifecycle(MultiComponentLifecycleResult result) {
        String components = result.componentResults().stream()
                .map(component -> "- " + component.componentId() + " = "
                        + component.observation().map(messages::lifecycle)
                                .orElse(messages.text("lifecycle.noObservation")))
                .reduce((left, right) -> left + "\n" + right).orElse("");
        String warnings = result.nonFatalFailures().stream().map(failure -> messages.text("failure.warning.summary",
                Map.of("code", failure.code(), "message", messages.catalog().text(failure.userMessage()), "recovery",
                        messages.text(
                                "failure.recovery." + failure.recoveryDisposition().name().toLowerCase(Locale.ROOT)))))
                .reduce((left, right) -> left + "\n" + right).orElse("");
        return messages.text("component.lifecycle.result",
                Map.of("accepted", messages.text(result.accepted() ? "value.yes" : "value.no"), "runtime",
                        messages.text("runtime.state." + result.runtimeState().name().toLowerCase(Locale.ROOT)),
                        "autostart",
                        messages.text("autostart.state." + result.autostartState().name().toLowerCase(Locale.ROOT)),
                        "components", components))
                + "\n\n"
                + messages.text("failure.operation.summary",
                        Map.of("operationId", result.operationIdentity().toString()))
                + result.failure()
                        .map(failure -> "\n" + failure.code() + ": " + messages.catalog().text(failure.userMessage()))
                        .orElse("")
                + (warnings.isBlank() ? "" : "\n" + warnings);
    }
}
