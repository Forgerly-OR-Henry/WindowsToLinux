...
final class AiProviderDragTransfer extends TransferHandler {
    ...
    protected Transferable createTransferable(JComponent source) {
        if (source != list || list.getSelectedValue() == null)
            return null;
        exported = new StringSelection(list.getSelectedValue());
        return exported;
    }
    ...
    public boolean importData(TransferSupport support) {
        ...
        try {
            ...
            for (int i = 0; i < list.getModel().getSize(); i++)
                order.add(list.getModel().getElementAt(i));
            if (!order.contains(id))
                return false;
            changed.accept(move(order, id, ((JList.DropLocation) support.getDropLocation()).getIndex()));
            return true;
        } catch (java.io.IOException | UnsupportedFlavorException | IllegalArgumentException failure) {
            return false;
        }
    }
    ...
    static List<String> move(List<String> order, String id, int dropPosition) {
        ...
        if (from < 0 || dropPosition < 0 || dropPosition > order.size())
            throw new IllegalArgumentException("invalid model move");
        result.remove(from);
        result.add(dropPosition > from ? dropPosition - 1 : dropPosition, id);
