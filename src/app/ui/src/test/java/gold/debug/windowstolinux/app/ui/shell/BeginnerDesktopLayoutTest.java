...
class BeginnerDesktopLayoutTest {
    @Test
    void navigationAnimatesWithoutVerticalJumpsAndCanReverseBeforeCompletion() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            DesktopFrame frame = new DesktopFrame(null, catalog, DesktopDisplayConfiguration.defaults(),
                    ThemePalette.light(), (source, selected) -> {
                    }, null);
            current.set(frame);
            ...
            JButton toggle = descendants(sidebar).filter(JButton.class::isInstance).map(JButton.class::cast)
                    .filter(button -> catalog.text("nav.collapse").equals(button.getToolTipText())).findFirst()
                    .orElseThrow();
            JButton deploy = descendants(sidebar).filter(JButton.class::isInstance).map(JButton.class::cast)
                    .filter(button -> catalog.text("nav.deployment").equals(button.getToolTipText())).findFirst()
                    .orElseThrow();
            ...
            Timer timer = new Timer(15, event -> {
                try {
                    ...
                    if (phase[0] == 0 && width > 80 && width < 155) {
                        ...
                        assertTrue(widths.stream().distinct().count() >= 4,
                                "The animation must render intermediate widths");
        ...
        try {
            ...
            SwingUtilities.invokeAndWait(() -> {
                ...
                Container sidebar = (Container) ((BorderLayout) frame.getContentPane().getLayout())
                ...
                frame.dispose();
                try {
                    var field = DesktopFrame.class.getDeclaredField("navigationAnimation");
    ...
    @Test
    void allPagesKeepTheirDrawerAndInputsAcrossLanguageAndThemeRebuild() throws Exception {
        org.junit.jupiter.api.Assumptions.assumeFalse(GraphicsEnvironment.isHeadless());
        SwingUtilities.invokeAndWait(() -> {
            DesktopThemeService.install(ThemeMode.LIGHT);
            DesktopFrame frame = new DesktopFrame(null, MessageCatalog.forLanguageTag("zh-CN"),
                    DesktopDisplayConfiguration.defaults(), ThemePalette.light(), (source, selected) -> {
                    }, null);
            DesktopFrame restored = null;
            try {
                var show = DesktopFrame.class.getDeclaredMethod("showPage", String.class, String.class, String.class);
                show.setAccessible(true);
                for (String page : List.of("deployment", "components", "applications", "backup", "servers", "ai",
                        "settings")) {
                    show.invoke(frame, page, "nav." + page, "page." + page + ".description");
                    var root = frame.getContentPane();
                    root.setSize(1060, 680);
                    layout(root);
                    var pane = descendants(root).filter(AdvancedOptionsPane.class::isInstance)
                            .map(AdvancedOptionsPane.class::cast).filter(Component::isVisible)
                            .filter(value -> value.getParent().isVisible()).findFirst().orElseThrow();
                    Container workspace = (Container) ((BorderLayout) root.getLayout())
                            .getLayoutComponent(BorderLayout.CENTER);
                    Container header = (Container) ((BorderLayout) workspace.getLayout())
                    ...
                    JButton toggle = descendants(header).filter(JButton.class::isInstance).map(JButton.class::cast)
                            .filter(button -> button.getText()
                                    .equals(MessageCatalog.forLanguageTag("zh-CN").text("advanced.show")))
                            .findFirst().orElseThrow();
                    Rectangle toggleBounds = SwingUtilities.convertRectangle(toggle.getParent(), toggle.getBounds(),
                            header);
                    Insets padding = header.getInsets();
                    assertEquals((header.getHeight() + padding.top - padding.bottom) / 2.0, toggleBounds.getCenterY(),
                            1, "Advanced options must be centered inside the header border");
                    assertFalse(SwingUtilities.isDescendingFrom(toggle, pane));
                    assertFalse(pane.expanded());
                    render(root, page + "-closed.png");
                    toggle.doClick(0);
                    assertTrue(pane.expanded());
                    layout(root);
                    render(root, page + "-expanded.png");
                    if (page.equals("components")) {
                        JButton lifecycle = descendants(pane).filter(JButton.class::isInstance).map(JButton.class::cast)
                                .filter(button -> button.getText().equals(
                                        MessageCatalog.forLanguageTag("zh-CN").text("component.button.lifecycle")))
                                .findFirst().orElseThrow();
                        Rectangle action = SwingUtilities.convertRectangle(lifecycle.getParent(), lifecycle.getBounds(),
                                pane);
                        assertTrue(action.y + action.height <= pane.getHeight(),
                                "Lifecycle action must not be clipped");
                    ...
                    if (page.equals("settings")) {
                        descendants(pane).filter(JButton.class::isInstance).map(JButton.class::cast)
                                .filter(button -> button.getText().equals(
                                        MessageCatalog.forLanguageTag("zh-CN").text("settings.diagnostics.open")))
                                .forEach(button -> assertTrue(button.getHeight() <= 40,
                                        "Diagnostic action must retain normal height"));
                    }
                    descendants(pane).filter(JButton.class::isInstance).map(JButton.class::cast)
                            .filter(button -> "help".equals(button.getClientProperty("JButton.buttonType")))
                            .forEach(button -> assertFalse(
                                    button.getAccessibleContext().getAccessibleName().startsWith("help."),
                ...
                try (var state = frame.captureViewState()) {
                    ...
                    restored = new DesktopFrame(null, MessageCatalog.forLanguageTag("en-US"),
                            new DesktopDisplayConfiguration("en-US", ThemeMode.DARK), ThemePalette.dark(),
                            (source, selected) -> {
                            }, state);
                    try (var after = restored.captureViewState()) {
                        assertEquals(state.expanded(), after.expanded());
                        assertEquals(state.deploymentSelection(), after.deploymentSelection());
    ...
    @Test
    void keepsPrimaryActionLogAndHelpVisibleAtSupportedSizes() throws Exception {
        org.junit.jupiter.api.Assumptions.assumeFalse(GraphicsEnvironment.isHeadless());
        SwingUtilities.invokeAndWait(() -> {
            DesktopThemeService.install(ThemeMode.LIGHT);
            var catalog = MessageCatalog.forLanguageTag("zh-CN");
            ...
            try {
                for (int[] size : List.of(new int[]{1180, 780}, new int[]{1060, 720})) {
                    ...
                    JButton deploy = descendants(deployment).filter(JButton.class::isInstance).map(JButton.class::cast)
                            ...
                            .orElseThrow();
                    JTextArea log = descendants(deployment).filter(JTextArea.class::isInstance)
                            .map(JTextArea.class::cast).filter(area -> "deployment.output".equals(area.getName()))
                            .findFirst().orElseThrow();
                    render(root, "deployment-" + size[0] + ".png");
                    assertTrue(log.getHeight() >= 200,
                            "Log must remain readable: " + log.getSize() + " parent=" + log.getParent().getSize());
                    ...
                    descendants(selection).filter(JButton.class::isInstance).map(JButton.class::cast)
                            ...
                            .forEach(button -> assertTrue(button.getHeight() >= 84,
                                    "Selection areas must not be compressed: " + button.getSize()));
                    ...
                    render(root, "deployment-" + size[0] + ".png");
                    var sourceCard = descendants((Container) source).filter(
                            ...
                            .findFirst().orElseThrow();
                    sourceCard.restore("https://github.com/MHSanaei/3x-ui.git", true);
                    layout(root);
                    render(root, "deployment-selected-" + size[0] + ".png");
                    sourceCard.restore("", false);
                    layout(root);
                    Dimension previousLogSize = log.getSize();
                    deployment.setExpanded(true);
                    frame.validate();
                    layout(root);
                    assertEquals(previousLogSize, log.getSize(),
                            "Opening an inspector must preserve the workspace size");
                    assertTrue(log.getWidth() >= 360, "Inspector must not overlap the log");
                    assertTrue(log.getHeight() >= 200);
                    long help = Stream
                            .concat(descendants(root),
                                    java.util.Arrays.stream(frame.getOwnedWindows())
                                            .flatMap(BeginnerDesktopLayoutTest::descendants))
    ...
    private static Stream<Component> descendants(Container parent) {
        return Stream.of(parent.getComponents())
                .flatMap(child -> child instanceof Container container
                        ? Stream.concat(Stream.of(child), descendants(container))
                        : Stream.of(child));
    }
    ...
    private static void render(Container component, String name) {
        try {
            Path directory = Path.of("target", "visual-checks");
