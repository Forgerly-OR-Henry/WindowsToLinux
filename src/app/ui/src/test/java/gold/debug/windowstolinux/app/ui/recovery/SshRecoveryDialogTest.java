...
class SshRecoveryDialogTest {
    static class Session implements SshRecoverySession {
        ...
        @Override
        public void bindTerminal(String target, String server, boolean consent, boolean reconciled) {
            fail("no binding during approval");
        }

        @Override
        public void confirmAction(String token, boolean high) {
            assertEquals("exact-action", token);
            assertFalse(high);
            approved = true;
        }
    ...
    @Test
    void exposesExactCommandAndCleansUpWhenWindowCloses() throws Exception {
        ...
        var components = new DesktopComponentFactory(ThemePalette.light());
        var session = new Session();
        var ai = (AiApplicationFacade) Proxy.newProxyInstance(getClass().getClassLoader(),
                new Class<?>[]{AiApplicationFacade.class}, (p, m, a) -> {
                    throw new AssertionError("no model configuration call expected");
                });
        SwingUtilities.invokeAndWait(() -> {
            com.formdev.flatlaf.FlatLightLaf.setup();
            new SshRecoveryDialog(new JPanel(), new ServerProfile("fixture", "example.test", 22, "user", "ssh/fixture",
                    CredentialStorageMode.MASTER_PASSWORD), session, ai, components, messages);
        });
        try {
            SwingUtilities.invokeAndWait(() -> {
                var dialog = Arrays.stream(Window.getWindows()).filter(JDialog.class::isInstance)
                        .map(JDialog.class::cast)
                        .filter(v -> v.isShowing() && v.getTitle().equals(messages.text("recovery.title"))).findFirst()
                        .orElseThrow();
                assertTrue(descendants(dialog).filter(JTextArea.class::isInstance).map(JTextArea.class::cast)
                        .anyMatch(v -> v.getText().contains("uname -a")));
                ...
                assertFalse(session.approved);
                button.doClick();
                assertTrue(session.approved);
                try {
                    var image = new java.awt.image.BufferedImage(dialog.getWidth(), dialog.getHeight(),
                    ...
                    javax.imageio.ImageIO.write(image, "png",
                            Path.of("target/visual-checks/ssh-recovery.png").toFile());
            ...
            SwingUtilities.invokeAndWait(() -> Arrays.stream(Window.getWindows()).filter(JDialog.class::isInstance)
                    .map(JDialog.class::cast).filter(v -> v.getTitle().equals(messages.text("recovery.title")))
    ...
    private Stream<Component> descendants(Component component) {
        return Stream.concat(Stream.of(component),
                component instanceof Container container
                        ? Arrays.stream(container.getComponents()).flatMap(this::descendants)
                        : Stream.empty());
    }
}
