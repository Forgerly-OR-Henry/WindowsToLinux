...
class BackupPageStateTest {
    @Test
    void appearanceRebuildRetainsEnteredPasswordsAndReleasesStateCopies() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            try (var restored = page.captureState()) {
                ...
                assertEquals(state.task(), restored.task());
                assertEquals(state.output(), restored.output());
            }
        });
        state.close();
        assertArrayEquals(new char[14], state.backupPassword());
        assertArrayEquals(new char[14], state.masterPassword());
    }
}
