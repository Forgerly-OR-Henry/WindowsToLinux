...
class AdvancedOptionsPaneTest {
    @Test void longLabelsWrapWithoutClippingHelpOrInputsAcrossWidthsLanguagesAndScales() throws Exception {
            ...
            for (String scale : java.util.List.of("1.0", "1.25", "1.5")) {
                for (String language : java.util.List.of("zh-CN", "en")) {
                    SwingUtilities.invokeAndWait(() -> {
                        ...
                        var keys = java.util.List.of("field.containerPorts", "field.containerVolumes", "field.configurationEntries",
                                "field.databaseReviewMode", "field.databaseDetails", "field.secretReferences");
                        var inputs = new java.util.ArrayList<JTextField>();
                        for (String key : keys) {
                            JTextField input = new JTextField(); inputs.add(input); pane.field(key, input);
                        }
                        JFrame frame = new JFrame();
                        JScrollPane drawer = (JScrollPane) pane.drawer();
                        frame.setContentPane(drawer); drawer.setVisible(true);
                        frame.setBounds(0, 0, 320, 680); frame.setVisible(true);
                        try {
                            int narrowHeight = 0;
                            for (int width : new int[]{320, 240, 420, 320}) {
                                frame.setSize(width, 680); frame.validate();
                                ...
                                for (int index = 0; index < inputs.size(); index++) {
                                    ...
                                    JTextArea label = descendants(row).filter(JTextArea.class::isInstance).map(JTextArea.class::cast)
                                    ...
                                    assertEquals(label.getText(), inputs.get(index).getAccessibleContext().getAccessibleName());
                                    var last = label.modelToView2D(label.getDocument().getLength() - 1);
                                    assertNotNull(last);
                                    assertTrue(last.getMaxY() <= label.getHeight(), "Last line must fit: " + label.getText());
                                    assertTrue(last.getMaxX() <= label.getWidth(), "Last character must fit: " + label.getText());
                                    Rectangle helpBounds = SwingUtilities.convertRectangle(help.getParent(), help.getBounds(), fields);
                                    assertTrue(helpBounds.getMaxX() <= drawer.getViewport().getWidth(), "Help must remain inside the drawer");
                                    ...
                                    assertTrue(inputs.get(index).getHeight() >= inputs.get(index).getPreferredSize().height);
                                    assertTrue(inputs.get(index).getX() + inputs.get(index).getWidth() <= row.getWidth());
                                }
                                int detailsHeight = fields.getComponent(4).getHeight();
                                if (width == 240) narrowHeight = detailsHeight;
                                if (width == 420) assertTrue(detailsHeight < narrowHeight, "A wider drawer must reclaim the extra wrapped lines");
                                if (width == 320) capture(drawer, "advanced-labels-" + language + "-" + scale + ".png");
                            }
    ...
    private static java.util.stream.Stream<Component> descendants(Container parent) {
        return java.util.Arrays.stream(parent.getComponents()).flatMap(child -> child instanceof Container nested
    ...
    private static void capture(JComponent component, String name) throws Exception {
        var image = new java.awt.image.BufferedImage(component.getWidth(), component.getHeight(), java.awt.image.BufferedImage.TYPE_INT_RGB);
    ...
    @Test void nestedInputsShowCustomizationAndBusyRestoresOriginalEnabledStates() throws Exception {
        SwingUtilities.invokeAndWait(() -> {
            ...
            ToggleSwitch required = new ToggleSwitch(); JPanel flags = new JPanel(); flags.add(required);
