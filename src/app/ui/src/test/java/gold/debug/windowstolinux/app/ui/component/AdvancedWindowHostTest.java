...
class AdvancedWindowHostTest {
    @Test void dockedAndFloatingInspectorsPreserveWorkspaceAndCloseWithTheirOwner() throws Exception {
        ...
        SwingUtilities.invokeAndWait(() -> {
            ...
            JPanel root = new JPanel(new BorderLayout()); root.add(pane);
            frame.setContentPane(root); frame.setBounds(0, 0, 520, 360);
            ...
            try {
                ...
                pane.setExpanded(true); frame.validate();
                assertEquals(size, workspace.getSize());
                assertEquals(840, frame.getWidth());
                pane.setExpanded(true); assertEquals(840, frame.getWidth());
                pane.setExpanded(false); frame.validate(); assertEquals(520, frame.getWidth());
            ...
            } finally { controller.close(); frame.dispose(); }
        });
    }
}
