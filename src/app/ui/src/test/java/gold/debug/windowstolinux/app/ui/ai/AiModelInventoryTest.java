...
class AiModelInventoryTest {
    ...
    private AiProviderSummary model(String id,int order){
        return new AiProviderSummary(new AiProviderProfile(id,URI.create("https://example.test/v1/chat/completions"),"model-"+id,"ai/"+id,CredentialStorageMode.MASTER_PASSWORD),
            id,order,1,Optional.of(Instant.now()),Optional.of(Instant.now()));
    }
    @Test void inventoryRemainsVisibleAndPurposeDraftDoesNotChangeDisplayOrder() throws Exception {
        var values=new AtomicReference<>(List.of(model("first",0),model("second",1),model("third",2)));
        var roles=new EnumMap<AiPurposeType,List<AiPurposeAssignment>>(AiPurposeType.class);for(var role:AiPurposeType.values())roles.put(role,List.of());
        var failSave=new AtomicBoolean();
        AiApplicationFacade service=(AiApplicationFacade)Proxy.newProxyInstance(getClass().getClassLoader(),new Class<?>[]{AiApplicationFacade.class},(p,m,a)->{
            return switch(m.getName()){
                case "listAiConfigurations"->values.get();
                case "listAiPurpose"->roles.get(a[0]);
                case "saveAiPurpose"->{if(failSave.get())throw new java.sql.SQLException("test save failure");roles.put((AiPurposeType)a[0],(List<AiPurposeAssignment>)a[1]);yield null;}
                case "reorderAiProviders"->{var prior=values.get();values.set(((List<String>)a[0]).stream().map(id->prior.stream().filter(v->v.profile().id().equals(id)).findFirst().orElseThrow()).toList());yield null;}
                default->throw new AssertionError(m);
            };
        });
        AtomicReference<AiModelInventoryPane> pane=new AtomicReference<>();
        SwingUtilities.invokeAndWait(()->{FlatLightLaf.setup();pane.set(new AiModelInventoryPane(service,c,messages,v->{}));pane.get().refresh();});idle();
        AtomicReference<JList<String>> original=new AtomicReference<>();
        SwingUtilities.invokeAndWait(()->{
            var inventory=field(pane.get(),"inventory",JList.class);original.set(inventory);
            pane.get().open(AiPurposeType.APPROVAL);assertSame(inventory,field(pane.get(),"inventory",JList.class));assertEquals(3,inventory.getModel().getSize());
            inventory.setSelectedIndex(0);field(pane.get(),"add",JButton.class).doClick();
            assertFalse(field(pane.get(),"add",JButton.class).isEnabled());assertEquals(3,inventory.getModel().getSize());
            inventory.setSelectedIndex(1);field(pane.get(),"add",JButton.class).doClick();
            var selected=field(pane.get(),"selected",JList.class);selected.setSelectedIndex(1);
            selected.getActionMap().get("alt UP").actionPerformed(new ActionEvent(selected,0,"alt UP"));
            assertEquals(List.of("second","first"),pane.get().capture().draft().stream().map(AiPurposeAssignment::profileId).toList());
            inventory.setSelectedIndex(0);inventory.getActionMap().get("alt DOWN").actionPerformed(new ActionEvent(inventory,0,"alt DOWN"));
        });idle();
        assertEquals(List.of("second","first","third"),values.get().stream().map(v->v.profile().id()).toList());
        failSave.set(true);SwingUtilities.invokeAndWait(()->button(pane.get(),"ai.purpose.save").doClick());idle();
        assertTrue(roles.get(AiPurposeType.APPROVAL).isEmpty());assertEquals(2,pane.get().capture().draft().size());
        failSave.set(false);SwingUtilities.invokeAndWait(()->button(pane.get(),"ai.purpose.save").doClick());idle();
        assertEquals(List.of("second","first"),roles.get(AiPurposeType.APPROVAL).stream().map(AiPurposeAssignment::profileId).toList());
        SwingUtilities.invokeAndWait(()->{
            assertTrue(pane.get().capture().purpose().isEmpty());assertSame(original.get(),field(pane.get(),"inventory",JList.class));
            pane.get().open(AiPurposeType.APPROVAL);field(pane.get(),"selected",JList.class).setSelectedIndex(0);
            field(pane.get(),"remove",JButton.class).doClick();button(pane.get(),"ai.purpose.cancel").doClick();
        });
        assertEquals(2,roles.get(AiPurposeType.APPROVAL).size());assertTrue(roles.get(AiPurposeType.DEPLOYMENT).isEmpty());
        assertEquals(List.of("b","c","a"),AiProviderDragTransfer.move(List.of("a","b","c"),"a",3));
    }
    @Test void failedModelSaveRetainsInputsAndCancelWaitsForWorkerCleanup() throws Exception {
        ...
        AiApplicationFacade service=(AiApplicationFacade)Proxy.newProxyInstance(getClass().getClassLoader(),new Class<?>[]{AiApplicationFacade.class},(p,m,a)->{
            if(!block[0])throw gold.debug.windowstolinux.app.service.failure.ApplicationServiceException.create(
                gold.debug.windowstolinux.app.service.failure.ApplicationServiceFailureType.AI_CONFIGURATION_TEST_FAILED,"test failed");
            started.countDown();try{new CountDownLatch(1).await();}catch(InterruptedException interrupted){cleanup.countDown();release.await();throw new CancellationException();}return null;
        });
        AtomicReference<AiProviderDialog> dialog=new AtomicReference<>();
        SwingUtilities.invokeAndWait(()->{dialog.set(new AiProviderDialog(null,service,c,messages,model("existing",1),()->fail("must not save")));
            dialog.get().setModalityType(Dialog.ModalityType.MODELESS);dialog.get().setVisible(true);field(dialog.get(),"save",JButton.class).doClick();});
        try{
            idle();SwingUtilities.invokeAndWait(()->{assertTrue(dialog.get().isShowing());assertEquals("existing",field(dialog.get(),"name",JTextField.class).getText());block[0]=true;field(dialog.get(),"save",JButton.class).doClick();});
    ...
    private static Stream<Component> descendants(Container root){return Stream.of(root.getComponents()).flatMap(v->v instanceof Container nested?Stream.concat(Stream.of(v),descendants(nested)):Stream.of(v));}
}
