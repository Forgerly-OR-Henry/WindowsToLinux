...
/**
 * Failures owned by the isolated Windows browser terminal. / Windows 隔离浏览器终端持有的失败。
 */
public enum BrowserRecoveryFailureType implements FailureDefinition {
    /**
     * OPERATION FAILED classification within browser recovery failure type.
     * <p>浏览器恢复失败类型中的操作失败分类。
     */
    OPERATION_FAILED("windows.browser.operation-failed", "browser", "windows.error.browserOperationFailed", FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * OBSERVATION FAILED classification within browser recovery failure type.
     * <p>浏览器恢复失败类型中的观测失败分类。
     */
    OBSERVATION_FAILED("windows.browser.observation-failed", "observation", "windows.error.browserObservationFailed", FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * CLEANUP FAILED classification within browser recovery failure type.
     * <p>浏览器恢复失败类型中的清理失败分类。
     */
    CLEANUP_FAILED("windows.browser.cleanup-failed", "cleanup", "windows.error.browserCleanupFailed", FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * INTERRUPTED classification within browser recovery failure type.
     * <p>浏览器恢复失败类型中的已中断分类。
     */
    INTERRUPTED("windows.browser.interrupted", "browser", "windows.error.browserInterrupted", FailureRecoveryAction.NONE);
    /**
    ...
    /**
     * Binds the supplied dependencies and state for browser recovery failure type.
     * <p>为浏览器恢复失败类型绑定传入的依赖及状态。
     *
     ...
     */
    BrowserRecoveryFailureType(String code, String phase, String messageKey, FailureRecoveryAction recoveryAction) {
        this.code=code; this.phase=phase; this.messageKey=messageKey; this.recoveryAction=recoveryAction;
    }
    /**
