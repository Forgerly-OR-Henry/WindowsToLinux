...
public final class PlaywrightTerminalSession implements BrowserTerminalSession {
    ...
    private final ScheduledExecutorService worker = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread thread = new Thread(r, "ssh-rescue-browser"); thread.setDaemon(true); return thread;
    });
    /**
     * Closed.
     * <p>已关闭。
     ...
     */
    public PlaywrightTerminalSession() { this(false, "about:blank"); }
    /**
     ...
     */
    PlaywrightTerminalSession(boolean headless, String initialUrl) { this.headless = headless; this.initialUrl = initialUrl; }
    ...
    @Override public void open() {
        call(() -> {
            if (browser != null && browser.isConnected()) {
                if (context.pages().isEmpty()) context.newPage();
                return null;
            }
            if (playwright != null) { playwright.close(); playwright = null; }
            bound = null; generation++; candidates.clear();
            playwright = Playwright.create(new Playwright.CreateOptions().setEnv(Map.of("PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD", "1", "DEBUG", "", "PWDEBUG", "0")));
            var launch = new BrowserType.LaunchOptions().setHeadless(headless);
            String channel = installedChannel();
            if (channel != null) launch.setChannel(channel);
            ...
            browser.onDisconnected(ignored -> { bound = null; generation++; });
    ...
    private void watch(Page page) {
        page.onWebSocket(socket -> socket.onClose(ignored -> {
            if (bound != null && bound.page() == page) { bound = null; generation++; }
        }));
        ...
        page.onClose(ignored -> { if (bound != null && bound.page() == page) { bound = null; generation++; } });
        page.onFrameNavigated(frame -> {
            if (bound != null && (bound.frame() == frame || bound.page().mainFrame() == frame)) { bound = null; generation++; }
        });
    }
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public List<TerminalTarget> targets() {
        return call(() -> {
            requireOpen(); candidates.clear(); List<TerminalTarget> result = new ArrayList<>();
            for (Page page : context.pages()) for (Frame frame : page.frames()) {
                if (result.size() >= 64) break;
                try {
                    for (ElementHandle element : frame.querySelectorAll(SELECTOR)) {
                        if (!element.isVisible() || result.size() >= 64) continue;
                        // Nested canvases inside xterm must not become a second input target. / xterm 内嵌画布不得成为第二个输入目标。
                        if (Boolean.TRUE.equals(element.evaluate("e => e.matches('canvas') && !!e.closest('.xterm')"))) continue;
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public long bind(String id) {
        return call(() -> {
            Binding candidate = candidates.get(id);
            if (candidate == null || !attached(candidate)) throw new IllegalStateException("terminal-unavailable");
            bound = candidate; generation++; return generation;
        });
    }
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public TerminalObservation observe(long expected) {
        try { return call(() -> {
            requireBound(expected);
            String text = (String) bound.element().evaluate("e => { const a = e.querySelector('.xterm-accessibility-tree'); "
            ...
            if (failure.failure().definition() == BrowserRecoveryFailureType.INTERRUPTED) throw failure;
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public void submit(long expected, String command) {
        ...
        call(() -> {
            ...
            if (input == null) input = bound.element();
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public boolean valid(long expected) {
        if (closed.get()) return false;
     ...
     * @throws BrowserRecoveryException if browser work, terminal observation, interruption or owned cleanup cannot complete / 浏览器工作、终端观测、中断处理或自有资源清理无法完成时
     */
    @Override public void invalidate() { if (!closed.get()) call(() -> { bound = null; generation++; return null; }); }
    ...
    private boolean attached(Binding value) {
        try { return browser.isConnected() && !value.page().isClosed() && !value.frame().isDetached()
    ...
    private void requireOpen() {
        if (browser == null || !browser.isConnected() || context.pages().isEmpty()) throw new IllegalStateException("browser-disconnected");
    }
    /**
    ...
    private void requireBound(long expected) {
        requireOpen(); if (expected != generation || bound == null || !attached(bound)) throw new IllegalStateException("terminal-changed");
    }
    /**
    ...
    private static String origin(String url) {
        try { URI uri = URI.create(url); return uri.getScheme() + "://" + Objects.toString(uri.getHost(), "local")
    ...
    private static String installedChannel() {
        for (String base : List.of("ProgramFiles(x86)", "ProgramFiles", "LOCALAPPDATA")) {
            String folder = System.getenv(base); if (folder == null) continue;
            if (java.nio.file.Files.isRegularFile(java.nio.file.Path.of(folder, "Microsoft", "Edge", "Application", "msedge.exe"))) return "msedge";
            if (java.nio.file.Files.isRegularFile(java.nio.file.Path.of(folder, "Google", "Chrome", "Application", "chrome.exe"))) return "chrome";
        }
        return null;
    }
    /**
    ...
    private <T> T call(Callable<T> action) {
        if (closed.get()) throw new BrowserRecoveryException(BrowserRecoveryFailureType.OPERATION_FAILED, null);
        try { return worker.submit(action).get(); }
        catch (InterruptedException failure) { Thread.currentThread().interrupt(); throw new BrowserRecoveryException(BrowserRecoveryFailureType.INTERRUPTED, failure); }
        catch (ExecutionException failure) { throw new BrowserRecoveryException(BrowserRecoveryFailureType.OPERATION_FAILED, failure.getCause()); }
    }
    /**
    ...
    @Override public void close() {
        ...
        Future<?> cleanup = worker.submit(() -> {
            ...
            finally { try { if (browser != null) browser.close(); } finally { if (playwright != null) playwright.close(); } }
        });
        long deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(15);
        try {
            while (true) {
                try { cleanup.get(Math.max(1, deadline - System.nanoTime()), TimeUnit.NANOSECONDS); break; }
                catch (InterruptedException failure) { interrupted = true; }
            }
        } catch (ExecutionException | TimeoutException failure) { throw new BrowserRecoveryException(BrowserRecoveryFailureType.CLEANUP_FAILED, null); }
        finally {
            worker.shutdownNow(); deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(5);
            try {
                while (true) {
                    try {
