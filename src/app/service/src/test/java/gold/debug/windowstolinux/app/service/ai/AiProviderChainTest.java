...
class AiProviderChainTest {
    ...
    private void seed(DesktopPersistence db, String... ids) throws Exception {
        for (String id : ids) {
            ...
            try (var store = new DesktopSecretStoreService(db.encryptedSecrets()).open(profile.credentialMode(), master())) {
                store.save(profile.credentialKey(), (id + "-key").toCharArray());
            }
            db.aiProfiles().saveVerified(profile.stored(), id, java.time.Instant.now());
            var members=new java.util.ArrayList<>(db.aiProfiles().purposes().list(gold.debug.windowstolinux.shared.model.ai.AiPurposeType.DEPLOYMENT));
            members.add(new gold.debug.windowstolinux.shared.model.ai.AiPurposeAssignment(id,true));
            db.aiProfiles().purposes().save(gold.debug.windowstolinux.shared.model.ai.AiPurposeType.DEPLOYMENT,members);
        }
    }
    private void enabled(DesktopPersistence db,String id,boolean enabled) throws Exception {
        ...
        db.aiProfiles().purposes().save(purpose,db.aiProfiles().purposes().list(purpose).stream()
            .map(v->v.profileId().equals(id)?new gold.debug.windowstolinux.shared.model.ai.AiPurposeAssignment(id,enabled):v).toList());
    }
    ...
    @Test void skipsDisabledAndFallsThroughHttpAndSchemaFailuresButStopsOnValidRefusal() throws Exception {
        ...
        try (var db = DesktopPersistence.open(directory)) {
            var facade = new AiUseCaseFacade(db.aiProfiles(), new DesktopSecretStoreService(db.encryptedSecrets()), client); seed(db,"disabled","http","schema","refusal","unused");
            enabled(db,"disabled", false); db.aiProfiles().saveRoleAssignment(new gold.debug.windowstolinux.app.db.entity.StoredAiRoleAssignment("PROJECT_ANALYSIS","unused"));
    ...
    @Test void usesSnapshotUntilNextCallAndReportsAllFailuresOncePerProvider() throws Exception {
        try (var db = DesktopPersistence.open(directory)) {
            ...
            var client = new OpenAiCompatibleRoleClient((endpoint,key,body) -> {
                String id = endpoint.getHost().split("\\.")[0]; calls.add(id);
                try { enabled(db,"b", false); db.aiProfiles().reorder(List.of("b","a")); } catch (Exception e) { throw new IllegalStateException(e); }
                return new RoleChatResult(500,"failed");
            }, Clock.systemUTC());
            var facade = new AiUseCaseFacade(db.aiProfiles(), new DesktopSecretStoreService(db.encryptedSecrets()), client); seed(db,"a","b");
            ...
            enabled(db,"a", false); assertTrue(facade.invokeRole(context,master()).isEmpty());
        }
    }
    @Test void cancellationStopsEntireChainAndClearsSecrets() throws Exception {
        ...
        try (var db = DesktopPersistence.open(directory)) {
            var facade = new AiUseCaseFacade(db.aiProfiles(),new DesktopSecretStoreService(db.encryptedSecrets()),client); seed(db,"a","b"); char[] unlock = master();
    ...
    @Test void testsOnlySelectedModelBeforeSavingAndRetainsImmutableCredentialVersions() throws Exception {
        ...
        try (var db = DesktopPersistence.open(directory)) {
            ...
            char[] key = "first-key".toCharArray(); assertThrows(Exception.class, () -> facade.saveConfiguration(profile("a"),"First",master(),key,gold.debug.windowstolinux.shared.model.ai.AiCapabilityType.TEXT)); assertArrayEquals(new char[key.length],key); assertTrue(facade.configurations().isEmpty());
            success[0] = true; facade.saveConfiguration(profile("a"),"First",master(),"first-key".toCharArray(),gold.debug.windowstolinux.shared.model.ai.AiCapabilityType.TEXT); var first = facade.configurations().getFirst();
            assertTrue(first.textVerifiedAt().isPresent()); assertTrue(facade.purpose(gold.debug.windowstolinux.shared.model.ai.AiPurposeType.DEPLOYMENT).isEmpty()); assertTrue(requests.stream().allMatch(body -> body.contains("connection-test") && !body.contains("local-project")));
            enabled(db,"a",false); facade.saveConfiguration(first.profile(),"Renamed",master(),new char[0],gold.debug.windowstolinux.shared.model.ai.AiCapabilityType.TEXT); assertEquals(first.profile().credentialKey(),facade.configurations().getFirst().profile().credentialKey()); assertTrue(facade.purpose(gold.debug.windowstolinux.shared.model.ai.AiPurposeType.DEPLOYMENT).isEmpty());
            facade.saveConfiguration(first.profile(),"Renamed",master(),"second-key".toCharArray(),gold.debug.windowstolinux.shared.model.ai.AiCapabilityType.TEXT); var second = facade.configurations().getFirst(); assertNotEquals(first.profile().credentialKey(),second.profile().credentialKey());
