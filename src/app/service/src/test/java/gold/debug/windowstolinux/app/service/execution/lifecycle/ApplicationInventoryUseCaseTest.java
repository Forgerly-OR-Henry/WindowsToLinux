...
class ApplicationInventoryUseCaseTest {
    ...
    @Test void combinesFiltersUsesStableDeploymentOrderAndNeverInventsAnExternalDeploymentDate() throws Exception {
        ...
        try (var db = DesktopPersistence.open(directory)) {
            ...
            db.externalApplications().adopt(new StoredExternalApplication(externalId, server.id(), server.host(), 22, "tester",
                    new DiscoveredApplication(new ExternalApplicationTarget(ExternalApplicationKind.SYSTEMD, "external.service", "c".repeat(64)), "External", RuntimeState.STOPPED, true, true, false), time.plusSeconds(1), time.plusSeconds(1)));
            var servers = new ServerUseCaseFacade(db.servers(), new DesktopSecretStoreService(db.encryptedSecrets()), (endpoint, credential, verifier) -> { throw new AssertionError("inventory reads must not connect"); });
            var inventory = new ApplicationInventoryUseCase(db.managedApplications(), db.externalApplications(), servers);
            var list = inventory.list();
            assertEquals(List.of(externalId, "managed:a", "managed:b", "managed:unknown"), list.stream().map(ApplicationSummary::key).toList());
