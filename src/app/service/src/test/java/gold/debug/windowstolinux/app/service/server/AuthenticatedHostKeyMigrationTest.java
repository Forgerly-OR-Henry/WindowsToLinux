...
class AuthenticatedHostKeyMigrationTest {
    ...
    @Test void migratesTheSameKeyOnlyAfterAuthenticationAndRejectsChangedKeys() throws Exception {
        try (var database = DesktopPersistence.open(directory)) {
            ...
            var service = new ServerUseCaseFacade(database.servers(), new DesktopSecretStoreService(database.encryptedSecrets()),
                    (endpoint, credential, verifier) -> { throw new AssertionError("No network needed"); });
            var observation = new HostKeyObservation("SHA256:standard-same-key", prior.hostKeySha256());
            var verifier = service.hostKeyVerifier(PROFILE, ignored -> { throw new AssertionError("Existing trust must not prompt"); });
    ...
    @Test void firstUseDoesNotPersistFailedOrDifferentAuthentication() throws Exception {
        try (var database = DesktopPersistence.open(directory)) {
            var service = new ServerUseCaseFacade(database.servers(), new DesktopSecretStoreService(database.encryptedSecrets()),
                    (endpoint, credential, verifier) -> { throw new AssertionError("No network needed"); });
            ...
            assertFalse(verifier.authenticated(PROFILE.endpoint(), new HostKeyObservation("SHA256:other-key", "SHA256:other-encoded")));
            assertTrue(database.servers().findServer(PROFILE.id()).isEmpty());
