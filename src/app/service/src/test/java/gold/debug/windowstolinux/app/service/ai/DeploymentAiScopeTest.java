...
class DeploymentAiScopeTest {
    ...
    private AiProviderProfile profile(String id){return new AiProviderProfile(id,URI.create("https://example.test/v1/chat/completions"),id,"ai/"+id,CredentialStorageMode.MASTER_PASSWORD);}
    @Test void staticScopeNeverInvokesTransportEvenWhenModelsExist() throws Exception {
        try(var db=DesktopPersistence.open(directory)){
            var calls=new AtomicInteger();var facade=new AiUseCaseFacade(db.aiProfiles(),new DesktopSecretStoreService(db.encryptedSecrets()),
                new OpenAiCompatibleRoleClient((url,key,body)->{calls.incrementAndGet();throw new AssertionError("static task must not invoke AI");},Clock.systemUTC()));
            ...
            try(var scope=facade.openDeployment(DeploymentAutomationMode.STATIC)){
                ...
                assertTrue(facade.invokeRole(new ErrorExplanationRoleContext("deploy","failure"),"master".toCharArray()).isEmpty());
            }
            assertEquals(0,calls.get());assertTrue(DeploymentAiScope.current().isEmpty());
        }
    }
    @Test void agentCannotOpenWithoutExplicitVerifiedApprovalMembership() throws Exception {
        try(var db=DesktopPersistence.open(directory)){
            ...
            for(var ignored:AgentApprovalMode.values()){
                var failure=assertThrows(ApplicationServiceException.class,()->facade.openDeployment(DeploymentAutomationMode.AGENT));
            ...
            assertThrows(ApplicationServiceException.class,()->facade.openDeployment(DeploymentAutomationMode.AGENT));
    ...
    @Test void deploymentPurposeRequiredAndInventoryOrderCannotChangeCapturedRouting() throws Exception {
        try(var db=DesktopPersistence.open(directory)){
            var facade=new AiUseCaseFacade(db.aiProfiles(),new DesktopSecretStoreService(db.encryptedSecrets()));
            assertThrows(ApplicationServiceException.class,()->facade.openDeployment(DeploymentAutomationMode.ASSISTED));
            for(var id:List.of("a","b"))db.aiProfiles().saveVerified(profile(id).stored(),id,Instant.now());
            db.aiProfiles().purposes().save(AiPurposeType.DEPLOYMENT,List.of(new AiPurposeAssignment("b",true),new AiPurposeAssignment("a",true)));
            try(var scope=facade.openDeployment(DeploymentAutomationMode.ASSISTED)){
                ...
                assertEquals(List.of("b","a"),scope.providers(AiPurposeType.DEPLOYMENT).stream().map(AiProviderProfile::id).toList());
            }
        }
    }
}
