...
class AutomaticDatabaseUseCaseTest {
    ...
    @Test
    void declaredInitializationForOneDatabaseDoesNotInitializeEveryOtherDatabase() throws Exception {
        Files.writeString(root.resolve("schema.sql"), "CREATE TABLE sample(id integer);");
        var assessment = new DatabaseProjectInspector.Assessment(
                List.of(requirement("main", List.of("schema.sql")), requirement("audit", List.of())),
                List.of("schema.sql"), true, false);
    ...
    @Test
    void initializationFilesAreValidatedBeforeAnyRemoteDependenciesAreNeeded() {
        var assessment = new DatabaseProjectInspector.Assessment(List.of(requirement("main", List.of("missing.sql"))),
                List.of(), true, false);
        assertThrows(IllegalArgumentException.class,
                () -> useCase().completeInputs(root, "demo", assessment, new char[0], interaction(false)));
    }

    @Test
    void cancellingUnknownDatabaseSelectionClearsUnlockCopyWithoutConnecting() {
    ...
    @Test
    void redisCannotBeMisclassifiedAsASqlDatasourceOrInitializer() {
    ...
    private static DatabaseRequirement requirement(String id, List<String> sql) {
        return new DatabaseRequirement(id, DatabaseEngineType.POSTGRESQL, "", id, id, id.toUpperCase(Locale.ROOT),
                id.toUpperCase(Locale.ROOT) + "_PASSWORD", sql, false, "fixture");
    }
    ...
    private static AutomaticDeploymentInteraction interaction(boolean cancel) {
        return new AutomaticDeploymentInteraction() {
            public Optional<Map<String, String>> requestInputs(List<DeploymentInputField> fields) {
                assertTrue(cancel, "declared database inputs must not be asked again");
                return Optional.empty();
            }

            public boolean confirm(String key, Map<String, ?> details) {
                throw new AssertionError("source planning has no remote risk");
            }

            public char[] requestSecret(String key) {
                throw new AssertionError("source planning must not ask for DB credentials");
            }
        };
    }
}
