...
class DeploymentFormUseCaseTest {
    @Test
    void preservesExplicitHealthChoicesWithoutInventingEndpoints() {
        ...
        var inferred = DeploymentFormUseCase.request(source, server(),
                input("AUTOMATIC", "http://127.0.0.1:18080/health"));
    ...
    @Test
    void resolvesGitReferencesAndRetainsTheAllowedHost() {
        var request = DeploymentFormUseCase.request(
                new DeploymentSourceInput(Optional.empty(), "https://example.test/team/demo.git", 1, "release-one"),
    ...
    private static DeploymentFormInput input(String healthMode, String endpoint) {
        return new DeploymentFormInput(true, DeploymentProjectType.NODE_SERVICE, "", "", "22", "", "PORT=18080", "",
                DatabaseReviewMode.UNREVIEWED, "", healthMode, endpoint, "200", "10", "1", "", "", "", "", "", null,
                false);
    }

    private static ServerProfile server() {
        return new ServerProfile("server", "example.test", 22, "root", "server/password",
