 ...
 */
public record AutomaticDeploymentRequest(Optional<Path> directory, Optional<GitSourceRequest> git, ServerProfile server,
        Map<String, String> overrides, DeploymentAutomationMode automationMode, AgentApprovalMode approvalMode,
        String taskId) {
     ...
     */
    public AutomaticDeploymentRequest(Optional<Path> directory, Optional<GitSourceRequest> git, ServerProfile server,
            Map<String, String> overrides) {
        this(directory, git, server, overrides, DeploymentAutomationMode.STATIC, AgentApprovalMode.AUTOMATIC,
     ...
     */
    public AutomaticDeploymentRequest withAutomation(DeploymentAutomationMode mode, AgentApprovalMode approval) {
        return new AutomaticDeploymentRequest(directory, git, server, overrides, mode, approval, taskId);
    }
    ...
    * @param taskId immutable click-time task identity / 点击时不可变任务身份
    ...
    public AutomaticDeploymentRequest {
        Objects.requireNonNull(automationMode);
        Objects.requireNonNull(approvalMode);
        java.util.UUID.fromString(taskId);
        Objects.requireNonNull(directory);
        Objects.requireNonNull(git);
        Objects.requireNonNull(server);
        if (directory.isPresent() == git.isPresent())
            throw new IllegalArgumentException("select exactly one source");
