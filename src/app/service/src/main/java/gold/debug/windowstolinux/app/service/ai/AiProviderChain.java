...
final class AiProviderChain {
     ...
     */
    AiProviderChain(AiProfileRepository profiles, DesktopSecretStoreService secrets) {
     ...
     */
    <T> Outcome<T> invoke(gold.debug.windowstolinux.shared.model.ai.AiPurposeType purpose, char[] master,
            Invocation<T> invocation) throws java.sql.SQLException {
        List<AiProviderAttempt> attempts = new ArrayList<>();
        Optional<T> last = Optional.empty();
        try {
            checkCancelled();
            var scope = DeploymentAiScope.current();
            var snapshot = scope.isPresent()
                    ? scope.get().remaining(purpose)
                    : profiles.configuredFor(purpose).stream()
                            .map(value -> AiProviderProfile.fromStored(value.profile())).toList();
            int position = 0;
            while (true) {
                scope.ifPresent(DeploymentAiScope::checkpoint);
                var candidates = scope.isPresent() ? scope.get().remaining(purpose) : snapshot;
                if (scope.isPresent() ? candidates.isEmpty() : position >= candidates.size())
                    break;
                AiProviderProfile profile = scope.isPresent() ? candidates.getFirst() : candidates.get(position++);
                checkCancelled();
                char[] key = null;
                Attempt<T> result = null;
                try (var store = secrets.open(profile.credentialMode(), master)) {
                    ...
                    checkCancelled();
                    scope.ifPresent(DeploymentAiScope::called);
                    result = invocation.call(profile, key);
                    checkCancelled();
                } catch (CancellationException cancelled) {
                    throw cancelled;
                } catch (InterruptedException interrupted) {
                    Thread.currentThread().interrupt();
                    throw new CancellationException("AI invocation cancelled");
                ...
                attempts.add(new AiProviderAttempt(profile.id(), profile.model(),
                        result == null ? AiInvocationStatus.UNAVAILABLE : result.status(),
                ...
                if (result != null) {
                    ...
                    if (result.status() == AiInvocationStatus.VALIDATED)
                        return new Outcome<>(last, List.copyOf(attempts), true, snapshot);
                }
     ...
     */
    record Outcome<T>(Optional<T> value, List<AiProviderAttempt> attempts, boolean valid,
            List<AiProviderProfile> snapshot) {
    }
}
