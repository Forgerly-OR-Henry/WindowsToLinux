...
public enum ApplicationServiceFailureType implements FailureDefinition {
    ...
    APPROVAL_MODEL_REQUIRED("service.ai.approval-model-required", "ai", "service.error.approvalModelRequired",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /** Assisted and Agent tasks require deployment models. / AI 辅助 及 Agent 任务需要部署模型。 */
    DEPLOYMENT_MODEL_REQUIRED("service.ai.deployment-model-required", "ai", "service.error.deploymentModelRequired",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
     ...
     */
    RECOVERY_PROBE_FAILED("service.recovery.probe-failed", "recovery", "service.error.recoveryProbeFailed",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * RECOVERY INTERRUPTED classification within application service failure type.
     * <p>应用服务失败类型中的恢复已中断分类。
     */
    RECOVERY_INTERRUPTED("service.recovery.interrupted", "recovery", "service.error.recoveryInterrupted",
            FailureSeverityLevel.WARNING, FailureRecoveryAction.NONE),
    /**
     * RECOVERY OBSERVATION FAILED classification within application service failure type.
     * <p>应用服务失败类型中的恢复观测失败分类。
     */
    RECOVERY_OBSERVATION_FAILED("service.recovery.observation-failed", "recovery",
            "service.error.recoveryObservationFailed", FailureSeverityLevel.ERROR,
     ...
     */
    RECOVERY_CLEANUP_FAILED("service.recovery.cleanup-failed", "cleanup", "service.error.recoveryCleanupFailed",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * AI CONFIGURATION TEST FAILED classification within application service failure type.
     * <p>应用服务失败类型中的AI配置测试失败分类。
     */
    AI_CONFIGURATION_TEST_FAILED("service.ai.configuration-test-failed", "ai",
            "service.error.aiConfigurationTestFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * DATABASE AUTH REQUIRED classification within application service failure type.
     * <p>应用服务失败类型中的数据库认证必需分类。
     */
    DATABASE_AUTH_REQUIRED("service.database.auth-required", "database", "service.error.databaseAuthRequired",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * DATABASE STATE CHANGED classification within application service failure type.
     * <p>应用服务失败类型中的数据库状态已变化分类。
     */
    DATABASE_STATE_CHANGED("service.database.state-changed", "database", "service.error.databaseStateChanged",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    DATABASE_INITIALIZATION_FAILED("service.database.initialization-failed", "database",
            "service.error.databaseInitializationFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    DATABASE_MANUAL_RESTORE_REQUIRED("service.database.manual-restore-required", "database",
            "service.error.databaseManualRestoreRequired", FailureSeverityLevel.ERROR,
    ...
    DATABASE_VERSION_UNSUPPORTED("service.database.version-unsupported", "database",
            "service.error.databaseVersionUnsupported", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * DATABASE ACTION FAILED classification within application service failure type.
     * <p>应用服务失败类型中的数据库动作失败分类。
     */
    DATABASE_ACTION_FAILED("service.database.action-failed", "database", "service.error.databaseActionFailed",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * STORAGE MODE MISMATCH classification within application service failure type.
     * <p>应用服务失败类型中的存储模式不匹配分类。
     */
    STORAGE_MODE_MISMATCH("service.validation.storage-mode-mismatch", "validation", "service.error.storageModeMismatch",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    DEPLOYMENT_ANALYSIS_REQUIRED("service.deployment.analysis-required", "deployment",
            "service.error.deploymentAnalysisRequired", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    APPLICATION_SECRET_REFERENCE_MISSING("service.secret.reference-missing", "secret",
            "service.error.applicationSecretReferenceMissing", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    LIFECYCLE_CONTEXT_MISMATCH("service.lifecycle.context-mismatch", "lifecycle",
            "service.error.lifecycleContextMismatch", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    MANAGED_SUMMARY_READ_FAILED("service.persistence.summary-read-failed", "persistence",
            "service.error.managedSummaryReadFailed", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.RESTART_APPLICATION),
    /**
    ...
    APPLICATION_NOT_SELECTED("service.lifecycle.application-not-selected", "lifecycle",
            "service.error.applicationNotSelected", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * LEGACY RUNTIME MISSING classification within application service failure type.
     * <p>应用服务失败类型中的历史运行时缺失分类。
     */
    LEGACY_RUNTIME_MISSING("service.lifecycle.runtime-missing", "lifecycle", "service.error.runtimeMissing",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
    ...
    SERVER_PROFILE_MISSING("service.lifecycle.server-profile-missing", "lifecycle",
            "service.error.serverProfileMissing", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    APPLICATION_NOT_MANAGED("service.lifecycle.application-not-managed", "lifecycle",
            "service.error.applicationNotManaged", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    APPLICATION_SERVER_CONFLICT("service.deployment.server-conflict", "deployment",
            "service.error.applicationServerConflict", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
    ...
    APPLICATION_IDENTITY_INVALID("service.deployment.identity-invalid", "deployment",
            "service.error.applicationIdentityInvalid", FailureSeverityLevel.ERROR,
            FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
    ...
    DEPLOYMENT_RECORD_SAVE_FAILED("service.persistence.deployment-record-save-failed", "persistence",
            "service.error.deploymentRecordSaveFailed", FailureSeverityLevel.WARNING,
            FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * BACKUP INPUT INCOMPLETE classification within application service failure type.
     * <p>应用服务失败类型中的备份输入未完成分类。
     */
    BACKUP_INPUT_INCOMPLETE("service.backup.input-incomplete", "backup", "service.error.backupInputIncomplete",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUEST_USER_CORRECTION),
    /**
     * BACKUP RECOVERY FAILED classification within application service failure type.
     * <p>应用服务失败类型中的备份恢复失败分类。
     */
    BACKUP_RECOVERY_FAILED("service.backup.recovery-failed", "backup", "service.error.backupRecoveryFailed",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY),
    /**
     * RESTORE RECORD SAVE FAILED classification within application service failure type.
     * <p>应用服务失败类型中的恢复记录保存失败分类。
     */
    RESTORE_RECORD_SAVE_FAILED("service.restore.record-save-failed", "restore", "service.error.restoreRecordSaveFailed",
            FailureSeverityLevel.ERROR, FailureRecoveryAction.REQUIRE_MANUAL_RECOVERY);
     ...
     */
    ApplicationServiceFailureType(String code, String phase, String messageKey, FailureSeverityLevel severity,
