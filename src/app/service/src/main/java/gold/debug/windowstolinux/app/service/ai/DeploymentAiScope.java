...
public final class DeploymentAiScope implements AutoCloseable {
     ...
     */
    public void beforeRequest(Runnable checkpoint) {
        requestCheckpoint = Objects.requireNonNull(checkpoint);
    }

    /** Stops before selecting the next model when pause or cancel is pending. / 暂停或取消待处理时在选择下一模型前停止。 */
    public void checkpoint() {
        requestCheckpoint.run();
    }
     ...
     */
    public void audit(java.util.function.Consumer<Map<String, String>> events) {
     ...
     */
    public void attempted(AiPurposeType purpose, String profile, String status) {
        events.accept(
                Map.of("type", "MODEL_ATTEMPT", "action", "", "detail", purpose.name() + ":" + profile + ":" + status));
    }
     ...
     */
    public List<AiProviderProfile> remaining(AiPurposeType purpose) {
        var all = providers(purpose);
        return all.subList(Math.min(positions.getOrDefault(purpose, 0), all.size()), all.size());
    }
     ...
     */
    public boolean advance(AiPurposeType purpose) {
        var remaining = remaining(purpose);
        if (!remaining.isEmpty())
            consumed.computeIfAbsent(purpose, p -> new HashSet<>()).add(remaining.getFirst());
        positions.put(purpose, positions.getOrDefault(purpose, 0) + 1);
        events.accept(Map.of("type", "MODEL_HANDOFF", "action", "", "detail",
                purpose.name() + ":" + (remaining.isEmpty() ? "exhausted" : remaining.getFirst().id()) + "->"
                        + (remaining(purpose).isEmpty() ? "exhausted" : remaining(purpose).getFirst().id())));
        return !remaining(purpose).isEmpty();
    }
     ...
     */
    public void usage(OptionalLong tokens) {
        if (tokens.isPresent()) {
            measuredCalls++;
            reportedTokens = Math.addExact(reportedTokens, tokens.getAsLong());
        }
    }
    ...
    public Map<String, String> metrics() {
        return Map.of("modelCalls", Long.toString(calls), "reportedTokens", Long.toString(reportedTokens),
                "callsWithTokenUsage", Long.toString(measuredCalls), "humanInterventions",
                Long.toString(humanInterventions));
    }
    ...
    public DeploymentAiScope(DeploymentAutomationMode mode, Map<AiPurposeType, List<AiProviderProfile>> providers) {
        if (CURRENT.get() != null)
            throw new IllegalStateException("deployment AI scope already active");
        this.mode = Objects.requireNonNull(mode);
        var frozen = new EnumMap<AiPurposeType, List<AiProviderProfile>>(AiPurposeType.class);
        for (var purpose : AiPurposeType.values())
            frozen.put(purpose, List.copyOf(providers.getOrDefault(purpose, List.of())));
     ...
     */
    public static Optional<DeploymentAiScope> current() {
    ...
    public boolean allowsAi() {
        return mode != DeploymentAutomationMode.STATIC;
    }
     ...
     */
    public List<AiProviderProfile> providers(AiPurposeType purpose) {
        return allowsAi() ? providers.get(purpose) : List.of();
    }
    ...
    public void replace(Map<AiPurposeType, List<AiProviderProfile>> snapshot) {
        if (Thread.currentThread() != owner)
            throw new IllegalStateException("wrong model snapshot owner");
        var next = new EnumMap<AiPurposeType, List<AiProviderProfile>>(AiPurposeType.class);
        for (var purpose : AiPurposeType.values())
            next.put(purpose, snapshot.getOrDefault(purpose, List.of()).stream()
    ...
    public String binding() {
        return revision + ":"
                + gold.debug.windowstolinux.shared.model.agent.AgentAction.digest(new TreeMap<>(providers).toString());
    }
    ...
    public void close() {
        if (Thread.currentThread() != owner || CURRENT.get() != this)
            throw new IllegalStateException("wrong deployment scope owner");
        CURRENT.remove();
    }
    ...
    private void recordSnapshot() {
        for (var purpose : AiPurposeType.values()) {
            int order = 0;
            for (var profile : providers(purpose))
                events.accept(Map.of("type", "MODEL_SNAPSHOT", "action", "", "detail",
                        revision + ":" + purpose.name() + ":" + (++order) + ":" + profile.id() + ":" + profile.model()
                                + ":"
