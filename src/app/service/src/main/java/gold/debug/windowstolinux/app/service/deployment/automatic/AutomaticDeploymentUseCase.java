...
public final class AutomaticDeploymentUseCase {
     ...
     * @param agent Agent execution boundary / Agent 执行边界
     */
    public AutomaticDeploymentUseCase(AutomaticDeploymentApplicationFacade service,SourcePreparationUseCase source,
            ServerOperationLockRegistry locks,AssistedDeploymentAdvisor assistance,AutomaticAgentBoundary agent){
        this.service=service;this.source=source;this.locks=locks;this.assistance=assistance;this.agent=agent;
    }
    ...
    public AutomaticDeploymentOutcome deploy(AutomaticDeploymentRequest request, char[] master,
        ...
        if(request.automationMode()==DeploymentAutomationMode.AGENT&&agent==null)throw new SecurityException("Agent execution requires independent approval boundary");
        if(request.automationMode()==DeploymentAutomationMode.ASSISTED&&assistance==null)throw new IllegalStateException("assisted deployment requires checkpoint advisor");
    ...
    private AutomaticDeploymentOutcome execute(AutomaticDeploymentRequest request, Path root, Optional<GitSnapshot> git,
        ...
        if(agent!=null)agent.freeze(root);
        var resolved = operation(AgentToolType.ANALYZE_SOURCE,Map.of("effect","Analyze private source snapshot; request unresolved nonsecret inputs; never modify original source"),
            ()->resolveInputs(request, root, master, interaction, progress));
        var sourceFacts=Map.of("components",String.join(",",resolved.discovered().stream().map(DiscoveredProjectComponent::id).toList()),
            "sourceRevisions",resolved.preparations().values().stream().map(p->p.archive().orElseThrow().contentSha256()).sorted().toList().toString(),
            "validatedInputsDigest",AgentAction.digest(new TreeMap<>(resolved.inputs()).toString()),"healthOwner",resolved.healthOwner());
        if(agent!=null)sourceFacts.forEach(agent::fact);
        if(assistance!=null)assistance.check("ANALYSIS",sourceFacts);
        ...
        var capabilities=operation(AgentToolType.VERIFY_SERVER, Map.of("effect","Read host identity and supported deployment capabilities"),
        ...
        if(agent!=null){serverFacts.forEach(agent::fact);
            agent.register("server-capabilities",AgentToolType.VERIFY_SERVER,Map.of("effect","Read current server deployment capabilities"),()->true,()->{
                ...
                return new gold.debug.windowstolinux.shared.model.agent.AgentObservation(true,true,Map.of("operatingSystem",observed.operatingSystem(),"architecture",observed.architecture(),"availableBytes",Long.toString(observed.availableBytes())));});
            agent.register("environment-preparation",AgentToolType.PREPARE_ENVIRONMENT,Map.of("effect","Recheck and prepare existing supported dependencies and helper; no unrelated packages or arbitrary shell"),()->true,()->{
                prepareEnvironmentOnce(request,master,fingerprint,interaction);return new gold.debug.windowstolinux.shared.model.agent.AgentObservation(true,true,Map.of("environment","prepared"));});
        }
    ...
    private void validateInputs(Map<String, Map<String,String>> inputs, Map<String, ReviewedSourcePreparation> preparations,
        ...
        for (var entry : inputs.entrySet()) {
            ...
            if (preparations.get(entry.getKey()).assessment().facts().orElseThrow().support().level() == DeploymentSupportLevel.EXPERIMENTAL_ADAPTER
                    ...
                    && agent==null && !interaction.confirm("auto.risk.experimental", Map.of("component", entry.getKey()))) throw new CancellationException();
            if (values.get("type").equals("DOCKERFILE_CONTAINER") && values.getOrDefault("containerEngine", "PODMAN").equals("DOCKER")
                    && agent==null && !interaction.confirm("auto.risk.docker", Map.of("component", entry.getKey()))) throw new CancellationException();
        }
    }
    ...
    private AutomaticDeploymentOutcome deploySingle(AutomaticDeploymentRequest request, Path root, Optional<GitSnapshot> git,
        ...
        var databaseAssessments = resolved.databases(); var inputs = resolved.inputs();
            ...
            if(agent!=null&&service instanceof gold.debug.windowstolinux.app.service.contract.ManagedApplicationFacade managed)
    ...
    private AutomaticDeploymentOutcome deployMultiple(AutomaticDeploymentRequest request, Path root, Optional<GitSnapshot> git,
        ...
        if(agent!=null&&service instanceof gold.debug.windowstolinux.app.service.contract.ManagedApplicationFacade managed)
            AgentManagedToolCatalog.register(agent,managed,request.server(),reviewed.components().stream().map(c->c.application().id()).collect(java.util.stream.Collectors.toSet()),master);
    ...
    private AutomaticDatabasePreparation prepareDatabases(Path root, String id,
        ...
        progress.accept(LocalizedMessage.of("auto.progress.environment"));
        prepareEnvironment(request, master, fingerprint, interaction);
        return operation(AgentToolType.PREPARE_DATABASE,Map.of("application",id,"effect","Prepare only the declared reviewed database bindings; preserve existing data unless separately authorized",
            "assessmentDigest",AgentAction.digest(assessment.toString())),()->service.prepareAutomaticDatabases(root, id, request.server(), assessment, master.clone(), interaction, fingerprint, progress));
    }
    ...
    private void prepareEnvironment(AutomaticDeploymentRequest request, char[] master,
        ...
        if (agent==null && !interaction.confirm("environment.confirm", Map.of("serverId", request.server().id(),
                "host", request.server().host(), "port", request.server().sshPort(),
                "username", request.server().username()))) throw new CancellationException();
        operation(AgentToolType.PREPARE_ENVIRONMENT,Map.of("effect","Install supported deployment tools and scoped privileged helper; separately review system-security preparation"),
            ()->{prepareEnvironmentOnce(request, master, fingerprint, interaction);return null;});
    }
    ...
    private void answerWithAi(List<DeploymentInputField> fields, Map<String, String> values,
        ...
        values.putAll(assistance==null ? AutomaticInputCompletion.ask(fields,interaction) : assistance.complete(fields));
    }
     ...
     */
    private <T> T operation(AgentToolType type,Map<String,String> parameters,java.util.concurrent.Callable<T> operation)throws Exception{
     ...
     */
    private void preflight(Map<String,String> description){if(assistance!=null){var facts=new LinkedHashMap<>(description);facts.putAll(serverFacts);assistance.check("PREFLIGHT",facts);}}
