...
final class DeploymentDiagnosticEvidence {
    ...
    static Map<String, String> from(Object result) {
        ...
        var events = new ArrayList<DeploymentEvent>();
        if (result instanceof DeploymentOutcome single) {
            evidence.put("deploymentStatus", single.status().name());
            events.addAll(single.events());
        } else if (result instanceof MultiComponentDeploymentResult multiple) {
            evidence.put("deploymentStatus", multiple.status().name());
            events.addAll(multiple.applicationEvents());
            multiple.componentResults().forEach(c -> events.addAll(c.events()));
        }
        int index = 0;
        for (var event : events)
            if (!event.succeeded() && index < 3) {
                evidence.put("failedStage/" + (++index),
                        event.step().name() + ":" + AgentEvidenceText.redact(event.evidence()));
            }
