...
public final class ExternalApplicationUseCase {
    ...
    public ApplicationScan scan(String serverId, char[] master, Predicate<String> confirmation) throws Exception {
        try {
            ...
            for (var app : managed.list())
                if (app.server().id().equals(serverId)) {
                    registrations.put("SYSTEMD/" + app.systemdUnit(), "managed:" + app.id());
                    result.applications().stream()
                            .filter(value -> value.target().kind() == ExternalApplicationKind.DOCKER
                                    && value.name().equals("windowstolinux-" + app.id()))
                            .forEach(value -> registrations.put(value.target().key(), "managed:" + app.id()));
                }
            for (var app : applications.list())
                if (app.serverId().equals(serverId))
                    registrations.put(app.application().target().key(), app.id());
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public String adopt(ApplicationScan scan, DiscoveredApplication candidate, char[] master,
            Predicate<String> confirmation) throws Exception {
        var lock = locks.forServer(scan.server().id());
        try {
            lock.lockInterruptibly();
            if (!scan.candidates().contains(candidate) || !scan.adoptable(candidate))
                throw ownershipRequired();
            ServerProfile server = requireServer(scan.server().id());
            if (!server.endpoint().equals(scan.server().endpoint()))
                throw changed();
            var fresh = withSession(server, master, confirmation,
                    port -> port.execute(candidate.target(), LifecycleAction.REFRESH_STATUS));
            if (fresh.managed()) {
                String existing = scan.registrations().getOrDefault(fresh.target().key(), "");
                if (!existing.startsWith("managed:") || managed.find(existing.substring(8))
                        .filter(app -> app.server().id().equals(server.id())
                                && (fresh.target().kind() == ExternalApplicationKind.SYSTEMD
                                        ? app.systemdUnit().equals(fresh.target().identity())
                                        : fresh.name().equals("windowstolinux-" + app.id())))
                        .isEmpty())
                    throw ownershipRequired();
                return existing;
            }
            Instant now = Instant.now();
            return applications.adopt(new StoredExternalApplication("external:" + UUID.randomUUID(), server.id(),
                    server.host(), server.sshPort(), server.username(), fresh, now, now), true);
        } finally {
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    public ApplicationLifecycleResult execute(String id, LifecycleAction action, char[] master,
            Predicate<String> confirmation) throws Exception {
        try {
            var stored = applications.find(id)
                    .orElseThrow(() -> new IllegalArgumentException("external application no longer exists"));
            var lock = locks.forServer(stored.serverId());
            lock.lockInterruptibly();
            try {
                var server = requireServer(stored.serverId());
                if (!server.host().equals(stored.host()) || server.sshPort() != stored.sshPort()
                        || !server.username().equals(stored.username()))
                    throw changed();
                var result = withSession(server, master, confirmation,
                        port -> port.execute(stored.application().target(), action));
                if (result.managed())
                    throw ownershipRequired();
                Instant now = Instant.now();
                applications.observe(id, result, now);
    ...
    private ServerProfile requireServer(String id) throws java.sql.SQLException {
        return servers.find(id).orElseThrow(() -> ApplicationServiceException
                .create(ApplicationServiceFailureType.SERVER_PROFILE_MISSING, "Saved server profile is required"));
    }
     ...
     * @throws Exception if the delegated operation or caller-provided interaction fails / 被委派操作或调用方提供的交互失败时
     */
    private <T> T withSession(ServerProfile server, char[] master, Predicate<String> confirmation, RemoteWork<T> work)
            throws Exception {
        try (var store = servers.secrets().open(server.credentialMode(), master);
                var session = gateway.connect(server.endpoint(), servers.loadPassword(server, store),
                        servers.hostKeyVerifier(server, confirmation))) {
    ...
    private static LinuxOperationException changed() {
        return LinuxOperationException.create(LinuxOperationFailureType.EXTERNAL_IDENTITY_CHANGED,
                "Selected runtime or server identity changed; scan again");
    }
    ...
    private static LinuxOperationException ownershipRequired() {
        return LinuxOperationException.create(LinuxOperationFailureType.EXTERNAL_OWNERSHIP_REQUIRED,
                "A managed application requires its original ownership contract");
    }
    /**
     ...
     */
    @FunctionalInterface
    private interface RemoteWork<T> {
        /**
         * Runs T.
         * <p>运行T。
         *
