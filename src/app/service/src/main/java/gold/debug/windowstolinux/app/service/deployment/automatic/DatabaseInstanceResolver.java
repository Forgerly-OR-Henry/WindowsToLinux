...
final class DatabaseInstanceResolver {
    ...
    static Instance resolve(NativeDatabasePort port, String serverId, DatabaseRequirement requirement,
        ...
        while (true) {
            ...
            if (!inventory.conflicts().isEmpty()) {
                if (!interaction.confirm("db.conflict", Map.of("conflicts", inventory.conflicts()))) throw new CancellationException();
                continue;
            }
            if (inventory.instances().isEmpty()) {
                if (inventory.candidate().isEmpty() || !version.accepts(inventory.candidate().orElseThrow().engineVersion())) {
                    if (!interaction.confirm("db.versionUnavailable", Map.of("required", requirement.version()))) throw new CancellationException();
                    continue;
                }
            ...
            if (!version.accepts(selected.version())) {
                if (inventory.instances().size() > 1) {
                    if (!interaction.confirm("db.multipleReplacement", Map.of("instance", selected.id(), "required", requirement.version())))
                ...
                Map<String, String> details = Map.of("server", serverId, "instance", selected.id(), "current", selected.version(),
                        "required", requirement.version(), "target", candidate.engineVersion(), "data", selected.dataDirectory());
                if (!interaction.confirmDatabaseReplacement(details)) throw new CancellationException();
    ...
    private static Instance choose(List<Instance> instances, AutomaticDeploymentInteraction interaction) {
        ...
        DeploymentInputField field = new DeploymentInputField("db/instance", "db.field.instance", "db.help.instance", "", options);
        String choice = AutomaticInputCompletion.ask(List.of(field), interaction).get(field.id());
