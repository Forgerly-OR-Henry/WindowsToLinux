...
final class RecoverySessionState {
    ...
    RecoverySnapshot snapshot() {
        return new RecoverySnapshot(id, phase, code, Optional.ofNullable(action), token, unknown, decisions, attempts, Optional.ofNullable(failure));
    }
    /**
     * Revokes recovery session state.
     * <p>撤销恢复会话状态。
     */
    void revoke() { action = null; token = ""; }
    /**
     ...
     */
    boolean active() { return phase == RecoverySnapshot.State.OBSERVING || phase == RecoverySnapshot.State.EXECUTING; }
}
