...
public final class AgentRemoteToolService {
     ...
     */
    public AgentRemoteToolService(ServerUseCaseFacade servers,DeploymentLinuxGateway gateway){this.servers=servers;this.gateway=gateway;}
     ...
     */
    public AgentObservation candidate(ServerProfile server,String task,RemoteWorkspace workspace,boolean cleanup,char[] master)throws Exception{
        try{
            if(!servers.find(server.id()).filter(server::equals).isPresent())throw new SecurityException("server identity changed");
            try(var store=servers.secrets().open(server.credentialMode(),master);
                var remote=gateway.connect(server.endpoint(),servers.loadPassword(server,store),servers.hostKeyVerifier(server,value->false))){
                var candidate=new gold.debug.windowstolinux.shared.linux.transfer.RemoteTaskCandidate(workspace,task);
                var before=remote.inspectTaskCandidate(candidate);
                if(!cleanup)return new AgentObservation(true,true,before);
                if(!remote.cleanupTaskCandidate(candidate).succeeded())return new AgentObservation(false,false,Map.of("candidate",workspace.candidateId(),"cleanup","unverified"));
                ...
                return new AgentObservation(cleared,cleared,after);
            }
