...
public final class RecoveryAiUseCase {
     ...
     */
    public RecoveryAiUseCase(AiProfileRepository profiles, DesktopSecretStoreService secrets,
            RecoveryModelClient client) {
    ...
    public record Result(String code, String content, Map<String, RecoveryModelClient.OutcomeStatus> attempts) {
        ...
        public String toString() {
            return "RecoveryModelResult[" + code + "]";
        }
    }
    /**
    ...
    public Result observe(TerminalObservation observation, char[] master) throws java.sql.SQLException {
        if (!observation.text().isBlank())
            return new Result("ok", observation.text(), Map.of());
        return invoke(profiles.configuredFor(AiPurposeType.VISION), true, profile -> key -> client
                .observe(profile.chatCompletionsEndpoint(), profile.model(), key, observation.image()), master);
    }
    ...
    public Result decide(String observation, char[] master) throws java.sql.SQLException {
        return invoke(profiles.configuredFor(AiPurposeType.DEPLOYMENT), false,
                profile -> key -> client.propose(profile.chatCompletionsEndpoint(), profile.model(), key, observation),
                master);
    }
        ...
        return route(snapshot, vision, (stored, ignored) -> {
            ...
            try {
                ...
                try (var store = secrets.open(profile.credentialMode(), master)) {
                    ...
                    if (key.length == 0)
                        return new RecoveryModelClient.Reply(RecoveryModelClient.OutcomeStatus.UNAVAILABLE, "");
    ...
    static Result route(List<StoredAiProviderConfiguration> snapshot, boolean vision,
        ...
        for (var profile : enabled) {
            checkCancelled();
            if (attempts.containsKey(profile.profile().id()))
                continue;
            var reply = call.apply(profile, vision);
            checkCancelled();
            attempts.put(profile.profile().id(), reply.status());
            if (reply.status() == RecoveryModelClient.OutcomeStatus.VALID)
                return new Result("ok", reply.content(), attempts);
        }
        String code = attempts.containsValue(RecoveryModelClient.OutcomeStatus.UNAVAILABLE)
                ? "modelUnavailable"
                : attempts.containsValue(RecoveryModelClient.OutcomeStatus.INVALID)
                        ? "modelInvalid"
