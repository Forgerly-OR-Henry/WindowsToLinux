...
final class RecoverySshProbe {
    ...
    RecoverySshProbe(ServerUseCaseFacade servers, DesktopSecretStoreService secrets, DeploymentLinuxGateway gateway) {
        this.servers = servers; this.secrets = secrets; this.gateway = gateway;
    }
    /**
    ...
    RecoveryProbeResult check(ServerProfile profile, char[] master, Predicate<String> fingerprint) {
        try (var store = secrets.open(profile.credentialMode(), master)) {
            ...
            try (var session = gateway.connect(profile.endpoint(), credential, servers.hostKeyVerifier(profile, fingerprint))) {
                session.verifyConnection(); return RecoveryProbeResult.recovered();
            } finally { credential.clear(); }
        } catch (Exception failure) { return RecoveryProbeResult.failed(failure); }
    }
}
