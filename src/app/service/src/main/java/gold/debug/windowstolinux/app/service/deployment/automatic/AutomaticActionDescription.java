...
public final class AutomaticActionDescription {
    ...
    public static Map<String, String> deployment(ReviewedDeploymentRequest request) {
        return Map.ofEntries(Map.entry("application", request.facts().applicationId()),
                Map.entry("sourceRevision", request.archive().contentSha256()),
                Map.entry("configurationRevision", Long.toString(request.configuration().revision())),
                Map.entry("configurationDigest", request.configuration().sha256()),
                Map.entry("requestDigest", AgentAction.digest(request.toString())),
                Map.entry("projectType", request.facts().projectType().name()),
                Map.entry("runtimeIdentity", request.runtime().identityPolicy().name()),
                Map.entry("healthCheckType", request.runtime().healthCheck().getClass().getSimpleName()),
                Map.entry("resourceBindings",
                        request.fileBindings().stream().map(b -> b.bindingId() + ":" + b.location()).toList()
                                .toString()),
                Map.entry("databaseCount", Integer.toString(request.databaseBindings().map(List::size).orElse(0))),
                Map.entry("effect",
                        "Upload frozen source; create isolated candidate; restricted non-root build; verify health; publish managed release; recover using existing verified transaction. No arbitrary commands."),
                Map.entry("scope",
                        "Only the validated application installation, configuration and data bindings; never unowned directories or symlink traversal"));
    }
    ...
    public static Map<String, String> deployment(ReviewedMultiComponentApplication request) {
        ...
        result.put("effect",
                "Existing ordered component deployment transaction with application health gate and supported recovery");
        ...
        if (result.size() > 128)
            throw new IllegalArgumentException("component review exceeds supported context");
