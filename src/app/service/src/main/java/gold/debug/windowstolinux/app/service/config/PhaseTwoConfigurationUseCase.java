...
/**
 * Coordinates immutable Phase Two configuration and platform-secret references without returning secret values.
 *
 * <p>协调不可变的二期配置和平台秘密引用，且不返回秘密值。
 */
public final class PhaseTwoConfigurationUseCase {
    private final DesktopDatabase database;
    private final DesktopSecretStores secretStores;

    /**
     * Creates a {@code PhaseTwoConfigurationUseCase} instance.
     *
     * <p>创建 {@code PhaseTwoConfigurationUseCase} 实例。
     */
    public PhaseTwoConfigurationUseCase(DesktopDatabase database, DesktopSecretStores secretStores) {
