...
fn decode(raw: &str) -> Result<String, &'static str> {
    ...
    while index < bytes.len() {
        if bytes[index] == b'%' {
            ...
            value.push(if bytes[index] == b'+' { b' ' } else { bytes[index] });
            index += 1;
        }
    }
...
pub fn route(configuration: &Configuration, target: &str) -> Response {
    let mut response = Response {
        ...
        body: configuration.label.clone(),
    };
    ...
    if path == "/api/summary" || configuration.mode == "json" {
        let result = (|| {
            ...
            if path == "/api/summary" {
                for field in query.split('&') {
                    ...
                    if decode(key)? == "values" {
                        raw = Some(decode(value)?);
                        break;
                    }
                }
            }
            service::summarize(raw.as_deref())?
                .json()
        ...
        match result {
            ...
            Err(message) => {
                response.status = if message == "serialization-failed" { 500 } else { 400 };
                response.body = message.into();
            }
        }
    }
    response
