    ...
    while (!query.empty()) {
        ...
        if (field.starts_with("values=")) {
            ...
            for (;;) {
                ...
                if (token.empty() || error != std::errc{} || last != token.data() + token.size() || item < 0 ||
                    item > 10000 || result.items.size() == 20)
                    throw std::invalid_argument("invalid-values");
...
std::string Summary::json() const {
    ...
    for (std::size_t i = 0; i < items.size(); ++i)
        result += (i ? "," : "") + std::to_string(items[i]);
