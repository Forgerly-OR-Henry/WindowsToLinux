...
源码服务按 **12 种语言、26 个语言/构建工具组合**组织，每个组合固定 **3 组正常部署 + 2 组失败部署**，共 130 组。这里是独立的示例项目，不属于根 Maven reactor；自动化验收代码位于各模块的 `src/test`。
...
`python/stdlib/http-service` 使用 `PYTHON_STDLIB`，五组保持相同接口与边界校验，仅使用标准库，`dependencies = []` 明确声明无第三方依赖。配置失败组故意省略这一声明，必须在分析阶段拒绝或要求补填。新增组的真实 Linux 发布仍为 `RUNTIME-PENDING`。
