...
目录迁移补充（2026-09-19）：本目录由原 `test/` 整体迁入 `test/single-language/`。迁移瞬间核对 1,080 个文件的相对清单和 SHA-256 一致，场景源码和锁文件保持原样；说明导航按新路径修正。迁移相关 138 项既有 JUnit 测试通过；连同新增混合矩阵门禁共 139 项通过。下列记录仍对应 2026-09-09 的原验证范围，不因目录迁移升级为新的 Linux 实机验收。
