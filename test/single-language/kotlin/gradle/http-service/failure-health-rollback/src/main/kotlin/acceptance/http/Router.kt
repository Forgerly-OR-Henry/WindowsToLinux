...
class Router(private val configuration: Configuration) {
    ...
    fun route(target: String): Response {
        if (configuration.status == 503)
            return Response(503, "text/plain; charset=utf-8", configuration.label)
            ...
            try {
                ...
                val pair =
                    query
                        ...
                        .firstOrNull {
                            URLDecoder.decode(it[0], StandardCharsets.UTF_8) == "values"
                        }
                val raw =
                    if (path == "/api/summary" && pair != null)
                        URLDecoder.decode(pair.getOrElse(1) { "" }, StandardCharsets.UTF_8)
                    else null
                return Response(
                    200,
                    "application/json; charset=utf-8",
                    service.summarize(raw).toJson(),
                )
            } catch (error: IllegalArgumentException) {
                return Response(400, "text/plain; charset=utf-8", "invalid-values")
            }
        }
