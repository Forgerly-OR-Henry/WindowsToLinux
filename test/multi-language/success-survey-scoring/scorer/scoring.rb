module Scoring
  def self.calculate(payload)
    ...
    raise ArgumentError,
          'invalid revision, questions or answers' unless revision.is_a?(Integer) && revision.positive? &&
                                                          questions.is_a?(Array) && (1..30).cover?(questions.size) &&
                                                          answers.is_a?(Hash)
    ...
    questions.each do |q|
      ...
      raise ArgumentError,
            "#{id}: invalid weight or dimension" unless weight.is_a?(Integer) && (1..10).cover?(weight) &&
                                                        dimension.is_a?(String) && !dimension.empty?
      ...
      when 'scale'
        ...
        raise ArgumentError,
              "#{id}: invalid scale" unless minimum.is_a?(Integer) && maximum_value.is_a?(Integer) &&
                                            minimum >= 0 && maximum_value <= 100 && minimum < maximum_value
        raise ArgumentError,
              "#{id}: answer outside scale" unless missing ||
                                                   (answer.is_a?(Integer) && (minimum..maximum_value).cover?(answer))

        maximum = maximum_value - minimum
        raw = rule['reverse'] ? maximum_value - answer : answer - minimum unless missing
        explanation = missing ? '未填写可选题，计 0 分' :
          (rule['reverse'] ? "反向计分 #{maximum_value} - #{answer}" : "量表计分 #{answer} - #{minimum}")
      when 'single', 'multi'
        options = q['options']
        ...
        raise ArgumentError,
              "#{id}: invalid option scores" unless scores.size == options.size && scores.values.all? { |s|
                                                      s.is_a?(Integer) && (0..100).cover?(s)
                                                    }

        selected = missing ? [] : (q['type'] == 'single' ? [answer] : answer)
        raise ArgumentError,
              "#{id}: invalid selection" unless selected.is_a?(Array) && selected.uniq == selected &&
                                                selected.all? { |s|
                                                  s.is_a?(String) && scores.key?(s)
                                                }
      ...
      parts << { 'id' => id, 'dimension' => dimension, 'raw' => raw, 'weight' => weight, 'weighted' => weighted,
                 'maximum' => ceiling, 'explanation' => "#{explanation}，权重 #{weight}" }
    ...
    { 'protocolVersion' => 2, 'component' => 'ruby-scoring', 'revisionId' => revision,
      'score' => maximum.zero? ? 0 : (total * 100.0 / maximum).round(2), 'weightedTotal' => total,
      'maximum' => maximum, 'parts' => parts, 'dimensions' => dimensions, 'hidden' => hidden }
  end
end
