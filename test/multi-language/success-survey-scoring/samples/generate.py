...
if not 1 <= args.count <= 100000 or args.revision < 1:
    parser.error('count/revision out of range')
ids = set()
    ...
    body = json.dumps(
        {
            ...
            'respondent': '林同学' if i % 2 == 0 else '周同学',
            'requestId': f'scale-{args.revision}-{args.seed}-{i}',
            'answers': answers,
        }
    ).encode()
