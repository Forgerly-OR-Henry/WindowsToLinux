let base = "",
  pending = 0,
  timeout = 15000;
export const node = <T extends HTMLElement = HTMLElement>(id: string) =>
...
const shape = (v: any, fields: Record<string, string>) =>
  v !== null &&
  typeof v === "object" &&
  Object.entries(fields).every(([key, type]) =>
    type === "array" ? Array.isArray(v[key]) : typeof v[key] === type,
  );
function valid(path: string, v: any) {
  ...
  if (path.endsWith("/stats"))
    return shape(v, { total: "number", revisions: "array" });
  if (path.startsWith("/api/submissions?"))
    return shape(v, { items: "array", total: "number" });
  if (path.startsWith("/api/submissions"))
    return (
      shape(v, {
        id: "number",
...
export async function configure() {
  ...
  if (c.requestTimeoutMs !== undefined) {
    if (
      !Number.isInteger(c.requestTimeoutMs) ||
      c.requestTimeoutMs < 100 ||
      c.requestTimeoutMs > 60000
    )
