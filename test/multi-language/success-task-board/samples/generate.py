...
if not 1<=args.count<=100000:parser.error('count must be 1..100000')
for i in range(args.count):
 project=1+i%2;member=1 if project==1 else 3
 body=json.dumps({'projectId':project,'title':f'规模任务 {args.seed}-{i}','description':'确定性样本','ownerId':member,'priority':1+i%3,'labels':['scale'],'dependsOn':[],'dueDate':'2030-10-01','version':0,'actorId':member}).encode()
