...
public class TaskStore {
    ...
    private static final String SELECT = "SELECT t.id,t.project_id AS projectId,t.title,t.description,t.owner_id AS ownerId,m.name AS "
    ...
    public TaskStore() throws Exception {
        try (var c = db.open()) {
            ...
            for (String sql : List.of(
                    ...
                    "CREATE INDEX IF NOT EXISTS tasks_filter ON " + "tasks(project_id,status,priority,owner_id,id)",
                    ...
                    "INSERT OR IGNORE INTO members " + "VALUES(1,1,'林同学'),(2,1,'周同学'),(3,2,'陈老师'),(4,2,'李同学')",
            ...
            if (scalar(c, "SELECT count(*) FROM tasks") == 0)
                db.write(tx -> {
                    exec(tx, "INSERT INTO "
                            ...
                            + "version) " + "VALUES(1,1,'确认活动场地','先完成场地确认，再发布活动公告',1,2,'[\"演示\"]','todo'"
    ...
    public Object members(long project) throws Exception {
        try (var c = db.open()) {
            project(c, project);
            return rows(c, "SELECT id,name,project_id AS projectId FROM members WHERE project_id=? ORDER BY id",
                    project);
        }
    }
    ...
    private Map<String, Object> detail(Connection c, long id, long project) throws Exception {
        ...
        task.put("dependencies", rows(c, "SELECT t.id,t.title,t.status FROM dependencies d JOIN tasks t ON "
                + "t.id=d.depends_on WHERE d.task_id=? ORDER BY t.id", id));
        task.put("comments", rows(c, "SELECT x.id,x.text,x.created,m.name AS actor FROM comments x JOIN "
                + "members m ON m.id=x.actor_id WHERE task_id=? ORDER BY x.id", id));
        task.put("events", rows(c, "SELECT e.action,e.detail,e.created,m.name AS actor FROM events e JOIN "
                + "members m ON m.id=e.actor_id WHERE task_id=? ORDER BY e.id", id));
        return task;
    }
    ...
    public Object list(long project, String q, String label, String status, int priority, long owner, boolean overdue,
        ...
        var values = new ArrayList<Object>(
                List.of(project, q, label, label, status, status, priority, priority, owner, owner, overdue ? 1 : 0));
        try (var c = db.open()) {
            project(c, project);
    ...
    public Object stats(long project) throws Exception {
        try (var c = db.open()) {
            project(c, project);
            return Map.of("total", scalar(c, "SELECT count(*) FROM tasks WHERE project_id=?", project), "statuses",
                    rows(c, "SELECT status,count(*) AS count FROM tasks WHERE project_id=? GROUP BY status", project),
                    "owners",
                    rows(c, "SELECT m.id,m.name,count(t.id) AS count FROM members m LEFT JOIN tasks t ON "
                            + "t.owner_id=m.id WHERE m.project_id=? GROUP BY m.id ORDER BY m.id", project),
                    "overdue",
                    scalar(c, "SELECT count(*) FROM tasks WHERE project_id=? AND status<>'done' AND due_date<>'' "
                            + "AND due_date<date('now')", project));
        }
    }

    private static void event(Connection c, long task, long actor, String action, String detail) throws Exception {
        exec(c, "INSERT INTO events(task_id,actor_id,action,detail) VALUES(?,?,?,?)", task, actor, action, detail);
    }

    public Object save(Long id, TaskInput v) throws Exception {
        text(v.title(), 120, "标题");
        if (v.description() == null || v.description().length() > 2000 || v.priority() < 1 || v.priority() > 3)
        ...
        if (v.dependsOn() == null || v.dependsOn().size() > 20 || v.dependsOn().stream().anyMatch(Objects::isNull)
        ...
        return db.write(c -> {
            ...
            if (id != null) {
                var old = one(c, "SELECT version,status FROM tasks WHERE id=? AND project_id=?", id, v.projectId());
            ...
            for (long dependency : v.dependsOn()) {
                var dep = one(c, "SELECT status FROM tasks WHERE id=? AND project_id=?", dependency, v.projectId());
                if (id != null && (dependency == id || scalar(c,
            ...
            if (id == null) {
                exec(c, "INSERT INTO "
                        ...
                        + "VALUES(?,?,?,?,?,?,'todo',?,1)", v.projectId(), v.title().trim(), v.description(),
                        v.ownerId(), v.priority(), json.writeValueAsString(v.labels()), v.dueDate());
                key = scalar(c, "SELECT last_insert_rowid()");
            } else {
                key = id;
                exec(c, "UPDATE tasks SET "
                        + "title=?,description=?,owner_id=?,priority=?,labels=?,due_date=?,version=version+1 "
                        + "WHERE id=?", v.title().trim(), v.description(), v.ownerId(), v.priority(),
    ...
    public Object transition(long id, long project, Transition v) throws Exception {
        return db.write(c -> {
            ...
            var allowed = Map.of("todo", Set.of("doing"), "doing", Set.of("review"), "review", Set.of("doing", "done"),
                    "done", Set.<String>of());
            if (v.status() == null || !allowed.get(status).contains(v.status()))
                throw new BusinessError(409, "非法状态转换");
            if (v.status().equals("done")
                    && scalar(c, "SELECT count(*) FROM dependencies d JOIN tasks t ON t.id=d.depends_on WHERE "
                            + "d.task_id=? AND t.status<>'done'", id) > 0)
    ...
    public Object comment(long id, long project, Comment v) throws Exception {
        ...
        return db.write(c -> {
            ...
            if (!old.isEmpty()) {
                ...
                exec(c, "INSERT INTO comments(task_id,actor_id,text,request_id) VALUES(?,?,?,?)", id, v.actorId(),
                        v.text(), v.requestId());
