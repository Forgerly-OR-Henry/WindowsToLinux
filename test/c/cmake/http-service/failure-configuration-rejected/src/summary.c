...
int summary_parse(const char *query, Summary *result) {
    *result = (Summary){{1, 2, 3}, 3, 6};
    ...
    for (const char *p = query; p != NULL && *p; ) {
        if (strncmp(p, "values=", 7) == 0) { value = p + 7; break; }
        if (strncmp(p, "values", 6) == 0 && (p[6] == '&' || p[6] == '\0')) return 0;
        p = strchr(p, '&');
        if (p != NULL) ++p;
    }
    if (value == NULL) return 1;
    char decoded[128]; size_t n = 0;
    for (const char *p = value; *p && *p != '&'; ++p) {
        int c = (unsigned char)*p;
        if (c == '%') {
            if (!p[1] || !p[2] || hex(p[1]) < 0 || hex(p[2]) < 0) return 0;
            c = hex(p[1]) * 16 + hex(p[2]); p += 2;
        }
        if (n + 1 >= sizeof(decoded) || !(isdigit(c) || c == ',')) return 0;
        decoded[n++] = (char)c;
    }
    decoded[n] = '\0';
    ...
    for (;;) {
        if (!isdigit((unsigned char)*p) || result->count == 20) return 0;
        int item = 0;
        while (isdigit((unsigned char)*p)) {
            item = item * 10 + (*p++ - '0');
            if (item > 10000) return 0;
        }
        result->items[result->count++] = item; result->total += item;
        if (!*p) return 1;
        if (*p++ != ',') return 0;
    }
}
