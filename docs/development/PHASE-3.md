...
- 阶段基线版本：`2.1.0-phase3-support-preview`
- 文档结构版本：`2.0.0-roadmap-rebaseline`
- 文档状态：**实施中；支持等级与禁止执行的语言识别预览已完成本地实现，其余三期范围待实施**
- 当前实现：支持等级、精确验证范围、六个高级适配候选及其余候选语言路径识别已接入 Swing 分析结果；识别预览不创建归档且不能进入计划、构建或远端修改
- 更新日期：2026-08-13
