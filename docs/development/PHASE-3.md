...
- 阶段基线版本：`2.3.0-phase3-multi-component-core`
- 文档结构版本：`2.0.0-roadmap-rebaseline`
- 文档状态：**实施中；支持等级、识别预览、六种试验适配器及混合项目/多组件核心已通过本地全量门禁，桌面入口、真实 Linux、多模型与新增发行版待实施**
- 当前实现：Go、Rust、.NET、Kotlin、PHP、Ruby 已接入固定试验链路；混合项目可生成稳定组件记录并在目标机修改前拒绝冲突，多组件核心可按依赖执行构建/快照/切换/健康/恢复和生命周期汇总。上述新增链路全部仍为 `RUNTIME-PENDING`，不得声称产品入口或正式支持已验收
- 更新日期：2026-08-13
