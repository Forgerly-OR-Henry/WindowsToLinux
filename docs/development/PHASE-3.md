...
- 阶段基线版本：`2.6.0-phase3-product-entry`
- 文档结构版本：`2.0.0-roadmap-rebaseline`
- 文档状态：**实施中；支持等级、试验适配器、混合项目/多组件产品入口、多模型角色链路及新增发行版静态矩阵已通过本地聚焦门禁；真实 Linux 部署验收仍在进行**
- 当前实现：Go、Rust、.NET、Kotlin、PHP、Ruby 已接入固定试验链路；桌面多组件页可执行显式图分析、逐组件审阅、整应用事务及依赖安全生命周期，SQLite v7 在成功事务中原子保存不含秘密的组件图并可在桌面重启后恢复控制；三个 AI 角色使用独立 Provider/模型和安全冲突裁决；Debian、Rocky Linux、AlmaLinux、Oracle Linux 已具有独立识别、兼容策略与环境准备脚本。新增部署链路仍为 `RUNTIME-PENDING`，不得声称真实部署或正式支持已验收
- 更新日期：2026-08-13
...
- [x] 桌面单组件和多组件审阅/结果均显示语言、框架、支持等级及精确验证范围；真实目标组合仍按证据保持 `RUNTIME-PENDING`。
...
- [x] 部分自启和部分运行/异常由专用汇总状态保留，不会被显示为整体正常；桌面结果页已接入，真实入口证据待补。
