...
- 阶段基线版本：`2.2.0-phase3-experimental-adapters`
- 文档结构版本：`2.0.0-roadmap-rebaseline`
- 文档状态：**实施中；支持等级、禁止执行的语言识别预览及六种高级语言试验适配器已通过本地全量门禁，真实 Linux 验收与其余三期范围待实施**
- 当前实现：Go、Rust、.NET、Kotlin、PHP、Ruby 已接入锁文件静态分析、类型化运行时、固定构建渲染、helper v3、工具链探测和逐次试验风险确认；全部仍为 `RUNTIME-PENDING`，不得声称正式支持
- 更新日期：2026-08-13
