...
- Java 文档：`shared`、`app`、`web` 的生产 Java 中，所有显式具名类型、构造器、方法、字段及枚举项均使用英文摘要在前、中文说明紧随的标准 Javadoc；record 组件由类型的 `@param` 说明。补全适用的参数、泛型参数、返回及真实异常标签，不为局部变量或编译器生成成员制造注释。关键约束、单位及资源清理责任须与执行路径一致。
- 注释门禁：`JavadocCoverageTest` 使用 JDK AST 检查声明覆盖、双语正文和标签，包含缺失注释、单语言、缺失标签及匿名类/record 反例；JDK 21 doclint 校验语法与引用。该门禁纳入 Maven `verify`，报告及生成文档位于 `src/app/main/target/`。纯注释批量修改须比较去除注释后的源码，确保表达式、字符串、SQL 和嵌入脚本未变。
...
2. 每期使用目标与承接、模块增量、功能说明、功能依赖、按功能归组的验收、剩余边界的结构；未实施能力可保留实施顺序；跨模块功能完整说明一次，其他入口只链接。
3. 现行功能以当前代码和适用实际行为为准。文档冲突时沿产品入口核对代码、资源、配置及证据，修正文档；不得为了符合旧文档倒改代码或削弱门禁。
4. 分开记录当前实现、真实验证和历史分期。类型或类存在不等于产品入口可用，本地测试不证明 Linux、数据库或 UI 全流程。代码缺陷如实标明，文档维护不自动扩大为代码修复。
5. 五期当前实现与六期预留分别记录，保留已确认需求。旧文档未落地设想不自动转成本期开发任务；变更发生期次不能确认时标为期次未核实。
6. 日期、版本和提交只作为元信息或证据字段；正文按功能组织。新增和修复直接更新所属功能，删除失效规则、重复叙述及已完成迁移步骤，不在首尾新增日期、批次或版本流水账。必要的原因、复验和未覆盖范围按功能保留。
