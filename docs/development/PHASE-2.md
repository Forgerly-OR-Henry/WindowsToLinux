...
- 阶段基线版本：`2.1.0-phase2-local-implementation`
- 文档结构版本：`2.0.0-roadmap-rebaseline`
- 文档状态：**本地实现与自动化验证已完成；真实 Linux、systemd、容器和各类型端到端验收待执行**
- 当前实现：Git 只读快照、二期静态分析与计划、不可变配置/密钥修订、主机矩阵、受限 Provider/Agent 已实现并经过本地测试；二期类型尚未获得正式运行环境支持结论
- 更新日期：2026-08-12
...
- 已完成本地实现与负向测试：Git 分支/Tag 固定 Commit、禁用 Hook、拒绝未经支持的 Submodule/LFS 和 URL 凭据；六种类型的有界静态识别、冲突/缺失输入与确定性计划；SQLite v4 不可变普通配置及秘密修订/发布绑定；Linux 事实采集与 Ubuntu、CentOS Stream、旧 CentOS 的保守矩阵；Docker/Podman 不可混淆的自启契约；命名 Provider 与仅含分析/计划工具的可选 Agent 表面。
- 本次没有连接二期目标机，也没有执行 Gradle、普通 JAR、Node、Python、静态站点、Docker 或 Podman 的目标机构建、发布、回滚或生命周期。因此所有二期类型与新增发行版均为 `RUNTIME-PENDING`，不得标记为正式支持。
...
- [~] 六个项目类型都有类型化静态分析和确定性计划；目标机构建、启动、健康、失败恢复和生命周期为 `RUNTIME-PENDING`。
- [~] Docker restart policy 与 Podman Quadlet 具有独立类型化契约；未在真实环境验证。
