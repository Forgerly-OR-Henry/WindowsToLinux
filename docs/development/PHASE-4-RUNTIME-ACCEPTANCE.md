...
- 文档版本：`1.6.0-managed-resource-bindings`
...
| --- | --- | --- |
| JDK 21 全仓静态门禁 | 是 | `mvn -q -B -ntp -o verify` 已通过 393 项测试，25 项真实环境条件测试默认跳过；共读取 124 份 Surefire 报告 |
| 桌面受管应用已保存备份输入检查 | 是 | 输入应用标识后只读桌面 SQLite v10，逐项显示运行时、文件/数据库资源范围、配置和秘密绑定缺失；完整结果仍明确不代表远端归档就绪 |
...
| SQLite、PostgreSQL、MySQL/MariaDB 真实一致性导出与候选恢复 | 否 | SQLite v10 已保存经审阅非秘密绑定且密码仅为精确秘密引用，helper v4 与数据库适配契约也已存在，但远端物理采集映射及桌面/service 备份创建产品入口尚未实现 |
| 远端候选启动、两级健康与提交 | 否 | schema v4 组件发布和秘密归属已精确绑定并由 deploy 独立复核；候选无冲突端口策略仍未确定，禁止复用正式端口强行启动。schema v3 或 v4 声明秘密但无 `secrets.enc` 时禁止远端暂存或激活 |
