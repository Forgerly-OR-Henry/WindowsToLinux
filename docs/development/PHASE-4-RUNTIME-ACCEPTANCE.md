...
- 文档版本：`1.5.0-schema-v4-activation-bindings`
...
| --- | --- | --- |
| JDK 21 全仓静态门禁 | 是 | `mvn -q -B -ntp -o verify` 已通过 380 项测试，25 项真实环境条件测试默认跳过；共读取 121 份 Surefire 报告 |
| 桌面受管应用已保存备份输入检查 | 是 | 输入应用标识后只读桌面 SQLite，逐项显示缺失输入；完整结果仍明确不代表远端归档就绪 |
