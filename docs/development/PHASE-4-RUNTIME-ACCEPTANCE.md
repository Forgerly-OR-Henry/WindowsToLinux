...
- 文档版本：`1.10.0-phase6-maintenance-boundary`
...
| --- | --- | --- |
| JDK 21 全仓静态门禁 | 是 | `mvn -q -B -ntp -o verify` 本次实际生成 128 份 Surefire 报告、414 项测试，0 失败、0 错误，27 项真实环境条件测试默认跳过；5 份已移动或已删除测试类留下的旧 XML 共 16 项不计入当前证据 |
...
2026-09-08 已在同一台 Ubuntu 24.04 x86-64 执行 helper v5 环境准备和全语言产品入口部署，当前结果、具体路径和错误修复见[实机记录](UBUNTU-24-LIVE-DEPLOYMENT-2026-09-08.md)。上表旧静态统计保留为当时记录，不作为本次结果。四期全链路仍缺真实备份/恢复/迁移矩阵；Windows 独立更新/卸载执行器已经移入六期。不能以单元测试、直接 helper 或手工 SSH 替代四期运行证据。
