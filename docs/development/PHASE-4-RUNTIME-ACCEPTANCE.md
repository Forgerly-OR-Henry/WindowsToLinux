...
因此，当前可以在环境到位后分别执行 helper v5 基线、桌面完整备份、受管恢复和双服务器离线迁移取证，但这些入口尚未产生新协议的真实成功证据。四期全链路仍缺真实备份/恢复/迁移矩阵；Windows 独立更新/卸载执行器已经移入六期，不再阻塞四期。不能以单元测试、直接 helper 或手工 SSH 替代四期运行证据。
