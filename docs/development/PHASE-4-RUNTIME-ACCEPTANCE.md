...
| --- | --- | --- |
| JDK 21 全仓静态门禁 | 是 | `mvn -q -B -ntp -o verify` 已通过 368 项测试，25 项真实环境条件测试默认跳过 |
| 桌面受管应用已保存备份输入检查 | 是 | 输入应用标识后只读桌面 SQLite，逐项显示缺失输入；完整结果仍明确不代表远端归档就绪 |
| 桌面本地归档完整校验、隔离候选准备与精确删除 | 是 | 只验证所选归档并管理一个从未激活的本地候选；删除要求页面保留工作区签发的内存授权，不连接服务器，也不在应用重启后扫描未知旧目录 |
