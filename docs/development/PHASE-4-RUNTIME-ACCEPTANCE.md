...
- 文档版本：`1.4.0-secret-authentication-ui`
...
| --- | --- | --- |
| JDK 21 全仓静态门禁 | 是 | `mvn -q -B -ntp -o verify` 已通过 369 项测试，25 项真实环境条件测试默认跳过 |
| 桌面受管应用已保存备份输入检查 | 是 | 输入应用标识后只读桌面 SQLite，逐项显示缺失输入；完整结果仍明确不代表远端归档就绪 |
| 桌面本地归档完整校验、秘密密码认证、隔离候选准备与精确删除 | 是 | 可在本地认证独立备份密码与完整秘密修订集合，后台返回前立即清零解码秘密；只管理一个从未激活的本地候选，不连接服务器，也不在应用重启后扫描未知旧目录 |
