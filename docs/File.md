...
- 文档版本：`3.40.0-authenticated-maintenance-handoff`
- 文档状态：**28-POM 模块边界保持不变；桌面更新/卸载主进程交接已有严格版本化、用途隔离和 HMAC-SHA256 认证的跨进程载荷，SQLite v9 备份输入与固定 `WindowsToLinux/*` 凭据删除能力保持完成；认证密钥交付、交接文件 ACL、生产维护执行器、远端物理数据映射及产品入口实机证据仍在开发并标记 `RUNTIME-PENDING`**
