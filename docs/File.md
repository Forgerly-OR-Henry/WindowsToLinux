...
- 文档版本：`3.43.0-local-backup-publication`
- 文档状态：**28-POM 模块边界保持不变；SQLite v9 备份输入准入与 Windows 同目录临时写入、双重完整校验、无覆盖原子发布边界已完成静态实现；桌面维护认证交接保持完成，远端物理数据映射、完整归档采集、生产维护执行器及产品入口实机证据仍标记 `RUNTIME-PENDING`**
