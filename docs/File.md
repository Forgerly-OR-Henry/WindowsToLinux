...
- 文档版本：`3.44.0-local-candidate-lifecycle`
- 文档状态：**28-POM 模块边界保持不变；SQLite v9 备份输入准入、Windows 归档安全发布及不可伪造本地恢复候选的精确删除生命周期已完成静态实现；桌面维护认证交接保持完成，远端物理数据映射、完整归档采集、生产维护执行器及产品入口实机证据仍标记 `RUNTIME-PENDING`**
