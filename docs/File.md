...
- 文档版本：`3.41.0-single-component-backup-input-persistence`
- 文档状态：**28-POM 模块边界保持不变；SQLite v9 现由单组件与多组件成功部署共同使用完整整应用图事务，精确保存后续备份所需的运行时、数据路径状态、发布配置与秘密引用；桌面维护认证交接和固定 `WindowsToLinux/*` 凭据删除能力保持完成，远端物理数据映射、生产维护执行器及产品入口实机证据仍标记 `RUNTIME-PENDING`**
