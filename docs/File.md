...
- 文档版本：`3.51.0-candidate-restore-ports`
- 文档状态：**28-POM 模块边界保持不变；完整远端备份产品入口已实现；恢复候选采用“可证明覆盖时回环并行、否则短停机”的混合端口策略，真实恢复与离线迁移产品链进入实现；生产维护执行器及新增协议实机证据仍标记 `RUNTIME-PENDING`**
