...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v3 类型化组件/依赖/两级健康、候选恢复和显式停写的离线迁移状态机；Linux/SSHD 可将精确成员暂存到摘要候选并独立回读，成功迁移只等待人工切流且保留源端 | 只证明本地静态契约、状态编排、数据库协议、单向暂存接缝和负向边界；候选端口隔离、解密秘密交接、deploy 激活/切换、双端同步、桌面用例及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
