...
- 文档版本：`2.35.0-managed-input-ui`
- 文档状态：**SQLite v9 备份输入准入已接入桌面可读检查入口，Windows 本地归档安全发布及不可伪造本地恢复候选精确删除保持完成；桌面维护认证交接保持完成，远端物理映射、完整归档采集、生产维护执行器及其产品入口仍标记 `RUNTIME-PENDING`**
- 更新日期：2026-08-22
...
| 桌面/Web 业务 | Swing 已提供单组件与多组件独立页面，以及十二类项目的类型选择、静态分析、类型化计划审阅、已保存凭据提交、整应用结果、依赖安全生命周期、本地备份输入检查及单候选准备与删除；Web 业务未实现 | 桌面入口统一使用 Reviewed API；备份输入检查只读本地 SQLite 并逐项解释缺失，不能宣称远端归档就绪；候选只在本地隔离提取且删除局限于工作区签发的精确当前候选；Web 不可部署或管理应用 |
...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v3 类型化组件/依赖/两级健康、候选恢复和显式停写的离线迁移状态机；SQLite v9 让新成功的单组件与多组件部署共同保存准确的已审阅运行时、逻辑数据路径状态及发布配置来源；第六个桌面备份门面及页面可只读核对并展示整应用图、当前发布、运行时、路径、发布配置和秘密绑定缺失项且拒绝状态漂移；Linux/SSHD 可暂存并回读精确成员，桌面可校验归档、管理一个隔离本地候选并交接认证秘密 | 只证明本地静态契约和接缝；页面明确本地输入完整不代表远端归档就绪，本地候选删除也不等于远端恢复；完整归档创建入口尚未接通，远端物理数据映射、候选端口隔离、deploy 激活/切换、双端同步及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
