...
- 文档版本：`2.24.0-phase4-secret-revision-handoff`
- 文档状态：**三期产品入口证据作为 helper v3 历史结果保留；四期已完成 `secrets.enc` 严格修订载荷、整体认证解码及 manifest 标识绑定交接；候选激活所需端口隔离、其余平台接线和真实产品入口验收继续标记 `RUNTIME-PENDING`**
- 更新日期：2026-08-22
...
| 四期桌面更新与卸载 | `app/windows` 已实现签名元数据、固定 Ed25519 信任策略、版本/架构/撤销验证、程序与 SQLite 成对回滚，以及无默认选择的受管卸载和精确残留模型 | 未内置测试密钥；生产发布公钥、独立 jpackage 更新器、真实文件替换、Credential Manager 删除、service/main/UI 接线均未完成，不可声称产品更新或卸载已可用 |
