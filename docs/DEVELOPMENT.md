...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v4 组件发布/精确秘密归属、候选恢复和显式停写的离线迁移状态机；schema v3 保留本地兼容但不允许自动激活。SQLite v9 让新成功部署保存准确的已审阅运行时、路径及发布配置来源；桌面可显示持久化输入缺失、校验归档、认证独立备份密码并管理一个隔离本地候选，已解码秘密在页面回调前关闭；Linux/SSHD 可暂存并回读精确成员 | 只证明本地静态契约和接缝；页面明确本地输入完整、密码认证或候选删除都不等于远端归档/恢复；v4 缺少 `secrets.enc` 时也不授权激活。完整归档创建入口尚未接通，远端物理数据映射、候选端口隔离、deploy 激活/切换、双端同步及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
