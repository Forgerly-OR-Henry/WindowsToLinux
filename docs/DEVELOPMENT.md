...
- 文档版本：`2.21.0-phase4-desktop-maintenance-core`
- 文档状态：**三期产品入口证据作为 helper v3 历史结果保留；四期备份/恢复/迁移及桌面更新/卸载安全核心仅完成本地静态实现与测试，具体平台接线和真实产品入口验收继续标记 `RUNTIME-PENDING`**
- 更新日期：2026-08-22
...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v2 平台事实、候选恢复和显式停写的离线迁移状态机；成功迁移只等待人工切流且保留源端 | 只证明本地静态契约、状态编排、数据库协议、模块依赖方向和负向边界；具体 deploy/Linux 恢复与双端同步、桌面用例及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
