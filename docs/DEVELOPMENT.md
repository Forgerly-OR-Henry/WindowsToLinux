...
| 桌面/Web 业务 | Swing 已提供单组件与多组件独立页面，以及十二类项目的类型选择、静态分析、类型化计划审阅、已保存凭据提交、整应用结果、依赖安全生命周期、完整受管备份创建、本地归档检查、秘密密码认证、单候选管理、受管目标恢复和双服务器离线迁移；Web 业务未实现 | 备份、恢复和迁移均使用已保存服务器资料及受控产品入口；迁移不会自动切换外部流量或删除源端。真实 helper v5、Linux/数据库执行仍待验收，Web 不可部署或管理应用 |
...
| 版本 | 日期 | 阶段 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| 2.40.0-managed-restore-migration | 2026-08-22 | 四期 | 受管恢复与双服务器离线迁移静态产品链完成；424 项全仓门禁通过，真实执行待验收 | 配置文档 v2 精确携带非秘密激活事实；恢复先做零写入目标预检，再暂存配置、秘密和制品，执行混合端口候选或整应用短停机、数据库恢复、两级健康、正式提交及失败恢复。离线迁移执行初始备份、明确停写、最终停写归档和目标恢复；成功保持源端停止、保留且等待人工切流，源端归属保留时目标本地接管明确延后。完整 28-POM JDK 21 离线门禁通过 424 项测试、0 失败、0 错误、25 项真实环境条件跳过，131 份 Surefire 报告。生产 Windows 独立更新/卸载执行器仍待讨论。 |
| 2.39.0-managed-remote-backup | 2026-08-22 | 四期 | 远端完整备份静态产品链和 397 项全仓门禁通过；实机仍待验收 | SQLite v11 原子保存整应用健康探针；普通文件进入固定受管根，容器卷和镜像要求可验证归属。helper v5 仅接受类型化取材、读取和清理动词，生成严格 PAX TAR、Podman OCI Archive 及固定数据库制品。桌面完整备份默认短停写，失败也恢复并复核原运行状态，秘密以独立密码加密后经 Windows 同目录无覆盖原子发布及最终复验。完整 28-POM JDK 21 离线门禁通过 397 项测试、0 失败、0 错误、25 项真实环境条件跳过，126 份 Surefire 报告；真实 Linux/数据库为 `RUNTIME-PENDING`。 |
