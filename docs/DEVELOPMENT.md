...
- 文档版本：`2.23.0-phase4-desktop-backup-entry`
- 文档状态：**三期产品入口证据作为 helper v3 历史结果保留；四期桌面已接通版本化备份完整校验和隔离本地候选准备，且明确不连接服务器或激活候选；候选激活所需端口隔离、解密秘密交接、其余平台接线和真实产品入口验收继续标记 `RUNTIME-PENDING`**
- 更新日期：2026-08-22
...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v3 类型化组件/依赖/两级健康、候选恢复和显式停写的离线迁移状态机；Linux/SSHD 可将精确成员暂存到摘要候选并独立回读；桌面可完整校验归档并在平台工作区准备隔离本地候选 | 只证明本地静态契约、状态编排、数据库协议、单向暂存接缝、桌面本地入口和负向边界；候选端口隔离、解密秘密交接、deploy 激活/切换、双端同步及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
