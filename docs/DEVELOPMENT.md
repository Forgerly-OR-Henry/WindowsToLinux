...
| 桌面/Web 业务 | Swing 已提供单组件与多组件独立页面，以及十二类项目的类型选择、静态分析、类型化计划审阅、已保存凭据提交、整应用结果、依赖安全生命周期、完整受管备份创建、本地归档检查、秘密密码认证及单候选准备与删除；Web 业务未实现 | 完整备份会使用已保存服务器资料执行受控短停写取材并在 Windows 最终路径独立复验；真实 Linux/数据库执行仍待验收，Web 不可部署或管理应用 |
...
| 四期备份、恢复与迁移 | `shared/backup` 已实现 schema v4 安全归档、秘密信封、数据库一致性策略、候选恢复和离线迁移状态机；SQLite v11 保存完整已审阅输入。普通文件固定映射到 `/var/lib/windowstolinux/data/<application>/<component>/files/<binding>`，容器只接收有归属的命名卷；helper v5 可创建严格 PAX TAR、Podman OCI Archive 和固定数据库制品。桌面入口默认统一短停写、恢复原运行状态、执行组件与整应用健康，并以无覆盖方式发布完整归档 | 397 项静态/集成测试证明产品组合和失败恢复；SQLite 仍因缺少已审阅应用相对物理路径而在远端前显式拒绝，多数据库超出 schema v4 单数据库范围也拒绝。真实 Linux/数据库、候选端口激活、恢复与双端迁移仍为 `RUNTIME-PENDING` |
