...
- 文档版本：`2.36.0-secret-authentication-ui`
- 文档状态：**SQLite v9 备份输入准入及 `secrets.enc` 独立密码认证已接入桌面入口，Windows 本地归档安全发布及不可伪造本地恢复候选精确删除保持完成；桌面维护认证交接保持完成，远端物理映射、完整归档采集、生产维护执行器及其产品入口仍标记 `RUNTIME-PENDING`**
- 更新日期：2026-08-22
...
| 四期备份、恢复与迁移 | `shared/backup` 已实现版本化安全归档、独立秘密信封、数据库一致性策略、schema v3 类型化组件/依赖/两级健康、候选恢复和显式停写的离线迁移状态机；SQLite v9 让新成功部署保存准确的已审阅运行时、路径及发布配置来源；桌面可显示持久化输入缺失、校验归档、认证独立备份密码并管理一个隔离本地候选，已解码秘密在页面回调前关闭；Linux/SSHD 可暂存并回读精确成员 | 只证明本地静态契约和接缝；页面明确本地输入完整、密码认证或候选删除都不等于远端归档/恢复；完整归档创建入口尚未接通，远端物理数据映射、候选端口隔离、deploy 激活/切换、双端同步及 helper v4 实机证据仍为 `RUNTIME-PENDING` |
