...
“一键部署”同行最左侧的标准三档滑块可用鼠标或键盘选择。每次启动默认为**静态**；本次会话的选择在主题、语言切换后保留。切换滑块或审批方式不会调用 AI；任务启动后冻结模式、审批策略、源码、目标及模型顺序，并锁定选择。

| 模式 | 行为 | 模型要求 |
| --- | --- | --- |
| 静态 | 确定性分析、人工补齐、固定部署事务和真实健康验证；部署任务全程不调用 AI，包括失败分支 | 无 |
| 半 AI | 系统控制流程；在分析补齐、执行前检查和失败解释三个固定节点提供结构化辅助，不自主运行修复命令 | 已启用且通过文字能力测试的部署模型 |
| Agent | 一个部署 Agent 根据实际反馈选择下一步；一个独立审批 Agent 审核动作；串行调用受控工具和既有部署事务 | 可用部署模型及审批模型 |

半 AI 只自动采用证据唯一、无冲突、非秘密且经确定性校验的候选值。密码、授权、缺少依据的参数仍需用户填写。部署模型全部不可用时须明确同意人工继续，不能静默降级。
...
本版真实模型接口及半 AI/Agent Linux 部署验收尚未运行，不能用历史静态部署证据代替。当前本地验证及剩余边界见 [App 开发说明](development/PHASE-4.md#agent-modes)。
